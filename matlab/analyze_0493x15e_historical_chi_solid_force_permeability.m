function result = analyze_0493x15e_historical_chi_solid_force_permeability(runRoot)
% 0493x15e historical chi-solid closure: force budget + permeability.
...
if nargin < 1 || isempty(runRoot)
    runRoot = fullfile('..','runs','0493x15e_historical_chi_solid_force_permeability','fresh');
end
...
assert(strcmp(forcingMode, 'mean_outward_bath'), ...
...
writetable(A, fullfile(analysisDir,'force_budget_steps_0493x15e.csv'));
writetable(P, fullfile(analysisDir,'permeability_steps_0493x15e.csv'));

fid = fopen(fullfile(analysisDir,'summary_0493x15e.txt'),'w');
assert(fid >= 0, 'Cannot create x15e summary');
cleanup = onCleanup(@() fclose(fid));
fprintf(fid,'0493x15e historical mean_outward_bath + chiVP force/permeability\n');
