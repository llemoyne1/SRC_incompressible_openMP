function prep = prepare_tranche2_step02_run_simple(overrides)
%PREPARE_TRANCHE2_STEP02_RUN_SIMPLE
% Prepare tranche 2 inputs and write a shell script for manual WSL launch.
%
% prep = prepare_tranche2_step02_run_simple()
% prep = prepare_tranche2_step02_run_simple(overrides)
%
% Recognized runtime override fields:
%   workdir, exePath, prefixIn, prefixOut, seed, layoutMode,
%   stateMode, amp, x, v, type, r0
...
exeDefault = fullfile(thisDir, '..', 'build', 'main_step02_zone_pass');
paramsOverrides = overrides;
runtimeFields = {'workdir','exePath','prefixIn','prefixOut','seed','layoutMode','stateMode','amp','x','v','type','r0'};
...
workdir = getfield_if_exists_local(overrides, 'workdir', fullfile(pwd, 'run_tranche2_step02_manual'));
...
layoutMode = char(string(getfield_if_exists_local(overrides, 'layoutMode', 'base')));
...
runScriptPath = fullfile(workdir, 'run_step02.sh');
...
write_shell_script_local(runScriptPath, exeLinux, paramsLinux, xLinux, vLinux, typeLinux, r0Linux, outLinux, layoutMode);
...
prep.layoutMode = layoutMode;
prep.hasType = true;
prep.hasR0 = true;

save(fullfile(workdir, 'prep_tranche2_step02.mat'), 'prep');

fprintf('\n=== prepare_tranche2_step02_run_simple ===\n');
fprintf('workdir        : %s\n', workdir);
fprintf('exePath        : %s\n', exePath);
fprintf('layoutMode     : %s\n', layoutMode);
fprintf('stateMode      : %s\n', stateInfo.mode);
...
function write_shell_script_local(scriptPath, exeLinux, paramsLinux, xLinux, vLinux, typeLinux, r0Linux, outLinux, layoutMode)
