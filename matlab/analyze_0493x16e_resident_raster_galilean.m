function result = analyze_0493x16e_resident_raster_galilean(pairRoot)
% 0493x16e CUDA-resident sub-cell rasterization / Galilean qualification for dynamic chi solids.
%
...
%   analyze_0493x16e_resident_raster_galilean('../runs/0493x16e_resident_raster_galilean_pair')

if nargin < 1 || isempty(pairRoot)
    pairRoot = fullfile('..','runs','0493x16e_resident_raster_galilean_pair');
end
restRoot = fullfile(pairRoot,'rest','fresh');
boostRoot = fullfile(pairRoot,'boost','fresh');
metaPath = fullfile(pairRoot,'pair_meta_0493x16e.txt');
assert(isfile(metaPath), 'Missing %s', metaPath);
UG = readKvNumber(metaPath, 'boostCommonUx');
assert(isfinite(UG) && abs(UG) > 0, 'x16e requires a non-zero boostCommonUx');
...
% Authoritative CUDA-rasterized solid volume. This directly detects any
% sub-cell volume pulsation in the resident geometry.
...
% x16e Galilean/residency criteria. These are deliberately dimensionless/statistical:
...
pass = mechanicsPass && initialPass && geometryPass && residentPass && phasePass && ...
...
writetable(T, fullfile(analysisDir,'galilean_steps_0493x16e.csv'));

H = table((1:4)',forceHarmonics(:),massHarmonics(:), ...
    'VariableNames',{'harmonic','forceAmplitudeOverRms','fictitiousMassAmplitudeOverRms'});
writetable(H, fullfile(analysisDir,'galilean_phase_harmonics_0493x16e.csv'));

fid = fopen(fullfile(analysisDir,'summary_0493x16e.txt'),'w');
assert(fid >= 0, 'Cannot create x16e summary');
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'0493x16e Galilean common-translation qualification / rigid slab 1D\n');
...
fprintf('\n===== 0493x16e GALILEAN COMMON-TRANSLATION =====\n');
fprintf('status                              = %s\n',status);
...
fprintf('CUDA resident/subcell                = %d / %d\n',residentPass,geometryPass);
...
title('0493x16e frame-invariant relative bulk motion'); grid on;

figure;
plot(B.phase(B.steady),B.totalFluidImpulseX(B.steady),'.');
xlabel('sub-cell slab phase'); ylabel('fluid impulse x / step');
title('0493x16e boosted reaction versus grid phase'); grid on;

figure;
plot(B.time,B.sampledSolidVolume,'-');
xlabel('time'); ylabel('sampled solid volume');
title('0493x16e sampled chi-solid volume, boosted frame'); grid on;
...
assert(~isempty(D) && ~isempty(S),'Empty x16e case %s',runRoot);
...
% x16e: the authoritative sampled solid volume is produced by the CUDA
% sub-cell rasterizer itself; do not reconstruct the superseded center-sampled
% center-sampled x16d geometry on the host.
...
C.subcellRaster=all(D.subcellRaster0493x16e(id)==1);
