function out = analyze_open_channel_cylinder_resampling_0138(runRoot, varargin)
%ANALYZE_OPEN_CHANNEL_CYLINDER_RESAMPLING_0138 Post-process open-channel-cylinder runs.
%
...
if nargin < 1 || isempty(runRoot)
    runRoot = fullfile('..','runs','open_channel_cylinder_resampling_0138');
end
runRoot = char(strrep(string(runRoot), '\\', filesep));

p = inputParser;
p.FunctionName = 'analyze_open_channel_cylinder_resampling_0138';
...
for i = 1:numel(labels)
    ...
    if ~isfolder(runDir)
        fprintf('[0138] skipping missing run directory: %s\n', runDir);
        continue;
    end
    ...
    writetable(r.metrics, fullfile(analysisDir, sprintf('open_channel_cylinder_metrics_%s.csv', label)));
    writetable(r.profile, fullfile(analysisDir, sprintf('open_channel_cylinder_profile_%s.csv', label)));
end

summary = local_make_summary(runs);
writetable(summary, fullfile(analysisDir, 'open_channel_cylinder_summary_0138.csv'));
...
disp('=== Open-channel cylinder resampling 0138 summary ===');
disp(summary);
fprintf('[0138] Analysis written to: %s\n', analysisDir);
...
if ~isfile(paramsFile)
    error('analyze_open_channel_cylinder_resampling_0138:missingParams', 'Cannot find params_used.kv in %s', runDir);
end
params = parse_smpcd_kv(paramsFile);
Lx = double(params.Lx); Ly = double(params.Ly);
Nx = double(params.Nx); Ny = double(params.Ny);
cx = local_getfield(params, 'immersedSolidCx', 1.0);
cy = local_getfield(params, 'immersedSolidCy', 0.5 * Ly);
R  = local_getfield(params, 'immersedSolidR', 0.10);
bodyAx = local_getfield(params, 'bodyAccelerationX', 0.0);

frames = list_smpcd_dumps(runDir);
if height(frames) == 0
    error('analyze_open_channel_cylinder_resampling_0138:noFrames', 'No state_step_*.smpcd dumps found in %s', runDir);
end
...
for i = 1:n
    ...
    probeX = min(max(cx + probeXOverR * R, fields.xc(1)), fields.xc(end));
...
fig = figure('Name', '0138 open-channel cylinder timeseries', 'Visible', 'on');
tiledlayout(6,1);
...
saveas(fig, fullfile(analysisDir, 'open_channel_cylinder_0138_timeseries.png'));
end

function local_plot_final_fields(runs, analysisDir)
for i = 1:numel(runs)
    f = runs(i).finalFields;
    fig = figure('Name', ['0138 final fields ' runs(i).label], 'Visible', 'on');
    tiledlayout(3,2);
    ...
    saveas(fig, fullfile(analysisDir, sprintf('open_channel_cylinder_0138_final_fields_%s.png', runs(i).label)));
end
end

function local_plot_profiles(runs, analysisDir)
fig = figure('Name', '0138 open-channel cylinder final profiles', 'Visible', 'on');
tiledlayout(1,2);
nexttile; hold on; grid on;
for i = 1:numel(runs)
...
saveas(fig, fullfile(analysisDir, 'open_channel_cylinder_0138_profiles.png'));
end
...
k = k + 1;
freq = (k - 1) / (n * dt);
D = 2.0 * local_getfield(params, 'immersedSolidR', 0.10);
