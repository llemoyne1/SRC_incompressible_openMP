function R = analyze_backward_step_mass_budget_0069(varargin)
%ANALYZE_BACKWARD_STEP_MASS_BUDGET_0069 Zone mass-budget diagnostics for hard-inlet backstep runs.
%
...
p = inputParser;
p.FunctionName = 'analyze_backward_step_mass_budget_0069';
...
outDir = fullfile(runRoot, 'analysis_0069');
...
for c = 1:numel(caseDirs)
    ...
    summaryRows = [summaryRows; local_summary_row(caseLabel, rows)]; %#ok<AGROW>
end
...
if ~isempty(summaryRows)
    writetable(summaryRows, fullfile(outDir, 'mass_budget_summary_0069.csv'));
end
...
function srow = local_summary_row(caseLabel, rows)
last = rows(end,:);
if height(rows) >= 2
    t = rows.time;
    ...
    massSlope = NaN; upSlope = NaN; downSlope = NaN;
end
srow = table(string(caseLabel), height(rows), rows.time(1), rows.time(end), ...
    last.totalFluidMass, massSlope, last.maxFluidN, last.stdFluidN, ...
    last.upstreamLowerMass, upSlope, last.upstreamLowerMaxN, ...
    last.downstreamLowerMass, downSlope, last.downstreamLowerMaxN, ...
    last.inletFluxProxy, last.outletFluxProxy, last.frontBandUpFluxProxy, last.frontBandMass, last.q9HaloMass, last.q9GeoActiveMass, ...
    'VariableNames', {'caseLabel','nFrames','timeFirst','timeFinal','totalFluidMassFinal','totalFluidMassSlope', ...
    'maxFluidNFinal','stdFluidNFinal','upstreamLowerMassFinal','upstreamLowerMassSlope','upstreamLowerMaxNFinal', ...
    'downstreamLowerMassFinal','downstreamLowerMassSlope','downstreamLowerMaxNFinal', ...
    'inletFluxProxyFinal','outletFluxProxyFinal','frontBandUpFluxProxyFinal','frontBandMassFinal','q9HaloMassFinal','q9GeoActiveMassFinal'});
end
...
for f = 1:numel(fields)
    ...
    title(sprintf('0069 mass budget: %s', fields{f}), 'Interpreter','none');
