function R = analyze_open_channel_wall_bands_0089(varargin)
%ANALYZE_OPEN_CHANNEL_WALL_BANDS_0089 Wall-band diagnostics for full-IO VP inlet/outlet runs.
%
% This diagnostic is intentionally state-dump based.  It separates inlet,
% interior, outlet, and full-domain x-regions into bottom-wall, core, top-wall,
% combined-wall, and all-y bands.  It is meant to quantify near-wall pockets
% that can pollute global density/profile statistics.
%
% Typical use:
%   R = analyze_open_channel_wall_bands_0089( ...
%       'root','..', ...
%       'runRoot','runs/poiseuille_ramped_softlimited_q9_0080_g30');

p = inputParser;
p.FunctionName = 'analyze_open_channel_wall_bands_0089';
addParameter(p, 'root', '..', @(s) ischar(s) || isstring(s));
addParameter(p, 'runRoot', 'runs/open_channel_full_io_vp_tapered_flat_0089_g30', @(s) ischar(s) || isstring(s));
...
outDir = fullfile(runRoot, 'analysis_0089_wall_bands');
...
for c = 1:numel(caseDirs)
    ...
    if isempty(frames)
        warning('No .smpcd dumps for %s; no wall-band diagnostics produced.', caseLabel);
        continue;
    end
    ...
    writetable(bandRows, fullfile(outDir, sprintf('wall_band_timeseries_%s.csv', caseLabel)));
    allBandRows = [allBandRows; bandRows]; %#ok<AGROW>

    summaryRows = local_summary_rows(caseLabel, bandRows, opts.lateFraction);
    writetable(summaryRows, fullfile(outDir, sprintf('wall_band_summary_%s.csv', caseLabel)));
    allSummaryRows = [allSummaryRows; summaryRows]; %#ok<AGROW>
end

if ~isempty(allBandRows)
    writetable(allBandRows, fullfile(outDir, 'wall_band_timeseries_all_cases.csv'));
end
if ~isempty(allSummaryRows)
    writetable(allSummaryRows, fullfile(outDir, 'wall_band_summary_all_cases.csv'));
    disp(allSummaryRows);
end
...
outletCells = max(1, round(local_get_num(params, 'inletReservoirCells', 3)));
ixInlet = 1:min(geom.Nx, inletCells);
ixOutlet = max(1, geom.Nx-outletCells+1):geom.Nx;
ixCore = (min(geom.Nx, inletCells)+1):max(min(geom.Nx, inletCells)+1, geom.Nx-outletCells);
if isempty(ixCore); ixCore = ixAll; end

iyBottom = 1:wallCells;
iyTop = (geom.Ny-wallCells+1):geom.Ny;
iyCore = (wallCells+1):(geom.Ny-wallCells);
if isempty(iyCore); iyCore = 1:geom.Ny; end

bandDef = struct();
bandDef.wallBandCells = wallCells;
bandDef.regions = struct( ...
    'name', {'inlet','interior','outlet','allX'}, ...
    'ix', {ixInlet, ixCore, ixOutlet, ixAll});
bandDef.ybands = struct( ...
    'name', {'bottom','core','top','walls','allY'}, ...
    'iy', {iyBottom, iyCore, iyTop, unique([iyBottom iyTop]), 1:geom.Ny});
end
...
metrics = {'mass','meanN','stdN','maxN','netFluxProxy','negativeFluxFraction','uyRms'};
for r = 1:numel(regions)
    Tr = T(T.region == regions(r), :);
    for m = 1:numel(metrics)
        local_plot_bands(Tr, outDir, sprintf('wallband_%s_%s', char(regions(r)), metrics{m}), metrics{m}, showFigures, closeFigures);
    end
end
end
