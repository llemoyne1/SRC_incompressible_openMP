function V = make_backward_step_hard_inlet_visual_report_0072(varargin)
%MAKE_BACKWARD_STEP_HARD_INLET_VISUAL_REPORT_0072 Systematic PNG visual diagnostics from .smpcd dumps.
%
% Creates per-case, per-frame panels for N, Ux, Uy, omega, q9 masks and low-mass ramp.

p = inputParser;
p.FunctionName = 'make_backward_step_hard_inlet_visual_report_0072';
...
addParameter(p, 'maxFramesPerCase', 10, @(x) isnumeric(x) && isscalar(x) && x >= 1);
addParameter(p, 'fields', {'N','Ux','Uy','speed','omega','q6Active','q9Active','q9OpenExcluded','q9ImmersedHalo','q9LowMassRamp','solidAny','maskCode'}, @(x) iscell(x) || isstring(x));
...
figRoot = fullfile(runRoot, 'visual_report_0072');
...
nCol = 3;
nRow = ceil(nF / nCol);
fig = figure('Visible','on','Color','w','Position',[50 50 420*nCol 360*nRow]);
for k = 1:nF
    ...
    elseif strcmpi(fld,'maskCode')
        caxis([0 8]);
    elseif any(strcmpi(fld, {'q6Active','q9Active','q9ImmersedHalo','q9OpenExcluded','q9LowMassRamp','solidAny'}))
        caxis([0 1]);
    end
end
...
maskCode(masks.q9GeoActive) = 1;
maskCode(masks.q9OpenExcluded & ~solid) = 2;
maskCode(masks.q9Halo) = 3;
