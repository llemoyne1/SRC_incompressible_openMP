function info = prepare_open_channel_cylinder_resampling_0138(varargin)
%PREPARE_OPEN_CHANNEL_CYLINDER_RESAMPLING_0138 Build an inlet/outlet channel with an immersed cylinder.
%
...
% Inlet/outlet, wallVP and immersed-cylinder couplings are configured later by params.kv.

p = inputParser;
p.FunctionName = 'prepare_open_channel_cylinder_resampling_0138';
addParameter(p, 'output', fullfile('..','init','open_channel_cylinder_resampling_0138','initial_state_open_channel_cylinder_0138.smpcd'), @(s) ischar(s) || isstring(s));
...
addParameter(p, 'cylinderCx', 1.0, @(x) isnumeric(x) && isscalar(x));
addParameter(p, 'cylinderCy', 0.5, @(x) isnumeric(x) && isscalar(x));
addParameter(p, 'cylinderR', 0.10, @(x) isnumeric(x) && isscalar(x) && x > 0);
...
if isempty(char(opt.layoutCsv))
    ...
    layoutCsv = fullfile(outDir, 'initial_open_channel_cylinder_layout_0138.csv');
else
...
if cx - R <= 0 || cx + R >= Lx || cy - R <= 0 || cy + R >= Ly
    error('prepare_open_channel_cylinder_resampling_0138:badCylinder', 'Cylinder must lie strictly inside the channel.');
end
...
if nFluidCells == 0
    error('prepare_open_channel_cylinder_resampling_0138:noFluidCells', 'Cylinder masks all cells.');
end
...
switch mode
    ...
    case {'random','heterogeneous','random_population'}
        ...
        counts(fluidIds) = raw;
    otherwise
        error('prepare_open_channel_cylinder_resampling_0138:badPopulationMode', 'Unknown populationMode: %s', mode);
end
...
if logical(opt.makePreview)
    fig = figure('Name', '0138 initial open-channel cylinder population', 'Visible', 'on'); %#ok<NASGU>
    imagesc(((0:Nx-1)+0.5)*dx, ((0:Ny-1)+0.5)*dy, counts);
    set(gca, 'YDir', 'normal'); axis equal tight; colorbar;
    title('initial cell population, open-channel cylinder'); xlabel('x'); ylabel('y');
    hold on; local_draw_circle(cx, cy, R, 'k-', 1.5); hold off;
    saveas(gcf, fullfile(layoutDir, 'initial_open_channel_cylinder_population_0138.png'));
end
...
fprintf('[0138] wrote open-channel-cylinder initial state: %s\n', output);
fprintf('[0138] grid=%dx%d fluidCells=%d gamma=%g Np=%d Lx=%g Ly=%g kBT=%g\n', Nx, Ny, nFluidCells, gamma, Np, Lx, Ly, double(opt.kBT));
fprintf('[0138] population over fluid cells: mean=%g std=%g min=%g max=%g\n', info.meanNFluidCells, info.stdNFluidCells, info.minNFluidCells, info.maxNFluidCells);
fprintf('[0138] wrote layout CSV: %s\n', layoutCsv);
end
...
switch profile
    ...
    otherwise
        error('prepare_open_channel_cylinder_resampling_0138:badProfile', 'Unknown initialProfile: %s', profile);
end
end
...
while delta ~= 0
    iter = iter + 1;
    if iter > 1000000
        error('prepare_open_channel_cylinder_resampling_0138:adjustFailed', 'Failed to adjust counts to requested total.');
    end
    if delta > 0
        cand = find(counts < nMax);
        if isempty(cand), error('prepare_open_channel_cylinder_resampling_0138:maxTooLow', 'populationMax too low.'); end
        k = cand(randi(numel(cand)));
        counts(k) = counts(k) + 1;
        delta = delta - 1;
    else
        cand = find(counts > nMin);
        if isempty(cand), error('prepare_open_channel_cylinder_resampling_0138:minTooHigh', 'populationMin too high.'); end
        k = cand(randi(numel(cand)));
        counts(k) = counts(k) - 1;
        delta = delta + 1;
    end
...
for q = 1:n
    ...
    if ~ok
        ...
        rr = sqrt((xt - cx)^2 + (yt - cy)^2);
        if rr <= R
            xt = cx + (R + 0.25 * min(dx,dy)) * (xt - cx) / max(rr, 1e-12);
            yt = cy + (R + 0.25 * min(dx,dy)) * (yt - cy) / max(rr, 1e-12);
        end
        xs(q) = min(max(xt, x0), x0+dx*(1-eps));
        ys(q) = min(max(yt, y0), y0+dy*(1-eps));
    end
end
end
