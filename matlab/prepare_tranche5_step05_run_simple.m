function prep = prepare_tranche5_step05_run_simple(overrides)
%PREPARE_TRANCHE5_STEP05_RUN_SIMPLE Prepare tranche 5 full-step inputs and a WSL shell script.
...
exeDefault = fullfile(rootDir, 'build', 'main_step05_full_step');
...
workdir = getfield_if_exists_local(overrides, 'workdir', fullfile(pwd, 'run_tranche5_step05_manual'));
...
runScriptPath = fullfile(workdir, 'run_step05.sh');
...
prep.hasType = true;
prep.hasR0 = true;
save(fullfile(workdir, 'prep_tranche5_step05.mat'), 'prep');

fprintf('\n=== prepare_tranche5_step05_run_simple ===\n');
fprintf('workdir        : %s\n', workdir);
fprintf('exePath        : %s\n', exePath);
fprintf('stateMode      : %s\n', stateInfo.mode);
