...
// - dPx_bot/top: for Couette viscosity measurement (momentum flux at walls)
    ...
    // Main loop
    cudaCheck(cudaDeviceSynchronize(), "bench pre-loop sync");
    const auto benchLoopT0_0333 = std::chrono::high_resolution_clock::now();
    ...
    cudaCheck(cudaDeviceSynchronize(), "bench post-loop sync");
    const auto benchLoopT1_0333 = std::chrono::high_resolution_clock::now();
    const double benchLoopSeconds_0333 = std::chrono::duration<double>(benchLoopT1_0333 - benchLoopT0_0333).count();
    const double benchMsPerStep_0333 = (P.nSteps > 0) ? (1000.0 * benchLoopSeconds_0333 / static_cast<double>(P.nSteps)) : 0.0;
    std::printf("BENCH_STEPS %d\n", P.nSteps);
    std::printf("BENCH_LOOP_SECONDS %.9f\n", benchLoopSeconds_0333);
    std::printf("BENCH_MS_PER_STEP %.9f\n", benchMsPerStep_0333);
