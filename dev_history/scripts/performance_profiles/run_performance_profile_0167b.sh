...
# 0167b — mass-guard buffer-only profiling probe.
# Requires an executable built from the validated 0167b base plus the 0167b buffer-only mass-guard change.
# Outputs:
#   RUN_ROOT/perf_summary_0167b.csv
#   RUN_ROOT/phase_profile_0167b.csv
#   RUN_ROOT/phase_profile_top_0167b.csv
#   RUN_ROOT/q6_cg_profile_0167b.csv
#   RUN_ROOT/q6_cg_profile_top_0167b.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0167b}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0167b.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0167b.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0167b.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0167b.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0167b.csv"
RESAMP_GUARD_CSV="$RUN_ROOT/resampling_guard_profile_0167b.csv"
RESAMP_GUARD_TOP_CSV="$RUN_ROOT/resampling_guard_profile_top_0167b.csv"
...
find_profile_file() {
  ...
  for tag in 0167b 0166 0165 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0167b-profile] root       : $ROOT_DIR
[0167b-profile] binary     : $BIN
[0167b-profile] run root   : $RUN_ROOT
[0167b-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0167b-profile] steps      : $STEPS
[0167b-profile] threads    : $THREAD_LIST
[0167b-profile] cases      : $CASE_LIST
[0167b-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0167b-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0167b.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0167b/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0167b.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0167b/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    resamp_guard_src="$out_dir/resampling_guard_profile_0167b.csv"
...
echo "[0167b-profile] wrote $PERF_CSV"
echo "[0167b-profile] wrote $PHASE_CSV"
echo "[0167b-profile] wrote $TOP_CSV"
echo "[0167b-profile] wrote $Q6_CG_CSV"
echo "[0167b-profile] wrote $Q6_CG_TOP_CSV"
echo "[0167b-profile] wrote $RESAMP_GUARD_CSV"
echo "[0167b-profile] wrote $RESAMP_GUARD_TOP_CSV"
