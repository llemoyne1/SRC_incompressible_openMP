...
# 0161 — general elliptic operator plan profiling probe for SRC/MPCD OpenMP performance work.
# Requires an executable built from the 0161 Q6/CG-instrumented sources.
# Outputs:
#   RUN_ROOT/perf_summary_0161.csv
#   RUN_ROOT/phase_profile_0161.csv
#   RUN_ROOT/phase_profile_top_0161.csv
#   RUN_ROOT/q6_cg_profile_0161.csv
#   RUN_ROOT/q6_cg_profile_top_0161.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0161}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0161.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0161.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0161.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0161.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0161.csv"

cat <<INFO
[0161-profile] root       : $ROOT_DIR
[0161-profile] binary     : $BIN
[0161-profile] run root   : $RUN_ROOT
[0161-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0161-profile] steps      : $STEPS
[0161-profile] threads    : $THREAD_LIST
[0161-profile] cases      : $CASE_LIST
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0161-profile] running $label threads=$threads"
...
PY
    ...
    if [[ -f "$out_dir/phase_profile_0161.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/phase_profile_0161.csv" >> "$PHASE_CSV"
    else
      echo "Missing phase profile: $out_dir/phase_profile_0161.csv" >&2
      exit 3
    fi

    if [[ -f "$out_dir/q6_cg_profile_0161.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/q6_cg_profile_0161.csv" >> "$Q6_CG_CSV"
    else
      echo "Missing Q6/CG profile: $out_dir/q6_cg_profile_0161.csv" >&2
      exit 3
    fi
  done
done
...
echo "[0161-profile] wrote $PERF_CSV"
echo "[0161-profile] wrote $PHASE_CSV"
echo "[0161-profile] wrote $TOP_CSV"
echo "[0161-profile] wrote $Q6_CG_CSV"
echo "[0161-profile] wrote $Q6_CG_TOP_CSV"
