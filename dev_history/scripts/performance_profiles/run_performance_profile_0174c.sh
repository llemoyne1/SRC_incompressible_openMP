...
# 0174c — post-guard deposit profiling probe.
# Requires an executable built from the validated 0171 mutation-profile baseline plus this 0174c post-thermal refresh patch.
# Outputs:
#   RUN_ROOT/perf_summary_0174c.csv
#   RUN_ROOT/phase_profile_0174c.csv
#   RUN_ROOT/phase_profile_top_0174c.csv
#   RUN_ROOT/q6_cg_profile_0174c.csv
#   RUN_ROOT/q6_cg_profile_top_0174c.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0174c}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0174c.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0174c.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0174c.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0174c.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0174c.csv"
RESAMP_GUARD_CSV="$RUN_ROOT/resampling_guard_profile_0174c.csv"
RESAMP_GUARD_TOP_CSV="$RUN_ROOT/resampling_guard_profile_top_0174c.csv"
DEPOSIT_CSV="$RUN_ROOT/deposit_profile_0174c.csv"
DEPOSIT_TOP_CSV="$RUN_ROOT/deposit_profile_top_0174c.csv"
POST_GUARD_CSV="$RUN_ROOT/post_guard_profile_0174c.csv"
POST_GUARD_TOP_CSV="$RUN_ROOT/post_guard_profile_top_0174c.csv"
POST_GUARD_EQUIV_CSV="$RUN_ROOT/post_guard_equivalence_profile_0174c.csv"
...
find_profile_file() {
  ...
  for tag in 0174c 0174b 0172 0171 0169 0168 0166 0165 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0174c-profile] root       : $ROOT_DIR
[0174c-profile] binary     : $BIN
[0174c-profile] run root   : $RUN_ROOT
[0174c-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0174c-profile] steps      : $STEPS
[0174c-profile] threads    : $THREAD_LIST
[0174c-profile] cases      : $CASE_LIST
[0174c-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0174c-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0174c/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0174c/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    resamp_guard_src=""
    if resamp_guard_src=$(find_profile_file "$out_dir" "resampling_guard_profile"); then
      cp "$resamp_guard_src" "$out_dir/resampling_guard_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$resamp_guard_src" >> "$RESAMP_GUARD_CSV"
    else
      echo "[warn] Missing resampling guard profile in $out_dir for tags 0174c/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    deposit_src=""
    if deposit_src=$(find_profile_file "$out_dir" "deposit_profile"); then
      cp "$deposit_src" "$out_dir/deposit_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
    ...
    if post_guard_src=$(find_profile_file "$out_dir" "post_guard_profile"); then
      cp "$post_guard_src" "$out_dir/post_guard_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
    ...
    if post_guard_equiv_src=$(find_profile_file "$out_dir" "post_guard_equivalence_profile"); then
      cp "$post_guard_equiv_src" "$out_dir/post_guard_equivalence_profile_0174c.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
...
echo "[0174c-profile] wrote $PERF_CSV"
echo "[0174c-profile] wrote $PHASE_CSV"
echo "[0174c-profile] wrote $TOP_CSV"
echo "[0174c-profile] wrote $Q6_CG_CSV"
echo "[0174c-profile] wrote $Q6_CG_TOP_CSV"
echo "[0174c-profile] wrote $RESAMP_GUARD_CSV"
echo "[0174c-profile] wrote $RESAMP_GUARD_TOP_CSV"
echo "[0174c-profile] wrote $DEPOSIT_CSV"
echo "[0174c-profile] wrote $DEPOSIT_TOP_CSV"
echo "[0174c-profile] wrote $POST_GUARD_CSV"
echo "[0174c-profile] wrote $POST_GUARD_TOP_CSV"
echo "[0174c-profile] wrote $POST_GUARD_EQUIV_CSV"
