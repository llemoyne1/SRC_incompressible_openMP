...
# 0159 — Q6/CG fine profiling probe for SRC/MPCD OpenMP performance work.
# Requires an executable built from the 0159 Q6/CG-instrumented sources.
# Outputs:
...
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0159}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0159.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0159.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0159.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0159.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0159.csv"

cat <<INFO
[0159-profile] root       : $ROOT_DIR
[0159-profile] binary     : $BIN
[0159-profile] run root   : $RUN_ROOT
[0159-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0159-profile] steps      : $STEPS
[0159-profile] threads    : $THREAD_LIST
[0159-profile] cases      : $CASE_LIST
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0159-profile] running $label threads=$threads"
...
PY
    ...
    if [[ -f "$out_dir/phase_profile_0159.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/phase_profile_0159.csv" >> "$PHASE_CSV"
    else
      echo "Missing phase profile: $out_dir/phase_profile_0159.csv" >&2
      exit 3
    fi

    if [[ -f "$out_dir/q6_cg_profile_0159.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/q6_cg_profile_0159.csv" >> "$Q6_CG_CSV"
    else
      echo "Missing Q6/CG profile: $out_dir/q6_cg_profile_0159.csv" >&2
      exit 3
    fi
  done
done
...
echo "[0159-profile] wrote $PERF_CSV"
echo "[0159-profile] wrote $PHASE_CSV"
echo "[0159-profile] wrote $TOP_CSV"
echo "[0159-profile] wrote $Q6_CG_CSV"
echo "[0159-profile] wrote $Q6_CG_TOP_CSV"
