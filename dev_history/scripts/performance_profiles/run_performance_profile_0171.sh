...
# 0171 — deposit fine-profile probe.
# Requires an executable built from the validated 0171 mutation-profile baseline plus this 0171 local role-write patch.
# Outputs:
#   RUN_ROOT/perf_summary_0171.csv
#   RUN_ROOT/phase_profile_0171.csv
#   RUN_ROOT/phase_profile_top_0171.csv
#   RUN_ROOT/q6_cg_profile_0171.csv
#   RUN_ROOT/q6_cg_profile_top_0171.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0171}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0171.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0171.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0171.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0171.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0171.csv"
RESAMP_GUARD_CSV="$RUN_ROOT/resampling_guard_profile_0171.csv"
RESAMP_GUARD_TOP_CSV="$RUN_ROOT/resampling_guard_profile_top_0171.csv"
DEPOSIT_CSV="$RUN_ROOT/deposit_profile_0171.csv"
DEPOSIT_TOP_CSV="$RUN_ROOT/deposit_profile_top_0171.csv"
...
find_profile_file() {
  ...
  for tag in 0171 0169 0168 0166 0165 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0171-profile] root       : $ROOT_DIR
[0171-profile] binary     : $BIN
[0171-profile] run root   : $RUN_ROOT
[0171-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0171-profile] steps      : $STEPS
[0171-profile] threads    : $THREAD_LIST
[0171-profile] cases      : $CASE_LIST
[0171-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0171-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0171.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0171.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    resamp_guard_src=""
    if resamp_guard_src=$(find_profile_file "$out_dir" "resampling_guard_profile"); then
      cp "$resamp_guard_src" "$out_dir/resampling_guard_profile_0171.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$resamp_guard_src" >> "$RESAMP_GUARD_CSV"
    else
      echo "[warn] Missing resampling guard profile in $out_dir for tags 0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    deposit_src="$out_dir/deposit_profile_0171.csv"
    if [[ -f "$deposit_src" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}'         "$deposit_src" >> "$DEPOSIT_CSV"
    else
      echo "[warn] Missing deposit profile: $deposit_src" >&2
    fi
  done
done
...
echo "[0171-profile] wrote $PERF_CSV"
echo "[0171-profile] wrote $PHASE_CSV"
echo "[0171-profile] wrote $TOP_CSV"
echo "[0171-profile] wrote $Q6_CG_CSV"
echo "[0171-profile] wrote $Q6_CG_TOP_CSV"
echo "[0171-profile] wrote $RESAMP_GUARD_CSV"
echo "[0171-profile] wrote $RESAMP_GUARD_TOP_CSV"
echo "[0171-profile] wrote $DEPOSIT_CSV"
echo "[0171-profile] wrote $DEPOSIT_TOP_CSV"
