...
# 0157 — fine phase profiling probe for SRC/MPCD OpenMP performance work.
# Requires an executable built from the 0157-instrumented sources.
# Outputs:
#   RUN_ROOT/perf_summary_0157.csv
#   RUN_ROOT/phase_profile_0157.csv
#   RUN_ROOT/phase_profile_top_0157.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0157}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0157.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0157.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0157.csv"

cat <<INFO
[0157-profile] root       : $ROOT_DIR
[0157-profile] binary     : $BIN
[0157-profile] run root   : $RUN_ROOT
[0157-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0157-profile] steps      : $STEPS
[0157-profile] threads    : $THREAD_LIST
[0157-profile] cases      : $CASE_LIST
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0157-profile] running $label threads=$threads"
...
PY
    ...
    if [[ -f "$out_dir/phase_profile_0157.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/phase_profile_0157.csv" >> "$PHASE_CSV"
    else
      echo "Missing phase profile: $out_dir/phase_profile_0157.csv" >&2
      exit 3
    fi
  done
done
...
echo "[0157-profile] wrote $PERF_CSV"
echo "[0157-profile] wrote $PHASE_CSV"
echo "[0157-profile] wrote $TOP_CSV"
