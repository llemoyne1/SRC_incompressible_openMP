...
# 0175 — post-guard strict cell-id reuse probe.
# Requires an executable built from the validated 0172/0173 baseline plus this 0175 post-guard strict reuse patch.
# Outputs:
#   RUN_ROOT/perf_summary_0175.csv
#   RUN_ROOT/phase_profile_0175.csv
#   RUN_ROOT/phase_profile_top_0175.csv
#   RUN_ROOT/q6_cg_profile_0175.csv
#   RUN_ROOT/q6_cg_profile_top_0175.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0175}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0175.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0175.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0175.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0175.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0175.csv"
RESAMP_GUARD_CSV="$RUN_ROOT/resampling_guard_profile_0175.csv"
RESAMP_GUARD_TOP_CSV="$RUN_ROOT/resampling_guard_profile_top_0175.csv"
DEPOSIT_CSV="$RUN_ROOT/deposit_profile_0175.csv"
DEPOSIT_TOP_CSV="$RUN_ROOT/deposit_profile_top_0175.csv"
...
find_profile_file() {
  ...
  for tag in 0175 0171 0169 0168 0166 0165 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0175-profile] root       : $ROOT_DIR
[0175-profile] binary     : $BIN
[0175-profile] run root   : $RUN_ROOT
[0175-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0175-profile] steps      : $STEPS
[0175-profile] threads    : $THREAD_LIST
[0175-profile] cases      : $CASE_LIST
[0175-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0175-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0175.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0175/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0175.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0175/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    resamp_guard_src=""
    if resamp_guard_src=$(find_profile_file "$out_dir" "resampling_guard_profile"); then
      cp "$resamp_guard_src" "$out_dir/resampling_guard_profile_0175.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$resamp_guard_src" >> "$RESAMP_GUARD_CSV"
    else
      echo "[warn] Missing resampling guard profile in $out_dir for tags 0175/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    deposit_src=""
    if deposit_src=$(find_profile_file "$out_dir" "deposit_profile"); then
      cp "$deposit_src" "$out_dir/deposit_profile_0175.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
...
echo "[0175-profile] wrote $PERF_CSV"
echo "[0175-profile] wrote $PHASE_CSV"
echo "[0175-profile] wrote $TOP_CSV"
echo "[0175-profile] wrote $Q6_CG_CSV"
echo "[0175-profile] wrote $Q6_CG_TOP_CSV"
echo "[0175-profile] wrote $RESAMP_GUARD_CSV"
echo "[0175-profile] wrote $RESAMP_GUARD_TOP_CSV"
echo "[0175-profile] wrote $DEPOSIT_CSV"
echo "[0175-profile] wrote $DEPOSIT_TOP_CSV"
