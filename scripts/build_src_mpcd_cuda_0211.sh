...
# 0211 — CUDA-enabled SRC/MPCD executable for SRC collision active validation.
# Includes CUDA Q6, CUDA cell moments, CUDA thermostat, and CUDA SRC collision.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0211}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0211-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0211-cuda-build] Built $OUT"
