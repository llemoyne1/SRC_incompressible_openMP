...
# 0291b — CUDA SRC classic build with thermostatEnable-gated CUDA thermostat.
# This build keeps the 0291 explicit outlet regimes and makes thermostatEnable the authoritative switch for both CPU and CUDA thermostat paths. Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0291b}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0291b-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0291b-cuda-build] Built $OUT"
