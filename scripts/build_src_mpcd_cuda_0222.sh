...
# 0222 — CUDA-enabled SRC/MPCD executable with optimized CudaParticleState uploads
...
OUT=${OUT:-build/src_mpcd_base_cuda_0222}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0222-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0222-cuda-build] Built $OUT"
