...
# 0207a A/B profile for fast O(1) removal from resampling freeInactiveSlots
# during insertion.  The legacy path uses std::find + vector erase over the
# inactive-slot pool and is expected to scale poorly with INACTIVE_FACTOR.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

ROOT_RUN="${ROOT_RUN:-runs/profile_fast_insert_remove_0207a}"
CXX="${CXX:-g++}"
...
MODES_LIST="${MODES_LIST:-legacy fast}"
...
BIN="build/src_mpcd_base_fast_insert_remove_0207a"
echo "[0207a-build] $BIN"
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_fast_insert_remove_0207a.csv"
...
run_one() {
  ...
  if [[ "$mode" == "legacy" ]]; then
    export MPCD_DISABLE_FAST_RESAMPLING_INSERT_REMOVE=1
  else
  ...
  echo "[0207a-run] factor=$factor mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
print('\n[0207a-summary] wall by inactive factor')
for r in rows:
    if r['run_kind'] == 'perf':
...
print('\n[0207a-summary] phase comparison, perf rows')
...
print('\n[0207a-summary] speedup fast vs legacy')
by={}
for r in rows:
    if r['run_kind'] == 'perf':
...
for factor in sorted(set(k[0] for k in by), key=lambda x: float(x)):
    for threads in sorted(set(k[1] for k in by if k[0]==factor), key=int):
        leg=by.get((factor,threads,'legacy'))
        fast=by.get((factor,threads,'fast'))
        if leg and fast:
            print(f"factor={factor} threads={threads} wall legacy={leg:.6g}s fast={fast:.6g}s speedup={leg/fast:.4g}")

print('\n[0207a-summary] audit scans by factor')
for r in rows:
