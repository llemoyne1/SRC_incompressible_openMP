...
# Keep a safe default target: cuda_cell_moments.cu uses double atomics.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0247b}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0247b-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0247b-cuda-build] Built $OUT"
