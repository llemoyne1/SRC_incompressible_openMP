...
# 0200 — CUDA-enabled SRC/MPCD executable with Q6 CUDA and cell-moments
# shadow validation. The simulation still uses the CPU collision deposit for
# dynamics; CUDA cell moments are compiled only for optional shadow checking.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0200}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0200-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0200-cuda-build] Built $OUT"
