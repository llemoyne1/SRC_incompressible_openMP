...
# 0296 — post-SRC CUDA mass reconditioning validation.
#
# This runner validates 0296 as the first mutating but support-preserving
# post-SRC CUDA resampling brick: it must not change particle roles, support or
# case summaries on the uniform-mass validation states.  It therefore runs each current CUDA
# demo twice:
#   1. mass disabled;
#   2. mass enabled.
...
# mass_off before the 0296 module is even exercised.
#
# Scope:
#   SRC classic CUDA resident -> post-SRC mass reconditioning on physical non-shifted grid.
#   Q6/resampling/virial remain disabled in the demo scripts.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0296}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_mass_recondition_0296}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-80}
THREADS=${THREADS:-8}
RECONDITION_EVERY=${RECONDITION_EVERY:-10}
RECONDITION_STRENGTH=${RECONDITION_STRENGTH:-1.0}
# Active inlet/outlet + immersed-circle VK is not part of the default strict
# 0296 suite because the thermostat-enabled case is not bit-reproducible OFF/OFF.
# It remains available as an optional diagnostic/stress test with RUN_VK=1.
# When enabled, keep the mass summary-aligned and the thermostat disabled by
# default for strict non-mutation checks.  Set VK_THERMOSTAT_ENABLE=1 only for a
# non-verdict stress run.
VK_RECONDITION_EVERY=${VK_RECONDITION_EVERY:-$STEPS}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0296-mass] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0296.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0296.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0296-mass] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_mass_recondition_0296_manifest.csv}
printf 'caseName,massOffRoot,massOnRoot,offExitCode,onExitCode,comparedMetrics,failedMetrics,verdict,reconditionOffFiles,massOffRows,reconditionOnFiles,massOnRows,compareCsv\n' > "$OUT_CSV"
...
recondition_count() {
  ...
  python3 - "$root" <<'PY'
import glob, os, sys
files=glob.glob(os.path.join(sys.argv[1], '**', 'cuda_resampling_mass_recondition_0296.csv'), recursive=True)
rows=0
for p in files:
    try:
...
run_demo_pair() {
  local case_name=$1 script=$2 nx=$3 ny=$4 steps=$5 extra_env=${6:-} mass_every=${7:-$RECONDITION_EVERY}
  local case_art="$ART_DIR/$case_name"
  local off_root="$case_art/mass_off"
  local on_root="$case_art/mass_on"
  mkdir -p "$case_art"

  echo "[0296-mass] running case=$case_name mass=0 every=$mass_every script=$script"
  local off_rc=0
  set +e
  env BIN="$BIN" AUTO_BUILD=0 LIVE_PROGRESS="$LIVE_PROGRESS" CLEAN_RUN_ROOT="$CLEAN_RUN_ROOT" \
      ...
      MPCD_CUDA_RESAMPLING_MASS_RECONDITION_0296_EVERY="$mass_every" \
      MPCD_CUDA_RESAMPLING_MASS_RECONDITION_0296_STRENGTH="$RECONDITION_STRENGTH" \
      $extra_env bash "$script" >"$case_art/mass_off.stdout.log" 2>"$case_art/mass_off.stderr.log"
  off_rc=$?
  set -e

  echo "[0296-mass] running case=$case_name mass=1 every=$mass_every script=$script"
  local on_rc=0
  set +e
  env BIN="$BIN" AUTO_BUILD=0 LIVE_PROGRESS="$LIVE_PROGRESS" CLEAN_RUN_ROOT="$CLEAN_RUN_ROOT" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$steps" DUMP_STATE_EVERY="$steps" \
      ...
      MPCD_CUDA_RESAMPLING_MASS_RECONDITION_0296_EVERY="$mass_every" \
      MPCD_CUDA_RESAMPLING_MASS_RECONDITION_0296_STRENGTH="$RECONDITION_STRENGTH" \
      $extra_env bash "$script" >"$case_art/mass_on.stdout.log" 2>"$case_art/mass_on.stderr.log"
  on_rc=$?
  set -e

  local off_files off_rows on_files on_rows
  IFS=, read -r off_files off_rows <<<"$(recondition_count "$off_root")"
  IFS=, read -r on_files on_rows <<<"$(recondition_count "$on_root")"

  local compared=0 failed=999999 verdict=FAIL
  local compare_csv="$case_art/${case_name}_mass_off_vs_on_compare.csv"
  ...
  echo "[0296-mass] case=$case_name verdict=$verdict compared=$compared failed=$failed reconditionRowsOff=$off_rows reconditionRowsOn=$on_rows"
  if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
    echo "[0296-mass] FAIL case=$case_name" >&2
    echo "[0296-mass] stdout/stderr: $case_art/mass_off.*.log $case_art/mass_on.*.log" >&2
    exit 1
  fi
}

# Common short-grid cases.  The default strict 0296 suite is intentionally limited
...
if [[ "$RUN_TG" != "0" ]]; then
  run_demo_pair tg_periodic scripts/run_demo_src_classic_cuda_taylor_green_forced_0283.sh "$NX" "$NY" "$STEPS" "" "$RECONDITION_EVERY"
fi
if [[ "$RUN_POISEUILLE" != "0" ]]; then
  run_demo_pair poiseuille_wall scripts/run_demo_src_classic_cuda_poiseuille_periodic_forced_0283.sh "$NX" "$NY" "$STEPS" "" "$RECONDITION_EVERY"
fi
if [[ "$RUN_STEP" != "0" ]]; then
  run_demo_pair backward_step_io scripts/run_demo_src_classic_cuda_backward_step_io_0283.sh "$NX" "$NY" "$STEPS" "" "$RECONDITION_EVERY"
fi
if [[ "$RUN_SEGMENTED" != "0" ]]; then
  run_demo_pair segmented_box_same_face scripts/run_demo_src_classic_cuda_box_same_face_io_0283.sh "$NX" "$NY" "$STEPS" "OUTLET_MODE=${SEGMENTED_OUTLET_MODE:-neumann}" "$RECONDITION_EVERY"
fi
if [[ "$RUN_VK" != "0" ]]; then
  run_demo_pair von_karman_circle_io scripts/run_demo_src_classic_cuda_von_karman_cylinder_0285.sh "$NX" "$NY" "$STEPS" "UIN=${VK_UIN:-0.30} INACTIVE_SLOTS=${VK_INACTIVE_SLOTS:-$((GAMMA * NY * 32))} OUTLET_MODE=${VK_OUTLET_MODE:-equilibrium_flux} THERMOSTAT_ENABLE=${VK_THERMOSTAT_ENABLE}" "$VK_RECONDITION_EVERY"
fi
...
on_rows=sum(int(r.get('massOnRows') or 0) for r in active)
off_rows=sum(int(r.get('massOffRows') or 0) for r in active)
print(f"[0296-mass] manifest={p}")
print(f"[0296-mass] cases={len(active)} failed={len(failed)} reconditionRowsOff={off_rows} reconditionRowsOn={on_rows} verdict={'PASS' if active and not failed and off_rows == 0 and on_rows > 0 else 'FAIL'}")
if failed:
    for r in failed:
        print(f"[0296-mass] failed-case={r.get('caseName')} offRc={r.get('offExitCode')} onRc={r.get('onExitCode')} compare={r.get('compareCsv')}")
if failed or not active or off_rows != 0 or on_rows <= 0:
    raise SystemExit(1)
PY

echo "[0296-mass] wrote $OUT_CSV"
