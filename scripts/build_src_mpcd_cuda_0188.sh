...
# 0188 — build a CUDA-enabled SRC/MPCD executable for the first integrated
# Q6 projection backend test.  The regular build_src_mpcd_base.sh remains the
# CPU/OpenMP reference and does not require CUDA.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0188}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0188-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0188-cuda-build] Built $OUT"
