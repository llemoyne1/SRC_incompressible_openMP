...
# 0493x15f — historical chi-solid closure force/permeability qualification.
...
CASE_LABEL="0493x15f_mean_chivp_force_permeability"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x15f.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x15f_mean_chivp_force_permeability}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x15f] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi

if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x15f] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
...
print(f"[0493x15f] chi={path} vertical_slab_cells=[{lo},{hi}) width={w}/{nx}")
PY_X15E_CHI
...
# 0493x15f: one run-local LiveVis control remains the single source
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x15f.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x15f.txt" <<META
case=$CASE_LABEL
...
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x15f] ERROR missing chi_solid_impulse_0493x15a.csv" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x15f] ERROR missing chi_vp_impulse_0493x15b.csv" >&2; exit 2; }
[[ -s "$OUT/chi_permeability_0493x15c.csv" ]] || { echo "[0493x15f] ERROR missing chi_permeability_0493x15c.csv" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x15f] ERROR missing summary_runtime.csv" >&2; exit 2; }

echo "[0493x15f] COMPLETE run=$RUN_ROOT"
echo "[0493x15f] next: analyze from matlab/ with analyze_0493x15f_mean_chivp_force_permeability('../runs/...')"
