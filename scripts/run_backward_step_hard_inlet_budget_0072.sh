...
# 0072 hard-inlet/open-outlet backward-step budget test.
# Purpose: after the 0071 straight-channel reference relaxed toward outlet~inlet,
# test the same hard_cell_density inlet / passive outlet configuration with the
# bottom obstacle enabled. Defaults are deliberately reduced resolution but fixed
# physical size: 48x24 at Lx=2, Ly=1, 80000 steps = t=80 = 2 advective times.
# The default run is the complete liquid closure chain only: q9_virial.
...
RUN_ROOT="${RUN_ROOT:-runs/backward_step_hard_inlet_budget_0072}"
...
STATE_FILE="${STATE_FILE:-initial_state_backward_step_exact_fluid_48x24_g20_kbt0p0025_ux0p05.smpcd}"
...
Q6_PROJECTION_STRENGTH="${Q6_PROJECTION_STRENGTH:-0.50}"
PROJECTION_MAX_ITERATIONS="${PROJECTION_MAX_ITERATIONS:-700}"
PROJECTION_TOLERANCE="${PROJECTION_TOLERANCE:-1.0e-9}"
...
EOF_KV
  ...
  echo "[0072] running ${label}"
  echo "[0072] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0072] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
if [[ "$RUN_Q9" == "1" ]]; then
  write_case "backstep_q9_hard_inlet_s003_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "q9" "true" "false" "${Q9_STRENGTH_REF}"
fi
if [[ "$RUN_Q9_VIRIAL" == "1" ]]; then
  write_case "backstep_q9_virial_hard_inlet_s003_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "q9_virial" "true" "true" "${Q9_STRENGTH_REF}"
fi

elapsed=$SECONDS
printf '[0072] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0072] Analyze with: cd matlab && R = analyze_backward_step_hard_inlet_budget_0072('root','..','runRoot','${RUN_ROOT}');"
echo "[0072] Visuals with: cd matlab && V = make_backward_step_hard_inlet_visual_report_0072('root','..','runRoot','${RUN_ROOT}');"
...
if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    ...
    echo "[0072] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
