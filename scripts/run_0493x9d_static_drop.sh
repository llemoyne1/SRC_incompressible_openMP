...
CASE_LABEL="static_drop_0493x9d"
TOPOLOGY="closed_box"
...
if abs(dx-dy)>1e-12*max(1.0,abs(dx),abs(dy)):
    raise SystemExit('[0493x9d] static drop requires square cells')
r=rc*dx
area=dx*dy
pth=g*kbt/area
pl=sigma/r
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x9d_static_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_sigma${SIGMA}}"
...
LIVE_VIS_CONTROL_FILE="${LIVE_VIS_CONTROL_FILE:-$BASE_RUN_ROOT/livevis_control_0493x9d.kv}"
...
if not (math.isfinite(sigma) and sigma>=0): raise SystemExit('[0493x9d] SIGMA must be finite and non-negative')
if min(rx,ry)<=0: raise SystemExit('[0493x9d] radius must be positive')
PY_VALIDATE
...
speciesDiagnosticsFilename = species_runtime_0493x9d.csv
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x9d.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x9d.env" <<META
GAMMA=$GAMMA
R_CELLS=$R_CELLS
RX=$RX
RY=$RY
...
printf '%s\n' \
  "[0493x9d] ACTIVE Laplace jump: p_l-p_g=sigma*kappa_p3; NO particle capillary kick" \
  "[0493x9d] grid=${NX}x${NY} gamma=$GAMMA Rcells=$R_CELLS R=$RX sigma=$SIGMA" \
  "[0493x9d] pThermal=$THERMAL_PRESSURE dPLaplace=$LAPLACE_PRESSURE ratio=$(awk -v p="$LAPLACE_PRESSURE" -v q="$THERMAL_PRESSURE" 'BEGIN{printf "%.6g",p/q}')" \
  "[0493x9d] curvature=p3 binomial3x3(3)+Scharr; LiveVis curvature now displays p3" \
  "[0493x9d] gas-pressure coupling=x6g EOS reference; density tau=$Q6_GF_DENSITY_RELAXATION_TIME" \
  "[0493x9d] LIVE_PROGRESS=$LIVE_PROGRESS LIVE_VIS_ENABLE=$LIVE_VIS_ENABLE"

suite_run_binary_0434 "$PARAMS" "$LOG" "$TIME_FILE" "$OUT"

if ! suite_truthy_0434 "$PREFLIGHT_ONLY" && awk -v s="$SIGMA" 'BEGIN{exit !(s>0)}'; then
  CAP="$OUT/cuda_surface_tension_0493x9d.csv"
  SPEC="$OUT/species_runtime_0493x9d.csv"
  REPORT="$BASE_RUN_ROOT/static_drop_0493x9d.json"
  python3 scripts/analyze_0493x9d_static_drop.py \
    --capillary "$CAP" --species "$SPEC" --json "$REPORT" \
    --liquid-type "$LIQUID_TYPE" --sigma "$SIGMA" --radius "$RX" \
    --thermal-pressure "$THERMAL_PRESSURE"
  echo "[0493x9d] capillary_audit=$CAP"
  echo "[0493x9d] report=$REPORT"
fi
