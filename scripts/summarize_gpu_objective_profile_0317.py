#!/usr/bin/env python3
"""Summarize 0317 objective GPU profiling artifacts.
...
def main(argv: List[str]) -> int:
    if len(argv) != 3:
        print("usage: summarize_gpu_objective_profile_0317.py MANIFEST ART_DIR", file=sys.stderr)
        return 2
    ...
    fields = [
        ...
        "binary", "stdoutFile", "stderrFile",
    ]
    write_csv(art_dir / "gpu_objective_profile_0317_summary.csv", summary, fields)
    ...
    for x in top_a:
        ...
        x["time_s"] = f"{x['time']:.9g}"
        x["count"] = int(x["count"]) if x["count"] else ""
    write_csv(art_dir / "gpu_objective_profile_0317_top_kernels.csv", top_k,
              ["target", "name", "time_s", "count", "percentOfKernelTime", "sources"])
    write_csv(art_dir / "gpu_objective_profile_0317_top_cuda_api.csv", top_a,
