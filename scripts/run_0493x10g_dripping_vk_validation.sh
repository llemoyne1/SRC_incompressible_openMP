...
CASE_LABEL="dripping_vk_0493x10g"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
TOPOLOGY=segmented
RUN_ROOT="${RUN_ROOT:-runs/0493x10i_dripping_b5_s4500_g01}"
...
PARAMS
  ...
  printf '%s\n' \
    ...
    "[0493x10g-drip] objective=retain x9q/x9r jet necking/pinch-off/drop fall/impact at VK kBT=0.125 without evaporation; x10g global reservoir is still single-component ablation physics"
...
if [[ -f "$KINCSV" ]]; then
  python3 scripts/analyze_0493x10g_dripping_vk_validation.py "$RUN_ROOT/capillary/output"
else
