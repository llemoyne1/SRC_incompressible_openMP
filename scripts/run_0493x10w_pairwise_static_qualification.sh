...
# 0493x10w-pairwise qualification: thickness-only trigger + conservative prestream pair rotation.
...
run_case() {
    ...
    echo
    echo "===== 0493x10w-pairwise QUALIFY $tag R=$R kBT=$kbt g=$g steps=$steps ====="
    RUN_ROOT="runs/0493x10w_pairwise_${tag}_R${R}_kBT${kbt}_g${g//-/m}" \
    DROP_RADIUS_CELLS="$R" \
    KBT="$kbt" \
    GRAVITY_Y="$g" \
    STEPS="$steps" \
...
# Negative controls: eta <= etaCap, so the pairwise limiter MUST be inactive.
# R4 remains the geometric/capillary under-resolution control; R8 cold should
# remain near its already-qualified ballistic response.
...
echo
echo "===== 0493x10w-pairwise STATIC QUALIFICATION COMPLETE ====="
