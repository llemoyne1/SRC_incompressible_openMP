...
# =============================================================================
# 0493x14aq-fix1 — DARCY/CHI NOZZLE + LATERAL AMBIENT GAS RESERVOIRS (2-D)
#
...
# =============================================================================
CASE_LABEL="${CASE_LABEL:-0493x14aq_fix1_planar_gas_jet_chi_nozzle_ambient_reservoir}"
RUN_MODE="src-q6-g-f"
TOPOLOGY="segmented"
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14aq_fix1_planar_gas_jet_chi_nozzle_ambient_reservoir_s${SURFACE_TENSION_SIGMA}_seed${SEED}}"
...
TOP_SEGMENTATION_VALUES="$(python3 - \
...
# Adjacent segments intentionally share their end/start coordinate:
# la1 == lo0, lo1 == left_wall, ro0 == right_wall, ro1 == ra0.
# x14aq incorrectly required a strict gap on the right.
if not (0 <= la0 < la1 <= lo0 < lo1 <= left_wall < right_wall <= ro0 < ro1 <= ra0 < ra1 <= 1):
    raise SystemExit(
...
echo "[gas-jet-chi-nozzle-ambient] topSegments: ambientL=[$LEFT_AMBIENT_SMIN,$LEFT_AMBIENT_SMAX] outletL=[$LEFT_OUTLET_SMIN,$LEFT_OUTLET_SMAX] wallL=[$LEFT_WALL_SMIN,$LEFT_WALL_SMAX] jet=[$JET_SMIN,$JET_SMAX] wallR=[$RIGHT_WALL_SMIN,$RIGHT_WALL_SMAX] outletR=[$RIGHT_OUTLET_SMIN,$RIGHT_OUTLET_SMAX] ambientR=[$RIGHT_AMBIENT_SMIN,$RIGHT_AMBIENT_SMAX]"
...
speciesDiagnosticsFilename = species_runtime_0493x14aq_fix1.csv
...
echo "PATHS: runner=$ROOT/scripts/run_0493x14aq_fix1_planar_gas_jet_chi_nozzle_ambient_reservoir.sh"
...
OUT_TAR="$CAMPAIGN_ROOT/0493x14aq_fix1_planar_gas_jet_chi_nozzle_ambient_reservoir_compact.tar.gz"
FILES=(
  analysis
  ...
  "output/species_runtime_0493x14aq_fix1.csv"
