...
# 0493x16b demonstration variant.
# Same qualified x16b coupling as run_0493x16b_spatial_solid_load.sh, but with
...
export CASE_LABEL="${CASE_LABEL:-0493x16b_rigid_slab_dynamic_demo}"
export BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x16b_rigid_slab_dynamic_demo}"

# Preserve x16a/x16b cell size and geometric proportions while doubling each
...
export LIVE_VIS_NX="${LIVE_VIS_NX:-512}"
export LIVE_VIS_NY="${LIVE_VIS_NY:-256}"
export LIVE_VIS_EVERY="${LIVE_VIS_EVERY:-1}"
export LIVE_VIS_FIELD="${LIVE_VIS_FIELD:-chi}"
export RECORD_FIELDS="${RECORD_FIELDS:-rho,ux,uy}"
...
exec bash "$ROOT/scripts/run_0493x16b_spatial_solid_load.sh"
