...
# 0493x10w qualified architecture.
...
run_case() {
    ...
    echo
    echo "===== 0493x10w QUALIFY $tag R=$R kBT=$kbt g=$g steps=$steps ====="
    RUN_ROOT="runs/0493x10w_${tag}_R${R}_kBT${kbt}_g${g//-/m}" \
    DROP_RADIUS_CELLS="$R" \
    KBT="$kbt" \
    GRAVITY_Y="$g" \
    STEPS="$steps" \
...
echo
echo "===== 0493x10w STATIC QUALIFICATION COMPLETE ====="
