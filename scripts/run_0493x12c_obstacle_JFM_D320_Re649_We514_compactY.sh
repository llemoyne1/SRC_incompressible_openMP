...
# 0493x12c — JFM-scaled D=320h obstacle splash, compact-Y production using the existing Darcy/chi
...
#   drop D = 320 h
#   tape height H = 6 h = 0.01875 D
#   tape leading-edge distance from impact centre d = 178 h = 0.55625 D
#
# This is deliberately a geometry-scaled exploratory case, NOT yet a
# quantitative reproduction of JFM 524 (2005).  Experimental H/D, d/D, Re and
# We can be substituted later without changing the runner architecture.
#
...
CASE_LABEL="0493x12c_obstacle_JFM_D320_Re649_We514_compactY"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
TOPOLOGY=closed_box

CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x12c_obstacle_JFM_D320_Re649_We514_compactY}"
CASES="${CASES:-rmin4}"
...
# From the 3200x1600 reference recording: ymax ~=1.33 at step 1 and
# ~=1.23 at step 801.  Ly=2.25 leaves ~0.92 above the initial drop top
# and ~1.02 above the observed step-801 sheet/crown.
GAMMA="${GAMMA:-20}"
DT="${DT:-0.002}"
KBT="${KBT:-0.125}"
...
DROP_VX="${DROP_VX:-0.0}"
DROP_VY="${DROP_VY:--0.3499248423769024}"
GRAVITY_Y="${GRAVITY_Y:--0.00033667}"
SEED="${SEED:-493950}"

# Tape-step geometry in grid cells.
OBSTACLE_HEIGHT_CELLS="${OBSTACLE_HEIGHT_CELLS:-6}"
OBSTACLE_DISTANCE_CELLS="${OBSTACLE_DISTANCE_CELLS:-178}"
...
# Compact live view + higher-cadence recording for the fast impact transient.
# 800x144 is an exact 4x downsample of the 3200x576 physical grid.
LIVE_VIS_ENABLE=1
LIVE_VIS_FIELD=mass
LIVE_VIS_EVERY=1
