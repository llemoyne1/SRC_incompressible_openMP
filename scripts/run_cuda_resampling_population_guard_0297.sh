...
# 0297 — post-SRC CUDA local population guard validation.
#
# Default validation is intentionally non-triggering: the guard is called and
# writes diagnostics, but NMIN=0 and NMAX=0 disable split/merge.  This gives the
# same strict OFF/ON non-perturbation check used by 0295/0296.  Active mutation
# experiments are enabled explicitly by setting GUARD_NMIN/GUARD_NMAX.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0297}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_population_guard_0297}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-80}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0297-guard] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0297.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0297.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0297-guard] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_population_guard_0297_manifest.csv}
...
run_demo_pair() {
  ...
  echo "[0297-guard] running case=$case_name guard=0 every=$guard_every script=$script"
  local off_rc=0
  set +e
  ...
  echo "[0297-guard] running case=$case_name guard=1 every=$guard_every script=$script"
  local on_rc=0
  set +e
  env BIN="$BIN" AUTO_BUILD=0 LIVE_PROGRESS="$LIVE_PROGRESS" CLEAN_RUN_ROOT="$CLEAN_RUN_ROOT" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$steps" DUMP_STATE_EVERY="$steps" \
  ...
  echo "[0297-guard] case=$case_name verdict=$verdict compared=$compared failed=$failed guardRowsOff=$off_rows guardRowsOn=$on_rows splitOn=$on_split mergeOn=$on_merge poorOn=$on_poor richOn=$on_rich"
  if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
    echo "[0297-guard] FAIL case=$case_name" >&2
    echo "[0297-guard] stdout/stderr: $case_art/guard_off.*.log $case_art/guard_on.*.log" >&2
    exit 1
  fi
}
...
print(f"[0297-guard] manifest={p}")
print(f"[0297-guard] cases={len(active)} failed={len(failed)} guardRowsOff={off_rows} guardRowsOn={on_rows} splitOn={on_split} mergeOn={on_merge} verdict={'PASS' if active and not failed and off_rows == 0 and on_rows > 0 else 'FAIL'}")
if failed:
    for r in failed:
        print(f"[0297-guard] failed-case={r.get('caseName')} offRc={r.get('offExitCode')} onRc={r.get('onExitCode')} compare={r.get('compareCsv')}")
if failed or not active or off_rows != 0 or on_rows <= 0:
    raise SystemExit(1)
PY

echo "[0297-guard] wrote $OUT_CSV"
