#!/usr/bin/env python3
"""Aggregate CUDA resident classic SRC performance artifacts for patch 0266.

The script scans an artifact tree produced by
scripts/run_cuda_classic_src_resident_perf_0266.sh and writes two CSV files:
  * cuda_classic_src_resident_perf_summary_0266.csv — one compact row per run
  * cuda_classic_src_resident_perf_phases_0266.csv — detailed phase rows
...
def main() -> int:
    ...
    summary_out = ns.summary_out or root / "cuda_classic_src_resident_perf_summary_0266.csv"
    phases_out = ns.phases_out or root / "cuda_classic_src_resident_perf_phases_0266.csv"
    ...
    print(f"[0266-perf-summary] cases={len(summary_rows)} summary={summary_out} phases={phases_out}")
