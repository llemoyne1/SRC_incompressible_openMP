...
# 0277 — immersed solid/rectangle validation for the CUDA persistent SRC+thermostat path.
# Scope: periodic box + static rectangular immersed solid, classic SRC only.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0277}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_solid_0277}
GRID_CASES=${GRID_CASES:-"64:64:120"}
CASES=${CASES:-"periodic_rect_obstacle_classic"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SEED_BASE=${SEED_BASE:-162770}
STOP_ON_FAIL=${STOP_ON_FAIL:-1}
FORCE_REBUILD=${FORCE_REBUILD:-1}
SUMMARY_EVERY_MODE=${SUMMARY_EVERY_MODE:-final}

BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_solid_src_thermostat}
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0277_cuda_solid_src_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0277-cuda-thermostat-solid] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0277.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0277.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0277-cuda-thermostat-solid] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_solid_0277.csv}
python3 - "$OUT_CSV" <<'PY'
import csv, sys
header = [
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    "$CUDA_MODE_NAME")
      ...
      metadata_cache=1
      ;;
    *) echo "[0277-cuda-thermostat-solid] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -z "${steps:-}" ]]; then
    echo "[0277-cuda-thermostat-solid] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "periodic_rect_obstacle_classic" ]]; then
      echo "[0277-cuda-thermostat-solid] ERROR: 0277 is limited to periodic_rect_obstacle_classic / static rectangular immersed solid, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0277-cuda-thermostat-solid] running baseline case=$case_name grid=$grid_tag"
    ...
    echo "[0277-cuda-thermostat-solid] running CUDA thermostat case=$case_name grid=$grid_tag"
...
PY
    ...
    echo "[0277-cuda-thermostat-solid] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0277-cuda-thermostat-solid] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0277-cuda-thermostat-solid] wrote $OUT_CSV"
