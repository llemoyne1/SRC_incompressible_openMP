...
# 0279b — full-face inlet/outlet validation for the CUDA persistent SRC+thermostat path.
#
...
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_io_fullface_0279b}
...
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0279b_cuda_io_fullface_resident_src_fused_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0279b-cuda-thermostat-io-fullface-resident] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0280c.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0280c.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0279b-cuda-thermostat-io-fullface-resident] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_io_fullface_0279b.csv}
python3 - "$OUT_CSV" <<'PY'
import csv, sys
header = [
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    "$CUDA_MODE_NAME")
      ...
      ;;
    *) echo "[0279b-cuda-thermostat-io-fullface-resident] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${steps:-}" ]]; then
    echo "[0279b-cuda-thermostat-io-fullface-resident] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "open_rect_obstacle_full" ]]; then
      echo "[0279b-cuda-thermostat-io-fullface-resident] ERROR: 0279b is limited to open_rect_obstacle_full, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0279b-cuda-thermostat-io-fullface-resident] running baseline case=$case_name grid=$grid_tag"
    base_rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_thermostat_io_fullface_0279b_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr" || base_rc=$?
    ...
    echo "[0279b-cuda-thermostat-io-fullface-resident] running CUDA fused thermostat case=$case_name grid=$grid_tag"
    rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_thermostat_io_fullface_0279b_${case_name}_${CUDA_MODE_NAME}_${grid_tag}" "$CUDA_MODE_NAME" "$stdout_log" "$stderr_log" || rc=$?
    cmp_rc=0
...
PY
    ...
    echo "[0279b-cuda-thermostat-io-fullface-resident] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0279b-cuda-thermostat-io-fullface-resident] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0279b-cuda-thermostat-io-fullface-resident] wrote $OUT_CSV"
echo "[0279b-cuda-thermostat-io-fullface-resident] fused SRC thermostat is enabled only for this classic-only inlet/outlet validation."
