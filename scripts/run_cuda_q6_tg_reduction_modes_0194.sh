...
# 0194 — CUDA Q6 reduction-mode regression.  0193 showed that reducing the
# block sums to one scalar on device is slower for the current TG sizes.  This
# script restores host-block reduction as the default and optionally runs the
# 0193 device-scalar mode as an ablation.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0194}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_integration_0194}
GRID_CASES=${GRID_CASES:-"64:1000 128:500"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-100}
SEED_BASE=${SEED_BASE:-1620194}
CUDA_Q6_DIV_AFTER_MAX=${CUDA_Q6_DIV_AFTER_MAX:-1e-8}
RUN_DEVICE_SCALAR_ABLATION=${RUN_DEVICE_SCALAR_ABLATION:-1}
RUN_UNSUPPORTED_CHECK=${RUN_UNSUPPORTED_CHECK:-1}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0194.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0194-cuda-reduction-modes] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

SCALING_CSV=${SCALING_CSV:-$ART_DIR/cuda_q6_tg_reduction_modes_0194.csv}
...
with open(out_csv, 'a', newline='') as f:
    ...
    writer.writerow(row)
print(f"[0194-cuda-reduction-modes] PASS {label}/{mode}: speedup wall CPU/CUDA={row['speedupWallCpuOverCuda']:.3f}, cudaTimingTotal={row['cudaTimingTotalSeconds']:.3f}s, hostReduction={row['cudaTimingHostReductionSeconds']:.3f}s")
PY
}
...
run_one() {
  ...
  local cpu_root="runs/cuda_q6_tg_cpu_ref_0194_${label}"

  echo "[0194-cuda-reduction-modes] CPU reference $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
      ...
      RUN_ROOT="$cpu_root" RUN_TAG="cuda0194_cpu_${label}" PROJECTION_BACKEND=cpu \
      bash scripts/run_validation_mono_config_0162.sh

  local mode default_root compare_out compare_summary_out
  for mode in host_blocks device_scalar; do
    if [[ "$mode" == "device_scalar" && "$RUN_DEVICE_SCALAR_ABLATION" != "1" ]]; then
      continue
    fi
    default_root="runs/cuda_q6_tg_cuda_0194_${mode}_${label}"
    compare_out="$ART_DIR/cuda_q6_tg_compare_0194_${mode}_${label}.csv"
    compare_summary_out="$ART_DIR/cuda_q6_tg_compare_summary_0194_${mode}_${label}.csv"
    echo "[0194-cuda-reduction-modes] CUDA $mode $label"
    env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
        NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
        THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
        RUN_ROOT="$default_root" RUN_TAG="cuda0194_${mode}_${label}" PROJECTION_BACKEND=cuda \
        MPCD_CUDA_Q6_TIMING=1 MPCD_CUDA_Q6_DISABLE_PLAN_CACHE=0 \
        MPCD_CUDA_Q6_DEVICE_SCALAR_REDUCTION=$([[ "$mode" == "device_scalar" ]] && echo 1 || echo 0) \
        MPCD_CUDA_Q6_HOST_BLOCK_SUM=$([[ "$mode" == "host_blocks" ]] && echo 1 || echo 0) \
        MPCD_CUDA_Q6_DEBUG_SYNC="${MPCD_CUDA_Q6_DEBUG_SYNC:-0}" \
        bash scripts/run_validation_mono_config_0162.sh

    python3 scripts/compare_validation_mono_config_0162.py \
      --origin "$cpu_root" \
      --optimized "$default_root" \
      --out "$compare_out" \
      --summary-out "$compare_summary_out"

    append_scaling_row "$label" "$mode" "$nx" "$ny" "$steps" \
      "$cpu_root/validation_summary_0162.csv" \
      "$default_root/validation_summary_0162.csv" \
      "$default_root/tg_periodic_full.time"
  done
}
...
if [[ "$RUN_UNSUPPORTED_CHECK" == "1" ]]; then
  echo "[0194-cuda-reduction-modes] Checking that non-periodic Poiseuille remains rejected by projectionBackend=cuda."
  FAIL_ROOT=${FAIL_ROOT:-runs/cuda_q6_unsupported_expected_fail_0194}
  rm -rf "$FAIL_ROOT"
  set +e
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="poiseuille_wall_full" \
      RUN_ROOT="$FAIL_ROOT" RUN_TAG="cuda0194_expected_unsupported" \
      PROJECTION_BACKEND=cuda NX=16 NY=16 GAMMA=4 STEPS=1 SUMMARY_EVERY=1 THREADS=2 \
      bash scripts/run_validation_mono_config_0162.sh \
      > "$ART_DIR/cuda_q6_unsupported_stdout_0194.log" \
      2> "$ART_DIR/cuda_q6_unsupported_stderr_0194.log"
  status=$?
  set -e
  if [[ $status -eq 0 ]]; then
    echo "[0194-cuda-reduction-modes] ERROR: unsupported non-periodic CUDA Q6 case unexpectedly succeeded." >&2
    exit 1
  fi
  if ! grep -R "fully periodic, unmasked" "$ART_DIR/cuda_q6_unsupported_stderr_0194.log" "$FAIL_ROOT" >/dev/null 2>&1; then
    echo "[0194-cuda-reduction-modes] ERROR: unsupported-case failure did not contain the expected guard message." >&2
    exit 1
  fi
  echo "[0194-cuda-reduction-modes] PASS: unsupported non-periodic CUDA Q6 case failed explicitly."
fi

echo "[0194-cuda-reduction-modes] wrote $SCALING_CSV"
echo "[0194-cuda-reduction-modes] PASS"
