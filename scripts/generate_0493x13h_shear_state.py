...
def main():
    a=args()
    if min(a.Lx,a.Ly,a.kBT,a.mass,a.amplitude)<=0 or min(a.Nx,a.Ny)<8 or a.gamma<2 or a.mode_y<1:
        raise SystemExit('[0493x13h-shear-state] invalid input')
    if a.mode_y*8>a.Ny: raise SystemExit('[0493x13h-shear-state] wavelength must contain >=8 cells')
    ...
    print(f'[0493x13h-shear-state] path={a.output} N={n} grid={a.Nx}x{a.Ny} gamma={a.gamma} modeY={a.mode_y} amplitude={a.amplitude:.9g}')
