...
# 0493x15e — historical chi-solid closure force/permeability qualification.
# Reuses the qualified 0425/0426 solid-aware chi closure on the x15c slab:
#   mean_outward_bath + chiVP, alphaMax=8e5, initial deactivation chi<0.05.
# No C++/CUDA change and no new user-facing flag/parameter is introduced.
# Existing x15a/x15b/x15c diagnostics separate deterministic Brinkman, outward
# bath, chiVP and exact mid-slab permeability.

ROOT="${ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
source "$ROOT/scripts/src_mpcd_run_common_0434.sh"
suite_root_cd_0434 || exit $?

CASE_LABEL="0493x15e_historical_chi_solid_force_permeability"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x15e.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x15e_historical_chi_solid_force_permeability}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x15e] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi

if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x15e] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
...
python3 - "$CHI" "$NX" "$NY" "$SLAB_CELLS" <<'PY_X15E_CHI'
import struct, sys
...
print(f"[0493x15e] chi={path} vertical_slab_cells=[{lo},{hi}) width={w}/{nx}")
PY_X15E_CHI
...
# 0493x15e: one run-local LiveVis control remains the single source
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x15e.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x15e.txt" <<META
case=$CASE_LABEL
historicalReference=0425_0426_mean_outward_bath_chiVP
mode=$MODE
topology=$TOPOLOGY
Lx=$Lx
Ly=$Ly
...
printf '\n===== 0493x15e HISTORICAL CHI-SOLID FORCE + PERMEABILITY =====\n'
printf 'run=%s\n' "$RUN_ROOT"
...
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x15e] ERROR missing chi_solid_impulse_0493x15a.csv" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x15e] ERROR missing chi_vp_impulse_0493x15b.csv" >&2; exit 2; }
[[ -s "$OUT/chi_permeability_0493x15c.csv" ]] || { echo "[0493x15e] ERROR missing chi_permeability_0493x15c.csv" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x15e] ERROR missing summary_runtime.csv" >&2; exit 2; }

echo "[0493x15e] COMPLETE run=$RUN_ROOT"
echo "[0493x15e] next: analyze from matlab/ with analyze_0493x15e_historical_chi_solid_force_permeability('../runs/...')"
