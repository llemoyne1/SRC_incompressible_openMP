...
# 0223 — compare persistent SRC+thermostat with CudaParticleState metadata-cache upload fast path.
# The physics subset matches 0215: periodic TG, no Q6, no virial/capacity, cell-relative thermostat.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0223}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_0223}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620223}
PROJECTION_BACKEND=${PROJECTION_BACKEND:-cpu}
PROJECTION_ENABLE=${PROJECTION_ENABLE:-false}
CASE_LIST=${CASE_LIST:-tg_periodic_full}
MODES=${MODES:-"legacy_internal_upload shared_particle_state_cache_off shared_particle_state"}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0223.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0223-persistent-upload-fastpath] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_particle_state_metadata_cache_0223.csv}
printf 'grid,NX,NY,steps,mode,case,baselineWallTime,modeWallTime,wallDelta_s,wallSpeedup,failed_metrics,compared_metrics,verdict,activeCalls,totalActiveSeconds,uploadSeconds,kernelSeconds,downloadSeconds,particleStateAllocateSeconds,particleStateUploadSeconds,particleStateAllocationCalls,particleStateReusedAllocationFraction,sharedParticleStateFraction,particleStateHostToDeviceBytes,particleStateMetadataUploadCalls,particleStateMetadataCacheHits,particleStateMetadataBytesSkipped,particleStateMetadataCacheHitFraction,thermostatAppliedCalls,thermostatKBTAfterLast,activeCsv,compareCsv,compareSummary\n' > "$OUT_CSV"
...
shared_vals=vals('sharedParticleStateEnabled')
reused_vals=vals('particleStateReusedAllocation')
row={
    'grid':label,
    'NX':nx,
    'NY':ny,
    ...
    'particleStateReusedAllocationFraction':(sum(reused_vals)/len(reused_vals)) if reused_vals else 0.0,
    'sharedParticleStateFraction':(sum(shared_vals)/len(shared_vals)) if shared_vals else 0.0,
    ...
    'particleStateMetadataCacheHits':sum_i('particleStateMetadataCacheHits'),
    'particleStateMetadataBytesSkipped':sum_i('particleStateMetadataBytesSkipped'),
    'particleStateMetadataCacheHitFraction':(sum_i('particleStateMetadataCacheHits')/len(rows)) if rows else 0.0,
...
with open(out_csv,'a',newline='') as fcsv:
    ...
    w.writerow(row)
print(f"[0223-persistent-upload-fastpath] {verdict} {label}/{mode}/{case_name}: wall={mode_wall:.6g}s baseline={base_wall:.6g}s speedup={row['wallSpeedup']:.4g} shared={row['sharedParticleStateFraction']:.2f} reused={row['particleStateReusedAllocationFraction']:.2f} metaHit={row['particleStateMetadataCacheHitFraction']:.2f} failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_mode() {
  ...
  local run_root="runs/cuda_persistent_particle_state_upload_0223_${mode}_${label}"
  local shared=0 metadata_cache=1
  case "$mode" in
    legacy_internal_upload) shared=0; metadata_cache=0 ;;
    shared_particle_state_cache_off) shared=1; metadata_cache=0 ;;
    shared_particle_state) shared=1; metadata_cache=1 ;;
    *) echo "[0223-persistent-metadata-cache] ERROR: unknown mode $mode" >&2; exit 2 ;;
  esac

  echo "[0223-persistent-upload-fastpath] mode=$mode $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      RUN_ROOT="$run_root" RUN_TAG="cuda0223_${mode}_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
      ...
      MPCD_CUDA_PERSISTENT_PARTICLE_STATE_USE="$shared" \
  ...
  local compare_csv="$ART_DIR/cuda_persistent_particle_state_upload_compare_0223_${mode}_${label}.csv"
  local compare_summary="$ART_DIR/cuda_persistent_particle_state_upload_compare_summary_0223_${mode}_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    ...
    if [[ ! -f "$active_csv" ]]; then
      echo "[0223-persistent-upload-fastpath] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
run_one() {
  ...
  local baseline_root="runs/cuda_persistent_particle_state_upload_0223_cpu_baseline_${label}"

  echo "[0223-persistent-upload-fastpath] baseline $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$baseline_root" RUN_TAG="cuda0223_cpu_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
      MPCD_CUDA_CELL_MOMENTS_USE=0 \
      MPCD_CUDA_THERMOSTAT_USE=0 \
      MPCD_CUDA_SRC_COLLISION_USE=0 \
      MPCD_CUDA_PERSISTENT_SRC_COLLISION_USE=0 \
...
for item in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0223-persistent-upload-fastpath] malformed GRID_CASES entry: $item" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0223-persistent-upload-fastpath] wrote $OUT_CSV"
