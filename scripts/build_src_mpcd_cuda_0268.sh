...
# 0268 — CUDA resident classic SRC performance-optimized build.
# The feature set deliberately mirrors 0267; the new patch optimizes the
# full-face hard-cell reservoir insertion while preserving the classic-only
...
OUT=${OUT:-build/src_mpcd_base_cuda_0268}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0268-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0268-cuda-build] Built $OUT"
