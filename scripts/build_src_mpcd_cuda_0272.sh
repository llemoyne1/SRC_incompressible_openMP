...
# 0272 — CUDA resident classic SRC performance-optimized build.
# The feature set deliberately builds on 0271; this patch reduces collision-side
# resident overheads while preserving the classic-only validation scope
...
OUT=${OUT:-build/src_mpcd_base_cuda_0272}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0272-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0272-cuda-build] Built $OUT"
