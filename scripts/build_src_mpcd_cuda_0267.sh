...
# 0267 — CUDA resident classic SRC performance-optimized build.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0267}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0267-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0267-cuda-build] Built $OUT"
