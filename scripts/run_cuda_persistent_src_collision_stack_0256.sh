...
# 0256 — consolidated validation of persistent CUDA SRC collision over four
# reference physics cases. Q6/projection, thermostat, virial closure and the
# deterministic inlet reservoir remain CPU.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0256}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_src_collision_0256}
...
collision_feature_flags_for_case() {
  ...
  case "$case_name" in
    ...
    piston_virial_full)
      piston=1 ;;
    *)
      echo "[0256-persistent-src-collision-stack] ERROR: unsupported case for consolidated collision validation: $case_name" >&2
      return 2 ;;
  esac
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0256-persistent-src-collision-stack] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${nx:-}" || -z "${ny:-}" ]]; then
    echo "[0256-persistent-src-collision-stack] ERROR: GRID_CASES entries must be NX:NY:STEPS, got '$spec'" >&2
    exit 2
  fi
  ...
  for case_name in $CASES; do
    echo "[0256-persistent-src-collision-stack] running baseline case=$case_name grid=$label"
    base_root="runs/cuda_src_collision_0256_${case_name}_baseline_${label}"
    ...
    for mode in "$CELL0251_MODE_NAME" "$COLL0256_MODE_NAME"; do
      echo "[0256-persistent-src-collision-stack] running case=$case_name mode=$mode grid=$label"
      run_root="runs/cuda_src_collision_0256_${case_name}_${mode}_${label}"
      ...
      python3 - "$OUT_CSV" "$case_name" "$label" "$nx" "$ny" "$steps" "$mode" \
        ...
        "$row_wall" "$row_immersed" "$row_piston" "${WALL_THERMAL_NOISE:-0}" \
...
row=[caseName,label,nx,ny,steps,mode,resamplingCuda,boundaryCuda,cellCuda,persistentCell,persistentCollision,
...
PY
      echo "[0256-persistent-src-collision-stack] $verdict case=$case_name grid=$label mode=$mode failed=$failed/$compared wall=$mode_total baseline=$base_total"
      if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
        echo "[0256-persistent-src-collision-stack] stopping after failure; see $stderr_log" >&2
        exit 1
      fi
    done
  done
...
echo "[0256-persistent-src-collision-stack] wrote $OUT_CSV"
echo "[0256-persistent-src-collision-stack] Q6/Q9, thermostat, virial closure and inlet reservoir stayed CPU in this validation."
