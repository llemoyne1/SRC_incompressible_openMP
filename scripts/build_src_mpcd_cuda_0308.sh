...
# 0308 — CUDA SRC classic build with nominal split-safety resampling consolidation.
# This build links the full SRC classic CUDA resident path plus 0295--0307 resampling diagnostics and split-safety modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0308}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0308-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0308-cuda-build] Built $OUT"
