...
# 0402 -- Poiseuille wall-channel comparison for the resident chain
#   CUDA wall stream -> CUDA SRC collision(shared 0251) -> CUDA Q6(channel) -> CUDA thermostat.
#
# The strict regression is CPU-Q6 vs CUDA-Q6. A separate classic SRC run is kept
# as a physical reference, but is not an equality test because Q6 changes the
...
if truthy_0402 "$LIVE_VIS_ENABLE" && [[ "$USER_BIN_SET" == "0" ]]; then
  BIN=build/src_mpcd_base_cuda_q6_resident_0400_livevis
else
  BIN=${BIN:-build/src_mpcd_base_cuda_q6_resident_0400}
fi

if truthy_0402 "$FORCE_BUILD" || [[ ! -x "$BIN" ]]; then
  if truthy_0402 "$LIVE_VIS_ENABLE"; then
...
if [[ ! -x "$BIN" ]]; then
  echo "[0402-poiseuille] ERROR missing binary: $BIN" >&2
  exit 127
fi
if truthy_0402 "$LIVE_VIS_ENABLE" && [[ "$USER_BIN_SET" == "1" ]]; then
  echo "[0402-poiseuille] livevis requested with user BIN=$BIN; assuming it was built with MPCD_ENABLE_LIVE_VIS=1" >&2
fi

RESAMPLING_ENABLE=${RESAMPLING_ENABLE:-false}
if truthy_0402 "$RESAMPLING_ENABLE"; then
  echo "[0402-poiseuille] ERROR: 0402 isolates SRC/Q6 and is disabled with RESAMPLING_ENABLE=true." >&2
  exit 2
fi

TAG="${NX}x${NY}_${STEPS}"
RUN_ROOT_CPU=${RUN_ROOT_CPU:-runs/q6_resident_0402_poiseuille_cpu_${TAG}}
RUN_ROOT_CLASSIC=${RUN_ROOT_CLASSIC:-runs/q6_resident_0402_poiseuille_classic_cuda_${TAG}}
RUN_ROOT_CUDA=${RUN_ROOT_CUDA:-runs/q6_resident_0402_poiseuille_cuda_${TAG}}
ART_DIR=${ART_DIR:-dev_history/artifacts/q6_resident_0402_poiseuille_${TAG}}
mkdir -p "$ART_DIR"
...
common=(
  ...
  WALL_THERMAL_NOISE=0.0 RESAMPLING_ENABLE="$RESAMPLING_ENABLE"
)
...
if truthy_0402 "$LIVE_VIS_ENABLE"; then
  ...
  SRC_LIVE_VIS_CONTROL_FILE=${SRC_LIVE_VIS_CONTROL_FILE:-${LIVE_VIS_CONTROL_FILE:-$ROOT/livevis_control.kv}}
...
if truthy_0402 "$LIVE_VIS_ENABLE"; then
  case "$LIVE_VIS_RUN" in
    ...
    none|off|0) ;;
    *) echo "[0402-poiseuille] ERROR unknown LIVE_VIS_RUN=$LIVE_VIS_RUN (expected cpu, classic, classic_q6, all, none)" >&2; exit 2 ;;
  esac
  echo "[0402-poiseuille] livevis enabled for run=$LIVE_VIS_RUN control=${SRC_LIVE_VIS_CONTROL_FILE:-none} field=${SRC_LIVE_VIS_FIELD:-unset}"
fi

echo "[0402-poiseuille] bin=$BIN steps=$STEPS summaryEvery=$SUMMARY_EVERY dumpStateEvery=$DUMP_STATE_EVERY"
...
print(f"[0402-poiseuille] Q6 PASS: iterations={r.get('q6Iterations')} divAfter={div:.6e}")
PY2
...
print(
    "[0402-poiseuille] classic vs classic+Q6 metrics: "
...
echo "[0402-poiseuille] strict Q6 regression summary: $ART_DIR/compare_summary.csv"
echo "[0402-poiseuille] physical classic-vs-Q6 compare summary: $ART_DIR/classic_vs_q6_compare_summary.csv (rc=$classic_vs_q6_rc, differences expected)"
echo "[0402-poiseuille] physical classic-vs-Q6 selected metrics: $ART_DIR/classic_vs_q6_metrics.csv"
