...
# 0089 open-channel full inlet/outlet with VP/no-slip walls and tapered-flat inlet/outlet profile.
#
# Purpose
# -------
...
# x-face velocity/flux profile is tapered-flat in y and uses UIN as the cross-
...
RUN_ROOT="${RUN_ROOT:-runs/open_channel_full_io_vp_tapered_flat_q9_virial_excl0_0089_g30}"
...
INLET_VELOCITY_SPATIAL_PROFILE="${INLET_VELOCITY_SPATIAL_PROFILE:-flat_taper_y}"
...
STATE_FILE="${STATE_FILE:-initial_state_open_channel_full_io_vp_tapered_flat_${Nx}x${Ny}_g${GAMMA}_kbt${KBT_TAG}_ux${UX_TAG}.smpcd}"
...
# Validated Q6/Q9 model parameters from feature/elliptic-q6-core tapered-flat.
...
# by the balanced open-boundary flux pair.  The spatial profile is tapered-flat
...
EOF_KV
  ...
  echo "[0089] running ${label}"
  echo "[0089] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0089] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
echo "[0089] full-height inlet/outlet, VP/no-slip y walls, tapered-flat x-profile, no immersed solid"
echo "[0089] Umean=Uin=Uex=${UIN}; spatialProfile=${INLET_VELOCITY_SPATIAL_PROFILE}; ramp ${INLET_RAMP_INITIAL_FACTOR}->${INLET_RAMP_FINAL_FACTOR}, t=${INLET_RAMP_START_TIME}->${INLET_RAMP_END_TIME}, profile=${INLET_RAMP_PROFILE}"
echo "[0089] Q9 limiter: mode=${Q9_CORRECTION_LIMITER_MODE}, overThermal=${Q9_CORRECTION_LIMITER_OVER_THERMAL}, thermalKBT=${Q9_CORRECTION_LIMITER_THERMAL_KBT}, legacyAbs=${Q9_CORRECTION_LIMITER}"
...
elapsed=$SECONDS
printf '[0089] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0089] Analyze with: cd matlab && R = analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}','caseGlob','openchan_*');"
...
if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    ...
    echo "[0089] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
