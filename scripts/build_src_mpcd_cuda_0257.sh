...
# 0257 — performance build for persistent CUDA SRC collision with minimal post-collision download.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0257}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0257-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0257-cuda-build] Built $OUT"
