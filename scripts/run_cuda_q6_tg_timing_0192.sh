...
# 0192 — CUDA Q6 timing/scaling regression for the integrated Taylor--Green
...
BIN=${BIN:-build/src_mpcd_base_cuda_0192}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_integration_0192}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0192.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0192-cuda-timing] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

SCALING_CSV=${SCALING_CSV:-$ART_DIR/cuda_q6_tg_timing_scaling_0192.csv}
...
run_one() {
  ...
  local cpu_root="runs/cuda_q6_tg_cpu_ref_0192_${label}"
  local cuda_root="runs/cuda_q6_tg_cuda_0192_${label}"
  local compare_out="$ART_DIR/cuda_q6_tg_compare_0192_${label}.csv"
  local compare_summary_out="$ART_DIR/cuda_q6_tg_compare_summary_0192_${label}.csv"

  echo "[0192-cuda-timing] CPU reference $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
      ...
      RUN_ROOT="$cpu_root" RUN_TAG="cuda0192_cpu_${label}" PROJECTION_BACKEND=cpu \
      bash scripts/run_validation_mono_config_0162.sh

  echo "[0192-cuda-timing] CUDA instrumented $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$cuda_root" RUN_TAG="cuda0192_cuda_${label}" PROJECTION_BACKEND=cuda \
...
with open(out_csv, 'a', newline='') as f:
    ...
    writer.writerow(row)
print(f"[0192-cuda-timing] PASS {label}: speedup wall CPU/CUDA={row['speedupWallCpuOverCuda']:.3f}, cudaTimingTotal={row['cudaTimingTotalSeconds']:.3f}s")
PY
}
...
if [[ "$RUN_UNSUPPORTED_CHECK" == "1" ]]; then
  echo "[0192-cuda-timing] Checking that non-periodic Poiseuille remains rejected by projectionBackend=cuda."
  FAIL_ROOT=${FAIL_ROOT:-runs/cuda_q6_unsupported_expected_fail_0192}
  rm -rf "$FAIL_ROOT"
  set +e
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="poiseuille_wall_full" \
      RUN_ROOT="$FAIL_ROOT" RUN_TAG="cuda0192_expected_unsupported" \
      PROJECTION_BACKEND=cuda NX=16 NY=16 GAMMA=4 STEPS=1 SUMMARY_EVERY=1 THREADS=2 \
      bash scripts/run_validation_mono_config_0162.sh \
      > "$ART_DIR/cuda_q6_unsupported_stdout_0192.log" \
      2> "$ART_DIR/cuda_q6_unsupported_stderr_0192.log"
  status=$?
  set -e
  if [[ $status -eq 0 ]]; then
    echo "[0192-cuda-timing] ERROR: unsupported non-periodic CUDA Q6 case unexpectedly succeeded." >&2
    exit 1
  fi
  if ! grep -R "fully periodic, unmasked" "$ART_DIR/cuda_q6_unsupported_stderr_0192.log" "$FAIL_ROOT" >/dev/null 2>&1; then
    echo "[0192-cuda-timing] ERROR: unsupported-case failure did not contain the expected guard message." >&2
    exit 1
  fi
  echo "[0192-cuda-timing] PASS: unsupported non-periodic CUDA Q6 case failed explicitly."
fi

echo "[0192-cuda-timing] wrote $SCALING_CSV"
echo "[0192-cuda-timing] PASS"
