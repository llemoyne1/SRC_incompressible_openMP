...
export MPCD_X10N_Q6_CONTINUOUS_INTERFACE_WALL=1
export MPCD_X10L_PREWALL_INTERFACE_DIAGNOSTICS=1
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x10n_continuous_interface_static_s4500}"
...
echo "===== 0493x10n STATIC DROP / Q6-CONTINUOUS MOVING INTERFACE ====="
echo "[0493x10m] sigma=$SIGMA_ACTIVE kBT=$KBT; continuous alpha=.5 moving polyline from shared Q6-style crossings; no B8/global receiver reaction"
echo "[0493x10m] moving plane velocity UGamma.n from post-Q6/B1 liquid; boundary impulse recorded, not fed back"
echo "[0493x10k] liveEvery=$LIVE_VIS_EVERY recordEvery=$RECORD_EVERY recordFields=$RECORD_FIELDS filterSampleEvery=$FILTER_SAMPLE_EVERY hold=$LIVE_VIS_HOLD_ON_EXIT"
