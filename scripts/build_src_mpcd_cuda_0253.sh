...
# 0253 — CUDA boundary stack + persistent SRC collision wall-simple from the shared
# 0251 particle state for the periodic TG subset.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0253}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0253-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0253-cuda-build] Built $OUT"
