...
# 0213 — validate the persistent-particle CUDA SRC collision path in the real step.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0213}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_0213}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620213}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0213.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0213-persistent-src] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_collision_active_0213.csv}
...
with open(out_csv, 'a', newline='') as fcsv:
    ...
    writer.writerow(row)
print(f"[0213-persistent-src] {verdict} {label}/{case_name}: activeWall={active_wall:.6g}s baselineWall={base_wall:.6g}s totalActive={row['totalActiveSeconds']:.6g}s kernel={row['kernelSeconds']:.6g}s failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_one() {
  ...
  local base_root="runs/cuda_persistent_src_collision_0213_baseline_${label}"
  local active_root="runs/cuda_persistent_src_collision_0213_active_${label}"

  if [[ "$RUN_BASELINE" != "1" ]]; then
    echo "[0213-persistent-src] ERROR: RUN_BASELINE=0 is not supported for this harness" >&2
    exit 4
  fi

  echo "[0213-persistent-src] baseline $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      RUN_ROOT="$base_root" RUN_TAG="cuda0213_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
  ...
  echo "[0213-persistent-src] active $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$active_root" RUN_TAG="cuda0213_persistent_src_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
      MPCD_CUDA_CELL_MOMENTS_USE=0 \
      MPCD_CUDA_THERMOSTAT_USE=0 \
      MPCD_CUDA_SRC_COLLISION_USE=0 \
  ...
  local compare_csv="$ART_DIR/cuda_persistent_src_collision_compare_0213_${label}.csv"
  local compare_summary="$ART_DIR/cuda_persistent_src_collision_compare_summary_0213_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    local active_csv="$active_root/$case_name/cuda_persistent_src_collision_active_0213.csv"
    if [[ ! -f "$active_csv" ]]; then
      echo "[0213-persistent-src] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
for item in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0213-persistent-src] malformed GRID_CASES entry: $item" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0213-persistent-src] wrote $OUT_CSV"
