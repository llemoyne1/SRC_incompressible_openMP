...
# 0260 — periodic classic SRC CUDA resident build.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0260}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0260-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0260-cuda-build] Built $OUT"
