...
# 0206 — CUDA-enabled SRC/MPCD executable for thermostat shadow validation.
# Builds the previously validated CUDA Q6, CUDA cell-moment deposit, and the
# new CUDA cell-relative thermostat primitive in shadow mode.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0206}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0206-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0206-cuda-build] Built $OUT"
