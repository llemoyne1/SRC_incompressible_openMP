...
for required in \
  ...
  if [[ ! -f "$required" ]]; then
    echo "[0493x14n-fix1] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14n-fix1 — RUNNER-ONLY NO-KINETIC-REFLECTION CONTROL (NO C++/CUDA PATCH)
#
# Purpose:
#   Fast causal control for the square-drop defect seen with x14l/x14m.
#   Keep explicit gas, x6g pressure coupling, common L/G SRC collision,
#   differentiated thermostat and x9 capillarity, but disable the ENTIRE kinetic
#   interface-reflection/retention chain for this short control:
#       phaseInterfaceKineticReflectionFraction = 0
#       phaseInterfaceKineticBilateralRelocation = false
#       x10o/CIC/Q2/x10p/x10u/x10v/x10w/x12a = OFF
#       MPCD_X14L_GAS_SPECULAR_REFLECTION = 0
#   Thus neither liquid nor gas receives any kinetic interface action. Natural
#   particle crossings are allowed. This is intentionally not the final liquid
#   production chain; it is only the cheapest visual causal ablation of the gas
#   specular-reflection hypothesis.
#
# The global KBT remains GAS_KBT because x6g uses it for the gas EOS. Actual
# liquid/gas thermostat targets remain species-specific TL/TG.
#
# ./livevis_control.kv is user-owned: this runner reads/uses it and never edits it.
# =============================================================================

# -----------------------------------------------------------------------------
# USER EDIT ZONE — physical/numerical parameters are intentionally visible here.
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14n_fix1_no_kinetic_reflection_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14n-fix1] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14n-fix1] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14n-fix1] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14n-fix1] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14n_fix1_no_kinetic_reflection_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14n_fix1.csv
...
# 0493x14n-fix1 runner-only ablation: absolutely no kinetic interface action.
# r=0 avoids the historical x9t B=vacuum guard and disables the kinetic wall.
# Explicitly keep every x10/x12 kinetic closure flag OFF to make the control
# unambiguous. x9 capillarity and x6g gas pressure remain active.
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14n_fix1.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14n_fix1.env" <<META
...
PHASE_INTERFACE_KINETIC_BILATERAL_RELOCATION=false
MPCD_X14L_GAS_SPECULAR_REFLECTION=0
PHASE_INTERFACE_KINETIC_REFLECTION_FRACTION=$PHASE_INTERFACE_KINETIC_REFLECTION_FRACTION
MPCD_X10O_Q6_THERMAL_INTERFACE_WALL=0
MPCD_X10_KINETIC_INTERFACE_ONE_FOR_ONE=0
...
run_ok_surface_print_0493x13zi "x9+x6g-explicit-gas-NO-kinetic-interface-reflection"
echo "===== 0493x14n-fix1 NO KINETIC INTERFACE REFLECTION — CAUSAL CONTROL ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14n_fix1_no_kinetic_reflection_drop.sh"
...
echo "KIN-L: OFF for this causal control — no x10o/CIC/Q2/x10p/q/x10u/x10v/x12a"
echo "KIN-G: OFF — no x10u, no specular reflection, no orphan guard; natural crossings allowed"
echo "THERM: species targets TL=$LIQUID_KBT TG=$GAS_KBT ; global KBT(x6g)=$KBT"
echo "RUN:   steps=$STEPS dt=$DT summaryEvery=$SUMMARY_EVERY dumpEvery=$DUMP_STATE_EVERY 0272=1 kineticReflection=$PHASE_INTERFACE_KINETIC_REFLECTION_FRACTION bilateralGeometry=false gasSpecular=false"
echo "NOTE:  LiveVis is normal/default ON; ./livevis_control.kv is user-owned and not modified"
echo "================================================================="
...
# Keep the existing x14j analyzer untouched, but publish x14n-named copies so
# gas-closure-OFF results cannot be confused with x14j/x14l/x14m.
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14n_fix1.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14n_fix1.csv"

echo "[0493x14n-fix1] DONE"
echo "[0493x14n-fix1] return: $ANALYSIS_DIR/radial_metrics_0493x14n_fix1.csv"
echo "[0493x14n-fix1] return: $ANALYSIS_DIR/radial_profiles_0493x14n_fix1.csv"
echo "[0493x14n-fix1] return: $ANALYSIS_DIR/laplace_static_drop_0493x14n_fix1.json (if produced)"
