...
# 0266 — CUDA resident classic SRC performance-instrumented build.
# The feature set deliberately mirrors 0265c/0264; the new patch adds timing
# CSVs and runners only, not new physics.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0266}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0266-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0266-cuda-build] Built $OUT"
