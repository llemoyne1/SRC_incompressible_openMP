...
# 0230 — CUDA-enabled SRC/MPCD executable for robust resampling shadow harness.
# Same CUDA feature set as 0229; only the validation harness changes.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0230}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0230-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0230-cuda-build] Built $OUT"
