...
# 0164 — minimal resampling pool-rebuild suppression profiling probe.
# Requires an executable built from 0161 plus the 0164 minimal resampling-pool optimization.
# Outputs:
#   RUN_ROOT/perf_summary_0164.csv
#   RUN_ROOT/phase_profile_0164.csv
#   RUN_ROOT/phase_profile_top_0164.csv
#   RUN_ROOT/q6_cg_profile_0164.csv
#   RUN_ROOT/q6_cg_profile_top_0164.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0164}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0164.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0164.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0164.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0164.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0164.csv"
...
find_profile_file() {
  ...
  for tag in 0164 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0164-profile] root       : $ROOT_DIR
[0164-profile] binary     : $BIN
[0164-profile] run root   : $RUN_ROOT
[0164-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0164-profile] steps      : $STEPS
[0164-profile] threads    : $THREAD_LIST
[0164-profile] cases      : $CASE_LIST
[0164-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0164-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0164.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0164/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0164.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0164/0163/0161/0160/0159/0158/0157" >&2
    fi
  done
done
...
echo "[0164-profile] wrote $PERF_CSV"
echo "[0164-profile] wrote $PHASE_CSV"
echo "[0164-profile] wrote $TOP_CSV"
echo "[0164-profile] wrote $Q6_CG_CSV"
echo "[0164-profile] wrote $Q6_CG_TOP_CSV"
