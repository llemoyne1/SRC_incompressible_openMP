...
# 0248 — combined CUDA boundary-condition stack validation binary.
# This is intentionally a build wrapper around the already validated CUDA
# resampling/streaming/immersed modules. Q6/Q9, collision, cell moments and
# virial response remain CPU unless independently enabled by future patches.
...
# Safe default for double atomics in existing CUDA cell-moment compilation units.
# Users may override, e.g. CUDA_ARCH_FLAGS="--arch=sm_89".
...
OUT=${OUT:-build/src_mpcd_base_cuda_0248}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0248-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0248-cuda-build] Built $OUT"
