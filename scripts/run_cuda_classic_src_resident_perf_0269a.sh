...
# 0269a — performance pass for the validated CUDA resident classic SRC stack.
# This runner keeps the 0265c validation structure and profiles the 0269a
# segmented inlet/outlet hard-cell reservoir pool allocator.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0269a}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_resident_perf_0269a}
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_resident_perf_validation_0269a.csv}
THREADS=${THREADS:-8}
GAMMA=${GAMMA:-20}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0269a-perf] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0269a.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0269a.sh
fi
...
export MPCD_CUDA_RESIDENT_PROFILE_0269A=1

# Keep child validation CSV parsing from 0265c, but place all artifacts under the
# 0269a performance root and use the 0269a binary.
BIN="$BIN" \
...
python3 scripts/summarize_cuda_resident_perf_0269a.py "$ART_DIR/validation_runs" \
  --summary-out "$ART_DIR/cuda_classic_src_resident_perf_summary_0269a.csv" \
  --phases-out "$ART_DIR/cuda_classic_src_resident_perf_phases_0269a.csv"

echo "[0269a-perf] validation manifest: $OUT_CSV"
echo "[0269a-perf] performance summary: $ART_DIR/cuda_classic_src_resident_perf_summary_0269a.csv"
echo "[0269a-perf] detailed phases: $ART_DIR/cuda_classic_src_resident_perf_phases_0269a.csv"
echo "[0269a-perf] Q6/resampling remain disabled only in these classic-only validators; reactivation paths are intentionally preserved."
echo "[0269a-perf] CUDA thermostat wall/solid/piston/IO-aware remains a separate future chantier."
