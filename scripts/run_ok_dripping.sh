...
# run_ok_dripping: stand-alone qualitative liquid/vacuum dripping demonstration.
# Only scripts/src_mpcd_run_ok_common.sh is sourced.
CASE_LABEL="dripping"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
BIN="${BIN:-${SRC_MPCD_DEFAULT_BIN_0434:-build/src_mpcd_base_cuda_q6_resident_livevis_0486}}"
PREFLIGHT_ONLY="${PREFLIGHT_ONLY:-0}"
TOPOLOGY=segmented
RUN_ROOT="${RUN_ROOT:-runs/run_ok_dripping_${NX:-256}x${NY:-384}_g${GAMMA:-8}_s${SIGMA_ACTIVE:-2500.0}}"
CASES="${CASES:-capillary}"

# Reference fluid G08-0.72: square cells h=1/256 by default.
Lx="${Lx:-1.0}"; Ly="${Ly:-1.5}"; NX="${NX:-256}"; NY="${NY:-384}"
GAMMA="${GAMMA:-8}"; DT="${DT:-0.0063471328149122585}"; KBT="${KBT:-0.125}"; STEPS="${STEPS:-50000}"
SEED="${SEED:-493940}"
...
# Qualified x10/x12 flags are exported immediately before launch by run_ok_common.sh.
LIVE_VIS_RECORD_ENABLE="${LIVE_VIS_RECORD_ENABLE:-1}"
LIVE_VIS_RECORD_EVERY="${LIVE_VIS_RECORD_EVERY:-100}"
LIVE_VIS_RECORD_FIELDS="${LIVE_VIS_RECORD_FIELDS:-mass,ux,uy}"
...
ROTATION_ANGLE="${ROTATION_ANGLE:-2.0943951023931953}"; RANDOM_ROTATION_SIGN=true; GRID_SHIFT_ENABLE=true
...
if suite_truthy_0434 "$VACUUM_BACKGROUND"; then
  ...
  RUN_OK_GENERATOR_PATH="embedded:vacuum_with_liquid_slots"
...
PY
  ...
  RUN_OK_GENERATOR_PATH="$ROOT/scripts/generate_0493x9q_gas_box_state.py"
  export RUN_OK_GENERATOR_PATH
...
PARAMS
  ...
  PHASE_INTERFACE_B_SELECTOR="$PHASE_B_SELECTOR"
  ...
  printf '%s\n' \
    "[run_ok_dripping] case=$name sigma=$sigma minRadiusCells=$SURFACE_TENSION_MIN_RADIUS_CELLS contactAngle=$contact gravityY=$GRAVITY_Y inlet=top[$INLET_SMIN,$INLET_SMAX] Uin=-$UIN vents=top[$VENT_LEFT_SMIN,$VENT_LEFT_SMAX]+[$VENT_RIGHT_SMIN,$VENT_RIGHT_SMAX] mode=hybrid-zero" \
    "[run_ok_dripping] grid=${NX}x${NY} h=$(awk -v l="$Lx" -v n="$NX" 'BEGIN{printf "%.7g",l/n}') gamma=$GAMMA massRatio=$(awk -v a="$LIQUID_MASS" -v b="$GAS_MASS" 'BEGIN{printf "%.6g",a/b}') steps=$STEPS dt=$DT" \
    "[run_ok_dripping] objective=Q6-hydrodynamic normal motion + finite thermal envelope around continuous alpha=.5; no B8/global reaction"
...
if suite_truthy_0434 "${PREFLIGHT_ONLY:-0}"; then
  echo "[run_ok_dripping] PREFLIGHT_ONLY complete; analysis skipped"
  exit 0
fi
