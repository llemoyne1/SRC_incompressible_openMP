...
# 0248 — end-of-boundary-stack validation.
# Runs the four physical validation cases while enabling all CUDA boundary
# modules validated so far. Each module remains internally gated, so unsupported
# physics falls back to the CPU path.
#
# Expected dispatch by case:
#   tg_periodic_full       -> CUDA periodic force/stream 0245
#   poiseuille_wall_full   -> CUDA wall-simple force/stream 0246
#   open_rect_obstacle_full-> CPU force/stream + CPU open-boundary + CUDA rectangle reflection 0247a
#   piston_virial_full     -> CUDA moving-y-wall piston force/stream 0247b
#
# Q6/Q9, collision, cell moments, inlet/outlet hard reservoir and virial response
# stay CPU in this validation.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0248}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_boundary_stack_0248}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0248.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0248-boundary-stack] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_boundary_stack_0248.csv}
printf 'caseList,grid,NX,NY,steps,mode,resamplingCuda,periodicStreamingCuda,wallSimpleStreamingCuda,immersedRectangleCuda,pistonStreamingCuda,projectionEnable,projectionBackend,runExitCode,baselineWallTime,modeWallTime,wallDelta_s,wallSpeedup,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0248-boundary-stack] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_boundary_stack_0248_${BASELINE_MODE_NAME}_${grid}"
  base_stdout="$ART_DIR/cuda_boundary_stack_0248_${BASELINE_MODE_NAME}_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_boundary_stack_0248_${BASELINE_MODE_NAME}_${grid}.stderr.log"
  echo "[0248-boundary-stack] running baseline grid=$grid cases=[$CASE_LIST]"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0248_${BASELINE_MODE_NAME}_${grid}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
  base_rc=$?
  base_wall=$(summary_wall "$base_root/validation_summary_0162.csv")
  printf '%s,%s,%s,%s,%s,%s,0,0,0,0,0,%s,%s,%s,%s,%s,0,1,0,0,%s,%s,%s,none,none\n' \
  ...
  for mode in "$RESAMPLING_MODE_NAME" "$STACK_MODE_NAME"; do
    run_root="runs/cuda_boundary_stack_0248_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_boundary_stack_0248_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_boundary_stack_0248_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_boundary_stack_compare_0248_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_boundary_stack_compare_summary_0248_${mode}_${grid}.csv"
    echo "[0248-boundary-stack] running mode=$mode grid=$grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0248_${mode}_${grid}" "$mode" "$stdout_log" "$stderr_log"
    rc=$?
    ...
    resampling_flag=1; periodic_flag=0; wall_flag=0; immersed_flag=0; piston_flag=0
    if [[ "$mode" == "$STACK_MODE_NAME" ]]; then
      periodic_flag=1; wall_flag=1; immersed_flag=1; piston_flag=1
    fi
    python3 - <<PY >> "$OUT_CSV"
import math
...
print(f"$case_list_csv,$grid,$nx,$ny,$steps,$mode,$resampling_flag,$periodic_flag,$wall_flag,$immersed_flag,$piston_flag,$PROJECTION_ENABLE,$PROJECTION_BACKEND,$rc,{base},{wall},{delta},{speed},$failed,$compared,$verdict,$stdout_log,$stderr_log,$compare_csv,$compare_summary")
PY
    echo "[0248-boundary-stack] $verdict mode=$mode grid=$grid: wall=$wall baseline=$base_wall failed=$failed/$compared"
...
echo "[0248-boundary-stack] wrote $OUT_CSV"
echo "[0248-boundary-stack] Q6/Q9, collision, cell moments, hard inlet/outlet reservoir and virial response stayed CPU."
