...
for ic=1:n
    ...
    cp=resolvepath(cpRaw,repoRoot,fileparts(P.__path));
...
if ~isfield(P,f)
    error('x8e:paramMissing','Missing parameter %s in %s',k,P.__path);
end
x=str2double(P.(f));
if ~isfinite(x)
    error('x8e:paramBad','Invalid numeric parameter %s in %s',k,P.__path);
end
end
