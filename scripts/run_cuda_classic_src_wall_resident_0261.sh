...
# 0261 — wall-simple classic SRC CUDA resident validation/performance runner.
# Scope: Poiseuille wall-simple only, Q6/Q9/resampling/virial disabled, and
# thermostat disabled for this first resident wall step.  The CUDA wall-aware
# thermostat remains a separate future work item.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0261}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_0261}
...
BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_classic_wall_no_thermostat}
HYBRID_MODE_NAME=${HYBRID_MODE_NAME:-0257_wall_cuda_download_each_step_no_thermostat}
RESIDENT_MODE_NAME=${RESIDENT_MODE_NAME:-0261_wall_resident_classic_cuda_no_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0261-classic-src-wall-resident] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0261.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0261.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0261-classic-src-wall-resident] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_wall_resident_0261.csv}
printf 'caseName,grid,NX,NY,steps,mode,classicSrcCudaMode,residentWall0261,summaryEvery,thermostatEnable,runExitCode,baselineTotalWallTime,modeTotalWallTime,totalWallDelta_s,totalWallSpeedup,baselineMaxCaseWallTime,modeMaxCaseWallTime,maxCaseWallDelta_s,maxCaseWallSpeedup,collisionActiveCalls,collisionTotalSeconds,collisionUploadSeconds,collisionKernelSeconds,collisionDownloadSeconds,collisionSharedParticleStateFraction,collisionSharedCellWorkspaceFraction,thermostatGpuAppliedFraction,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  local wall_stream=0 wall_download=1 resident=0 persistent_collision=0 collision_shared=0 minimal_download=0
  case "$mode" in
    "$BASELINE_MODE_NAME") ;;
    "$HYBRID_MODE_NAME")
      wall_stream=1; wall_download=1; persistent_collision=1; collision_shared=1; minimal_download=1 ;;
    "$RESIDENT_MODE_NAME")
      wall_stream=1; wall_download=0; resident=1; persistent_collision=1; collision_shared=1; minimal_download=1 ;;
    *) echo "[0261-classic-src-wall-resident] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$case_name" WALL_THERMAL_NOISE=0 \
      ...
      MPCD_CUDA_CLASSIC_SRC_WALL_RESIDENT_0261="$resident" \
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -z "${steps:-}" ]]; then
    echo "[0261-classic-src-wall-resident] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "poiseuille_wall_full" ]]; then
      echo "[0261-classic-src-wall-resident] ERROR: 0261 is limited to poiseuille_wall_full / wall-simple CL, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0261-classic-src-wall-resident] running baseline case=$case_name grid=$grid_tag"
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_classic_src_0261_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
    base_rc=$?
    ...
    for mode in "$HYBRID_MODE_NAME" "$RESIDENT_MODE_NAME"; do
      ...
      echo "[0261-classic-src-wall-resident] running case=$case_name mode=$mode grid=$grid_tag"
      run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_classic_src_0261_${case_name}_${mode}_${grid_tag}" "$mode" "$stdout_log" "$stderr_log"
      rc=$?
      cmp_rc=0
...
PY
      ...
      echo "[0261-classic-src-wall-resident] $verdict case=$case_name grid=$grid_tag mode=$mode failed=$failed/$compared wall=$mode_total baseline=$base_total"
      if [[ "$STOP_ON_FAIL" == "1" && ( "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
        echo "[0261-classic-src-wall-resident] stopping after failure; see $stderr_log" >&2
        exit 1
      fi
    done
  done
...
echo "[0261-classic-src-wall-resident] wrote $OUT_CSV"
echo "[0261-classic-src-wall-resident] scope: wall-simple CL only; Q6/Q9/resampling/virial and thermostat disabled; host downloads deferred to summaries in resident mode."
