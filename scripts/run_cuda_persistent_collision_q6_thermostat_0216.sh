...
# 0216 — first integration step toward a persistent GPU pipeline with Q6 still isolated.
# Modes compared:
#   cpu_baseline                  : CPU collision + CPU Q6 + CPU thermostat
#   q6_cuda                       : CPU collision + CUDA Q6 + CPU thermostat
#   persistent_collision          : persistent CUDA deposit+SRC collision + CPU Q6 + CPU thermostat
#   persistent_collision_q6_cuda  : persistent CUDA deposit+SRC collision + CUDA Q6 + CPU thermostat
#
# This intentionally keeps the CPU boundary between persistent collision and Q6.
# The goal is to measure the cost/benefit before attempting to keep projected velocities resident on GPU.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0216}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_0216}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620216}
BATCH_SIZE=${BATCH_SIZE:-20}
CASE_LIST=${CASE_LIST:-tg_periodic_full}
CUDA_Q6_DIV_AFTER_MAX=${CUDA_Q6_DIV_AFTER_MAX:-1e-8}
STOP_ON_FAIL=${STOP_ON_FAIL:-0}
MODES=${MODES:-"cpu_baseline q6_cuda persistent_collision persistent_collision_q6_cuda"}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0216.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0216-persistent-q6] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_collision_q6_thermostat_0216.csv}
printf 'grid,mode,NX,NY,steps,batchSize,elapsed_s,wallTime,speedupWallBaselineOverMode,failed_metrics,compared_metrics,verdict,q6Iterations,q6DivAfterProjectedFluxRms,persistentActiveCalls,persistentTotalSeconds,persistentUploadSeconds,persistentKernelSeconds,persistentDownloadSeconds,persistentInvalidCellParticles,persistentParticlesVisitedPerCall,persistentFluidParticlesPerCall,persistentParticlesRotatedPerCall,cudaTimingSolves,cudaTimingIterations,cudaTimingBatches,cudaTimingConvergenceDownloads,cudaTimingTotalSeconds,cudaTimingHostReductionSeconds,cudaTimingApplyOperatorSeconds,runRoot,compareCsv,compareSummary\n' > "$OUT_CSV"

mode_flags() {
  local mode=$1
  case "$mode" in
    cpu_baseline)                 echo "cpu 0" ;;
    q6_cuda)                      echo "cuda 0" ;;
    persistent_collision)         echo "cpu 1" ;;
    persistent_collision_q6_cuda) echo "cuda 1" ;;
    *) echo "[0216-persistent-q6] unknown mode '$mode'" >&2; return 2 ;;
  esac
}

run_validation() {
  local nx=$1 ny=$2 steps=$3 root=$4 tag=$5 projection=$6 persistent_collision=$7
...
if mode in ('q6_cuda','persistent_collision_q6_cuda') and (not math.isfinite(q6_div) or q6_div > div_max):
    verdict = 'FAIL'
    failed = max(failed, 1)
...
if os.path.exists(persistent_csv):
    with open(persistent_csv, newline='') as fp:
        persistent_rows = list(csv.DictReader(fp))
...
row = {
    ...
    'NY': ny,
    'steps': steps,
    'batchSize': batch_size if mode in ('q6_cuda','persistent_collision_q6_cuda') else 0,
...
with open(out_csv, 'a', newline='') as fcsv:
    ...
    writer.writerow(row)
print(f"[0216-persistent-q6] {verdict} {label}/{mode}: wall={wall:.6g}s baselineSpeed={row['speedupWallBaselineOverMode']:.3f} failed={failed}/{compared} persistent={row['persistentTotalSeconds']:.3g}s q6={row['cudaTimingTotalSeconds']:.3g}s")
PY
}

run_and_record() {
  local label=$1 mode=$2 nx=$3 ny=$4 steps=$5 base_root=$6
  read -r projection persistent_collision < <(mode_flags "$mode")
  local root="runs/cuda_persistent_collision_q6_thermostat_0216_${mode}_${label}"
  local tag="cuda0216_${mode}_${label}"
  echo "[0216-persistent-q6] running $label/$mode projection=$projection persistentCollision=$persistent_collision"
  run_validation "$nx" "$ny" "$steps" "$root" "$tag" "$projection" "$persistent_collision"
  local compare_csv="none" compare_summary="none"
  if [[ "$mode" != "cpu_baseline" ]]; then
    local prefix="cuda_persistent_collision_q6_compare_0216_${mode}_${label}"
...
for spec in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0216-persistent-q6] invalid GRID_CASES entry '$spec' (expected NX:steps)" >&2
    exit 2
  fi
  ny=${NY_OVERRIDE:-$nx}
  label="${nx}x${ny}_s${steps}"

  base_root="runs/cuda_persistent_collision_q6_thermostat_0216_cpu_baseline_${label}"
  base_done=0
...
echo "[0216-persistent-q6] wrote $OUT_CSV"
