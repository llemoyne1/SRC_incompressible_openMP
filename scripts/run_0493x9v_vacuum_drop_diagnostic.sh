...
# 0493x9v is diagnostic-only. It runs exactly the x9u physics on the same
# stationary VK-fluid vacuum drop, but audits escape paths every SUMMARY_EVERY.
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x9v_vacuum_drop_diagnostic}"
...
# Halo was already obvious near step 500-650 in x9u. 700 steps is sufficient
# and cheaper than repeating the old 1200-step run. Extra x9v work happens only
# on step 1 and these summary steps; there is no new per-step particle pass.
...
printf '%s\n' \
  "[0493x9v-suite] DIAGNOSTIC ONLY: x9u physics unchanged" \
  "[0493x9v-suite] no new particle pass; extra geometry only on audit cadence" \
  "[0493x9v-suite] VK fluid gamma=$GAMMA dt=$DT kBT=$KBT mass=$LIQUID_MASS" \
  "[0493x9v-suite] sigma=$SIGMA_ACTIVE r=$KINETIC_REFLECTION_FRACTION steps=$STEPS summaryEvery=$SUMMARY_EVERY"

bash scripts/run_0493x9s_splash.sh

CSV="$RUN_ROOT/output/cuda_phase_kinetic_escape_diagnostic_0493x9v.csv"
if [[ ! -f "$CSV" ]]; then
  echo "[0493x9v-suite] ERROR diagnostic CSV absent: $CSV" >&2
  exit 2
fi
python3 scripts/analyze_0493x9v_escape.py --csv "$CSV"
