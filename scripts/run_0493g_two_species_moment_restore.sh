...
CASE_LABEL="two_species_moment_restore_0493g"
BIN="${BIN:-build/src_mpcd_base_cuda_q6_resident_livevis_0486}"
RUN_ROOT="${RUN_ROOT:-runs/0493g_two_species_moment_restore}"
NX="${NX:-16}"
NY="${NY:-8}"
GAMMA="${GAMMA:-10}"
...
if (( GAMMA < 4 || GAMMA % 2 != 0 )); then
  echo "[0493g] ERROR GAMMA must be even and >=4" >&2
  exit 2
fi
if (( (GAMMA - 2) % 2 != 0 || (GAMMA + 2) % 2 != 0 )); then
  echo "[0493g] ERROR per-species checkerboard counts must be integral" >&2
  exit 2
fi
if (( SHORT_STEPS < 1 )); then
  echo "[0493g] ERROR SHORT_STEPS must be >=1" >&2
  exit 2
fi
if (( GUARD_NMIN < 1 || GUARD_NMIN > GUARD_NTARGET || GUARD_NTARGET > GUARD_NMAX )); then
  echo "[0493g] ERROR invalid guard thresholds $GUARD_NMIN/$GUARD_NTARGET/$GUARD_NMAX" >&2
  exit 2
fi
...
if not (math.isfinite(total) and math.isfinite(species) and total > 0.0 and species > 0.0):
    raise SystemExit("[0493g] ERROR target masses must be finite and positive")
if abs(total - 2.0 * species) > 1.0e-12 * max(1.0, abs(total)):
    raise SystemExit("[0493g] ERROR TARGET_CELL_MASS must equal 2*SPECIES_TARGET_CELL_MASS")
PY_VALIDATE_MASS
...
STATE="$RUN_ROOT/init/two_species_neutral_checkerboard_0493g.smpcd"
...
print(
    f"[0493g-state] path={path} grid={nx}x{ny} gamma={gamma} "
...
write_params_0493g() {
  ...
  local params="$case_dir/params/params_0493g.kv"
...
EOF_PARAMS
  ...
  cat >> "$params" <<EOF_PARAMS
# 0493g: physically neutral checkerboard validating species-local moment restoration.
...
run_case_0493g() {
  ...
  params="$(write_params_0493g "$case_dir" "$mode" "$steps" "$type1_switch" "$type2_switch")"
  log="$case_dir/logs/run_0493g.log"
  time_file="$case_dir/logs/time_0493g.txt"
  ...
  if ! suite_preflight_run_ok_0492 "$params"; then
    echo "$name,FAIL,preflight" >> "$RUN_ROOT/status_0493g.csv"
    return 0
  fi
  if [[ "$PREFLIGHT_ONLY" == 1 ]]; then
    echo "$name,PASS,preflight-only" >> "$RUN_ROOT/status_0493g.csv"
    return 0
  fi
  echo "[0493g] case=$name mode=$mode steps=$steps type1=$type1_switch type2=$type2_switch"
  set +e
  ...
  echo "$name,$status,$reason" >> "$RUN_ROOT/status_0493g.csv"
  [[ "$status" == PASS ]] || tail -100 "$log" >&2 || true
}

if [[ "$ANALYZE_ONLY" != 1 ]]; then
  make_state_0493g "$STATE"
  printf 'case,status,reason\n' > "$RUN_ROOT/status_0493g.csv"
  run_case_0493g 00_no_resampling src "$SHORT_STEPS" false false
  run_case_0493g 01_both_species src-resampling "$SHORT_STEPS" true true
  run_case_0493g 02_type1_only src-resampling "$SHORT_STEPS" true false
  run_case_0493g 03_type2_only src-resampling "$SHORT_STEPS" false true
  run_case_0493g 04_no_species_enabled src-resampling "$SHORT_STEPS" false false
  cat "$RUN_ROOT/status_0493g.csv"
  if grep -q ',FAIL,' "$RUN_ROOT/status_0493g.csv"; then
    echo "[0493g] FAIL runner" >&2
    exit 2
  fi
  if [[ "$PREFLIGHT_ONLY" == 1 ]]; then
    echo "[0493g] PASS preflight"
    exit 0
  fi
fi

python3 scripts/analyze_0493g_two_species_moment_restore.py \
