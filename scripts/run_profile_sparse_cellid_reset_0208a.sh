...
# 0208a A/B profile for sparse/active-only resampling cellId reset.
# The full path resets cellId over Ntotal. The sparse path resets only current
# fluidSlots when the resampling pool is valid, avoiding inactive-slot writes.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

ROOT_RUN="${ROOT_RUN:-runs/profile_sparse_cellid_reset_0208a}"
CXX="${CXX:-g++}"
...
MODES_LIST="${MODES_LIST:-full sparse}"
...
BIN="build/src_mpcd_base_sparse_cellid_reset_0208a"
echo "[0208a-build] $BIN"
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_sparse_cellid_reset_0208a.csv"
...
run_one() {
  ...
  if [[ "$mode" == "full" ]]; then
    export MPCD_DISABLE_SPARSE_RESAMPLING_CELLID_RESET=1
  else
  ...
  echo "[0208a-run] factor=$factor mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
print('\n[0208a-summary] wall by inactive factor')
for r in rows:
    if r['run_kind'] == 'perf':
...
print('\n[0208a-summary] phase comparison, perf rows')
keys=['total','resamp_insert','resamp_extract','resamp_pool_edit','resamp_deposit_edit','stream','boundary','resamp_pool0','collision','q6','thermostat','resamp_deposit0','resamp_deposit_guard','resamp_thermal_late']
for r in rows:
...
print('\n[0208a-summary] speedup sparse vs full')
by={}
for r in rows:
    if r['run_kind'] == 'perf':
...
for factor in sorted(set(k[0] for k in by), key=lambda x: float(x)):
    for threads in sorted(set(k[1] for k in by if k[0]==factor), key=int):
        full=by.get((factor,threads,'full'))
        sparse=by.get((factor,threads,'sparse'))
        if full and sparse:
            print(f"factor={factor} threads={threads} wall full={full:.6g}s sparse={sparse:.6g}s speedup={full/sparse:.4g}")

print('\n[0208a-summary] audit scans by factor')
for r in rows:
