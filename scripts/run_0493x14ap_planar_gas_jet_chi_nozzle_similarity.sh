...
for f in "$GENERATOR" "$ANALYZER" "$SRC_Q6" "$SRC_BASE" "$SRC_PARAMS"; do
  [[ -f "$f" ]] || { echo "[gas-jet-chi-nozzle-sim] ERROR missing $f" >&2; exit 2; }
done
grep -q '0493x14ad — local-x6g-face gauge sampling' "$SRC_Q6" || {
  echo '[gas-jet-chi-nozzle-sim] ERROR current local gas-traction projection source marker not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f prestream Darcy/chi was requested but not handled' "$SRC_BASE" || {
  echo '[gas-jet-chi-nozzle-sim] ERROR Q6-g-f + resident Darcy/chi integration (0493x7g) not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f Darcy requires darcyInitialDeactivateBelowChi<0' "$SRC_PARAMS" || {
  echo '[gas-jet-chi-nozzle-sim] ERROR Q6-g-f Darcy/chi parameter contract (0493x7g) not found' >&2; exit 2;
}
grep -q -- '--incident-height-over-jet' "$ANALYZER" || {
  echo '[gas-jet-chi-nozzle-sim] ERROR local jet analyzer with --incident-height-over-jet is required' >&2; exit 2;
}

# =============================================================================
# 0493x14ap — DARCY/CHI NOZZLE, EXPERIMENTAL-SIMILARITY DEFAULTS (2-D)
#
# Purpose: retain the current gas -> liquid interaction chain while inserting a
# finite-length nozzle represented by the already-qualified Q6-g-f resident
# Darcy/Brinkman chi fictitious-domain path.
#
# Geometry:
...
# =============================================================================
CASE_LABEL="${CASE_LABEL:-0493x14ap_planar_gas_jet_chi_nozzle_similarity}"
RUN_MODE="src-q6-g-f"
TOPOLOGY="segmented"
...
case "$DARCY_FORCING_MODE" in
  ...
  *) echo "[gas-jet-chi-nozzle-sim] ERROR unsupported Q6-g-f Darcy forcing mode: $DARCY_FORCING_MODE" >&2; exit 2 ;;
esac
...
if [[ "$OUTLET_MODE" != "hybrid" ]]; then
  echo "[gas-jet-chi-nozzle-sim] ERROR first top-same-face qualification requires OUTLET_MODE=hybrid; got $OUTLET_MODE" >&2
  exit 2
fi
...
if ! awk -v x="$INLET_THERMAL_NOISE" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-sim] ERROR resident segmented inlet requires INLET_THERMAL_NOISE=0" >&2
  exit 2
fi
...
if ! awk -v x="$WALL_ACCOMMODATION" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-sim] ERROR first gas/liquid jet qualification fixes WALL_ACCOMMODATION=0" >&2
  exit 2
fi
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14ap_planar_gas_jet_chi_nozzle_similarity_s${SURFACE_TENSION_SIGMA}_seed${SEED}}"
...
hx=lx/nx; hy=ly/ny
if abs(hx-hy)>1e-12*max(1,abs(hx),abs(hy)): raise SystemExit('[gas-jet-chi-nozzle-sim] square cells required')
if abs(hx-1/256)>1e-12: raise SystemExit(f'[gas-jet-chi-nozzle-sim] first qualification keeps h=1/256, got {hx:.17g}')
if abs(bh/hy-round(bh/hy))>1e-10: raise SystemExit('[gas-jet-chi-nozzle-sim] BATH_HEIGHT must lie on a cell boundary')
W=wc*hx; smin=(xc-.5*W)/lx; smax=(xc+.5*W)/lx
if not (0.02<smin<smax<0.98): raise SystemExit('[gas-jet-chi-nozzle-sim] jet segment too close to top corners')
A=hx*hy; pref=gam*kg/A; rhoG=gam*mg/A; rhoL=gam*ml/A
cth=math.sqrt(kg/mg)*dt/hx; cjet=abs(uj)*dt/hx; ccomb=cth+cjet
if ccomb>0.80: raise SystemExit(f'[gas-jet-chi-nozzle-sim] unresolved gas flight: Cthermal+Cjet={ccomb:.6g} > 0.80 cell/step')
gabs=abs(gy)
if gy>1e-15: raise SystemExit('[gas-jet-chi-nozzle-sim] default geometry expects GRAVITY_Y <= 0')
if gabs>0:
...
if abs(hx-hy) > 1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit("[gas-jet-chi-nozzle-sim] square cells required")
if wcell < 2 or tcell < 1 or lcell < 1:
    raise SystemExit("[gas-jet-chi-nozzle-sim] require innerWidthCells>=2, wallThicknessCells>=1, lengthCells>=1")
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1)):
    if not (0.0 < v < lx):
        raise SystemExit(f"[gas-jet-chi-nozzle-sim] {name}={v} outside domain")
if not (outer0 < inner0 < inner1 < outer1):
    raise SystemExit("[gas-jet-chi-nozzle-sim] invalid nozzle x ordering")
if exit_y <= bath + 4*hy:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-sim] nozzle exit y={exit_y:.9g} too close to bath={bath:.9g}; "
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1),("exit_y",exit_y)):
    ...
    if abs(q-round(q)) > 1e-9:
        raise SystemExit(f"[gas-jet-chi-nozzle-sim] {name}={v:.17g} is not aligned with a cell face")
W=inner1-inner0
stand=exit_y-bath
...
# Top face: outlet -- chi wall -- inlet/nozzle throat -- chi wall -- outlet.
...
if not (0 <= lo0 < lo1 <= lw0 < lw1 <= ji0 < ji1 <= rw0 < rw1 <= ro0 < ro1 <= 1):
    raise SystemExit(
        "[gas-jet-chi-nozzle-sim] invalid top segmentation; expected "
...
if solid != expected or left_solid != tcell*lcell or right_solid != tcell*lcell:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-sim] chi cell-count mismatch solid={solid} expected={expected} "
...
print(
    f"[gas-jet-chi-nozzle-sim] chi={out} solidCells={solid} "
...
speciesDiagnosticsFilename = species_runtime_0493x14ap.csv
...
echo "BOUNDARIES: solid base faces; top = outlet | chi wall | inlet throat | chi wall | outlet (mode=$OUTLET_MODE)"
...
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[gas-jet-chi-nozzle-sim] PREFLIGHT_ONLY complete'
  exit 0
fi
...
OUT_TAR="$CAMPAIGN_ROOT/0493x14ap_planar_gas_jet_chi_nozzle_similarity_compact.tar.gz"
FILES=(
  analysis
  "chi/${CASE_LABEL}_${NX}x${NY}.f32"
  "chi/${CASE_LABEL}_${NX}x${NY}.f32.json"
  "output/darcy_cost_0343.csv"
  "output/species_runtime_0493x14ap.csv"
...
echo "[gas-jet-chi-nozzle-sim] COMPLETE"
echo "[gas-jet-chi-nozzle-sim] compact=$OUT_TAR"
