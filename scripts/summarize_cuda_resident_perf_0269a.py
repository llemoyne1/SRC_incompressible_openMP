#!/usr/bin/env python3
"""Aggregate CUDA resident classic SRC performance artifacts for patch 0269a.

The script scans an artifact tree produced by
scripts/run_cuda_classic_src_resident_perf_0269a.sh and writes two CSV files:
  * cuda_classic_src_resident_perf_summary_0269a.csv — one compact row per run
  * cuda_classic_src_resident_perf_phases_0269a.csv — detailed phase rows
...
def main() -> int:
    ...
    summary_out = ns.summary_out or root / "cuda_classic_src_resident_perf_summary_0269a.csv"
    phases_out = ns.phases_out or root / "cuda_classic_src_resident_perf_phases_0269a.csv"
    ...
    print(f"[0269a-perf-summary] cases={len(summary_rows)} summary={summary_out} phases={phases_out}")
