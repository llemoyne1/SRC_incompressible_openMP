...
# 0258 — validation/performance runner for the persistent CUDA SRC collision
# stack plus persistent CUDA cell-relative thermostat. Q6/Q9, virial closure
# and inlet reservoirs remain CPU; the thermostat is applied after those CPU
# stages, exactly where the baseline applies it.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0258}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_src_collision_0258}
...
COLL0257_MODE_NAME=${COLL0257_MODE_NAME:-0257_minimal_collision_download}
THERM0258_MODE_NAME=${THERM0258_MODE_NAME:-0258_persistent_collision_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0258-src-collision-thermostat] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0258.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0258.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0258-src-collision-thermostat] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_collision_thermostat_0258.csv}
printf 'caseName,grid,NX,NY,steps,mode,resamplingCuda,boundaryStackCuda,persistentSrcCollision,minimalCollisionDownload0257,persistentThermostat0258,collisionWallSimple0253,collisionImmersedRect0254,collisionPiston0255,wallThermalNoise,projectionEnable,projectionBackend,runExitCode,baselineTotalWallTime,modeTotalWallTime,totalWallDelta_s,totalWallSpeedup,baselineMaxCaseWallTime,modeMaxCaseWallTime,maxCaseWallDelta_s,maxCaseWallSpeedup,collisionActiveCalls,collisionTotalSeconds,collisionUploadSeconds,collisionKernelSeconds,collisionDownloadSeconds,collisionSharedParticleStateFraction,collisionSharedCellWorkspaceFraction,thermostatActiveCalls,thermostatTotalSeconds,thermostatUploadSeconds,thermostatKineticKernelSeconds,thermostatScaleKernelSeconds,thermostatApplyKernelSeconds,thermostatDownloadSeconds,thermostatParticlesVisitedPerCall,thermostatCellsRescaledPerCall,thermostatParticlesRescaledPerCall,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
collision_feature_flags_for_case() {
  ...
  case "$case_name" in
    ...
    *) echo "[0258-src-collision-thermostat] ERROR: unsupported case $case_name" >&2; return 2 ;;
  esac
...
run_validation_logged() {
  ...
  local boundary_cuda=0 persistent_collision=0 collision_shared_0251=0 minimal_download_0257=0 persistent_thermostat_0258=0
  ...
  case "$mode" in
    ...
    "$THERM0258_MODE_NAME")
      resampling_cuda=1; resampling_download_all=0; resampling_host_shadow=1; resampling_upload_mode=roles_only
      boundary_cuda=1; persistent_collision=1; collision_shared_0251=1; minimal_download_0257=1; persistent_thermostat_0258=1
      IFS=, read -r coll_wall coll_immersed coll_piston <<<"$(collision_feature_flags_for_case "$case_name")" ;;
    *) echo "[0258-src-collision-thermostat] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${nx:-}" || -z "${ny:-}" ]]; then
    echo "[0258-src-collision-thermostat] ERROR: GRID_CASES entries must be NX:NY:STEPS, got '$spec'" >&2
    exit 2
  fi
  ...
  for case_name in $CASES; do
    echo "[0258-src-collision-thermostat] running baseline case=$case_name grid=$label"
    base_root="runs/cuda_src_collision_0258_${case_name}_baseline_${label}"
    ...
    for mode in "$COLL0257_MODE_NAME" "$THERM0258_MODE_NAME"; do
      echo "[0258-src-collision-thermostat] running case=$case_name mode=$mode grid=$label"
      run_root="runs/cuda_src_collision_0258_${case_name}_${mode}_${label}"
      ...
      row_thermostat=0
      if [[ "$mode" == "$THERM0258_MODE_NAME" ]]; then row_thermostat=1; fi

      python3 - "$OUT_CSV" "$case_name" "$label" "$nx" "$ny" "$steps" "$mode" \
        1 1 1 1 "$row_thermostat" "$row_wall" "$row_immersed" "$row_piston" "${WALL_THERMAL_NOISE:-0}" \
...
(out, caseName, label, nx, ny, steps, mode,
 resamplingCuda, boundaryCuda, persistentCollision, minimalDownload0257, persistentThermostat0258,
...
row=[caseName,label,nx,ny,steps,mode,resamplingCuda,boundaryCuda,persistentCollision,minimalDownload0257,persistentThermostat0258,
...
PY
      echo "[0258-src-collision-thermostat] $verdict case=$case_name grid=$label mode=$mode failed=$failed/$compared wall=$mode_total baseline=$base_total"
      if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
        echo "[0258-src-collision-thermostat] stopping after failure; see $stderr_log" >&2
        exit 1
      fi
    done
  done
...
echo "[0258-src-collision-thermostat] wrote $OUT_CSV"
echo "[0258-src-collision-thermostat] Q6/Q9 and virial closure stayed CPU; 0258 applies the cell-relative thermostat on CUDA after those CPU stages."
