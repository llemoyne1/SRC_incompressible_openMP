...
SRC_IO="$ROOT/src/cuda_classic_src_io_resident_0263.cu"
for f in "$SRC_Q6" "$SRC_BASE" "$SRC_PARAMS" "$SRC_IO"; do
  [[ -f "$f" ]] || { echo "[0493x14av] ERROR missing $f" >&2; exit 2; }
done

grep -q '0493x14ad — local-x6g-face gauge sampling' "$SRC_Q6" || {
  echo '[0493x14av] ERROR x14ad local gas-traction projection marker not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f prestream Darcy/chi was requested but not handled' "$SRC_BASE" || {
  echo '[0493x14av] ERROR Q6-g-f + resident Darcy/chi integration marker not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f Darcy requires darcyInitialDeactivateBelowChi<0' "$SRC_PARAMS" || {
  echo '[0493x14av] ERROR Q6-g-f Darcy/chi parameter contract marker not found' >&2; exit 2;
}
grep -q 'allow multi-face segmented IO' "$SRC_IO" || {
  echo '[0493x14av] ERROR 0412 multi-face segmented resident IO marker not found' >&2; exit 2;
}

# =============================================================================
# 0493x14av-s1 — AIR-ASSISTED ATOMIZER DEMONSTRATION (2-D, stand-alone runner)
#
...
Lx="${Lx:-2.0}"; Ly="${Ly:-1.0}"; NX="${NX:-800}"; NY="${NY:-400}"
GAMMA="${GAMMA:-8}"
DT="${DT:-0.006}"
STEPS="${STEPS:-4000}"
...
LIQUID_MASS="${LIQUID_MASS:-1.0}"; GAS_MASS="${GAS_MASS:-1.}"
LIQUID_KBT="${LIQUID_KBT:-0.01}"; GAS_KBT="${GAS_KBT:-0.02}"
...
SURFACE_TENSION_SIGMA="${SURFACE_TENSION_SIGMA:-29.4461365748622}"
...
LIQUID_NOZZLE_CENTER_Y="${LIQUID_NOZZLE_CENTER_Y:-0.5}"
LIQUID_NOZZLE_DIAMETER_CELLS="${LIQUID_NOZZLE_DIAMETER_CELLS:-12}"
LIQUID_NOZZLE_LENGTH_CELLS="${LIQUID_NOZZLE_LENGTH_CELLS:-64}"
LIQUID_NOZZLE_WALL_CELLS="${LIQUID_NOZZLE_WALL_CELLS:-14}"
...
AIR_NOZZLE_LENGTH_CELLS="${AIR_NOZZLE_LENGTH_CELLS:-80}"
...
AIR_TOP_SPEED="${AIR_TOP_SPEED:-1.50}"
AIR_BOTTOM_SPEED="${AIR_BOTTOM_SPEED:-$AIR_TOP_SPEED}"
...
INLET_RAMP_END_TIME="${INLET_RAMP_END_TIME:-0.20}"
...
DARCY_ALPHA_MAX="${DARCY_ALPHA_MAX:-80000.0}"
DARCY_Q="${DARCY_Q:-0.1}"
...
case "$DARCY_FORCING_MODE" in
  ...
  *) echo "[0493x14av] ERROR unsupported Darcy mode=$DARCY_FORCING_MODE" >&2; exit 2 ;;
esac
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14av_air_assisted_atomizer_seed${SEED}}"
if [[ "$RESTART" == "1" ]]; then
...
DERIVED_LINE="$(python3 - \
  ...
  "$LIQUID_NU_REFERENCE" "$GAS_NU_REFERENCE" "$DARCY_ALPHA_MAX" <<'PY'
import math,sys
...
echo "===== 0493x14av AIR-ASSISTED ATOMIZER DEMO ====="
...
echo "DARCY: alpha=[$DARCY_ALPHA_MIN,$DARCY_ALPHA_MAX] lambdaSolidPerStep=$DARCY_LAMBDA forcing=$DARCY_FORCING_MODE chiCollisionVP=$DARCY_CHI_COLLISION_VP_ENABLE"
...
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[0493x14av] PREFLIGHT_ONLY complete'
  exit 0
fi
...
echo "[0493x14av] COMPLETE root=$RUN_ROOT"
echo "[0493x14av] compact=$OUT_TAR"
