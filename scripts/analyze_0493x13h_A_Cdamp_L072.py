...
def load_group(w1, row, run_root):
    ...
    reps = sorted(p for p in group.glob('rep*')
                  if p.is_dir() and (p / 'RUN_COMPLETE_0493x13h_A_Cdamp').exists())
    if not reps:
...
def main():
    ...
    p.add_argument('--campaign-root', type=Path, default=Path('runs/0493x13h_L072_qualification'))
    ...
    write_csv(output_root / 'A_Cdamp_individual_replicates_0493x13h.csv', individual)
    write_csv(output_root / 'A_Cdamp_group_statistics_0493x13h.csv', groups)
    ...
    write_csv(output_root / 'A_Cdamp_fluid_qualification_0493x13h.csv', qual)
    ...
    print(f'[{FIX_TAG}] ANALYSIS COMPLETE elapsed={total_elapsed:.2f}s output={output_root}')
