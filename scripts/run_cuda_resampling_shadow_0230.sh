...
# 0230 — robust harness for CUDA resampling shadow on real deposits.
# This fixes the 0229 ambiguity where the consolidated CSV could contain only
# the CPU baseline if the shadow run aborted before append_summary.
#
# Design:
...
BIN=${BIN:-build/src_mpcd_base_cuda_0230}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_0230}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620230}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0230.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0230-resampling-shadow] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_shadow_0230.csv}
...
print(f"[0230-resampling-shadow] {verdict} {label}/{mode}: rc={rc} wall={wall:.6g}s baseline={bwall:.6g}s shadowRows={len(shadow_rows)} shadowPass={shadow_pass} poorMismatch={shadow_poor} richMismatch={shadow_rich} failed={failed}/{compared}")
if verdict!='PASS':
    raise SystemExit(1)
PY
}
...
for spec in $GRID_CASES; do
  ...
  [[ -n "${nx:-}" && -n "${steps:-}" ]] || { echo "[0230-resampling-shadow] invalid GRID_CASES entry $spec" >&2; exit 2; }
  ny=${NY_OVERRIDE:-$nx}
  label="${nx}x${ny}_s${steps}"
  base_root="runs/cuda_resampling_shadow_0230_cpu_baseline_${label}"
  shadow_root="runs/cuda_resampling_shadow_0230_shadow_${label}"

  if [[ "$MODES" == *"cpu_baseline"* ]]; then
    stdout_log="$ART_DIR/cuda_resampling_shadow_0230_cpu_${label}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_shadow_0230_cpu_${label}.stderr.log"
    echo "[0230-resampling-shadow] running baseline $label"
    run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0230_cpu_${label}" 0 "none" "$stdout_log" "$stderr_log"; rc=$?
  ...
  if [[ "$MODES" == *"cuda_resampling_shadow"* ]]; then
    shadow_csv="$ART_DIR/cuda_resampling_shadow_detail_0230_${label}.csv"
    compare_csv="$ART_DIR/cuda_resampling_shadow_compare_0230_${label}.csv"
    compare_summary="$ART_DIR/cuda_resampling_shadow_compare_summary_0230_${label}.csv"
    stdout_log="$ART_DIR/cuda_resampling_shadow_0230_shadow_${label}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_shadow_0230_shadow_${label}.stderr.log"
    rm -f "$shadow_csv" "$compare_csv" "$compare_summary"
    echo "[0230-resampling-shadow] running shadow $label"
    run_validation_logged "$nx" "$ny" "$steps" "$shadow_root" "cuda0230_shadow_${label}" 1 "$shadow_csv" "$stdout_log" "$stderr_log"; rc=$?
...
echo "[0230-resampling-shadow] wrote $OUT_CSV"
if [[ $fail_count -ne 0 ]]; then
  echo "[0230-resampling-shadow] completed with $fail_count failing row(s); see $OUT_CSV and logs in $ART_DIR" >&2
  exit 1
fi
