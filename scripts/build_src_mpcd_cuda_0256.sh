...
# 0256 — consolidated validation build for persistent CUDA SRC collision over the four reference physics cases.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0256}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0256-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0256-cuda-build] Built $OUT"
