...
# 0242 — active-path transfer-mode validation.
# Compares:
#   1) CPU baseline
#   2) 0241 roundtrip mode: GPU edit + full download_all
#   3) 0242 host-shadow-authoritative mode: GPU edit + no full download_all
# The third mode is not a final GPU-owner architecture; it isolates the cost of
# the post-edit D2H copy while keeping the CPU-owned pipeline deterministic.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0242}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_0242}
GRID_CASES=${GRID_CASES:-"64:100 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620242}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0242.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0242-active-path] ERROR: missing binary $BIN" >&2
  exit 127
fi
...
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_persistent_active_path_0242.csv}
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0242-active-path] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_resampling_persistent_active_path_0242_cpu_baseline_${grid}"
  base_stdout="$ART_DIR/cuda_resampling_persistent_active_path_0242_cpu_baseline_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_resampling_persistent_active_path_0242_cpu_baseline_${grid}.stderr.log"
  echo "[0242-active-path] running baseline $grid"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0242_cpu_baseline_${grid}" cpu_baseline "$base_stdout" "$base_stderr"
  base_rc=$?
  ...
  for mode in persistent_active_path_0241_roundtrip persistent_active_path_0242_shadow; do
    run_root="runs/cuda_resampling_persistent_active_path_0242_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_resampling_persistent_active_path_0242_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_persistent_active_path_0242_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_resampling_persistent_active_path_compare_0242_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_resampling_persistent_active_path_compare_summary_0242_${mode}_${grid}.csv"
    echo "[0242-active-path] running $mode $grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0242_${mode}_${grid}" "$mode" "$stdout_log" "$stderr_log"
    rc=$?
...
PY
    echo "[0242-active-path] $verdict $mode $grid: wall=$wall baseline=$base_wall failed=$failed/$compared"
...
echo "[0242-active-path] wrote $OUT_CSV"
