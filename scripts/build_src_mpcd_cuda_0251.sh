...
# 0251 — CUDA boundary stack + active CUDA cell moments from the shared
# persistent particle state when that state is known to be fresh.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0251}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0251-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0251-cuda-build] Built $OUT"
