...
# 0202a A/B for resampling fluidSlots scan path.
...
ROOT_RUN="${ROOT_RUN:-runs/profile_resampling_fluid_slots_0202a}"
CXX="${CXX:-g++}"
...
BIN="build/src_mpcd_base_resampling_fluid_slots_0202a"
echo "[0202a-build] $BIN"
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_resampling_fluid_slots_0202a.csv"
...
run_one() {
  ...
  echo "[0202a-run] mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
print('\n[0202a-summary] phase comparison, perf rows')
for threads in sorted(set(k[0] for k in perf)):
    legacy=perf.get((threads,'legacy'))
    slots=perf.get((threads,'slots'))
...
print('\n[0202a-summary] audit particle-scan comparison')
