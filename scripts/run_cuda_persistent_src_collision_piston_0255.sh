...
# 0255 — validate active CUDA SRC collision for piston/closed-capacity wall subset; Q6/projection remains CPU.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0255}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_src_collision_0255}
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0255-persistent-src-collision-piston] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac

  rm -rf "$root"
  set +e
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" WALL_THERMAL_NOISE="${WALL_THERMAL_NOISE:-0}" \
...
for spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${nx:-}" || -z "${ny:-}" ]]; then
    echo "[0255-persistent-src-collision-piston] ERROR: GRID_CASES entries must be NX:NY:STEPS, got '$spec'" >&2
    exit 2
  fi
  if [[ -z "${steps:-}" ]]; then steps=300; fi
  label="${nx}x${ny}_s${steps}"
  echo "[0255-persistent-src-collision-piston] running baseline grid=$label cases=[$CASE_LIST]"
  base_root="runs/cuda_src_collision_0255_baseline_${label}"
  ...
  for mode in "$CELL0251_MODE_NAME" "$COLL0255_MODE_NAME"; do
    echo "[0255-persistent-src-collision-piston] running mode=$mode grid=$label"
    run_root="runs/cuda_src_collision_0255_${mode}_${label}"
    ...
    python3 - "$OUT_CSV" "$CASE_LIST" "$label" "$nx" "$ny" "$steps" "$mode" \
      "$row_resampling_cuda" "$row_boundary_cuda" "$row_cell_cuda" "$row_persistent_cell" "$row_persistent_collision" "${WALL_THERMAL_NOISE:-0}" \
...
(out, caseList, label, nx, ny, steps, mode,
 resamplingCuda, boundaryCuda, cellCuda, persistentCell, persistentCollision, wallThermalNoise,
...
row=[caseList,label,nx,ny,steps,mode,resamplingCuda,boundaryCuda,cellCuda,persistentCell,persistentCollision,wallThermalNoise,
...
PY
    echo "[0255-persistent-src-collision-piston] $verdict grid=$label mode=$mode failed=$failed/$compared wall=$mode_total baseline=$base_total"
    if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
      echo "[0255-persistent-src-collision-piston] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0255-persistent-src-collision-piston] wrote $OUT_CSV"
echo "[0255-persistent-src-collision-piston] Q6/Q9 stayed CPU; collision CUDA is restricted to deterministic open-rectangle immersed solid (wallThermalNoise=0)."
