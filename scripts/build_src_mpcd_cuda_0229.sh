...
# 0229 — CUDA-enabled SRC/MPCD executable with resampling shadow
# classification/compaction/plan checks on the real weighted deposit.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0229}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0229-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0229-cuda-build] Built $OUT"
