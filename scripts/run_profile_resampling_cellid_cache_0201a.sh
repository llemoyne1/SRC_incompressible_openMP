...
# 0201a A/B for intra-step resampling cellId reuse.
...
ROOT_RUN="${ROOT_RUN:-runs/profile_resampling_cellid_cache_0201a}"
CXX="${CXX:-g++}"
...
BIN="build/src_mpcd_base_resampling_cellid_cache_0201a"
echo "[0201a-build] $BIN"
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_resampling_cellid_cache_0201a.csv"
...
run_one() {
  ...
  echo "[0201a-run] mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
print('\n[0201a-summary] phase comparison, perf rows')
for threads in sorted(set(k[0] for k in perf)):
    legacy=perf.get((threads,'legacy'))
    cached=perf.get((threads,'cached'))
...
print('\n[0201a-summary] audit cell-index comparison')
