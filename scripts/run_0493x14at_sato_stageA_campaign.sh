...
PRESSURE_COVARIATE_ANALYZER="$ROOT/scripts/analyze_cavity_gas_pressure_covariate.py"
RECORDING_ANALYZER="$ROOT/scripts/analyze_0493x14at_sato_stageA_recording.py"
SRC_Q6="$ROOT/src/cuda_q6_resident_0400.cu"
SRC_BASE="$ROOT/src/src_mpcd_base.cpp"
SRC_PARAMS="$ROOT/src/params_io_base.cpp"
for f in "$GENERATOR" "$ANALYZER" "$PRESSURE_COVARIATE_ANALYZER" "$RECORDING_ANALYZER" "$SRC_Q6" "$SRC_BASE" "$SRC_PARAMS"; do
  [[ -f "$f" ]] || { echo "[gas-jet-chi-nozzle-wide] ERROR missing $f" >&2; exit 2; }
done
grep -q '0493x14ad — local-x6g-face gauge sampling' "$SRC_Q6" || {
  echo '[gas-jet-chi-nozzle-wide] ERROR current local gas-traction projection source marker not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f prestream Darcy/chi was requested but not handled' "$SRC_BASE" || {
  echo '[gas-jet-chi-nozzle-wide] ERROR Q6-g-f + resident Darcy/chi integration (0493x7g) not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f Darcy requires darcyInitialDeactivateBelowChi<0' "$SRC_PARAMS" || {
  echo '[gas-jet-chi-nozzle-wide] ERROR Q6-g-f Darcy/chi parameter contract (0493x7g) not found' >&2; exit 2;
}
grep -q -- '--incident-height-over-jet' "$ANALYZER" || {
  echo '[0493x14at] ERROR local jet analyzer with --incident-height-over-jet is required' >&2; exit 2;
}
CAMPAIGN_ANALYZER="$ROOT/scripts/analyze_0493x14at_sato_stageA_campaign.py"
[[ -f "$CAMPAIGN_ANALYZER" ]] || { echo "[0493x14at] ERROR missing $CAMPAIGN_ANALYZER" >&2; exit 2; }

# Campaign mode recursively invokes this same file in case mode. Runs are
# intentionally serial so each case can be inspected with LiveVis.
# 0493x14at-r3500: resume mode preserves already-computed cases.  A case that
# has already reached >= TARGET_STEPS is post-processed only; it is never rerun.
if [[ "${CAMPAIGN_MODE:-campaign}" == "campaign" ]]; then
  BASE_ROOT="${CAMPAIGN_BASE_ROOT:-runs/0493x14at_sato_stageA_seed${SEED:-493205}}"
  FRM_TARGETS="${FRM_TARGETS:-0.20 0.40 0.660364520158346 0.90}"
  H_OVER_D_LIST="${H_OVER_D_LIST:-0.8 1.7}"
  TARGET_STEPS="${STEPS:-4000}"
  ANALYSIS_START_STEP="${ANALYSIS_START_STEP:-3000}"
  ANALYSIS_END_STEP="${ANALYSIS_END_STEP:-4000}"
  RESUME_CAMPAIGN="${RESUME_CAMPAIGN:-0}"
  if [[ "$ANALYSIS_END_STEP" -gt "$TARGET_STEPS" ]]; then
    echo "[0493x14at] ERROR ANALYSIS_END_STEP=$ANALYSIS_END_STEP > STEPS=$TARGET_STEPS" >&2; exit 2
  fi
  if [[ "$RESUME_CAMPAIGN" == "1" ]]; then
    CLEAN_CAMPAIGN_ROOT=0
  else
    CLEAN_CAMPAIGN_ROOT="${CLEAN_CAMPAIGN_ROOT:-1}"
  fi
  if [[ "$CLEAN_CAMPAIGN_ROOT" == "1" ]]; then rm -rf "$BASE_ROOT"; fi
  mkdir -p "$BASE_ROOT"

  case_progress_step() {
    local root="$1"
    python3 - "$root" <<'PYX14AT_PROGRESS'
import re,sys
from pathlib import Path
root=Path(sys.argv[1]); best=0
for p in root.glob('logs/*.log'):
    try: txt=p.read_text(errors='replace')
    except Exception: continue
    for m in re.finditer(r'step=(\d+)/(\d+)',txt): best=max(best,int(m.group(1)))
print(best)
PYX14AT_PROGRESS
  }

  case_analysis_matches_window() {
    local root="$1" start="$2" end="$3"
    python3 - "$root" "$start" "$end" <<'PYX14AT_ANALYSIS_MATCH'
import csv,sys
from pathlib import Path
p=Path(sys.argv[1])/"analysis"/"sato_stageA_recording_summary.csv"
want=(int(sys.argv[2]),int(sys.argv[3]))
try:
    r=next(csv.DictReader(p.open()))
    got=(int(float(r.get("analysisStartStep","-1"))),int(float(r.get("analysisEndStep","-1"))))
    print(1 if got==want else 0)
except Exception:
    print(0)
PYX14AT_ANALYSIS_MATCH
  }

  for hd in $H_OVER_D_LIST; do
    for frm in $FRM_TARGETS; do
      safe_hd="${hd//./p}"; safe_frm="${frm//./p}"
      case_name="0493x14at_H${safe_hd}_Fr${safe_frm}_seed${SEED:-493205}"
      case_root="$BASE_ROOT/$case_name"
      reached=0
      [[ -d "$case_root" ]] && reached="$(case_progress_step "$case_root")"
      echo "[0493x14at] CAMPAIGN case=$case_name targetH/D=$hd targetFr_m=$frm reached=$reached targetSteps=$TARGET_STEPS"

      if [[ "$RESUME_CAMPAIGN" == "1" && "$reached" -ge "$TARGET_STEPS" ]]; then
        analyzed="$(case_analysis_matches_window "$case_root" "$ANALYSIS_START_STEP" "$ANALYSIS_END_STEP")"
        if [[ "$analyzed" == "1" ]]; then
          echo "[0493x14at] REUSE case=$case_name reached=$reached; simulation and postprocess already complete for ${ANALYSIS_START_STEP}:${ANALYSIS_END_STEP}"
        else
          echo "[0493x14at] REUSE case=$case_name reached=$reached >= $TARGET_STEPS; simulation skipped, common-window postprocess only"
          env CAMPAIGN_MODE=case CASE_LABEL="$case_name" CAMPAIGN_ROOT="$case_root" \
              TARGET_H_OVER_D="$hd" TARGET_FRM_SATO="$frm" CLEAN_RUN_ROOT=0 POSTPROCESS_ONLY=1 \
              STEPS="$TARGET_STEPS" ANALYSIS_START_STEP="$ANALYSIS_START_STEP" ANALYSIS_END_STEP="$ANALYSIS_END_STEP" \
              LIVE_VIS_ENABLE="${LIVE_VIS_ENABLE:-1}" LIVE_VIS_EVERY="${LIVE_VIS_EVERY:-1}" \
              RECORD_EVERY="${RECORD_EVERY:-100}" DUMP_STATE_EVERY="${DUMP_STATE_EVERY:-1000}" \
              bash "$ROOT/scripts/run_0493x14at_sato_stageA_campaign.sh"
        fi
      elif [[ "$RESUME_CAMPAIGN" == "1" && "$reached" -gt 0 ]]; then
        echo "[0493x14at] ERROR partial case $case_name reached only $reached < $TARGET_STEPS." >&2
        echo "[0493x14at]       Physical RESTART remains available, but automatic segment merging is intentionally not guessed." >&2
        exit 3
      else
        env CAMPAIGN_MODE=case CASE_LABEL="$case_name" CAMPAIGN_ROOT="$case_root" \
            TARGET_H_OVER_D="$hd" TARGET_FRM_SATO="$frm" CLEAN_RUN_ROOT=1 \
            STEPS="$TARGET_STEPS" ANALYSIS_START_STEP="$ANALYSIS_START_STEP" ANALYSIS_END_STEP="$ANALYSIS_END_STEP" \
            LIVE_VIS_ENABLE="${LIVE_VIS_ENABLE:-1}" LIVE_VIS_EVERY="${LIVE_VIS_EVERY:-1}" \
            LIVE_VIS_HOLD_ON_EXIT=0 FILTERED_RECORDING_ENABLE="${FILTERED_RECORDING_ENABLE:-1}" \
            RECORD_ENABLE="${RECORD_ENABLE:-true}" RECORD_EVERY="${RECORD_EVERY:-100}" \
  ...
  if [[ "${PREFLIGHT_ONLY:-0}" != "1" ]]; then
    python3 "$CAMPAIGN_ANALYZER" --campaign-root "$BASE_ROOT" \
      --analysis-start-step "$ANALYSIS_START_STEP" --analysis-end-step "$ANALYSIS_END_STEP" --expected-cases 8
  fi
  echo "[0493x14at] CAMPAIGN COMPLETE root=$BASE_ROOT steps=$TARGET_STEPS analysisWindow=${ANALYSIS_START_STEP}:${ANALYSIS_END_STEP}"
  exit 0
fi
[[ "$CAMPAIGN_MODE" == "case" ]] || { echo "[0493x14at] ERROR CAMPAIGN_MODE must be campaign or case" >&2; exit 2; }

# =============================================================================
# 0493x14at — SATO NEAR-FIELD STAGE-A CAMPAIGN (2-D PLANAR ANALOGUE)
#
# Campaign objective: qualify the integrated normal gas->liquid coupling in the
# no-topology-change indentation regime against the Sato et al. near-field
# scaling, while keeping the already-exercised x14as-fix1 wide-outlet topology.
#
# Similarity deliberately matched by default:
#   Lx/D = 20, liquidDepth/D = 10, H/D = 0.8 and 1.7,
#   Bo_D = 13.584125 (water/air experiment),
#   and target modified Froude values Fr_m' in the Stage-A range.
# The 2-D slot geometry, rhoL/rhoG, Mach, viscosities/Reynolds and atmospheric
# return topology are NOT identical to the experiment and are reported as such.
#
# Purpose: freeze the experimentally-scaled nozzle case on the already-working
# wide-outlet segmented topology:
#
#     wide outlet | chi wall | driven gas jet | chi wall | wide outlet
#
# Gas pressure is NOT required to match the initial reference pressure in this
# campaign. It is recorded as part of the generating condition and aligned with
# the liquid cavity observables for later compensation/normalisation.
# No lateral atmospheric reservoir is used.
#
# Geometry:
...
# =============================================================================
CASE_LABEL="${CASE_LABEL:-0493x14at_sato_stageA_case}"
CAMPAIGN_MODE="${CAMPAIGN_MODE:-campaign}"
...
DT="${DT:-0.001}"
STEPS="${STEPS:-4000}"
BATH_HEIGHT="${BATH_HEIGHT:-0.78125}"
...
# The nozzle length is derived from TARGET_H_OVER_D so H/D is exact on the default 20-cell slot.
...
NOZZLE_LENGTH_CELLS="${NOZZLE_LENGTH_CELLS:-$(python3 - \
  "$Ly" "$NY" "$BATH_HEIGHT" "$NOZZLE_INNER_WIDTH_CELLS" "$TARGET_H_OVER_D" <<'PYX14AT_H'
import sys
ly=float(sys.argv[1]); ny=int(sys.argv[2]); bath=float(sys.argv[3]); dcell=int(sys.argv[4]); hd=float(sys.argv[5])
h=ly/ny
head=(ly-bath)/h
if abs(head-round(head))>1e-10: raise SystemExit('[0493x14at] gas headspace must be an integer cell count')
stand=hd*dcell
if abs(stand-round(stand))>1e-10: raise SystemExit('[0493x14at] TARGET_H_OVER_D * D_cells must be integer')
lcell=int(round(head-stand))
if lcell < 1: raise SystemExit('[0493x14at] derived nozzle length must be >=1 cell')
print(lcell)
PYX14AT_H
)}"
...
JET_SPEED="${JET_SPEED:-$(python3 - \
  "$Lx" "$NX" "$NOZZLE_INNER_WIDTH_CELLS" "$TARGET_FRM_SATO" "$LIQUID_MASS" "$GAS_MASS" "$GRAVITY_Y" <<'PYX14AT_U'
import math,sys
lx=float(sys.argv[1]); nx=int(sys.argv[2]); dc=int(sys.argv[3]); frm=float(sys.argv[4]); ml=float(sys.argv[5]); mg=float(sys.argv[6]); gy=abs(float(sys.argv[7]))
D=dc*(lx/nx)
if min(frm,ml,mg,gy,D)<=0: raise SystemExit('[0493x14at] positive Fr_m, masses, |g| and D required')
c=(math.pi/4.0)**2
print(f'{math.sqrt(frm*(ml/mg)*gy*D/c):.17g}')
PYX14AT_U
)}"
...
# With the default dt=0.001 and alphaMax=2500, lambda=1-exp(-alpha*dt)=0.9179 per step
...
case "$DARCY_FORCING_MODE" in
  ...
  *) echo "[gas-jet-chi-nozzle-wide] ERROR unsupported Q6-g-f Darcy forcing mode: $DARCY_FORCING_MODE" >&2; exit 2 ;;
esac
...
if [[ "$OUTLET_MODE" != "hybrid" ]]; then
  echo "[gas-jet-chi-nozzle-wide] ERROR first top-same-face qualification requires OUTLET_MODE=hybrid; got $OUTLET_MODE" >&2
  exit 2
fi
...
if ! awk -v x="$INLET_THERMAL_NOISE" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-wide] ERROR resident segmented inlet requires INLET_THERMAL_NOISE=0" >&2
  exit 2
fi
...
if ! awk -v x="$WALL_ACCOMMODATION" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-wide] ERROR first gas/liquid jet qualification fixes WALL_ACCOMMODATION=0" >&2
  exit 2
fi
...
DUMP_STATE_EVERY="${DUMP_STATE_EVERY:-1000}"   # restart checkpoints; films use reduced-grid recording
INACTIVE_SLOTS_CELL_FRACTION="${INACTIVE_SLOTS_CELL_FRACTION:-1.0}"
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14at_sato_stageA_H${TARGET_H_OVER_D}_Fr${TARGET_FRM_SATO}_seed${SEED}}"
...
if [[ "$ANALYSIS_END_STEP" -gt "$STEPS" ]]; then
  echo "[0493x14at] ERROR ANALYSIS_END_STEP=$ANALYSIS_END_STEP > STEPS=$STEPS" >&2; exit 2
fi
...
hx=lx/nx; hy=ly/ny
if abs(hx-hy)>1e-12*max(1,abs(hx),abs(hy)): raise SystemExit('[gas-jet-chi-nozzle-wide] square cells required')
if abs(hx-1/256)>1e-12: raise SystemExit(f'[gas-jet-chi-nozzle-wide] first qualification keeps h=1/256, got {hx:.17g}')
if abs(bh/hy-round(bh/hy))>1e-10: raise SystemExit('[gas-jet-chi-nozzle-wide] BATH_HEIGHT must lie on a cell boundary')
W=wc*hx; smin=(xc-.5*W)/lx; smax=(xc+.5*W)/lx
if not (0.02<smin<smax<0.98): raise SystemExit('[gas-jet-chi-nozzle-wide] jet segment too close to top corners')
A=hx*hy; pref=gam*kg/A; rhoG=gam*mg/A; rhoL=gam*ml/A
cth=math.sqrt(kg/mg)*dt/hx; cjet=abs(uj)*dt/hx; ccomb=cth+cjet
if ccomb>0.80: raise SystemExit(f'[gas-jet-chi-nozzle-wide] unresolved gas flight: Cthermal+Cjet={ccomb:.6g} > 0.80 cell/step')
gabs=abs(gy)
if gy>1e-15: raise SystemExit('[gas-jet-chi-nozzle-wide] default geometry expects GRAVITY_Y <= 0')
if gabs>0:
...
if abs(ff-tf)>5e-10*max(1.0,abs(tf)):
    raise SystemExit(f'[0493x14at] nominal Sato Fr mismatch target={tf:.12g} actual={ff:.12g}')
if abs(bb-tb)>2e-3*max(1.0,abs(tb)):
    raise SystemExit(f'[0493x14at] Bond mismatch target={tb:.12g} actual={bb:.12g}; override sigma only deliberately')
PYX14AT_SIM
...
if abs(hx-hy) > 1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit("[gas-jet-chi-nozzle-wide] square cells required")
if wcell < 2 or tcell < 1 or lcell < 1:
    raise SystemExit("[gas-jet-chi-nozzle-wide] require innerWidthCells>=2, wallThicknessCells>=1, lengthCells>=1")
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1)):
    if not (0.0 < v < lx):
        raise SystemExit(f"[gas-jet-chi-nozzle-wide] {name}={v} outside domain")
if not (outer0 < inner0 < inner1 < outer1):
    raise SystemExit("[gas-jet-chi-nozzle-wide] invalid nozzle x ordering")
if exit_y <= bath + 4*hy:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-wide] nozzle exit y={exit_y:.9g} too close to bath={bath:.9g}; "
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1),("exit_y",exit_y)):
    ...
    if abs(q-round(q)) > 1e-9:
        raise SystemExit(f"[gas-jet-chi-nozzle-wide] {name}={v:.17g} is not aligned with a cell face")
W=inner1-inner0
stand=exit_y-bath
...
if abs(a-b)>1e-12*max(1.0,abs(a)):
    raise SystemExit(f'[0493x14at] H/D mismatch target={a:.12g} actual={b:.12g}')
PYX14AT_HD
...
if not (0 <= lo0 < lo1 <= lw0 < lw1 <= ji0 < ji1 <= rw0 < rw1 <= ro0 < ro1 <= 1):
    raise SystemExit(
        "[gas-jet-chi-nozzle-wide] invalid top segmentation; expected "
...
if [[ "$RESTART" == "1" ]]; then
  [[ -n "$RESTART_STATE" && -s "$RESTART_STATE" ]] || { echo '[0493x14at] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd' >&2; exit 2; }
  if [[ -z "$RESTART_FROM_STEP" ]]; then
    b="$(basename "$RESTART_STATE")"
    [[ "$b" =~ ^state_step_([0-9]+)\.smpcd$ ]] || { echo '[0493x14at] ERROR cannot infer RESTART_FROM_STEP from dump name' >&2; exit 2; }
    RESTART_FROM_STEP=$((10#${BASH_REMATCH[1]}))
  fi
  [[ "$RESTART_FROM_STEP" =~ ^[0-9]+$ ]] || { echo '[0493x14at] ERROR RESTART_FROM_STEP must be an integer' >&2; exit 2; }
...
if suite_truthy_0434 "$POSTPROCESS_ONLY"; then
  [[ "$RESTART" == "0" ]] || { echo '[0493x14at] ERROR POSTPROCESS_ONLY is for the original case root, not a restart segment' >&2; exit 2; }
  [[ -d "$RUN_ROOT/output" ]] || { echo "[0493x14at] ERROR postprocess output missing: $RUN_ROOT/output" >&2; exit 2; }
  mkdir -p "$RUN_ROOT/analysis"
  # The sparse gas history is required because the current dense recording is liquid-filtered.
  if [[ ! -s "$RUN_ROOT/analysis/indentation_history.csv" ]]; then
    python3 "$ANALYZER" \
      --run-root "$RUN_ROOT" --nx "$NX" --ny "$NY" --Lx "$Lx" --Ly "$Ly" \
  ...
  python3 "$PRESSURE_COVARIATE_ANALYZER" \
    --run-root "$RUN_ROOT" \
    --rho-liquid "$LIQUID_RHO" \
    --gravity-abs "$(awk -v g="$GRAVITY_Y" 'BEGIN{if(g<0)g=-g; printf "%.17g",g}')" \
    --sigma "$SURFACE_TENSION_SIGMA" --jet-width "$JET_WIDTH" \
    --post-start-time "$(awk -v s="$ANALYSIS_START_STEP" -v d="$DT" 'BEGIN{printf "%.17g",s*d}')"
  python3 "$RECORDING_ANALYZER" \
    --run-root "$RUN_ROOT" --Lx "$Lx" --Ly "$Ly" \
    --rho-liquid "$LIQUID_RHO" \
    --gravity-abs "$(awk -v g="$GRAVITY_Y" 'BEGIN{if(g<0)g=-g; printf "%.17g",g}')" \
    --sigma "$SURFACE_TENSION_SIGMA" \
    --jet-center-x "$JET_CENTER_X" --jet-width "$JET_WIDTH" --bath-height "$BATH_HEIGHT" \
    --target-frm "$TARGET_FRM_SATO" --target-h-over-d "$TARGET_H_OVER_D" \
    --analysis-start-step "$ANALYSIS_START_STEP" --analysis-end-step "$ANALYSIS_END_STEP" \
    --incident-height-over-jet "$ANALYSIS_INCIDENT_HEIGHT_OVER_JET"
  OUT_TAR="$RUN_ROOT/0493x14at_sato_stageA_case_compact.tar.gz"
  FILES=(analysis output/recordings "output/darcy_cost_0343.csv" "output/species_runtime_0493x14at.csv" \
    "output/cuda_phase_interface_pressure_0493x6g.csv" "logs/${CASE_LABEL}.log" "logs/${CASE_LABEL}.time" \
    "logs/environment_${CASE_LABEL}.env" "params/${CASE_LABEL}.kv" "init/${CASE_LABEL}.smpcd.json")
  PRESENT=(); for f in "${FILES[@]}"; do [[ -e "$RUN_ROOT/$f" ]] && PRESENT+=("$f"); done
  tar -czf "$OUT_TAR" -C "$RUN_ROOT" "${PRESENT[@]}"
  echo "[0493x14at] POSTPROCESS COMPLETE existingCase=$RUN_ROOT window=${ANALYSIS_START_STEP}:${ANALYSIS_END_STEP}"
  echo "[0493x14at] compact=$OUT_TAR"
  exit 0
fi
...
if solid != expected or left_solid != tcell*lcell or right_solid != tcell*lcell:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-wide] chi cell-count mismatch solid={solid} expected={expected} "
...
print(
    f"[gas-jet-chi-nozzle-wide] chi={out} solidCells={solid} "
...
CAMPAIGN=0493x14at_sato_stageA
...
echo "PATHS: runner=$ROOT/scripts/run_0493x14at_sato_stageA_campaign.sh"
...
echo "DIMENSIONLESS: SatoFr_m_nom=$FRM_SATO_NOMINAL target=$TARGET_FRM_SATO  We_g(D)=$WE_G  Bo_l(D)=$BO_W targetBo=$TARGET_BO_SATO"
...
echo "BOUNDARIES: solid base faces; top = WIDE outlet | chi wall | jet throat | chi wall | WIDE outlet (mode=$OUTLET_MODE)"
...
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[gas-jet-chi-nozzle-wide] PREFLIGHT_ONLY complete'
  exit 0
fi
