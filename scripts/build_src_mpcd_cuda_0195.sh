...
# 0195 — CUDA-enabled SRC/MPCD executable with Q6 CG residual mean-removal
# shortcut controls.  The regular CPU/OpenMP build remains unchanged.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0195}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0195-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0195-cuda-build] Built $OUT"
