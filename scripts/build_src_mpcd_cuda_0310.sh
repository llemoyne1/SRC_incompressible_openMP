...
# 0310 — CUDA SRC classic build for inactive-slot scaling audit.
# This build links the full SRC classic CUDA resident path plus 0295--0307 resampling diagnostics/split-safety modules used by the inactive-slot scaling audit.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0310}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0310-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0310-cuda-build] Built $OUT"
