...
# 0263 — full-face inlet/outlet classic SRC CUDA resident validation/performance runner.
# Scope: open_rect_obstacle_full only, x-left inlet / x-right outlet, static y walls,
# static rectangular obstacle, no Q6/Q9, no resampling, no virial, no thermostat.
# The hard-cell reservoir is resident but deliberately deterministic/single-threaded
# in this first correctness-first step; segmentation and thermal inlet noise are left
# to follow-up patches.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0263}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_io_0263}
GRID_CASES=${GRID_CASES:-"64:64:300 128:128:300"}
CASES=${CASES:-"open_rect_obstacle_full"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SEED_BASE=${SEED_BASE:-162063}
STOP_ON_FAIL=${STOP_ON_FAIL:-1}
FORCE_REBUILD=${FORCE_REBUILD:-1}
SUMMARY_EVERY_MODE=${SUMMARY_EVERY_MODE:-final}

BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_classic_io_fullface_no_thermostat}
RESIDENT_MODE_NAME=${RESIDENT_MODE_NAME:-0263_io_fullface_resident_classic_cuda_no_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0263-classic-src-io-resident] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0263.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0263.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0263-classic-src-io-resident] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_io_fullface_resident_0263.csv}
printf 'caseName,grid,NX,NY,steps,mode,classicSrcCudaMode,residentIo0263,summaryEvery,thermostatEnable,runExitCode,baselineTotalWallTime,modeTotalWallTime,totalWallDelta_s,totalWallSpeedup,baselineMaxCaseWallTime,modeMaxCaseWallTime,maxCaseWallDelta_s,maxCaseWallSpeedup,collisionActiveCalls,collisionTotalSeconds,collisionUploadSeconds,collisionKernelSeconds,collisionDownloadSeconds,collisionSharedParticleStateFraction,collisionSharedCellWorkspaceFraction,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  case "$mode" in
    "$BASELINE_MODE_NAME") ;;
    "$RESIDENT_MODE_NAME")
      resident=1; persistent_collision=1; collision_shared=1; minimal_download=1; immersed_download=0 ;;
    *) echo "[0263-classic-src-io-resident] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$case_name" \
      ...
      MPCD_CUDA_CLASSIC_SRC_IO_FULLFACE_RESIDENT_0263="$resident" \
      ...
      MPCD_CUDA_IMMERSED_RECTANGLE_0247="$resident" \
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -z "${steps:-}" ]]; then
    echo "[0263-classic-src-io-resident] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "open_rect_obstacle_full" ]]; then
      echo "[0263-classic-src-io-resident] ERROR: 0263 is limited to open_rect_obstacle_full, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0263-classic-src-io-resident] running baseline case=$case_name grid=$grid_tag"
    base_rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_classic_src_0263_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr" || base_rc=$?
    if [[ "$base_rc" != "0" ]]; then
      echo "[0263-classic-src-io-resident] baseline returned rc=$base_rc; continuing to write diagnostic CSV row" >&2
    fi
    ...
    echo "[0263-classic-src-io-resident] running case=$case_name mode=$mode grid=$grid_tag"
    rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_classic_src_0263_${case_name}_${mode}_${grid_tag}" "$mode" "$stdout_log" "$stderr_log" || rc=$?
    if [[ "$rc" != "0" ]]; then
      echo "[0263-classic-src-io-resident] resident mode returned rc=$rc; continuing to write diagnostic CSV row" >&2
    fi
    cmp_rc=0
...
PY
    ...
    echo "[0263-classic-src-io-resident] $verdict case=$case_name grid=$grid_tag mode=$mode baselineRc=$base_rc runRc=$rc failed=$failed/$compared wall=$mode_total baseline=$base_total"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0263-classic-src-io-resident] stopping after failure; see baseline stderr: $base_stderr ; resident stderr: $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0263-classic-src-io-resident] wrote $OUT_CSV"
