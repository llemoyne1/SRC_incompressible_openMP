...
# 0269a — CUDA resident classic SRC performance-optimized build.
# The feature set deliberately mirrors 0268; the new patch optimizes the
# segmented inlet/outlet hard-cell reservoir insertion while preserving the classic-only
# validation scope and future Q6/resampling reactivation paths.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0269a}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0269a-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0269a-cuda-build] Built $OUT"
