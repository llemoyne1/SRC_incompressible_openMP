...
# 0204a A/B for early resampling pool reuse for inactive-slot operator scans.
# early: build/reuse the resampling pool before collision so collision, thermostat,
#        mean-flow and resampling deposits can use fluidSlots.
# late : disable only the early-pool path, preserving the validated 0202a resampling
#        fluidSlots path for a fair A/B comparison.
# Both modes keep the 0201b cellId reuse path enabled unless explicitly disabled outside.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

ROOT_RUN="${ROOT_RUN:-runs/profile_inactive_early_early_pool_0204a}"
CXX="${CXX:-g++}"
...
BIN="build/src_mpcd_base_inactive_early_early_pool_0204a"
echo "[0204a-build] $BIN"
...
python3 scripts/augment_smpcd_inactive_early_0204a.py \
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_inactive_early_early_pool_0204a.csv"
echo "mode,run_kind,threads,elapsed,user,sys,maxrss_kb,phase_csv,audit_csv,stdout" > "$GLOBAL_CSV"

run_one() {
  local mode="$1"        # early or late
  ...
  if [[ "$mode" == "late" ]]; then
    export MPCD_DISABLE_EARLY_RESAMPLING_POOL=1
  else
  ...
  echo "[0204a-run] mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
for threads in $THREADS_LIST; do
  run_one late perf "$threads"
  run_one early perf "$threads"
  if [[ "$AUDIT_ENABLE" != "0" ]]; then
    run_one late audit "$threads"
...
print('\n[0204a-summary] perf comparison')
perf={(r['threads'],r['mode']):r for r in rows if r['run_kind']=='perf'}
for threads in sorted(set(k[0] for k in perf)):
    late=perf.get((threads,'late'))
    early=perf.get((threads,'early'))
    if late and early:
        lw=float(late['elapsed']); cw=float(early['elapsed'])
        print(f"threads={threads} wall late={lw:.6g}s early={cw:.6g}s speedup={lw/cw if cw else 0:.4g}")
...
print('\n[0204a-summary] phase comparison, perf rows')
for threads in sorted(set(k[0] for k in perf)):
    late=perf.get((threads,'late'))
    early=perf.get((threads,'early'))
    if not late or not early:
        continue
    lph=phase_sum(late['phase_csv'])
    cph=phase_sum(early['phase_csv'])
    for key in ['total','stream','boundary','immersed','resamp_pool0','collision','q6','thermostat','mean_flow','resamp_deposit0','resamp_deposit_edit','resamp_deposit_remap','resamp_velocity_refresh']:
        if key in lph and key in cph:
            print(f"threads={threads} {key:24s} late={lph[key]:.6g}s early={cph[key]:.6g}s ratio={lph[key]/cph[key] if cph[key] else 0:.4g}")

print('\n[0204a-summary] audit particle-scan comparison')
audit={(r['threads'],r['mode']):r for r in rows if r['run_kind']=='audit'}
for threads in sorted(set(k[0] for k in audit)):
    late=audit.get((threads,'late'))
    early=audit.get((threads,'early'))
    if not late or not early:
        continue
    for label,r in [('late',late),('early',early)]:
        p=Path(r['audit_csv'])
        if not p.exists():
