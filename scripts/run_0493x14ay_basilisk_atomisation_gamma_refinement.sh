...
# 0493x14ay — particle-resolution refinement of the 2-D Basilisk atomisation surrogate.
# Stage A-gamma: same h, D/h, dt, alpha, mean inlet U and We_G design as x14aw,
# but gamma is raised from 8 to 12 or 16 while particle masses and kBT are scaled
# by 8/gamma.  This preserves, before transport recalibration:
#   gamma*m, gamma*kBT, kBT/m, rho_L/rho_G, lambda/h, thermal flight and gas p_ref.
# The gamma=8 TG viscosities are retained only as design proxies.  Actual Re at
# gamma=12/16 remains REVIEW until dedicated liquid/gas TG calibration is run.
#
# Reference Basilisk case:
#   radius = 1/12, D = 1/6, box = 3 = 18 D
#   Re = 5800, rho_L/rho_G = mu_L/mu_G = 27.84
#   sigma = 3e-5, u0 = 1, au/u0 = 0.05, T0 = 0.1
#   => We_G ~= 199.5, We_L ~= 5555.6, St = D/(U T0) = 5/3.
#
# 0493x14ba adds an optional global inlet oscillation, independent of the startup
# ramp.  It remains OFF by default here so gamma-refinement comparisons stay
# directly comparable to x14aw; set BASILISK_PULSE_RUNTIME_ENABLE=1 explicitly
# to reproduce the Basilisk 5% sinusoidal inlet forcing.
#
# The x14 liquid/gas physics is kept identical to x14aw:
# Q6-g-f liquid + x6g/x9/x10/x12 + x14l/x14v, no resampling, no virial kick.
# x14ax diagnostic alpha_x6c recording is supported by the canonical binary.
...
case "$GAMMA" in
  12|16) ;;
  *) echo "[0493x14ay] ERROR GAMMA must be 12 or 16 for this controlled refinement (got $GAMMA)" >&2; exit 2 ;;
esac
CASE_LABEL="${CASE_LABEL:-0493x14ay_basilisk2d_Re500_WeG200_D96_gamma${GAMMA}}"
RUN_MODE="src-q6-g-f"
TOPOLOGY="segmented"
...
BASILISK_PULSE_RUNTIME_ENABLE="${BASILISK_PULSE_RUNTIME_ENABLE:-0}"
...
# Keep h, dt and alpha=90 fixed.  Increase particle resolution at constant
# gamma*m and gamma*kBT, with the gamma=8 low-kBT x14aw fluid as reference.
...
GAS_TYPE="${GAS_TYPE:-2}"
REFERENCE_GAMMA=8
REFERENCE_LIQUID_MASS=1.0
REFERENCE_LIQUID_KBT=0.03125
REFERENCE_GAS_MASS=0.03591954022988506
REFERENCE_GAS_KBT=0.00575

GAMMA_SCALE="$(awk -v g="$GAMMA" -v g0="$REFERENCE_GAMMA" 'BEGIN{printf "%.17g",g0/g}')"
LIQUID_MASS="${LIQUID_MASS:-$(awk -v x="$REFERENCE_LIQUID_MASS" -v s="$GAMMA_SCALE" 'BEGIN{printf "%.17g",x*s}')}"
LIQUID_KBT="${LIQUID_KBT:-$(awk -v x="$REFERENCE_LIQUID_KBT" -v s="$GAMMA_SCALE" 'BEGIN{printf "%.17g",x*s}')}"
GAS_MASS="${GAS_MASS:-$(awk -v x="$REFERENCE_GAS_MASS" -v s="$GAMMA_SCALE" 'BEGIN{printf "%.17g",x*s}')}"
GAS_KBT="${GAS_KBT:-$(awk -v x="$REFERENCE_GAS_KBT" -v s="$GAMMA_SCALE" 'BEGIN{printf "%.17g",x*s}')}"
KBT="$GAS_KBT"
PARTICLE_MASS="$GAS_MASS"
RUN_OK_REFERENCE_PARTICLE_MASS="$LIQUID_MASS"

# Transport proxies only.  They are the measured gamma=8 values and determine
# the first similarity run's U and reported proxy Re.  Override after TG12/TG16.
LIQUID_NU_REFERENCE="${LIQUID_NU_REFERENCE:-0.0003303602194}"
GAS_NU_REFERENCE="${GAS_NU_REFERENCE:-0.0003536191886}"
LIQUID_NU_REFERENCE_STATUS="gamma8_TG128_PROXY_NOT_RECALIBRATED_AT_gamma${GAMMA}"
GAS_NU_REFERENCE_STATUS="gamma8_TG64_PROXY_NOT_RECALIBRATED_AT_gamma${GAMMA}"
...
read -r NX NY Lx Ly JET_D JET_CENTER_Y LIQUID_SPEED SURFACE_TENSION_SIGMA \
  ...
  "$LIQUID_NU_REFERENCE" "${STEPS:-3057}" <<'PY'
import math,sys
...
# 3057 steps gives Basilisk-equivalent t ~= 3.80 using the gamma=8 viscosity proxy.
STEPS="${STEPS:-3057}"
...
# Similarity guards relative to the gamma=8 reference fluid.
python3 - "$GAMMA" "$LIQUID_MASS" "$LIQUID_KBT" "$GAS_MASS" "$GAS_KBT" <<'PYINV'
import math,sys
g,ml,kl,mg,kg=map(float,sys.argv[1:])
ref=(8.0,1.0,0.03125,0.03591954022988506,0.00575)
g0,ml0,kl0,mg0,kg0=ref
checks={
 'gamma*mL':(g*ml,g0*ml0), 'gamma*mG':(g*mg,g0*mg0),
 'gamma*kBTL':(g*kl,g0*kl0), 'gamma*kBTG':(g*kg,g0*kg0),
 'kBTL/mL':(kl/ml,kl0/ml0), 'kBTG/mG':(kg/mg,kg0/mg0)}
for name,(a,b) in checks.items():
    if not math.isclose(a,b,rel_tol=2e-12,abs_tol=2e-14):
        raise SystemExit(f'[0493x14ay] ERROR similarity invariant {name}: {a:.17g} != {b:.17g}')
print('[0493x14ay] similarity invariants PASS')
PYINV
rc=$?
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14ay_basilisk_atomisation_Re500_gamma${GAMMA}_seed${SEED}}"
if [[ "$RESTART" == "1" ]]; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x14ay] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
...
case "$BASILISK_PULSE_RUNTIME_ENABLE" in
  0|1) ;;
  *) echo "[0493x14ay] ERROR BASILISK_PULSE_RUNTIME_ENABLE must be 0 or 1" >&2; exit 2 ;;
esac
if [[ -z "$INLET_OSCILLATION_TIME_OFFSET" ]]; then
  ...
  if [[ "$RESTART" == "1" && "$BASILISK_PULSE_RUNTIME_ENABLE" == "1" ]]; then
    ...
    if [[ "$restart_base" =~ ^state_step_0*([0-9]+)\.smpcd$ ]]; then
      ...
      echo "[0493x14ay] oscillation restart phase offset auto=$INLET_OSCILLATION_TIME_OFFSET from step=$restart_step"
    else
      echo "[0493x14ay] ERROR oscillating restart requires INLET_OSCILLATION_TIME_OFFSET" >&2
      echo "[0493x14ay]       when RESTART_STATE is not named state_step_N.smpcd" >&2
      exit 2
    fi
  fi
fi
...
printf '%s\n' \
  "===== 0493x14ay BASILISK ATOMISATION — GAMMA REFINEMENT =====" \
  "reference=https://basilisk.fr/src/examples/atomisation.c" \
  "stage=2D gamma-refinement surrogate with optional x14ba global inlet oscillation" \
  ...
  "fluid: gamma=${GAMMA} scale8/gamma=${GAMMA_SCALE} alpha=90deg dt=${DT} kBT_L=${LIQUID_KBT} m_L=${LIQUID_MASS} kBT_G=${GAS_KBT} m_G=${GAS_MASS}" \
  "similarity invariants: gamma*m and gamma*kBT fixed; kBT/m fixed; density ratio fixed; lambda/h fixed" \
  "transport: nuL=${LIQUID_NU_REFERENCE} [${LIQUID_NU_REFERENCE_STATUS}] nuG=${GAS_NU_REFERENCE} [${GAS_NU_REFERENCE_STATUS}]" \
...
if awk -v x="$FLIGHT_L" -v w="$FLIGHT_WARN_CELLS" 'BEGIN{exit !(x>w)}'; then
  echo "[0493x14ay] WARNING liquid thermal+advective flight=${FLIGHT_L} cells/step > ${FLIGHT_WARN_CELLS}" >&2
  if suite_truthy_0434 "$STRICT_FLIGHT_GUARD"; then
    echo '[0493x14ay] ERROR STRICT_FLIGHT_GUARD=1' >&2; exit 2
  fi
fi

if suite_truthy_0434 "$DESIGN_ONLY"; then
  echo '[0493x14ay] DESIGN_ONLY=1: no state generation and no solver launch'
  exit 0
fi
...
if [[ "$RESTART" == "1" ]]; then
  STATE="$RESTART_STATE"
  echo "[0493x14ay] RESTART inputState=$STATE"
else
...
with path.open('wb') as f:
    ...
    f.write(R)
meta={
 'profile':'0493x14ay_basilisk_atomisation_gamma_refinement_2d',
 ...
 'note':'Stage A uses steady mean inlet; Basilisk periodic 5% sinusoidal inlet is not available in current qualified binary.'}
path.with_suffix(path.suffix+'.json').write_text(json.dumps(meta,indent=2)+'\n')
print(f'[0493x14ay-generate] grid={NX}x{NY} h={h:.9g} N={len(X)} liquid={nl} gas={ng}')
print(f'[0493x14ay-generate] D={D:.9g} L0={L0:.9g} U={U:.9g}')
PYGEN
rc=$?
if [[ $rc -ne 0 ]]; then echo "[0493x14ay] ERROR initial-state generation failed rc=$rc" >&2; exit $rc; fi
fi
...
speciesDiagnosticsFilename = species_runtime_0493x14ay.csv
...
case "$NEUMANN_PROFILE" in
  ...
  x9e)
    ...
    ;;
  *) echo "[0493x14ay] ERROR bad NEUMANN_PROFILE=$NEUMANN_PROFILE" >&2; exit 2 ;;
esac
...
BENCHMARK=basilisk_atomisation_2d_stageA_gamma_refinement
REFERENCE_URL=https://basilisk.fr/src/examples/atomisation.c
TARGET_RE_L=$TARGET_RE_L
GAMMA=$GAMMA
GAMMA_SCALE_8_OVER_GAMMA=$GAMMA_SCALE
...
run_ok_surface_print_0493x13zi "Basilisk 2-D jet gamma refinement; optional x14ba global inlet oscillation"
echo "[0493x14ay] PARAMS=$PARAMS"
echo "[0493x14ay] STATE=$STATE"
echo "[0493x14ay] RUN_ROOT=$RUN_ROOT"

# Contract guards.
grep -Fqx "openBoundarySegmentCount = 2" "$PARAMS" || { echo '[0493x14ay] ERROR segment count contract' >&2; exit 2; }
grep -Fq "openBoundarySegment0 = left inlet" "$PARAMS" || { echo '[0493x14ay] ERROR liquid inlet contract' >&2; exit 2; }
grep -Fq "openBoundarySegment1 = right outlet" "$PARAMS" || { echo '[0493x14ay] ERROR outlet contract' >&2; exit 2; }
if grep -Eq '^darcyBrinkmanEnable[[:space:]]*=[[:space:]]*true' "$PARAMS"; then
  echo '[0493x14ay] ERROR this benchmark must not contain chi/Darcy nozzle geometry' >&2
  exit 2
fi

suite_run_binary_0434 "$PARAMS" "$LOG" "$TF" "$OUT"
rc=$?
if [[ $rc -ne 0 ]]; then echo "[0493x14ay] ERROR solver failed rc=$rc" >&2; exit $rc; fi
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[0493x14ay] PREFLIGHT_ONLY complete'
  exit 0
fi
...
if rows:
  ...
  print('[0493x14ay-analysis] wrote',out)
else: print('[0493x14ay-analysis] no state dumps found')
PYANA
fi

echo "[0493x14ay] COMPLETE root=$RUN_ROOT"
echo "[0493x14ay] NOTE gamma12/16 transport remains proxy-level until TG recalibration; exact Basilisk 5% sinusoidal inlet remains separate."
