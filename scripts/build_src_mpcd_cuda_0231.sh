...
# 0231 — CUDA-enabled SRC/MPCD executable for resampling shadow invariant validation.
# Same CUDA feature set as 0230; weighted_resampling shadow pass ignores CPU-specific transfer-plan mass by default.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0231}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0231-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0231-cuda-build] Built $OUT"
