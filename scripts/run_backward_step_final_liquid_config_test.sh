...
# By default this script only runs the missing final virial case. The no-virial
# baseline is normally supplied by the hard-wall calibration suite:
#   runs/backward_step_hardwall_q9_s0p50_b0p00100_k4_96x48
RUN_BASELINE="${RUN_BASELINE:-0}"
RUN_VIRIAL="${RUN_VIRIAL:-1}"
...
CONFIG_DIR="runs/backward_step_final_liquid_config_configs"
mkdir -p "$CONFIG_DIR"
...
write_case() {
  ...
  cat > "$file" <<EOF_INNER
# Auto-generated by scripts/run_backward_step_final_liquid_config_test.sh.
# Final immersed-step liquid-closure candidate test.
# The no-virial default can be compared to the hard-wall calibration baseline:
#   runs/backward_step_hardwall_q9_s0p50_b0p00100_k4_96x48
# This remains a periodic-x forced separated-flow stress test, not yet an
# inlet/outlet backward-facing-step benchmark.
...
run_case() {
  local desc="$1"
  local cfg="$2"
  echo "[backward-step-final-liquid] running ${desc}: ${cfg}"
  ./build/src_mpcd_base "$cfg"
}

if [[ "$RUN_BASELINE" != "0" ]]; then
  cfg="$CONFIG_DIR/params_backward_step_final_q9_k4_96x48.kv"
  out="runs/backward_step_final_q9_k4_96x48"
  write_case "q9" "$out" "$cfg" "false"
  run_case "Q9 hard-wall final candidate, no virial" "$cfg"
fi

if [[ "$RUN_VIRIAL" != "0" ]]; then
  cfg="$CONFIG_DIR/params_backward_step_final_q9_k4_virial_96x48.kv"
  out="runs/backward_step_final_q9_k4_virial_96x48"
  write_case "q9_virial" "$out" "$cfg" "true"
  run_case "Q9 hard-wall final candidate + virial" "$cfg"
fi

if [[ "$RUN_ANALYSIS" != "0" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    echo "[backward-step-final-liquid] running MATLAB analysis"
    matlab -batch "cd('matlab'); suite = validate_backward_step_final_liquid_config('makePlots',false);"
  else
    echo "[backward-step-final-liquid] MATLAB not found; analyze later with:" >&2
    echo "  cd matlab" >&2
    echo "  suite = validate_backward_step_final_liquid_config();" >&2
  fi
fi

echo "[backward-step-final-liquid] done"
