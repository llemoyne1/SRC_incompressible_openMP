...
# 0493x14aw — 2-D SRC/MPCD analogue of Basilisk src/examples/atomisation.c
# Stage A-lowkBT: Re_L target = 500, We_G target = 200, rho_L/rho_G = 27.84.
# Fluid recalibrated after the first smoke revealed a liquid thermal escape halo.
#
...
#
# IMPORTANT: the qualified current binary accepts inletVelocityRampProfile only
# as linear/smoothstep. It has no periodic sinusoidal inlet law. This Stage-A
# runner therefore uses the *mean* Basilisk inlet velocity (steady inlet) and
# records the exact missing pulse parameters. No C++/CUDA modification is made.
#
# The x14 liquid/gas physics is kept identical to run_ok_air_assisted_atomizer.sh:
...
# Low-kBT liquid selected after the first Re500 smoke showed a thermal escape halo.
# TG128, 8 seeds, alpha=90 deg: PASS, nu=3.303602194e-4, CV=11.28%.
LIQUID_KBT="${LIQUID_KBT:-0.03125}"
# Exact density ratio at equal gamma: rho_L/rho_G = m_L/m_G.
GAS_MASS="${GAS_MASS:-0.03591954022988506}"
# Low-kBT gas rematch at alpha=90. TG64, 8 seeds: REVIEW,
# nu=3.536191886e-4, CV=13.22%. This gives Re_G ~= 467 when Re_L=500.
GAS_KBT="${GAS_KBT:-0.00575}"
KBT="$GAS_KBT"
PARTICLE_MASS="$GAS_MASS"
RUN_OK_REFERENCE_PARTICLE_MASS="$LIQUID_MASS"

# Measured transport coefficients for this low-kBT benchmark fluid.
# Liquid: TG128, 8/8 PASS, CV=11.28%.
LIQUID_NU_REFERENCE="${LIQUID_NU_REFERENCE:-0.0003303602194}"
# Gas: TG64 ensemble REVIEW, CV=13.22%.
...
# 3057 steps gives Basilisk-equivalent t ~= 3.80 with the calibrated low-kBT U.
STEPS="${STEPS:-3057}"
...
# Basilisk pulse cannot be enabled on this binary without changing runtime code.
if [[ "$BASILISK_PULSE_RUNTIME_ENABLE" != "0" ]]; then
  echo "[0493x14aw] ERROR periodic sinusoidal inlet is not supported by the current qualified binary." >&2
  echo "[0493x14aw]       Keep BASILISK_PULSE_RUNTIME_ENABLE=0 for Stage A." >&2
  exit 2
fi
...
# Mean inlet only in Stage A; no start-up ramp so t=0 matches Basilisk mean phase.
...
LIVE_VIS_NX="${LIVE_VIS_NX:-432}"
LIVE_VIS_NY="${LIVE_VIS_NY:-432}"
...
printf '%s\n' \
  "===== 0493x14aw BASILISK ATOMISATION — DESIGN =====" \
  "reference=https://basilisk.fr/src/examples/atomisation.c" \
  "stage=2D steady-inlet low-kBT surrogate (periodic 5% pulse NOT YET implemented)" \
  ...
  "pulse target (not runtime): A/U=${BASILISK_PULSE_REL_AMPLITUDE} St=${TARGET_ST} period=${PULSE_PERIOD} = ${PULSE_PERIOD_STEPS} steps" \
  ...
  "gas viscosity reference status=${GAS_NU_REFERENCE_STATUS}" \
  "======================================================"

if awk -v x="$FLIGHT_L" -v w="$FLIGHT_WARN_CELLS" 'BEGIN{exit !(x>w)}'; then
  echo "[0493x14aw] WARNING liquid thermal+advective flight=${FLIGHT_L} cells/step > ${FLIGHT_WARN_CELLS}" >&2
  if suite_truthy_0434 "$STRICT_FLIGHT_GUARD"; then
    echo '[0493x14aw] ERROR STRICT_FLIGHT_GUARD=1' >&2; exit 2
  fi
fi

if suite_truthy_0434 "$DESIGN_ONLY"; then
  echo '[0493x14aw] DESIGN_ONLY=1: no state generation and no solver launch'
  exit 0
fi
...
with path.open('wb') as f:
    ...
    f.write(R)
meta={
 'profile':'0493x14aw_basilisk_atomisation_2d',
...
print(f'[0493x14aw-generate] grid={NX}x{NY} h={h:.9g} N={len(X)} liquid={nl} gas={ng}')
print(f'[0493x14aw-generate] D={D:.9g} L0={L0:.9g} U={U:.9g}')
PYGEN
...
# Stage-A Basilisk analogue: steady mean inlet. Current parser supports only
# linear/smoothstep ramps and no periodic sine law.
...
speciesDiagnosticsFilename = species_runtime_0493x14aw.csv
...
case "$NEUMANN_PROFILE" in
  ...
  x9e)
    ...
    ;;
  *) echo "[0493x14aw] ERROR bad NEUMANN_PROFILE=$NEUMANN_PROFILE" >&2; exit 2 ;;
esac
...
BENCHMARK=basilisk_atomisation_2d_stageA_lowkbt
...
BASILISK_PULSE_RUNTIME_ENABLE=0
...
run_ok_surface_print_0493x13zi "Basilisk 2-D pulsed-jet Stage-A steady-inlet analogue"
echo "[0493x14aw] PARAMS=$PARAMS"
echo "[0493x14aw] STATE=$STATE"
echo "[0493x14aw] RUN_ROOT=$RUN_ROOT"

# Contract guards.
grep -Fqx "openBoundarySegmentCount = 2" "$PARAMS"
grep -Fq "openBoundarySegment0 = left inlet" "$PARAMS"
grep -Fq "openBoundarySegment1 = right outlet" "$PARAMS"
if grep -Eq '^darcyBrinkmanEnable[[:space:]]*=[[:space:]]*true' "$PARAMS"; then
  echo '[0493x14aw] ERROR this benchmark must not contain chi/Darcy nozzle geometry' >&2
  exit 2
fi

suite_run_binary_0434 "$PARAMS" "$LOG" "$TF" "$OUT"
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[0493x14aw] PREFLIGHT_ONLY complete'
  exit 0
fi
...
if rows:
  ...
  print('[0493x14aw-analysis] wrote',out)
else: print('[0493x14aw-analysis] no state dumps found')
PYANA
fi

echo "[0493x14aw] COMPLETE root=$RUN_ROOT"
echo "[0493x14aw] NOTE exact Basilisk 5% sinusoidal inlet remains the next controlled runtime addition."
