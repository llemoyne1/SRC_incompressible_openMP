...
# run_ok_puddle: stand-alone liquid/vacuum droplet-impact demonstration.
...
SIGMA_ACTIVE="${SIGMA_ACTIVE:-945.0}"
...
RUN_ROOT="${RUN_ROOT:-runs/run_ok_puddle_${NX}x${NY}_g${GAMMA}_s${SIGMA_ACTIVE}}"
...
printf '%s\n' \
  "[run_ok_puddle] target=$TARGET sigma=$SIGMA_ACTIVE minRadiusCells=$SURFACE_TENSION_MIN_RADIUS_CELLS contactAngle=$CONTACT_ANGLE_DEG" \
  "[0493x9t-suite] kineticReflectionFraction=$KINETIC_REFLECTION_FRACTION evaporationTargetType=$EVAPORATION_TARGET_TYPE" \
  "[run_ok_puddle] drop R/h=$DROP_RADIUS_CELLS center=($DROP_CENTER_X,$DROP_CENTER_Y) v=($DROP_VX,$DROP_VY) gravityY=$GRAVITY_Y" \
  "[run_ok_puddle] vacuum=1 puddleDepthCells=$([[ $TARGET == puddle ]] && echo "$PUDDLE_DEPTH_CELLS" || echo 0) dumpsEvery=$DUMP_STATE_EVERY steps=$STEPS dt=$DT" \
  "[run_ok_puddle] objective=qualitative impact/sheet/rim/splash; wall and liquid-puddle targets use identical bulk/capillary settings"
...
echo "[run_ok_puddle] completed target=$TARGET output=$OUT"
echo "[run_ok_puddle] state dumps requested every $DUMP_STATE_EVERY steps"
