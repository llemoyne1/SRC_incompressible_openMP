...
# 0296 — CUDA SRC classic build plus post-SRC conservative mass reconditioning without support change.
# This build keeps the 0295 passive survey and adds the first mutating but support-preserving CUDA resampling brick.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0296}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0296-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0296-cuda-build] Built $OUT"
