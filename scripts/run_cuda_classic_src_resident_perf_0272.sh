...
# 0272 — performance pass for the validated CUDA resident classic SRC stack.
# This runner keeps the 0265c validation structure and profiles the 0272
# collision-overhead reduction.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0272}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_resident_perf_0272}
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_resident_perf_validation_0272.csv}
THREADS=${THREADS:-8}
GAMMA=${GAMMA:-20}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0272-perf] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0272.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0272.sh
fi
...
export MPCD_CUDA_RESIDENT_PROFILE_0272=1
# Keep 0271 fixed-overhead optimizations active; 0272 adds collision-specific reductions.
...
# 0272 performance root and use the 0272 binary.
BIN="$BIN" \
...
python3 scripts/summarize_cuda_resident_perf_0272.py "$ART_DIR/validation_runs" \
  --summary-out "$ART_DIR/cuda_classic_src_resident_perf_summary_0272.csv" \
  --phases-out "$ART_DIR/cuda_classic_src_resident_perf_phases_0272.csv"

echo "[0272-perf] validation manifest: $OUT_CSV"
echo "[0272-perf] performance summary: $ART_DIR/cuda_classic_src_resident_perf_summary_0272.csv"
echo "[0272-perf] detailed phases: $ART_DIR/cuda_classic_src_resident_perf_phases_0272.csv"
echo "[0272-perf] Q6/resampling remain disabled only in these classic-only validators; reactivation paths are intentionally preserved."
echo "[0272-perf] CUDA thermostat wall/solid/piston/IO-aware remains a separate future chantier."
