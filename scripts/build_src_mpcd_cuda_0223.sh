...
# 0223 — CUDA-enabled SRC/MPCD executable with CudaParticleState metadata-cache uploads
# for the persistent deposit->SRC collision->thermostat substep.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0223}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0223-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0223-cuda-build] Built $OUT"
