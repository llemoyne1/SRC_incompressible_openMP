...
# 0493x15b — fixed chi Brinkman slab + chiVP collision action/reaction audit.
# Second dependent step of the chi-solid force audit.
# Physics remains minimal: periodic classic SRC + body acceleration + deterministic
# mean Brinkman, now with chiVP ON, no resampling and no Q6.  The audit checks
# Delta P = body impulse + exact Brinkman impulse + exact chiVP collision impulse.
# No new user-facing physics mode/flag is introduced by this qualification.

ROOT="${ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
source "$ROOT/scripts/src_mpcd_run_common_0434.sh"
suite_root_cd_0434 || exit $?

CASE_LABEL="0493x15b_fixed_chi_vp_force_budget"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x15b.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x15b_fixed_chi_vp_force_budget}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x15b] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi

if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x15b] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
...
python3 - "$CHI" "$NX" "$NY" "$SLAB_CELLS" <<'PY_X15B_CHI'
import struct, sys
...
print(f"[0493x15b] chi={path} vertical_slab_cells=[{lo},{hi}) width={w}/{nx}")
PY_X15B_CHI
...
# 0493x15b: one run-local LiveVis control remains the single source
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x15b.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x15b.txt" <<META
case=$CASE_LABEL
mode=$MODE
topology=$TOPOLOGY
Lx=$Lx
...
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x15b] ERROR missing chi_solid_impulse_0493x15a.csv" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x15b] ERROR missing chi_vp_impulse_0493x15b.csv" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x15b] ERROR missing summary_runtime.csv" >&2; exit 2; }

echo "[0493x15b] COMPLETE run=$RUN_ROOT"
echo "[0493x15b] next: analyze from matlab/ with analyze_0493x15b_fixed_chi_vp_force_budget('../runs/...')"
