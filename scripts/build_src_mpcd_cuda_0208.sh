...
# 0208 — CUDA-enabled SRC/MPCD executable for combined TG GPU-path validation.
# Includes CUDA Q6, CUDA cell moments, and CUDA cell-relative thermostat.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0208}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0208-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0208-cuda-build] Built $OUT"
