...
# 0291 — CUDA SRC classic build with explicit outlet regimes.
# This build starts from the 0288/0290 SRC classic CUDA stack and adds explicit outlet regimes: neumann, equilibrium_flux, and forced_flux. Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0291}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0291-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0291-cuda-build] Built $OUT"
