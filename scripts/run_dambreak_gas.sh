...
GAMMA="${GAMMA:-8}"
STEPS="${STEPS:-5000}"
DT="${DT:-0.0012063471328149122585}"
SEED="${SEED:-493963}"
...
GAS_MASS="${GAS_MASS:-0.001}"
LIQUID_KBT="${LIQUID_KBT:-0.02}"
GAS_KBT="${GAS_KBT:-0.00224}"
...
# Current x14 liquid/gas coupling. x14ai must stay OFF here because the liquid
# touches solid walls; its global Q6-resultant closure is only valid for a
# closed liquid component isolated from all external Q6 boundaries.
...
if ! awk -v kg="$KBT" -v gg="$GAS_KBT" 'BEGIN{d=kg-gg; if(d<0)d=-d; exit !(d <= 1e-12*(1+(gg<0?-gg:gg)))}'; then
  echo "[run_dambreak_gas] ERROR global KBT=$KBT must equal GAS_KBT=$GAS_KBT because x6g EOS reads params.kBT" >&2
  exit 2
fi
if [[ "$X14_DEVICE_Q6_RESULTANT_CLOSURE" != 0 && "$X14_DEVICE_Q6_RESULTANT_CLOSURE" != false ]]; then
  echo "[run_dambreak_gas] ERROR x14ai device-resultant closure is invalid for this wall-connected liquid; keep X14_DEVICE_Q6_RESULTANT_CLOSURE=0" >&2
  exit 2
fi
...
# x6g gas EOS: params.kBT is the gas EOS temperature on the current path.
...
if suite_truthy_0434 "$X14_COUPLING_ENABLE"; then
  ...
  # Current gas coupling: x14l + x14v + x14ad. x14ai explicitly OFF: wall-connected liquid.
...
run_ok_surface_print_0493x13zi "x9+x6g+x10/x12+x14l+x14v+x14ad-wall-connected-no-x14ai"
...
echo "PATH:  src-q6-g-f + x9/x6g + x10o/CIC/Q2/x10p/q/x10u/x10v/x12a + x14l/x14v/x14ad"
echo "X14:   coupling=$X14_COUPLING_ENABLE x14ai=$X14_DEVICE_Q6_RESULTANT_CLOSURE (forced OFF for wall-connected liquid) pRefGas=$GAS_PRESSURE_REFERENCE"
