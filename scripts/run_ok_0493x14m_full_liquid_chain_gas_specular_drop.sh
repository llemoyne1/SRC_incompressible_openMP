...
for required in \
  ...
  if [[ ! -f "$required" ]]; then
    echo "[0493x14m] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14m — full qualified liquid free-surface chain + gas-side specular reflection
#
# Purpose:
#   Develop ONLY the gas-side closure while restoring the complete qualified
#   liquid free-surface chain: x10o+CIC+Q2+x10p/q+x10u+x10v full-vector+x12a.
#   None of the liquid laws is modified by x14m.  Gas B keeps x14l local-moving-
#   frame specular reflection with no wall-impulse feedback to liquid/Q6.
#   x12a is phase-A-only both for the kinetic envelope and the differentiated
#   species thermostat; gas keeps its own kBT target unchanged.
#   x10w and orphan cleanup remain OFF.
#
...
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14m_full_liquid_chain_gas_specular_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14m] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14m] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14m] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14m] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14m_full_liquid_chain_gas_specular_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14m.csv
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14m.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14m.env" <<META
...
run_ok_surface_print_0493x13zi "x9+x6g-liquid-qualified-full-x10uv-x12a-gas-specular-no-impulse-feedback"
echo "===== 0493x14m FULL QUALIFIED LIQUID CHAIN + GAS SPECULAR DROP ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14m_full_liquid_chain_gas_specular_drop.sh"
...
# Keep the existing x14j analyzer untouched, but publish x14m-named copies so
# full-liquid-chain gas-specular results cannot be confused with x14j/x14k/x14l.
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14m.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14m.csv"

echo "[0493x14m] DONE"
echo "[0493x14m] return: $ANALYSIS_DIR/radial_metrics_0493x14m.csv"
echo "[0493x14m] return: $ANALYSIS_DIR/radial_profiles_0493x14m.csv"
echo "[0493x14m] return: $ANALYSIS_DIR/laplace_static_drop_0493x14m.json (if produced)"
