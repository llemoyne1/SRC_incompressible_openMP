...
# 0271 — CUDA resident classic SRC performance-optimized build.
# The feature set deliberately mirrors 0270; this patch reduces wall/periodic
# resident fixed overheads while preserving the classic-only validation scope
# and future Q6/resampling reactivation paths.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0271}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0271-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0271-cuda-build] Built $OUT"
