...
# 0294/0295 — CUDA SRC classic build plus passive post-SRC support survey for future non-destructive resampling.
# This build keeps the 0293 outlet ordering and adds only a non-mutating support survey. Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0295}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0294-0295-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0294-0295-cuda-build] Built $OUT"
