...
RUN_DIR="runs/resampling_donor_particle_selection_smoke_0117"
mkdir -p "$RUN_DIR"
STATE="$RUN_DIR/initial_donor_selection_v2.smpcd"
...
root = pathlib.Path("runs/resampling_donor_particle_selection_smoke_0117")
root.mkdir(parents=True, exist_ok=True)
path = root / "initial_donor_selection_v2.smpcd"
Nx = 8
Ny = 4
gamma = 4
Nlatent = 5
...
cat > "$RUN_DIR/params_donor_selection.kv" <<KV
inputState = $STATE
...
./build/src_mpcd_base "$RUN_DIR/params_donor_selection.kv"
...
base = pathlib.Path("runs/resampling_donor_particle_selection_smoke_0117")
...
print(
    "donor selection:",
    f"selected={last['resampSelectedDonorParticles']}",
    f"selectedMass={last['resampSelectedDonorParticleMass']}",
    f"overshoot={last['resampSelectedDonorMassOvershoot']}",
    f"coverage={last['resampSelectedDonorMassCoverageFraction']}",
    f"firstParticle={last['resampFirstSelectedDonorParticle']}",
    f"lastParticle={last['resampLastSelectedDonorParticle']}",
    f"maxPerEntry={last['resampMaxSelectedParticlesForTransferEntry']}",
    f"maxPerCell={last['resampMaxSelectedParticlesPerDonorCell']}",
)
PY
