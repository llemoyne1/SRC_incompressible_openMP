...
# 0280c — segmented inlet/outlet validation for CUDA persistent SRC+thermostat (resident thermostat support).
#
...
BIN=${BIN:-build/src_mpcd_base_cuda_0280c}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_io_segmented_0280c}
...
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0280c_cuda_io_segmented_resident_src_fused_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0280c-cuda-thermostat-io-segmented] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0280c.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0280c.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0280c-cuda-thermostat-io-segmented] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_io_segmented_0280c.csv}
python3 - "$OUT_CSV" <<'PY'
import csv, sys
header = [
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    "$CUDA_MODE_NAME")
      io_segmented=1
      # 0280c source fix: resident segmented inlet/outlet now accepts
      # thermostatEnable=true only when the fused persistent SRC+thermostat
      # consumer is requested.  Keep the fused consumer on the fresh shared
      # 0251 state produced by the segmented resident boundary.
      ...
      ;;
    *) echo "[0280c-cuda-thermostat-io-segmented] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
dump_failure_context() {
  ...
  {
    echo "--- 0280c failure context: check resident support and shared thermostat state ---"
    echo "root=$root"
    ...
    echo "--- end 0280c failure context ---"
  } >> "$stderr_log" 2>&1 || true
}
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${steps:-}" ]]; then
    echo "[0280c-cuda-thermostat-io-segmented] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "segmented_u_turn_full" ]]; then
      echo "[0280c-cuda-thermostat-io-segmented] ERROR: 0280 is limited to segmented_u_turn_full, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0280c-cuda-thermostat-io-segmented] running baseline case=$case_name grid=$grid_tag"
    base_rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_thermostat_io_segmented_0280c_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr" || base_rc=$?
    ...
    echo "[0280c-cuda-thermostat-io-segmented] running CUDA fused thermostat case=$case_name grid=$grid_tag"
    rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_thermostat_io_segmented_0280c_${case_name}_${CUDA_MODE_NAME}_${grid_tag}" "$CUDA_MODE_NAME" "$stdout_log" "$stderr_log" || rc=$?
    cmp_rc=0
...
PY
    ...
    echo "[0280c-cuda-thermostat-io-segmented] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu sharedThermo=$coll_shared_p"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      dump_failure_context "$run_root" "$stdout_log" "$stderr_log"
      echo "[0280c-cuda-thermostat-io-segmented] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0280c-cuda-thermostat-io-segmented] wrote $OUT_CSV"
echo "[0280c-cuda-thermostat-io-segmented] shared-state fused SRC thermostat is enabled only for this classic-only segmented inlet/outlet validation."
