...
ROOT=${BASE_0475A_ROOT:-runs/0475a_materializer_on_plan_probe}
BIN=${BIN:-build/src_mpcd_base_cuda_q6_resident_periodic_equiv_0475a}
...
# Keep the legacy cadence effectively disabled; 0475a triggers the materializer
...
if [[ ! -x "$BIN" ]]; then
  echo "[0475a] missing executable: $BIN" >&2
  exit 2
fi
...
if [[ -x scripts/run_0464_scaling_cuda_vs_cpu.sh ]]; then
  ...
  echo "[0475a] missing scaling runner 0464/0463" >&2
  exit 2
fi
...
echo "[0475a] scaling runner: $SCALING_RUNNER"
echo "[0475a] root: $ROOT"
echo "[0475a] cases: $SCALE_CASES"
echo "[0475a] materializer every: $MATERIALIZER_EVERY"
echo "[0475a] on-plan trigger: enabled"
...
summary = root / 'materializer_on_plan_summary_0475a.csv'
report = root / 'materializer_on_plan_report_0475a.md'
...
with report.open('w') as out:
    out.write('# 0475a materializer on-plan shared-state probe\n\n')
    out.write('Scope: keep 0475 shared-state materializer, but trigger it opportunistically when the real CPU transfer plan exists, instead of sampling only fixed cadence steps where the plan can be empty.\n\n')
