...
# 0493x3 closed-box hypothesis test.  Requires the 0493x2 liquid-only extension
# of generate_0493x0_dam_break_state.py already present in the working tree.
CASE_LABEL="liquid_only_q6_force_prestream_0493x3"
TOPOLOGY="closed_box"
Lx="${Lx:-1.0}"; Ly="${Ly:-1.0}"; NX="${NX:-200}"; NY="${NY:-200}"
GAMMA="${GAMMA:-10}"; STEPS="${STEPS:-1000}"; DT="${DT:-0.005}"; KBT="${KBT:-0.05}"
SEED="${SEED:-493900}"; GRAVITY_Y="${GRAVITY_Y:--0.5}"
...
Q6_FORCE_PROJECTION_MODE="${Q6_FORCE_PROJECTION_MODE:-prestream}"
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x3_liquid_only_q6_force_${Q6_FORCE_PROJECTION_MODE}_${NX}x${NY}_g${GAMMA}}"
...
# Closed walls must be allowed to transmit the hydrostatic pressure reaction.
...
# Variables required by suite_defaults_common_0434 / suite_compute_derived_0434.
GEN_CASE="tg"; U0=0.0; VELOCITY_MODE="zero"; PARTICLE_MASS="$LIQUID_MASS"
TG_HOLE_ENABLE=false
suite_defaults_common_0434
suite_compute_derived_0434

if ! python3 scripts/generate_0493x0_dam_break_state.py --help 2>&1 | grep -q -- '--liquid-only'; then
  echo "[0493x3] ERROR: missing 0493x2 --liquid-only state-generator extension" >&2
  exit 2
fi
...
speciesCount = 2
species0 = $LIQUID_TYPE liquid_only liquid $LIQUID_Q6_STRENGTH 1.0 $LIQUID_REFERENCE_CELL_MASS
...
speciesDiagnosticsFilename = species_runtime_0493x3.csv
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x3.env" "$RUN_MODE"
echo "[0493x3] liquid-only Q6 force projection mode=$Q6_FORCE_PROJECTION_MODE grid=${NX}x${NY} gamma=$GAMMA steps=$STEPS gravityY=$GRAVITY_Y"
if [[ "$Q6_FORCE_PROJECTION_MODE" == "prestream_single_fused" ]]; then
  echo "[0493x4b] ordering=tentative-force deposit -> Q6 solve -> fused force+Q6 apply -> stream -> collision -> thermostat"
elif [[ "$Q6_FORCE_PROJECTION_MODE" == "prestream_single" ]]; then
  echo "[0493x4a] ordering=force-kick -> Q6 -> stream -> collision -> thermostat"
else
  echo "[0493x3] ordering=force-kick -> Q6 -> stream -> collision -> Q6 -> thermostat"
fi
