...
RUN_ROOT="${RUN_ROOT:-runs/example_backward_step}"
prepare_run_root
STATE_FILE="$RUN_ROOT/init/backward_step_${NX}x${NY}_g${GAMMA}_seed${SEED}.smpcd"
...
generate_state "$STATE_FILE" "$Lx" "$Ly" "$NX" "$NY" "$GAMMA" "$KBT" $((SEED+22)) uniform "$UIN" 0.0 0.0 -1.0 "$SOLID_RECT"
mkdir -p "$OUT_DIR"
