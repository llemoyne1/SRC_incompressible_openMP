...
# 0174d — post-guard deposit profiling probe.
# Requires an executable built from the validated 0171 mutation-profile baseline plus this 0174d post-thermal refresh patch.
# Outputs:
#   RUN_ROOT/perf_summary_0174d.csv
#   RUN_ROOT/phase_profile_0174d.csv
#   RUN_ROOT/phase_profile_top_0174d.csv
#   RUN_ROOT/q6_cg_profile_0174d.csv
#   RUN_ROOT/q6_cg_profile_top_0174d.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0174d}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0174d.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0174d.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0174d.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0174d.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0174d.csv"
RESAMP_GUARD_CSV="$RUN_ROOT/resampling_guard_profile_0174d.csv"
RESAMP_GUARD_TOP_CSV="$RUN_ROOT/resampling_guard_profile_top_0174d.csv"
DEPOSIT_CSV="$RUN_ROOT/deposit_profile_0174d.csv"
DEPOSIT_TOP_CSV="$RUN_ROOT/deposit_profile_top_0174d.csv"
POST_GUARD_CSV="$RUN_ROOT/post_guard_profile_0174d.csv"
POST_GUARD_TOP_CSV="$RUN_ROOT/post_guard_profile_top_0174d.csv"
POST_GUARD_EQUIV_CSV="$RUN_ROOT/post_guard_equivalence_profile_0174d.csv"
POST_GUARD_TRACE_CSV="$RUN_ROOT/post_guard_equivalence_trace_0174d.csv"
...
find_profile_file() {
  ...
  for tag in 0174d 0174b 0172 0171 0169 0168 0166 0165 0163 0161 0160 0159 0158 0157; do
...
cat <<INFO
[0174d-profile] root       : $ROOT_DIR
[0174d-profile] binary     : $BIN
[0174d-profile] run root   : $RUN_ROOT
[0174d-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0174d-profile] steps      : $STEPS
[0174d-profile] threads    : $THREAD_LIST
[0174d-profile] cases      : $CASE_LIST
[0174d-profile] profile tag: $PROFILE_SOURCE_TAG
INFO
...
printf 'case,threads,metric,value_total,value_per_call,calls\n' > "$POST_GUARD_EQUIV_CSV"
printf 'case,threads,step,affected_cells,candidate_cells,candidate_particle_refs,scanned_particle_refs,eligible_particle_refs,split_particles,extracted_particles,full_deposit_particles_visited,full_deposit_fluid_particles,full_deposit_cells,count_changed_cells,mass_changed_cells,momentum_changed_cells,classification_changed_cells,any_field_changed_cells,non_count_changed_mass_momentum_cells,non_count_changed_classification_cells,candidate_list_size_changed,candidate_list_content_changed,snapshot_cell_mismatch,full_to_scanned_ref_ratio\n' > "$POST_GUARD_TRACE_CSV"
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0174d-profile] running $label threads=$threads"
...
PY
    ...
    if phase_src=$(find_profile_file "$out_dir" "phase_profile"); then
      cp "$phase_src" "$out_dir/phase_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$phase_src" >> "$PHASE_CSV"
    else
      echo "[warn] Missing phase profile in $out_dir for tags 0174d/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    q6cg_src=""
    if q6cg_src=$(find_profile_file "$out_dir" "q6_cg_profile"); then
      cp "$q6cg_src" "$out_dir/q6_cg_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$q6cg_src" >> "$Q6_CG_CSV"
    else
      echo "[warn] Missing Q6/CG profile in $out_dir for tags 0174d/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    resamp_guard_src=""
    if resamp_guard_src=$(find_profile_file "$out_dir" "resampling_guard_profile"); then
      cp "$resamp_guard_src" "$out_dir/resampling_guard_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$resamp_guard_src" >> "$RESAMP_GUARD_CSV"
    else
      echo "[warn] Missing resampling guard profile in $out_dir for tags 0174d/0172/0171/0169/0168/0166/0165/0163/0161/0160/0159/0158/0157" >&2
    fi

    deposit_src=""
    if deposit_src=$(find_profile_file "$out_dir" "deposit_profile"); then
      cp "$deposit_src" "$out_dir/deposit_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
    ...
    if post_guard_src=$(find_profile_file "$out_dir" "post_guard_profile"); then
      cp "$post_guard_src" "$out_dir/post_guard_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
    ...
    post_guard_equiv_src=""
    if post_guard_equiv_src=$(find_profile_file "$out_dir" "post_guard_equivalence_profile"); then
      cp "$post_guard_equiv_src" "$out_dir/post_guard_equivalence_profile_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$post_guard_equiv_src" >> "$POST_GUARD_EQUIV_CSV"
    else
      echo "[warn] Missing post-guard equivalence profile in $out_dir for available profile tags" >&2
    fi

    post_guard_trace_src=""
    if post_guard_trace_src=$(find_profile_file "$out_dir" "post_guard_equivalence_trace"); then
      cp "$post_guard_trace_src" "$out_dir/post_guard_equivalence_trace_0174d.csv"
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$post_guard_trace_src" >> "$POST_GUARD_TRACE_CSV"
    else
      echo "[warn] Missing post-guard equivalence trace in $out_dir for available profile tags" >&2
    fi
  done
done
...
echo "[0174d-profile] wrote $PERF_CSV"
echo "[0174d-profile] wrote $PHASE_CSV"
echo "[0174d-profile] wrote $TOP_CSV"
echo "[0174d-profile] wrote $Q6_CG_CSV"
echo "[0174d-profile] wrote $Q6_CG_TOP_CSV"
echo "[0174d-profile] wrote $RESAMP_GUARD_CSV"
echo "[0174d-profile] wrote $RESAMP_GUARD_TOP_CSV"
echo "[0174d-profile] wrote $DEPOSIT_CSV"
echo "[0174d-profile] wrote $DEPOSIT_TOP_CSV"
echo "[0174d-profile] wrote $POST_GUARD_CSV"
echo "[0174d-profile] wrote $POST_GUARD_TOP_CSV"
echo "[0174d-profile] wrote $POST_GUARD_EQUIV_CSV"
echo "[0174d-profile] wrote $POST_GUARD_TRACE_CSV"
