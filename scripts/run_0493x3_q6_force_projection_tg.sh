...
# 0493x3: periodic Taylor--Green identity matrix for the experimental
# force-kick -> Q6 -> stream ordering.  The standard post-collision Q6 remains
# active in every case.  Null-force legacy/prestream runs must be bitwise equal;
# forced runs are compared off-line by modal amplitude and dump differences.
CASE_LABEL="q6_force_projection_tg_0493x3"
GEN_CASE="tg"
TOPOLOGY="periodic"
Lx="${Lx:-1.0}"; Ly="${Ly:-1.0}"; NX="${NX:-64}"; NY="${NY:-64}"
GAMMA="${GAMMA:-40}"; STEPS="${STEPS:-500}"; DT="${DT:-0.001}"; KBT="${KBT:-0.001}"
SEED="${SEED:-493300}"; U0="${U0:-0.04}"; VELOCITY_MODE="${VELOCITY_MODE:-taylor_green}"
TG_FORCING_AMPLITUDE="${TG_FORCING_AMPLITUDE:-0.02}"
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x3_q6_force_projection_tg_${NX}x${NY}_g${GAMMA}_s${STEPS}}"
...
FAIL_ON_NULL_MISMATCH="${FAIL_ON_NULL_MISMATCH:-1}"
...
run_case_0493x3() {
  ...
  mkdir -p "$out"
  write_params_0493x3 "$out" "$params" "$force_mode" "$forcing_enable"
...
META
  ...
  echo "[0493x3] case=$case_name mode=$force_mode forcing=$forcing_enable amplitude=$case_amplitude"
...
for spec in \
  ...
  "forced_prestream prestream true"; do
  read -r case_name force_mode forcing_enable <<< "$spec"
  set +e
  run_case_0493x3 "$case_name" "$force_mode" "$forcing_enable"
  rc=$?
  set -e
...
if [[ "$failures" != 0 ]]; then
  echo "[0493x3] FAIL: run failures=$failures" >&2
  exit 1
fi
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo "[0493x3] preflight PASS root=$BASE_RUN_ROOT"
  exit 0
fi

python3 scripts/analyze_0493x3_q6_force_projection_tg.py \
  --root "$BASE_RUN_ROOT" --Lx "$Lx" --Ly "$Ly" \
  --csv "$BASE_RUN_ROOT/q6_force_projection_tg_0493x3.csv" \
  --report "$BASE_RUN_ROOT/q6_force_projection_tg_0493x3.md" \
  --fail-on-null-mismatch "$FAIL_ON_NULL_MISMATCH"

echo "[0493x3] root=$BASE_RUN_ROOT"
echo "[0493x3] csv=$BASE_RUN_ROOT/q6_force_projection_tg_0493x3.csv"
echo "[0493x3] report=$BASE_RUN_ROOT/q6_force_projection_tg_0493x3.md"
