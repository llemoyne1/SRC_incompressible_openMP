...
# 0193 — CUDA-enabled SRC/MPCD executable with CUDA Q6 device scalar reduction.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0193}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0193-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0193-cuda-build] Built $OUT"
