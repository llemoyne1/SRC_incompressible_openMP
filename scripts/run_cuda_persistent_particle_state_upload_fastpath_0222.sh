...
with open(out_csv,'a',newline='') as fcsv:
    ...
    w.writerow(row)
print(f"[0222-persistent-upload-fastpath] {verdict} {label}/{mode}/{case_name}: wall={mode_wall:.6g}s baseline={base_wall:.6g}s speedup={row['wallSpeedup']:.4g} shared={row['sharedParticleStateFraction']:.2f} reused={row['particleStateReusedAllocationFraction']:.2f} failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_mode() {
  ...
  local run_root="runs/cuda_persistent_particle_state_upload_0222_${mode}_${label}"
  local shared=0
  case "$mode" in
    legacy_internal_upload) shared=0 ;;
    shared_particle_state) shared=1 ;;
    *) echo "[0222-persistent-upload-fastpath] ERROR: unknown mode $mode" >&2; exit 2 ;;
  esac

  echo "[0222-persistent-upload-fastpath] mode=$mode $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      RUN_ROOT="$run_root" RUN_TAG="cuda0222_${mode}_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
  ...
  local compare_csv="$ART_DIR/cuda_persistent_particle_state_upload_compare_0222_${mode}_${label}.csv"
  local compare_summary="$ART_DIR/cuda_persistent_particle_state_upload_compare_summary_0222_${mode}_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    ...
    if [[ ! -f "$active_csv" ]]; then
      echo "[0222-persistent-upload-fastpath] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
run_one() {
  ...
  local baseline_root="runs/cuda_persistent_particle_state_upload_0222_cpu_baseline_${label}"

  echo "[0222-persistent-upload-fastpath] baseline $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$baseline_root" RUN_TAG="cuda0222_cpu_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
      MPCD_CUDA_CELL_MOMENTS_USE=0 \
      MPCD_CUDA_THERMOSTAT_USE=0 \
      MPCD_CUDA_SRC_COLLISION_USE=0 \
      MPCD_CUDA_PERSISTENT_SRC_COLLISION_USE=0 \
...
for item in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0222-persistent-upload-fastpath] malformed GRID_CASES entry: $item" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0222-persistent-upload-fastpath] wrote $OUT_CSV"
