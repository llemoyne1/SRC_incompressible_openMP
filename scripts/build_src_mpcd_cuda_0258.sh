...
# 0258 — performance build for persistent CUDA SRC collision plus persistent CUDA thermostat.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0258}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0258-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0258-cuda-build] Built $OUT"
