...
# 0247b — CUDA streaming for the moving-y-wall piston subset. This keeps the
# validated 0244 resampling path available, but leaves Q6/Q9, closed-capacity
# virial response, collision, cell moments and diagnostics on CPU.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0247b}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_streaming_piston_0247b}
GRID_CASES=${GRID_CASES:-"64:300 128:300"}
CASE_LIST=${CASE_LIST:-"piston_virial_full"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
...
PISTON_MODE_NAME=${PISTON_MODE_NAME:-cuda_streaming_piston_0247b}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0247b.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0247b-piston-stream] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_streaming_piston_0247b.csv}
printf 'caseList,grid,NX,NY,steps,mode,pistonStreamingCuda,resamplingCuda,projectionEnable,projectionBackend,runExitCode,baselineWallTime,modeWallTime,wallDelta_s,wallSpeedup,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  case "$mode" in
    "$BASELINE_MODE_NAME")
      piston_cuda=0; resampling_cuda=0 ;;
    "$RESAMPLING_MODE_NAME")
      piston_cuda=0; resampling_cuda=1; resampling_download_all=0; resampling_host_shadow=1; resampling_upload_mode=roles_only ;;
    "$PISTON_MODE_NAME")
      piston_cuda=1; resampling_cuda=1; resampling_download_all=0; resampling_host_shadow=1; resampling_upload_mode=roles_only ;;
    *) echo "[0247b-piston-stream] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_streaming_piston_0247b_${BASELINE_MODE_NAME}_${grid}"
  base_stdout="$ART_DIR/cuda_streaming_piston_0247b_${BASELINE_MODE_NAME}_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_streaming_piston_0247b_${BASELINE_MODE_NAME}_${grid}.stderr.log"
  echo "[0247b-piston-stream] running baseline grid=$grid cases=[$CASE_LIST]"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0247b_${BASELINE_MODE_NAME}_${grid}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
  base_rc=$?
  ...
  for mode in "$RESAMPLING_MODE_NAME" "$PISTON_MODE_NAME"; do
    run_root="runs/cuda_streaming_piston_0247b_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_streaming_piston_0247b_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_streaming_piston_0247b_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_streaming_piston_compare_0247b_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_streaming_piston_compare_summary_0247b_${mode}_${grid}.csv"
    echo "[0247b-piston-stream] running mode=$mode grid=$grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0247b_${mode}_${grid}" "$mode" "$stdout_log" "$stderr_log"
    rc=$?
    ...
    if [[ "$mode" == "$PISTON_MODE_NAME" ]]; then piston_flag=1; else piston_flag=0; fi
    resampling_flag=1
    python3 - <<PY >> "$OUT_CSV"
import math
...
print(f"$case_list_csv,$grid,$nx,$ny,$steps,$mode,$piston_flag,$resampling_flag,$PROJECTION_ENABLE,$PROJECTION_BACKEND,$rc,{base},{wall},{delta},{speed},$failed,$compared,$verdict,$stdout_log,$stderr_log,$compare_csv,$compare_summary")
PY
    echo "[0247b-piston-stream] $verdict mode=$mode grid=$grid: wall=$wall baseline=$base_wall failed=$failed/$compared"
...
echo "[0247b-piston-stream] wrote $OUT_CSV"
echo "[0247b-piston-stream] scope: CUDA moving-y-wall piston streaming only; Q6/Q9 and closed-capacity virial stayed CPU."
