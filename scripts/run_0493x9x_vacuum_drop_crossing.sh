...
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x9x_vacuum_drop_crossing}"
...
# x9w diverged sharply between 150 and 200; 400 steps are enough for the first gate.
...
printf '%s\n' \
  "[0493x9x-suite] PRE-crossing reflection: actual x+v*dt alpha crossing" \
  "[0493x9x-suite] coherent advection gate: (v-u_bulk).n > 0" \
  "[0493x9x-suite] one-cell shell guard only; no multi-cell halo recovery" \
  "[0493x9x-suite] same 3 particle passes; no new O(Ncell)/O(Nparticle) buffer" \
  "[0493x9x-suite] steps=$STEPS summaryEvery=$SUMMARY_EVERY livevis=$LIVE_VIS_ENABLE recorder=$FILTERED_RECORDING_ENABLE"

bash scripts/run_0493x9s_splash.sh

CSV="$RUN_ROOT/output/cuda_phase_kinetic_crossing_0493x9x.csv"
[[ -f "$CSV" ]] || { echo "[0493x9x-suite] ERROR missing $CSV" >&2; exit 2; }
python3 scripts/analyze_0493x9x_crossing.py "$CSV"
