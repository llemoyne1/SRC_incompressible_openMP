...
# 0084 open-channel full inlet/outlet with slip/specular top/bottom walls.
# Purpose: isolate the core inlet/outlet + Q6/Q9/virial machinery before
# reintroducing VP/no-slip walls and non-compatible inlet profiles.
#
# Configuration intent:
#   - left boundary  : full-height hard_cell_density inlet, uniform Uin(t)
#   - right boundary : full-height outlet; Q6/Q9 impose the same open flux Uex=Uin(t)
#   - bottom/top     : specular/slip walls, no virtual-particle wall coupling
#   - no immersed solid
#   - q9_virial by default, Q9 validated parameters, thermal soft limiter
#   - no changes to the C++ core are required by this script.
...
RUN_ROOT="${RUN_ROOT:-runs/open_channel_full_io_slip_softlimited_q9_0084}"
...
# by the balanced open-boundary flux pair xLow=xHigh=Uin(t).
inletUxLeft = ${UIN}
inletUyLeft = 0.0
inletUxRight = ${UIN}
inletUyRight = 0.0
...
# Slip/specular top/bottom walls: impermeable, no VP/no-slip coupling.
wallVpEnable = false
...
EOF_KV
  ...
  echo "[0084] running ${label}"
  echo "[0084] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0084] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
echo "[0084] full-height inlet/outlet, slip/specular y walls, no immersed solid"
echo "[0084] Uin=Uex=${UIN}; ramp ${INLET_RAMP_INITIAL_FACTOR}->${INLET_RAMP_FINAL_FACTOR}, t=${INLET_RAMP_START_TIME}->${INLET_RAMP_END_TIME}, profile=${INLET_RAMP_PROFILE}"
echo "[0084] Q9 limiter: mode=${Q9_CORRECTION_LIMITER_MODE}, overThermal=${Q9_CORRECTION_LIMITER_OVER_THERMAL}, thermalKBT=${Q9_CORRECTION_LIMITER_THERMAL_KBT}, legacyAbs=${Q9_CORRECTION_LIMITER}"

SECONDS=0
TAG="u${UIN//./p}_${Nx}x${Ny}"
if [[ "$RUN_CLASSIC" == "1" ]]; then
  write_case "openchan_classic_fullio_slip_ramped_softlimited_${TAG}" "classic" "false" "false"
fi
if [[ "$RUN_Q6" == "1" ]]; then
  write_case "openchan_q6_fullio_slip_ramped_softlimited_${TAG}" "q6" "false" "false"
fi
if [[ "$RUN_Q9" == "1" ]]; then
  write_case "openchan_q9_fullio_slip_ramped_softlimited_${TAG}" "q9" "true" "false"
fi
if [[ "$RUN_Q9_VIRIAL" == "1" ]]; then
  write_case "openchan_q9_virial_fullio_slip_ramped_softlimited_${TAG}" "q9_virial" "true" "true"
fi

elapsed=$SECONDS
printf '[0084] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0084] Analyze with: cd matlab && R = analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"

if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    matlab -batch "cd('matlab'); analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"
  else
    echo "[0084] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
