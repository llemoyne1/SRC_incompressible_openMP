...
for required in \
  ...
  if [[ ! -f "$required" ]]; then
    echo "[0493x14l] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14l — qualified liquid x10u + gas-side specular reflection
#
# Purpose:
#   First physical validation of 0493x14l.  The liquid A treatment is NOT
#   changed: x10o+CIC+Q2+x10p+x10u remains position-only exactly as in the
#   qualified liquid path.  Only gas B changes response: an outward-from-gas
#   Q2 hit keeps the existing local-moving-frame specular velocity instead of
#   being overwritten by x10u.  No wall impulse is fed back to liquid/Q6.
#   x10v/x10w/x12a and orphan cleanup remain OFF.
#
...
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14l_gas_specular_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14l] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14l] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14l] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14l] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14l_gas_specular_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14l.csv
...
# 0493x14l: one existing particle pass handles both A and B.  The liquid
# defines the x10o/Q2 moving wall.  A keeps x10u position-only exactly; B
# retains the already-computed specular response.  No extra pass/buffer.
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14l.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14l.env" <<META
...
run_ok_surface_print_0493x13zi "x9+x6g-liquid-x10u-gas-specular-no-impulse-feedback"
echo "===== 0493x14l LIQUID X10U + GAS SPECULAR DROP ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14l_gas_specular_drop.sh"
...
echo "KIN:   liquid A=x10u position-only UNCHANGED; gas B=specular normal reflection in moving-interface frame
       tangential gas velocity unchanged; wall impulse diagnostic only / NOT applied; x10v/x10w/x12a=OFF"
echo "RUN:   steps=$STEPS dt=$DT summaryEvery=$SUMMARY_EVERY dumpEvery=$DUMP_STATE_EVERY 0272=1 bilateralGeometry=true gasSpecular=true"
...
# Keep the existing x14j analyzer untouched, but publish x14l-named copies so
# gas-specular results cannot be confused with either x14j or invalid x14k.
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14l.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14l.csv"

echo "[0493x14l] DONE"
echo "[0493x14l] return: $ANALYSIS_DIR/radial_metrics_0493x14l.csv"
echo "[0493x14l] return: $ANALYSIS_DIR/radial_profiles_0493x14l.csv"
echo "[0493x14l] return: $ANALYSIS_DIR/laplace_static_drop_0493x14l.json (if produced)"
