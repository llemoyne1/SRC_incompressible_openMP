...
RUN_ROOT="${RUN_ROOT:-runs/poiseuille_segmented_inlet_outlet_softlimited_q9_0083}"
...
LEFT_OPEN_YMIN="${LEFT_OPEN_YMIN:-0.08333333333333333}"
LEFT_OPEN_YMAX="${LEFT_OPEN_YMAX:-0.9166666666666666}"
RIGHT_OPEN_YMIN="${RIGHT_OPEN_YMIN:-0.08333333333333333}"
RIGHT_OPEN_YMAX="${RIGHT_OPEN_YMAX:-0.9166666666666666}"
...
EOF_KV
  ...
  echo "[0083] running ${label}"
  echo "[0083] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0083] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
elapsed=$SECONDS
printf '[0083] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0083] Analyze with: cd matlab && R = analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"

if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    matlab -batch "cd('matlab'); analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"
  else
    echo "[0083] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
