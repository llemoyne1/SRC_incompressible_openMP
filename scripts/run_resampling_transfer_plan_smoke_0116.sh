...
RUN_DIR="runs/resampling_transfer_plan_smoke_0116"
mkdir -p "$RUN_DIR"
STATE="$RUN_DIR/initial_transfer_plan_v2.smpcd"
...
root = pathlib.Path("runs/resampling_transfer_plan_smoke_0116")
root.mkdir(parents=True, exist_ok=True)
path = root / "initial_transfer_plan_v2.smpcd"
Nx = 8
Ny = 4
gamma = 4
Nlatent = 5
...
cat > "$RUN_DIR/params_transfer_plan.kv" <<KV
inputState = $STATE
...
./build/src_mpcd_base "$RUN_DIR/params_transfer_plan.kv"
...
base = pathlib.Path("runs/resampling_transfer_plan_smoke_0116")
...
print(
    ...
    f"pairs={last['resampTransferPairs']}",
    f"planned={last['resampPlannedTransferMass']}",
    f"remainingReceiver={last['resampRemainingReceiverDeficitAfterPlan']}",
    f"remainingDonor={last['resampRemainingDonorExcessAfterPlan']}",
    f"coverage={last['resampTransferMassCoverageFraction']}",
    f"meanDistance={last['resampTransferMeanCellDistance']}",
)
PY

printf '\n[0116 resampling transfer plan smoke] OK: local donor/receiver transfer plan is passive and deterministic.\n'
