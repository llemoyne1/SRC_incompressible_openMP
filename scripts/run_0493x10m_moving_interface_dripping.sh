...
CASE_LABEL="dripping_vk_0493x10m_moving_interface"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
TOPOLOGY=segmented
RUN_ROOT="${RUN_ROOT:-runs/0493x10m_moving_interface_dripping_s4500_g01}"
CASES="${CASES:-capillary}"

# Square cells: h=0.005 by default.
Lx="${Lx:-1.0}"; Ly="${Ly:-1.5}"; NX="${NX:-200}"; NY="${NY:-300}"
GAMMA="${GAMMA:-20}"; DT="${DT:-0.002}"; KBT="${KBT:-0.125}"; STEPS="${STEPS:-8000}"
SEED="${SEED:-493940}"
...
export MPCD_X10J_SIMPLE_SPECULAR_ABLATION MPCD_X10K_LOCAL_FRAME_SPECULAR_ABLATION MPCD_X10M_MOVING_INTERFACE_WALL MPCD_X10L_PREWALL_INTERFACE_DIAGNOSTICS
...
PARAMS
  ...
  printf '%s\n' \
    ...
    "[0493x10m-drip] objective=alpha=.5 local moving-interface wall driven by post-Q6/B1 normal liquid velocity; no collective receiver reaction"
