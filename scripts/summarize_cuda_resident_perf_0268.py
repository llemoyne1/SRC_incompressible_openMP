#!/usr/bin/env python3
"""Aggregate CUDA resident classic SRC performance artifacts for patch 0268.

The script scans an artifact tree produced by
scripts/run_cuda_classic_src_resident_perf_0268.sh and writes two CSV files:
  * cuda_classic_src_resident_perf_summary_0268.csv — one compact row per run
  * cuda_classic_src_resident_perf_phases_0268.csv — detailed phase rows
...
def main() -> int:
    ...
    summary_out = ns.summary_out or root / "cuda_classic_src_resident_perf_summary_0268.csv"
    phases_out = ns.phases_out or root / "cuda_classic_src_resident_perf_phases_0268.csv"
    ...
    print(f"[0268-perf-summary] cases={len(summary_rows)} summary={summary_out} phases={phases_out}")
