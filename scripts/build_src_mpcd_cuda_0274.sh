...
# 0274 — CUDA resident classic SRC collision wrapper/per-step overhead build.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0274}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0274-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0274-cuda-build] Built $OUT"
