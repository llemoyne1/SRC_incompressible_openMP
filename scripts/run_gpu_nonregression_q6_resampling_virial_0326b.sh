...
# 0326b — script-only non-regression harness for the post-0322 clean CUDA branch.
...
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_nonregression_q6_resampling_virial_0326b}
...
if [[ "$SRC_BUILD" == "1" ]]; then
  ...
  echo "[0326b-nonreg] building SRC binary: $BIN via $SRC_BUILD_SCRIPT"
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash "$SRC_BUILD_SCRIPT"
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0326b-nonreg] ERROR: missing executable BIN=$BIN" >&2
  echo "[0326b-nonreg]        Build once or rerun with SRC_BUILD=1." >&2
  exit 127
fi
if [[ ! -f scripts/run_validation_mono_config_0162.sh ]]; then
  echo "[0326b-nonreg] ERROR: missing scripts/run_validation_mono_config_0162.sh" >&2
  exit 127
fi
...
MANIFEST="$ART_DIR/gpu_nonregression_q6_resampling_virial_0326b_manifest.csv"
...
# 0326b correction: when Q6/resampling/virial are enabled, do NOT request
# the fused CUDA collision+thermostat path. Q6 and virial modify velocities on
# the CPU between collision and thermostat, so the correct non-regression smoke
# is:
...
# collision subset.
export_cuda_collision_only_flags_0326b() {
...
run_target() {
  ...
  case "$target" in
    ...
    hybrid_cuda_q6_resampling_tg)
      ...
      note="CUDA persistent collision + CPU Q6/resampling + CPU thermostat"
      ;;
    hybrid_cuda_piston_virial)
      ...
      note="CUDA persistent piston collision + CPU Q6/resampling/virial + CPU thermostat"
      ;;
    *)
      echo "[0326b-nonreg] ERROR: unknown target=$target" >&2
      exit 2
      ;;
  esac
  ...
  echo "[0326b-nonreg] run target=$target cases=$case_list steps=$steps note=$note"
  if [[ "$src_classic_cuda" == "true" ]]; then
    export_cuda_collision_only_flags_0326b "$cuda_piston_subset"
  else
    clear_cuda_hybrid_flags_0326b
  fi
  ...
  if [[ "$rc" != "0" ]]; then
    echo "[0326b-nonreg] WARNING: target=$target failed rc=$rc" >&2
...
python3 scripts/summarize_gpu_nonregression_q6_resampling_virial_0326b.py "$MANIFEST" "$ART_DIR"

echo "[0326b-nonreg] manifest: $MANIFEST"
echo "[0326b-nonreg] summary : $ART_DIR/gpu_nonregression_q6_resampling_virial_0326b_summary.csv"
echo "[0326b-nonreg] flags   : $GREP_0325"
