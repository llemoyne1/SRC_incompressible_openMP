...
# 0075 receded/thin backward-step hard-inlet run, lower-Uin variant with Q9 parameters reset to the
...
# The geometry is a 48x24 receded/thin backward-step inlet/outlet case, 80000 steps, with Uin=0.025 by default.
...
RUN_ROOT="${RUN_ROOT:-runs/backward_step_receded_thin_validated_q9_uin025_0075}"
...
# Receded/thin backward-step obstacle geometry.
# With Nx=48, xMin=0.625 leaves about 11 Q9-active columns between
# the inlet open-boundary exclusion and the immersed-solid halo. yMax=0.25
# eases the bypass test. This 0075 variant lowers Uin to test whether the
# validated Q9/virial parameters remain stable when the inlet transient is milder.
SOLID_XMIN="${SOLID_XMIN:-0.625}"
SOLID_XMAX="${SOLID_XMAX:-0.750}"
SOLID_YMIN="${SOLID_YMIN:-0.0}"
SOLID_YMAX="${SOLID_YMAX:-0.25}"
STATE_FILE="${STATE_FILE:-initial_state_backward_step_receded_thin_48x24_g20_kbt0p0025_ux0p025_x0p625_0p750_y0p25.smpcd}"
...
EOF_KV
  ...
  echo "[0075] running ${label}"
  echo "[0075] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0075] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
if [[ "$RUN_CLASSIC" == "1" ]]; then
  write_case "backstep_receded_y025_uin025_classic_hard_inlet_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "classic" "false" "false" "0.0"
fi
if [[ "$RUN_Q6" == "1" ]]; then
  write_case "backstep_receded_y025_uin025_q6_hard_inlet_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "q6" "false" "false" "0.0"
fi
if [[ "$RUN_Q9" == "1" ]]; then
  write_case "backstep_receded_y025_uin025_q9_validated_hard_inlet_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "q9" "true" "false" "${Q9_STRENGTH_REF}"
fi
if [[ "$RUN_Q9_VIRIAL" == "1" ]]; then
  write_case "backstep_receded_y025_uin025_q9_virial_validated_hard_inlet_48x24_halo${Q9_IMMERSED_HALO_CELLS}_long" "q9_virial" "true" "true" "${Q9_STRENGTH_REF}"
fi

elapsed=$SECONDS
printf '[0075] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0075] Analyze with: cd matlab && R = analyze_backward_step_hard_inlet_budget_0072('root','..','runRoot','${RUN_ROOT}');"
echo "[0075] Visuals with: cd matlab && V = make_backward_step_hard_inlet_visual_report_0072('root','..','runRoot','${RUN_ROOT}');"
...
if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    ...
    echo "[0075] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
