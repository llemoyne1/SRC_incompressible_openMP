...
# 0202 — run active CUDA particle->cell deposit inside the real SRC/MPCD
...
BIN=${BIN:-build/src_mpcd_base_cuda_0202}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_deposit_0202}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620202}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0202.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0202-cell-fastpath] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_cell_moments_fastpath_0202.csv}
printf 'grid,NX,NY,steps,case,mode,baselineElapsed_s,activeElapsed_s,baselineWallTime,activeWallTime,elapsedDelta_s,wallDelta_s,activeCalls,particlesVisitedPerCall,fluidParticlesPerCall,numCells,totalActiveSeconds,uploadSeconds,kernelSeconds,downloadSeconds,avgActiveTotalSeconds,avgKernelSeconds,reuseBufferFraction,allFluidFastPathFraction,uniformMassFastPathFraction,downloadedCellVelocitiesFraction,failed_metrics,compared_metrics,verdict,activeCsv\n' > "$OUT_CSV"
...
reuse_frac = sum(sf('reusedDeviceBuffers')) / calls if calls else float('nan')
all_fluid_frac = sum(sf('allFluidFastPath')) / calls if calls else float('nan')
uniform_mass_frac = sum(sf('uniformMassFastPath')) / calls if calls else float('nan')
download_vel_frac = sum(sf('downloadedCellVelocities')) / calls if calls else float('nan')
...
row = {
    ...
    'reuseBufferFraction': reuse_frac,
    'allFluidFastPathFraction': all_fluid_frac,
    'uniformMassFastPathFraction': uniform_mass_frac,
    'downloadedCellVelocitiesFraction': download_vel_frac,
...
with open(out_csv, 'a', newline='') as fcsv:
    ...
    writer.writerow(row)
print(f"[0202-cell-fastpath] {verdict} {label}/{case_name}: activeWall={active_wall:.6g}s baselineWall={base_wall:.6g}s totalActive={total_active:.6g}s kernel={kernel:.6g}s failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_one() {
  ...
  local base_root="runs/cuda_cell_moments_active_0202_baseline_${label}"
  local active_root="runs/cuda_cell_moments_active_0202_${label}"
  local base_summary="none"
  local base_elapsed="nan"

  if [[ "$RUN_BASELINE" == "1" ]]; then
    echo "[0202-cell-fastpath] baseline $label"
    env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
        ...
        RUN_ROOT="$base_root" RUN_TAG="cuda0202_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
  ...
  echo "[0202-cell-fastpath] active $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$active_root" RUN_TAG="cuda0202_active_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
  ...
  if [[ "$RUN_BASELINE" != "1" ]]; then
    echo "[0202-cell-fastpath] ERROR: RUN_BASELINE=0 is not supported for comparison summary yet" >&2
    exit 4
  fi

  local compare_csv="$ART_DIR/cuda_cell_moments_active_compare_0202_${label}.csv"
  local compare_summary="$ART_DIR/cuda_cell_moments_active_compare_summary_0202_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    local active_csv="$active_root/$case_name/cuda_cell_moments_active_0202.csv"
    if [[ ! -f "$active_csv" ]]; then
      echo "[0202-cell-fastpath] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
for spec in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0202-cell-fastpath] invalid GRID_CASES entry '$spec' (expected NX:steps)" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0202-cell-fastpath] wrote $OUT_CSV"
