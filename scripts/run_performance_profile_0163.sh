...
# 0163 — resampling guard/deposit optimization profiling probe for SRC/MPCD OpenMP performance work.
# Requires an executable built from the 0163 resampling-optimized sources.
# Outputs:
#   RUN_ROOT/perf_summary_0163.csv
#   RUN_ROOT/phase_profile_0163.csv
#   RUN_ROOT/phase_profile_top_0163.csv
#   RUN_ROOT/q6_cg_profile_0163.csv
#   RUN_ROOT/q6_cg_profile_top_0163.csv

ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$ROOT_DIR"

BIN=${BIN:-build/src_mpcd_base}
RUN_ROOT=${RUN_ROOT:-runs/performance_profile_0163}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-500}
...
PERF_CSV="$RUN_ROOT/perf_summary_0163.csv"
PHASE_CSV="$RUN_ROOT/phase_profile_0163.csv"
TOP_CSV="$RUN_ROOT/phase_profile_top_0163.csv"
Q6_CG_CSV="$RUN_ROOT/q6_cg_profile_0163.csv"
Q6_CG_TOP_CSV="$RUN_ROOT/q6_cg_profile_top_0163.csv"

cat <<INFO
[0163-profile] root       : $ROOT_DIR
[0163-profile] binary     : $BIN
[0163-profile] run root   : $RUN_ROOT
[0163-profile] grid/gamma : ${NX}x${NY} / $GAMMA
[0163-profile] steps      : $STEPS
[0163-profile] threads    : $THREAD_LIST
[0163-profile] cases      : $CASE_LIST
INFO
...
for label in $CASE_LIST; do
  for threads in $THREAD_LIST; do
    ...
    echo "[0163-profile] running $label threads=$threads"
...
PY
    ...
    if [[ -f "$out_dir/phase_profile_0163.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/phase_profile_0163.csv" >> "$PHASE_CSV"
    else
      echo "Missing phase profile: $out_dir/phase_profile_0163.csv" >&2
      exit 3
    fi

    if [[ -f "$out_dir/q6_cg_profile_0163.csv" ]]; then
      awk -F',' -v label="$label" -v threads="$threads" 'NR>1 {print label "," threads "," $0}' \
        "$out_dir/q6_cg_profile_0163.csv" >> "$Q6_CG_CSV"
    else
      echo "Missing Q6/CG profile: $out_dir/q6_cg_profile_0163.csv" >&2
      exit 3
    fi
  done
done
...
echo "[0163-profile] wrote $PERF_CSV"
echo "[0163-profile] wrote $PHASE_CSV"
echo "[0163-profile] wrote $TOP_CSV"
echo "[0163-profile] wrote $Q6_CG_CSV"
echo "[0163-profile] wrote $Q6_CG_TOP_CSV"
