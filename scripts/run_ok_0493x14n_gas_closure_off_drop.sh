...
for required in \
  ...
  if [[ ! -f "$required" ]]; then
    echo "[0493x14n] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14n — RUNNER-ONLY GAS-CLOSURE ABLATION (NO C++/CUDA PATCH)
#
# Purpose:
#   Causal control for the square-drop defect seen with x14l/x14m.
#   Keep the same explicit gas, x6g pressure coupling, common L/G SRC collision,
#   differentiated thermostat, x9 capillarity and the historical liquid-side
#   x10o+CIC+Q2+x10p/q+x10u position-only path used before x10v/x12a were
#   restored.  Disable ALL gas kinetic-interface handling:
#       phaseInterfaceKineticBilateralRelocation = false
#       MPCD_X14L_GAS_SPECULAR_REFLECTION = 0
#   Therefore the x10 particle pass sees phase A/liquid only; phase B/gas is
#   untouched by x10 and may cross naturally.  x10v/x10w/x12a remain OFF.
#
# Important scalar-target detail:
#   With bilateral geometry OFF, historical x10o resolves its thermal envelope
#   from thermostatTargetKBT.  Set that scalar to LIQUID_KBT so the liquid x10o
#   envelope remains the same as in x14l.  This does NOT change the actual gas
#   thermostat because speciesThermostatEnable=true and both species targets are
#   explicitly declared; x6g continues to use global KBT=GAS_KBT.
#
...
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14n_gas_closure_off_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
SURFACE_TENSION_SIGMA="${SURFACE_TENSION_SIGMA:-256.0}"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14n] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14n] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14n] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14n] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14n_gas_closure_off_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14n.csv
...
# 0493x14n runner-only ablation: disable bilateral B geometry and gas specular.
# The existing x10o/Q2 particle pass therefore filters back to phase A/liquid
# only. No source/CUDA change is needed.
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14n.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14n.env" <<META
...
run_ok_surface_print_0493x13zi "x9+x6g-liquid-x10u-gas-kinetic-closure-OFF"
echo "===== 0493x14n GAS KINETIC CLOSURE OFF — CAUSAL ABLATION ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14n_gas_closure_off_drop.sh"
...
echo "KIN-L: x10o+CIC+Q2+x10p/q+x10u position-only; x10v/x10w/x12a=OFF"
echo "KIN-G: OFF — no x10u, no specular reflection, no orphan guard; natural crossings allowed"
echo "THERM: species targets TL=$LIQUID_KBT TG=$GAS_KBT ; global KBT(x6g)=$KBT ; scalar thermostatTargetKBT(x10o)=$THERMOSTAT_TARGET_KBT"
echo "RUN:   steps=$STEPS dt=$DT summaryEvery=$SUMMARY_EVERY dumpEvery=$DUMP_STATE_EVERY 0272=1 bilateralGeometry=false gasSpecular=false"
...
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14n.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14n.csv"

echo "[0493x14n] DONE"
echo "[0493x14n] return: $ANALYSIS_DIR/radial_metrics_0493x14n.csv"
echo "[0493x14n] return: $ANALYSIS_DIR/radial_profiles_0493x14n.csv"
echo "[0493x14n] return: $ANALYSIS_DIR/laplace_static_drop_0493x14n.json (if produced)"
