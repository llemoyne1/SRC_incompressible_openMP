...
# 0253 — validate active CUDA SRC collision on top of the persistent 0251
# particle state for the deterministic wall-simple Poiseuille subset; Q6/projection remains CPU.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0253}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_src_collision_0253}
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0253-persistent-src-collision-wall] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
...
for spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${nx:-}" || -z "${ny:-}" ]]; then
    echo "[0253-persistent-src-collision-wall] ERROR: GRID_CASES entries must be NX:NY:STEPS, got '$spec'" >&2
    exit 2
  fi
  if [[ -z "${steps:-}" ]]; then steps=300; fi
  label="${nx}x${ny}_s${steps}"
  echo "[0253-persistent-src-collision-wall] running baseline grid=$label cases=[$CASE_LIST]"
  base_root="runs/cuda_src_collision_0253_baseline_${label}"
  ...
  for mode in "$CELL0251_MODE_NAME" "$COLL0252_MODE_NAME"; do
    echo "[0253-persistent-src-collision-wall] running mode=$mode grid=$label"
    run_root="runs/cuda_src_collision_0253_${mode}_${label}"
...
PY
    echo "[0253-persistent-src-collision-wall] $verdict grid=$label mode=$mode failed=$failed/$compared wall=$mode_total baseline=$base_total"
    if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
      echo "[0253-persistent-src-collision-wall] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0253-persistent-src-collision-wall] wrote $OUT_CSV"
echo "[0253-persistent-src-collision-wall] Q6/Q9 stayed CPU; collision CUDA is restricted to deterministic wall-simple Poiseuille (wallThermalNoise=0)."
