...
# 0493x17d-fix6 coherent thin-strip membrane with cross-thickness triangulation.
...
CASE_LABEL="${CASE_LABEL:-0493x17d_visible_membrane_fix6}"
MODE="${MODE:-src-q6}"
TOPOLOGY=periodic
GEN_CASE=uniform
...
MEMBRANE_K_CROSSBRACE="${MEMBRANE_K_CROSSBRACE:-150000.0}"
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x17d_visible_membrane_fix6}"
...
python3 - "$REFINE" "$Lx" "$Ly" "$NX" "$NY" "$H" "$DT" "$KBT" "$PARTICLE_MASS" \
  ...
  "$MEMBRANE_K_STRETCH" "$MEMBRANE_K_BENDING" "$MEMBRANE_K_CROSSBRACE" "$SOLID_MASS" <<'PY'
import math,sys
(r,Lx,Ly,nx,ny,h,dt,kbt,m,xmin,xmax,ymin,ymax,ks,kb,kc,ms) = map(float,sys.argv[1:])
nx=int(nx); ny=int(ny)
...
Sstretch=dt*math.sqrt((ks+2.0*kc)/node_mass)
...
printf 'membrane M=%s kS=%s kA=%s damping=%s bending=%s crossBrace=%s\n\n' "$SOLID_MASS" "$MEMBRANE_K_STRETCH" "$MEMBRANE_K_AREA" "$MEMBRANE_DAMPING" "$MEMBRANE_K_BENDING" "$MEMBRANE_K_CROSSBRACE"
