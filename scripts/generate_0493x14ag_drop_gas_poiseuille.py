#!/usr/bin/env python3
"""0493x14ag — circular liquid drop initially at rest in a developed gas Poiseuille profile."""
...
def main():
    ...
    ap.add_argument('--gas-umax',type=nn,default=0.075); ap.add_argument('--drop-ux0',type=float,default=0.0); ap.add_argument('--drop-uy0',type=float,default=0.0)
    ap.add_argument('--seed',type=int,default=493190)
    a=ap.parse_args()
    ...
    if min(cx-R,a.Lx-cx-R,cy-R,a.Ly-cy-R)<=8*h:ap.error('drop clearance must exceed 8h')
    ax=coprime(a.gamma,3); ay=coprime(a.gamma,7,ax)
    rl=random.Random(a.seed^0x14A61); rg=random.Random(a.seed^0x14A72)
    x=array('d');y=array('d');vx=array('d');vy=array('d');typ=array('I');mass=array('d');role=bytearray(); nL=nG=mixed=0
    def ugas(py):
        eta=2.0*py/a.Ly-1.0
        return max(0.0,a.gas_umax*(1.0-eta*eta))
    ...
    meta={'profile':'two_phase_drop_gas_poiseuille_0493x14ag','Lx':a.Lx,'Ly':a.Ly,'nx':a.nx,'ny':a.ny,'h':h,'gamma':a.gamma,'centerX':cx,'centerY':cy,'radius':R,'radiusCells':a.radius_cells,'liquidType':a.liquid_type,'gasType':a.gas_type,'liquidMass':a.liquid_mass,'gasMass':a.gas_mass,'liquidKBT':a.liquid_kBT,'gasKBT':a.gas_kBT,'gasUmax':a.gas_umax,'gasUmeanNominal':2*a.gas_umax/3,'dropUx0':a.drop_ux0,'dropUy0':a.drop_uy0,'particles':len(x),'liquidParticles':nL,'gasParticles':nG,'mixedCells':mixed,'seed':a.seed}
    a.output.with_suffix(a.output.suffix+'.json').write_text(json.dumps(meta,indent=2)+'\n')
    print(f"[0493x14ag-generate] grid={a.nx}x{a.ny} h={h:.10g} N={len(x)} R/h={a.radius_cells:g} center=({cx:.8g},{cy:.8g})")
    print(f"[0493x14ag-generate] liquid={nL} gas={nG} mixedCells={mixed} gasUmax={a.gas_umax:.8g} gasUmean={2*a.gas_umax/3:.8g} dropU0=({a.drop_ux0:.8g},{a.drop_uy0:.8g})")
    return 0
