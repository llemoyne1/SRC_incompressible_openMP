...
for f in "$GENERATOR" "$ANALYZER" "$PRESSURE_COVARIATE_ANALYZER" "$SRC_Q6" "$SRC_BASE" "$SRC_PARAMS"; do
...
grep -q -- '--incident-height-over-jet' "$ANALYZER" || {
  echo '[gas-jet-chi-nozzle-wide] ERROR local jet analyzer with --incident-height-over-jet is required' >&2; exit 2;
}

# =============================================================================
# 0493x14as-fix1 — DARCY/CHI NOZZLE, WIDE OUTLETS, GAS PRESSURE AS MEASURED COVARIATE (2-D)
#
...
# =============================================================================
CASE_LABEL="${CASE_LABEL:-0493x14as_fix1_planar_gas_jet_chi_nozzle_wide_outlets}"
RUN_MODE="src-q6-g-f"
TOPOLOGY="segmented"
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14as_fix1_planar_gas_jet_chi_nozzle_wide_outlets_s${SURFACE_TENSION_SIGMA}_seed${SEED}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14as_fix1.csv
...
echo "PATHS: runner=$ROOT/scripts/run_0493x14as_fix1_planar_gas_jet_chi_nozzle_wide_outlets.sh"
...
OUT_TAR="$CAMPAIGN_ROOT/0493x14as_fix1_planar_gas_jet_chi_nozzle_wide_outlets_compact.tar.gz"
FILES=(
  analysis
  "chi/${CASE_LABEL}_${NX}x${NY}.f32"
  "chi/${CASE_LABEL}_${NX}x${NY}.f32.json"
  "output/darcy_cost_0343.csv"
  "output/species_runtime_0493x14as_fix1.csv"
...
echo "[gas-jet-chi-nozzle-wide] COMPLETE"
echo "[gas-jet-chi-nozzle-wide] compact=$OUT_TAR"
