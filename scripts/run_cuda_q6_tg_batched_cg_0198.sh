...
# 0198 — CUDA Q6 TG regression for batched device-side CG.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0198}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_integration_0198}
GRID_CASES=${GRID_CASES:-"64:1000"}
BATCH_CASES=${BATCH_CASES:-"1 5 10 20"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-100}
SEED_BASE=${SEED_BASE:-1620198}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0198.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0198-cuda-batched-cg] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

SCALING_CSV=${SCALING_CSV:-$ART_DIR/cuda_q6_tg_batched_cg_0198.csv}
...
with open(out_csv, 'a', newline='') as f:
    ...
    writer.writerow(row)
print(f"[0198-cuda-batched-cg] PASS {label}/{mode}: wall CPU/CUDA={row['speedupWallCpuOverCuda']:.3f}, reductions={row['cudaTimingReductions']}, batches={row['cudaTimingDeviceScalarCgBatches']}, convergenceDownloads={row['cudaTimingDeviceScalarCgConvergenceDownloads']}, hostReduction={row['cudaTimingHostReductionSeconds']:.3f}s")
PY
}
...
run_one() {
  ...
  local cpu_root="runs/cuda_q6_tg_cpu_ref_0198_${label}"

  echo "[0198-cuda-batched-cg] CPU reference $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
      ...
      RUN_ROOT="$cpu_root" RUN_TAG="cuda0198_cpu_${label}" PROJECTION_BACKEND=cpu \
  ...
  for batch in $BATCH_CASES; do
    mode="batch${batch}"
    cuda_root="runs/cuda_q6_tg_cuda_0198_${mode}_${label}"
    compare_out="$ART_DIR/cuda_q6_tg_compare_0198_${mode}_${label}.csv"
    compare_summary_out="$ART_DIR/cuda_q6_tg_compare_summary_0198_${mode}_${label}.csv"
    echo "[0198-cuda-batched-cg] CUDA $mode $label"
    env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
        NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
        THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
        RUN_ROOT="$cuda_root" RUN_TAG="cuda0198_${mode}_${label}" PROJECTION_BACKEND=cuda \
...
if [[ "$RUN_UNSUPPORTED_CHECK" == "1" ]]; then
  echo "[0198-cuda-batched-cg] Checking that non-periodic Poiseuille remains rejected by projectionBackend=cuda."
  FAIL_ROOT=${FAIL_ROOT:-runs/cuda_q6_unsupported_expected_fail_0198}
  rm -rf "$FAIL_ROOT"
  set +e
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="poiseuille_wall_full" \
      RUN_ROOT="$FAIL_ROOT" RUN_TAG="cuda0198_expected_unsupported" \
      PROJECTION_BACKEND=cuda NX=16 NY=16 GAMMA=4 STEPS=1 SUMMARY_EVERY=1 THREADS=2 \
      bash scripts/run_validation_mono_config_0162.sh \
      > "$ART_DIR/cuda_q6_unsupported_stdout_0198.log" \
      2> "$ART_DIR/cuda_q6_unsupported_stderr_0198.log"
  status=$?
  set -e
  if [[ $status -eq 0 ]]; then
    echo "[0198-cuda-batched-cg] ERROR: unsupported non-periodic CUDA Q6 case unexpectedly succeeded." >&2
    exit 1
  fi
  if ! grep -R "fully periodic, unmasked" "$ART_DIR/cuda_q6_unsupported_stderr_0198.log" "$FAIL_ROOT" >/dev/null 2>&1; then
    echo "[0198-cuda-batched-cg] ERROR: unsupported-case failure did not contain the expected guard message." >&2
    exit 1
  fi
  echo "[0198-cuda-batched-cg] PASS: unsupported non-periodic CUDA Q6 case failed explicitly."
fi

echo "[0198-cuda-batched-cg] wrote $SCALING_CSV"
echo "[0198-cuda-batched-cg] PASS"
