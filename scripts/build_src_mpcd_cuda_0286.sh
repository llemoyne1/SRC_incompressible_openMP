...
# 0286 — consolidated CUDA SRC classic build with immersed circle and inlet/outlet support.
# This final 0286 build consolidates 0276--0285: boundary-aware thermostat, rectangle/step, circular immersed solid, full-face and segmented inlet/outlet. It is still a SRC classic build; Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0286}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0286-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0286-cuda-build] Built $OUT"
