...
# 0226 — CUDA-enabled SRC/MPCD executable with shared CudaParticleState
# and shared CudaCellWorkspace for the persistent SRC+thermostat substep.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0226}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0226-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0226-cuda-build] Built $OUT"
