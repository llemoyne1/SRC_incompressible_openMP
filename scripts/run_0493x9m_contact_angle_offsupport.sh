...
CASE_LABEL="contact_angle_offsupport_0493x9m"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"; TOPOLOGY=closed_box
RUN_ROOT="${RUN_ROOT:-runs/0493x9m_contact_angle_offsupport}"
BASELINE_X9L_ROOT="${BASELINE_X9L_ROOT:-runs/0493x9l_contact_angle_wallface}"
BASELINE_X9K_ROOT="${BASELINE_X9K_ROOT:-runs/0493x9k_contact_angle_mirror}"
ANGLES="${ANGLES:-30 45 60 75 90 105 120 135 150}"
...
if ((${#ANGLE_LIST[@]} == 0)); then
  echo "[0493x9m] ERROR: ANGLES is empty" >&2
  exit 2
fi
RADIUS="$(awk -v lx="$Lx" -v nx="$NX" -v rc="$RADIUS_CELLS" 'BEGIN{printf "%.17g",lx/nx*rc}')"
python3 - "$Lx" "$Ly" "$NX" "$NY" "$RADIUS" "$SIGMA" "${ANGLE_LIST[@]}" <<'PY'
import math,sys
lx,ly=map(float,sys.argv[1:3]); nx,ny=map(int,sys.argv[3:5]); r,s=map(float,sys.argv[5:7]); angles=list(map(float,sys.argv[7:]))
if abs(lx/nx-ly/ny)>1e-12: raise SystemExit('[0493x9m] square cells required')
if not (r>0 and s>0): raise SystemExit('[0493x9m] radius and sigma must be positive')
for a in angles:
    if not (math.isfinite(a) and 0.0 < a < 180.0):
        raise SystemExit('[0493x9m] contact angles must satisfy 0 < theta < 180; endpoints are intentionally unsupported')
PY
...
run_one(){
  local theta="$1" label="${1//./p}" dir="$RUN_ROOT/theta${1//./p}"
  mkdir -p "$dir/init" "$dir/params" "$dir/output" "$dir/logs"
  local state="$dir/init/sessile_cap_0493x9m.smpcd" params="$dir/params/contact_angle_offsupport_0493x9m.kv" log="$dir/logs/contact_angle_offsupport_0493x9m.log" tf="$dir/logs/contact_angle_offsupport_0493x9m.time"
...
speciesDiagnosticsFilename = species_runtime_0493x9m.csv
...
PARAMS_EOF
  ...
  BASE_RUN_ROOT="$dir"; LIVE_VIS_CONTROL_FILE="$dir/livevis_control_0493x9m.kv"; suite_prepare_livevis_control_0434 "$dir" "$RUN_MODE"; suite_export_livevis_0434
  echo "[0493x9m-suite] theta=$theta radius=$RADIUS exactKappa=$(awk -v r="$RADIUS" 'BEGIN{printf "%.8g",1/r}') sigma=$SIGMA closure=off_support_secant_curvature anchorLayer=4 anchorCenter=4.5h"
...
if ! suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  python3 scripts/analyze_0493x9m_contact_angle_offsupport.py \
    --root "$RUN_ROOT" --angles "${ANGLE_LIST[@]}" --radius "$RADIUS" \
    --baseline-x9l-root "$BASELINE_X9L_ROOT" --baseline-x9k-root "$BASELINE_X9K_ROOT"
fi
