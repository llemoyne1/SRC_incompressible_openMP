...
# 0203 — CUDA-enabled SRC/MPCD executable with both experimental GPU paths:
#   - Q6 CUDA projection backend from the 0198/0196 line;
#   - CUDA particle->cell moment deposit from the 0202 line.
# CPU/OpenMP remains the default runtime path.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0203}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0203-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0203-cuda-build] Built $OUT"
