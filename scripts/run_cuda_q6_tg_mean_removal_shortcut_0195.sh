...
# 0195 — CUDA Q6 TG regression for the residual mean-removal norm shortcut.
# Default CUDA mode avoids an explicit post-gauge-fix sum(r^2) reduction by using
# ||r - mean(r)||^2 = ||r||^2 - sum(r)^2 / n.  The legacy explicit recomputation
# remains available as an ablation.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0195}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_integration_0195}
GRID_CASES=${GRID_CASES:-"64:1000"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-100}
SEED_BASE=${SEED_BASE:-1620195}
...
shortcuts = ti('residualNormShortcuts')
if mode == 'shortcut' and shortcuts <= 0:
    raise SystemExit(f"expected residualNormShortcuts > 0 for {label}/{mode}, got {shortcuts}")
if mode == 'legacy' and shortcuts != 0:
    raise SystemExit(f"expected residualNormShortcuts == 0 for legacy mode, got {shortcuts}")
row = {
    ...
    'cudaTimingResidualNormShortcuts': shortcuts,
...
with open(out_csv, 'a', newline='') as f:
    ...
    writer.writerow(row)
print(f"[0195-cuda-mean-shortcut] PASS {label}/{mode}: speedup wall CPU/CUDA={row['speedupWallCpuOverCuda']:.3f}, reductions={row['cudaTimingReductions']}, shortcuts={shortcuts}, hostReduction={row['cudaTimingHostReductionSeconds']:.3f}s")
PY
}
...
run_one() {
  ...
  local cpu_root="runs/cuda_q6_tg_cpu_ref_0195_${label}"

  echo "[0195-cuda-mean-shortcut] CPU reference $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
      ...
      RUN_ROOT="$cpu_root" RUN_TAG="cuda0195_cpu_${label}" PROJECTION_BACKEND=cpu \
      bash scripts/run_validation_mono_config_0162.sh

  local mode cuda_root compare_out compare_summary_out legacy_flag shortcut_flag
  for mode in shortcut legacy; do
    if [[ "$mode" == "legacy" && "$RUN_LEGACY_ABLATION" != "1" ]]; then
      continue
    fi
    cuda_root="runs/cuda_q6_tg_cuda_0195_${mode}_${label}"
    compare_out="$ART_DIR/cuda_q6_tg_compare_0195_${mode}_${label}.csv"
    compare_summary_out="$ART_DIR/cuda_q6_tg_compare_summary_0195_${mode}_${label}.csv"
    if [[ "$mode" == "legacy" ]]; then
      legacy_flag=1
      shortcut_flag=0
    else
    ...
    echo "[0195-cuda-mean-shortcut] CUDA $mode $label"
    env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="tg_periodic_full" \
        NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
        THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
        RUN_ROOT="$cuda_root" RUN_TAG="cuda0195_${mode}_${label}" PROJECTION_BACKEND=cuda \
        ...
        MPCD_CUDA_Q6_RESIDUAL_NORM_SHORTCUT="$shortcut_flag" \
        MPCD_CUDA_Q6_LEGACY_MEAN_REMOVAL_RESIDUAL_NORM="$legacy_flag" \
...
if [[ "$RUN_UNSUPPORTED_CHECK" == "1" ]]; then
  echo "[0195-cuda-mean-shortcut] Checking that non-periodic Poiseuille remains rejected by projectionBackend=cuda."
  FAIL_ROOT=${FAIL_ROOT:-runs/cuda_q6_unsupported_expected_fail_0195}
  rm -rf "$FAIL_ROOT"
  set +e
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="poiseuille_wall_full" \
      RUN_ROOT="$FAIL_ROOT" RUN_TAG="cuda0195_expected_unsupported" \
      PROJECTION_BACKEND=cuda NX=16 NY=16 GAMMA=4 STEPS=1 SUMMARY_EVERY=1 THREADS=2 \
      bash scripts/run_validation_mono_config_0162.sh \
      > "$ART_DIR/cuda_q6_unsupported_stdout_0195.log" \
      2> "$ART_DIR/cuda_q6_unsupported_stderr_0195.log"
  status=$?
  set -e
  if [[ $status -eq 0 ]]; then
    echo "[0195-cuda-mean-shortcut] ERROR: unsupported non-periodic CUDA Q6 case unexpectedly succeeded." >&2
    exit 1
  fi
  if ! grep -R "fully periodic, unmasked" "$ART_DIR/cuda_q6_unsupported_stderr_0195.log" "$FAIL_ROOT" >/dev/null 2>&1; then
    echo "[0195-cuda-mean-shortcut] ERROR: unsupported-case failure did not contain the expected guard message." >&2
    exit 1
  fi
  echo "[0195-cuda-mean-shortcut] PASS: unsupported non-periodic CUDA Q6 case failed explicitly."
fi

echo "[0195-cuda-mean-shortcut] wrote $SCALING_CSV"
echo "[0195-cuda-mean-shortcut] PASS"
