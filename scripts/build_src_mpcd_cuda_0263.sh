...
# 0263 — full-face inlet/outlet classic SRC CUDA resident build.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0263}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0263-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0263-cuda-build] Built $OUT"
