...
# 0249b — segmented inlet/outlet CUDA validation.
# Scope: segmented_u_turn_full only. The CUDA kernel marks particles crossing
# openBoundarySegment intervals inactive before the existing CPU hard reservoir
# rebuild inserts the deterministic inlet band. The uncovered parts of the face
# remain solid and are intentionally left to the CPU boundary pass.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0249b}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_inlet_outlet_0249b}
GRID_CASES=${GRID_CASES:-"64:300 128:300"}
CASE_LIST=${CASE_LIST:-"segmented_u_turn_full"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
...
IO_MODE_NAME=${IO_MODE_NAME:-cuda_inlet_outlet_segmented_0249b}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0249b.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0249b-segmented-io] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_inlet_outlet_segmented_0249b.csv}
printf 'caseList,grid,NX,NY,steps,mode,resamplingCuda,periodicStreamingCuda,wallSimpleStreamingCuda,immersedRectangleCuda,pistonStreamingCuda,inletOutletSegmentedCuda,projectionEnable,projectionBackend,runExitCode,baselineWallTime,modeWallTime,wallDelta_s,wallSpeedup,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0249b-segmented-io] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      MPCD_CUDA_INLET_OUTLET_SEGMENTED_0249B="$io_cuda" \
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_inlet_outlet_0249b_${BASELINE_MODE_NAME}_${grid}"
  base_stdout="$ART_DIR/cuda_inlet_outlet_0249b_${BASELINE_MODE_NAME}_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_inlet_outlet_0249b_${BASELINE_MODE_NAME}_${grid}.stderr.log"
  echo "[0249b-segmented-io] running baseline grid=$grid cases=[$CASE_LIST]"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0249b_${BASELINE_MODE_NAME}_${grid}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
  base_rc=$?
  ...
  for mode in "$STACK_MODE_NAME" "$IO_MODE_NAME"; do
    run_root="runs/cuda_inlet_outlet_0249b_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_inlet_outlet_0249b_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_inlet_outlet_0249b_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_inlet_outlet_compare_0249b_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_inlet_outlet_compare_summary_0249b_${mode}_${grid}.csv"
    echo "[0249b-segmented-io] running mode=$mode grid=$grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0249b_${mode}_${grid}" "$mode" "$stdout_log" "$stderr_log"
    rc=$?
...
PY
    echo "[0249b-segmented-io] $verdict mode=$mode grid=$grid: wall=$wall baseline=$base_wall failed=$failed/$compared"
...
echo "[0249b-segmented-io] wrote $OUT_CSV"
echo "[0249b-segmented-io] Segmented openBoundarySegment* extraction is CUDA; Q6/Q9, collision and hard-reservoir insertion stay CPU."
