...
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x9z_vacuum_drop_individual}"

# Exact x9y qualification fluid/physics; only reflection mechanics changes.
...
# x9y had deepOuter=690 by step75 and 2375 by step100. 150 steps gives a
# decisive first verdict while keeping the individualization iteration cheap.
...
printf '%s\n' \
  "[0493x9z-suite] INDIVIDUAL donor reflection: each crossing uses its own normal" \
  "[0493x9z-suite] three existing x9 particle passes; NO extra full particle pass" \
  "[0493x9z-suite] one O(Ncell) reaction kernel; receiver affine P/E compensation" \
  "[0493x9z-suite] no merge, no resampling cleanup, evaporation path retained" \
  "[0493x9z-suite] steps=$STEPS summaryEvery=$SUMMARY_EVERY livevis=$LIVE_VIS_ENABLE"

bash scripts/run_0493x9s_splash.sh

CSV="$RUN_ROOT/output/cuda_phase_kinetic_crossing_0493x9z.csv"
[[ -f "$CSV" ]] || { echo "[0493x9z-suite] ERROR missing $CSV" >&2; exit 2; }
python3 scripts/analyze_0493x9z_individual.py "$CSV"
