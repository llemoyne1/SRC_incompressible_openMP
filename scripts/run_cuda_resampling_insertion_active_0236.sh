...
# 0236 — active CUDA insertion stage for the weighted resampling pipeline.
# CPU extraction remains active and authoritative.  CUDA replaces only the
# insertion write-back over the already-selected passive extraction operations.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0236}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_0236}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620236}
...
MODES=${MODES:-"cpu_baseline cuda_resampling_insertion"}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0236.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0236-resampling-insertion] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_insertion_active_0236.csv}
...
run_validation_logged() {
  local nx=$1 ny=$2 steps=$3 root=$4 tag=$5 use_cuda=$6 stdout_log=$7 stderr_log=$8
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      MPCD_CUDA_RESAMPLING_INSERTION_USE="$use_cuda" \
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_resampling_insertion_0236_baseline_${grid}"
  base_stdout="$ART_DIR/cuda_resampling_insertion_0236_baseline_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_resampling_insertion_0236_baseline_${grid}.stderr.log"
  echo "[0236-resampling-insertion] running baseline $grid"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0236_baseline" 0 "$base_stdout" "$base_stderr"
  base_rc=$?
  ...
  for mode in $MODES; do
    ...
    case "$mode" in
      cuda_resampling_insertion) use_cuda=1 ;;
      *) echo "[0236-resampling-insertion] unknown mode $mode" >&2; exit 2 ;;
    esac
    run_root="runs/cuda_resampling_insertion_0236_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_resampling_insertion_0236_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_insertion_0236_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_resampling_insertion_compare_0236_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_resampling_insertion_compare_summary_0236_${mode}_${grid}.csv"
    echo "[0236-resampling-insertion] running $mode $grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0236_${mode}" "$use_cuda" "$stdout_log" "$stderr_log"
    rc=$?
...
PY
    echo "[0236-resampling-insertion] $verdict $grid/$mode: wall=$wall baseline=$base_wall speedup=$(python3 - <<PY
...
echo "[0236-resampling-insertion] wrote $OUT_CSV"
