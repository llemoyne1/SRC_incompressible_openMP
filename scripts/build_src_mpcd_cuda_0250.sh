...
# 0250 — CUDA boundary stack + active CUDA cell-moments executable.
# The CUDA cell-moments path is still opt-in at runtime through
# MPCD_CUDA_CELL_MOMENTS_USE=1.  The default build keeps all previously
# validated CUDA boundary/resampling modules available, with CPU fallback.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0250}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0250-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0250-cuda-build] Built $OUT"
