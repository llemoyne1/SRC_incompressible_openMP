...
for required in \
  ...
  if [[ ! -f "$required" ]]; then
    echo "[0493x14k] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14k — bilateral x10u position-only liquid/gas drop
#
# Purpose:
#   First physical validation of 0493x14k: reuse the qualified liquid-side
#   x10o+CIC+Q2+x10p+x10u support relocation and apply the same position-only
#   semantics to one explicit gas B species with reversed phase sense.
...
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14k_bilateral_x10u_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14k] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14k] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14k] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14k] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14k_bilateral_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}}"
...
speciesDiagnosticsFilename = species_runtime_0493x14k.csv
...
# 0493x14k V1: one existing particle pass handles both A and B.  The liquid
# defines the x10o/Q2 moving wall; both types are position-mirrored only.
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14k.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14k.env" <<META
...
run_ok_surface_print_0493x13zi "x9+x6g-liquid-gas-x10u-bilateral-position-only"
echo "===== 0493x14k BILATERAL X10U DROP ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14k_bilateral_x10u_drop.sh"
...
echo "KIN:   x10o+CIC+Q2+x10p+x10u BILATERAL position-only; x10v/x10w/x12a=OFF; no orphan guard"
echo "RUN:   steps=$STEPS dt=$DT summaryEvery=$SUMMARY_EVERY dumpEvery=$DUMP_STATE_EVERY 0272=1 bilateral=true"
...
# Keep the existing x14j analyzer untouched, but publish x14k-named copies so
# bilateral results cannot be confused with the no-closure baseline.
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14k.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14k.csv"

echo "[0493x14k] DONE"
echo "[0493x14k] return: $ANALYSIS_DIR/radial_metrics_0493x14k.csv"
echo "[0493x14k] return: $ANALYSIS_DIR/radial_profiles_0493x14k.csv"
echo "[0493x14k] return: $ANALYSIS_DIR/laplace_static_drop_0493x14k.json (if produced)"
