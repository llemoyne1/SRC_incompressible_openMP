#!/usr/bin/env python3
"""0493x14aj — offline two-phase n=3 modal-drop analysis from liquid particle-state dumps.
...
def modal_state(path: Path, mode: int, R: float, phase: float):
    ...
    im_m = 0.0
    rpow = 0.0
    # Explicit n=3 path is both faster and numerically transparent.  Keep a
    # generic complex-power fallback so the offline tool can later audit n=4.
    if mode == 3:
        for i in idx:
            w = 1.0 if mass is None else mass[i]
            dx = x[i] - cx
            dy = y[i] - cy
            re_m += w * (dx * dx * dx - 3.0 * dx * dy * dy)
            im_m += w * (3.0 * dx * dx * dy - dy * dy * dy)
            rpow += w * (dx * dx + dy * dy) ** 1.5
    else:
        for i in idx:
            w = 1.0 if mass is None else mass[i]
            dx = x[i] - cx
...
def main():
    ...
    ap.add_argument("--gas-mass", type=float, default=0.1)
    ap.add_argument("--h", type=float, default=1.0 / 256.0)
    ap.add_argument("--nu-fit-scale", type=float, default=5.1e-4, help="Numerical beta search scale only; not a two-fluid damping theory claim")
    ...
    args = ap.parse_args()
    if args.mode != 3:
        raise SystemExit("[0493x14aj-analysis] this analyzer is locked to mode=3")
    ...
    if len(states) < 20:
        raise SystemExit(f"[0493x14aj-analysis] need >=20 state dumps, got {len(states)}")

    R = args.radius_cells * args.h
    rho_l = args.gamma * args.liquid_mass / (args.h * args.h)
    rho_g = args.gamma * args.gas_mass / (args.h * args.h)
    rho_total = rho_l + rho_g
    n = args.mode
    omega0 = math.sqrt(n * (n * n - 1.0) * args.sigma / (rho_total * R ** 3))
    T0 = 2.0 * math.pi / omega0
    # Only a numerical search scale for beta.  No analytical two-fluid damping
    # law is claimed here because the gas viscosity/slip closure is path-dependent.
    beta_ref = 2.0 * n * (n - 1.0) * args.nu_fit_scale / (R * R)
    omega_ref = omega0
    ...
    if len(fit) < 16:
        raise SystemExit(f"[0493x14aj-analysis] only {len(fit)} states in fit window")
    tt = [r["time"] for r in fit]
    yy = [r["qParallel"] for r in fit]
    best = fit_damped_signed(tt, yy, omega0, beta_ref)
    if best is None:
        raise SystemExit("[0493x14aj-analysis] damped fit failed")
    _sse, omega, beta, r2, coef, pred = best
    offset = coef[2]
    omega_zc, zc_count = zero_cross_omega(tt, yy, offset)

    analysis = args.run_root / "analysis_0493x14aj"
    analysis.mkdir(parents=True, exist_ok=True)
    trace = analysis / "oscillating_drop_n3_state_moment_trace.csv"
    with trace.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["step", "time", "q3Parallel", "q3Quadrature", "q3Abs", "xCM", "yCM", "particles", "mass", "momentReal", "momentImag", "radialMoment"])
        for r in series:
    ...
    lines = [
        "===== 0493x14aj TWO-PHASE 2-D OSCILLATING DROP / n=3 =====",
        ...
        f"R={R:.12g} R/h={args.radius_cells:g} rhoL={rho_l:.12g} rhoG={rho_g:.12g} rhoTotal={rho_total:.12g} sigma={args.sigma:.12g}",
        f"theory2D.inviscid omega0={omega0:.12g} period0={T0:.12g}",
        f"fitSearch betaScale={beta_ref:.12g} (numerical only; no two-fluid damping-theory claim)",
        f"fit omega={omega:.12g} beta={beta:.12g} R2={r2:.9g} offset={offset:.9g} C={coef[0]:.9g} D={coef[1]:.9g}",
        f"gainTwoFluid Gomega={omega/omega0:.9g}",
        ...
        f"modal q3Initial={q0:.9g} q3MinFit={min(yy):.9g} q3MaxFit={max(yy):.9g} quadratureRmsFit={qquad_rms:.9g}",
        ...
        "observable=q3=Re(<(z-zCM)^3> exp(+i*phase))/R^3; quadrature is audited separately.",
        "interpretation=Gomega uses the inviscid 2-D two-fluid inertia rhoL+rhoG; beta is measured empirically only.",
    ]
    report = "\n".join(lines) + "\n"
    (analysis / "oscillating_drop_n3_two_phase_report.txt").write_text(report)

    cm_dx = series[-1]["xCM"] - series[0]["xCM"]
    cm_dy = series[-1]["yCM"] - series[0]["yCM"]
    cm_norm = math.hypot(cm_dx, cm_dy)
    summary_json = {
        "benchmark": "0493x14aj_two_phase_oscillating_drop_n3",
        "mode": n,
        "stateRows": len(series),
        "fitRows": len(fit),
        "fitWindow": [fit[0]["time"], fit[-1]["time"]],
        "radius": R,
        "radiusCells": args.radius_cells,
        "sigma": args.sigma,
        "rhoLiquid": rho_l,
        "rhoGas": rho_g,
        "rhoTotal": rho_total,
        "omegaTheoryTwoFluid": omega0,
        "periodTheoryTwoFluid": T0,
        "omegaMeasured": omega,
        "GomegaTwoFluid": omega / omega0,
        "betaMeasured": beta,
        "fitR2": r2,
        "zeroCrossOmega": omega_zc,
        "zeroCrossings": zc_count,
        "quadratureRmsFit": qquad_rms,
        "cmDisplacement": {"dx": cm_dx, "dy": cm_dy, "norm": cm_norm, "inCells": cm_norm / args.h, "overR": cm_norm / R},
        "massMin": min(r["mass"] for r in series),
        "massMax": max(r["mass"] for r in series),
        "particlesMin": min(r["particles"] for r in series),
        "particlesMax": max(r["particles"] for r in series),
        "dampingTheoryStatus": "NOT_CLAIMED_PATH_DEPENDENT_TWO_FLUID",
    }
    (analysis / "oscillating_drop_n3_two_phase_summary_0493x14aj.json").write_text(json.dumps(summary_json, indent=2, sort_keys=True) + "\n")
    print(report, end="")
    print(f"twoPhaseSummary={analysis / 'oscillating_drop_n3_two_phase_summary_0493x14aj.json'}")
