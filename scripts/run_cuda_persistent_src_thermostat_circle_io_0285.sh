...
# 0285 — full-face inlet/outlet + circular immersed solid validation for the CUDA persistent SRC+thermostat path.
#
# Scope: open_circle_obstacle_classic, classic SRC only, Q6/resampling/virial disabled.
# Goal: compare CPU collision+CPU thermostat against the resident CUDA full-face inlet/outlet boundary path + CUDA circular solid
# reflection + persistent CUDA SRC collision + fused persistent CUDA thermostat.
#
...
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_circle_io_0285}
GRID_CASES=${GRID_CASES:-"64:64:120"}
CASES=${CASES:-"open_circle_obstacle_classic"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SEED_BASE=${SEED_BASE:-162850}
STOP_ON_FAIL=${STOP_ON_FAIL:-1}
FORCE_REBUILD=${FORCE_REBUILD:-1}
SUMMARY_EVERY_MODE=${SUMMARY_EVERY_MODE:-final}
VALIDATION_INACTIVE_SLOTS=${VALIDATION_INACTIVE_SLOTS:-}

BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_circle_io_src_thermostat}
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0285_cuda_circle_io_fullface_resident_src_fused_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0285-cuda-thermostat-circle-io] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0285.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0285.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0285-cuda-thermostat-circle-io] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_circle_io_0285.csv}
python3 - "$OUT_CSV" <<'PY'
import csv, sys
header = [
    ...
    'inletThermalNoise','ioFullfaceResidentFlag','immersedCircleFlag','fusedSrcThermostatUse',
...
run_validation_logged() {
  ...
  if [[ -n "${VALIDATION_INACTIVE_SLOTS:-}" ]]; then
    ...
    inactive_slots=$((GAMMA * ny * 16))
  fi

  local io_fullface=0 immersed_circle=0 immersed_download=1
  ...
  case "$mode" in
    ...
    "$CUDA_MODE_NAME")
      ...
      ;;
    *) echo "[0285-cuda-thermostat-circle-io] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$case_name" \
      ...
      MPCD_CUDA_IMMERSED_CIRCLE_0284="$immersed_circle" \
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -n "${extra:-}" || -z "${steps:-}" ]]; then
    echo "[0285-cuda-thermostat-circle-io] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "open_circle_obstacle_classic" ]]; then
      echo "[0285-cuda-thermostat-circle-io] ERROR: 0285 is limited to open_circle_obstacle_classic, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0285-cuda-thermostat-circle-io] running baseline case=$case_name grid=$grid_tag"
    base_rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_thermostat_circle_io_0285_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr" || base_rc=$?
    ...
    echo "[0285-cuda-thermostat-circle-io] running CUDA fused thermostat case=$case_name grid=$grid_tag"
    rc=0
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_thermostat_circle_io_0285_${case_name}_${CUDA_MODE_NAME}_${grid_tag}" "$CUDA_MODE_NAME" "$stdout_log" "$stderr_log" || rc=$?
    cmp_rc=0
...
PY
    ...
    echo "[0285-cuda-thermostat-circle-io] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0285-cuda-thermostat-circle-io] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0285-cuda-thermostat-circle-io] wrote $OUT_CSV"
echo "[0285-cuda-thermostat-circle-io] fused SRC thermostat is enabled only for this classic-only inlet/outlet + circular-solid validation."
