...
cd "$ROOT"
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x9u_vacuum_drop_reflection}"
...
printf '%s\n' \
  "[0493x9u-suite] qualification=vacuum-drop VK-fluid support-edge" \
  "[0493x9u-suite] gamma=$GAMMA dt=$DT kBT=$KBT mass=$LIQUID_MASS" \
  "[0493x9u-suite] sigma=$SIGMA_ACTIVE r=$KINETIC_REFLECTION_FRACTION" \
  "[0493x9u-suite] drop R/h=$DROP_RADIUS_CELLS center=($DROP_CENTER_X,$DROP_CENTER_Y)"
bash scripts/run_0493x9s_splash.sh
CSV="$RUN_ROOT/output/cuda_phase_kinetic_reflection_0493x9u.csv"
if [[ -f "$CSV" ]]; then
  python3 scripts/analyze_0493x9u_reflection.py --csv "$CSV"
else
  echo "[0493x9u-suite] ERROR audit CSV absent: $CSV" >&2
  exit 2
fi
