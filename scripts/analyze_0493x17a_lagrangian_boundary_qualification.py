...
ROOT=Path(sys.argv[1]) if len(sys.argv)>1 else Path('runs/0493x17a_lagrangian_boundary_qualification')
CASES=('static_rest','static_boost','rigid_rest','rigid_boost','deform_rest','deform_boost')

def rows(p):
    if not p.is_file(): raise SystemExit(f'[0493x17a] missing {p}')
...
def metric(name):
    ...
    if len(A)<20: raise SystemExit(f'[0493x17a] too few aligned rows {name}: {len(A)}')
...
lines=[
 '0493x17a persistent-Lagrangian-boundary qualification',
...
out=ROOT/'analysis'/'summary_0493x17a.txt'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text('\n'.join(lines)+'\n')
