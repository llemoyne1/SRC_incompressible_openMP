...
# 0198 — CUDA-enabled SRC/MPCD executable with experimental batched device-side CG.
# CPU/OpenMP remains the default reference path; projectionBackend=cuda is still
# restricted to the validated periodic, unmasked Q6/TG subset.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0198}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0198-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0198-cuda-build] Built $OUT"
