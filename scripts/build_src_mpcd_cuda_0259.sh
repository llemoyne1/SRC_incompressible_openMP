...
# 0259 — classic SRC CUDA build with Q6/virial short-circuit and fused deterministic thermostat.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0259}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0259-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0259-cuda-build] Built $OUT"
