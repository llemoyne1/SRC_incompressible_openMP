...
# 0313 — CUDA SRC classic build with nominal split-safety resampling consolidation.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0313}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0313-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0313-cuda-build] Built $OUT"
