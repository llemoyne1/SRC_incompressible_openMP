...
# 0220 — compare legacy persistent SRC+thermostat vs shared CudaParticleState active path.
# The physics subset matches 0215: periodic TG, no Q6, no virial/capacity, cell-relative thermostat.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0220}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_0220}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620220}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0220.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0220-persistent-particle-state] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_particle_state_active_0220.csv}
...
with open(out_csv,'a',newline='') as fcsv:
    ...
    w.writerow(row)
print(f"[0220-persistent-particle-state] {verdict} {label}/{mode}/{case_name}: wall={mode_wall:.6g}s baseline={base_wall:.6g}s speedup={row['wallSpeedup']:.4g} shared={row['sharedParticleStateFraction']:.2f} reused={row['particleStateReusedAllocationFraction']:.2f} failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_mode() {
  ...
  local run_root="runs/cuda_persistent_particle_state_0220_${mode}_${label}"
  local shared=0
  case "$mode" in
    legacy_internal_upload) shared=0 ;;
    shared_particle_state) shared=1 ;;
    *) echo "[0220-persistent-particle-state] ERROR: unknown mode $mode" >&2; exit 2 ;;
  esac

  echo "[0220-persistent-particle-state] mode=$mode $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      RUN_ROOT="$run_root" RUN_TAG="cuda0220_${mode}_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
  ...
  local compare_csv="$ART_DIR/cuda_persistent_particle_state_compare_0220_${mode}_${label}.csv"
  local compare_summary="$ART_DIR/cuda_persistent_particle_state_compare_summary_0220_${mode}_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    ...
    if [[ ! -f "$active_csv" ]]; then
      echo "[0220-persistent-particle-state] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
run_one() {
  ...
  local baseline_root="runs/cuda_persistent_particle_state_0220_cpu_baseline_${label}"

  echo "[0220-persistent-particle-state] baseline $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$baseline_root" RUN_TAG="cuda0220_cpu_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" PROJECTION_ENABLE="$PROJECTION_ENABLE" \
      MPCD_CUDA_CELL_MOMENTS_USE=0 \
      MPCD_CUDA_THERMOSTAT_USE=0 \
      MPCD_CUDA_SRC_COLLISION_USE=0 \
      MPCD_CUDA_PERSISTENT_SRC_COLLISION_USE=0 \
...
for item in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0220-persistent-particle-state] malformed GRID_CASES entry: $item" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0220-persistent-particle-state] wrote $OUT_CSV"
