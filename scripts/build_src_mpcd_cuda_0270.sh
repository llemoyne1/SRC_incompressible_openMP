...
# 0270 — CUDA resident classic SRC performance-optimized build.
# The feature set deliberately mirrors the validated 0269a stack; the new patch
# skips the redundant CPU boundary scan for wall-simple resident CUDA while
# preserving the classic-only validation scope and future Q6/resampling paths.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0270}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0270-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0270-cuda-build] Built $OUT"
