...
# 0245 — first CUDA streaming validation, deliberately restricted to the
# strictly periodic subset.  Q6/Q9, collision, cell moments and diagnostics stay
# CPU.  The 0244 CUDA resampling active path is kept as an optional already-
# validated companion path so this script can test whether adding CUDA streaming
# perturbs the periodic physical run.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0245}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_streaming_0245}
GRID_CASES=${GRID_CASES:-"64:300 128:300"}
CASE_LIST=${CASE_LIST:-"tg_periodic_full"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620245}
...
STREAMING_MODE_NAME=${STREAMING_MODE_NAME:-cuda_streaming_periodic_0245}

mkdir -p "$ART_DIR"

if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0245.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0245-streaming] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_streaming_periodic_0245.csv}
...
run_validation_logged() {
  ...
  case "$mode" in
    ...
    *) echo "[0245-streaming] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      MPCD_CUDA_STREAMING_PERIODIC_0245="$streaming_cuda" \
...
for spec in $GRID_CASES; do
  ...
  base_root="runs/cuda_streaming_periodic_0245_${BASELINE_MODE_NAME}_${grid}"
  base_stdout="$ART_DIR/cuda_streaming_periodic_0245_${BASELINE_MODE_NAME}_${grid}.stdout.log"
  base_stderr="$ART_DIR/cuda_streaming_periodic_0245_${BASELINE_MODE_NAME}_${grid}.stderr.log"
  echo "[0245-streaming] running baseline grid=$grid cases=[$CASE_LIST]"
  run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0245_${BASELINE_MODE_NAME}_${grid}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
  base_rc=$?
  ...
  for mode in "$RESAMPLING_MODE_NAME" "$STREAMING_MODE_NAME"; do
    run_root="runs/cuda_streaming_periodic_0245_${mode}_${grid}"
    stdout_log="$ART_DIR/cuda_streaming_periodic_0245_${mode}_${grid}.stdout.log"
    stderr_log="$ART_DIR/cuda_streaming_periodic_0245_${mode}_${grid}.stderr.log"
    compare_csv="$ART_DIR/cuda_streaming_periodic_compare_0245_${mode}_${grid}.csv"
    compare_summary="$ART_DIR/cuda_streaming_periodic_compare_summary_0245_${mode}_${grid}.csv"
    echo "[0245-streaming] running mode=$mode grid=$grid"
    run_validation_logged "$nx" "$ny" "$steps" "$run_root" "cuda0245_${mode}_${grid}" "$mode" "$stdout_log" "$stderr_log"
    rc=$?
...
PY
    echo "[0245-streaming] $verdict mode=$mode grid=$grid: wall=$wall baseline=$base_wall failed=$failed/$compared"
...
echo "[0245-streaming] wrote $OUT_CSV"
echo "[0245-streaming] scope: periodic streaming only; Q6/Q9 stayed CPU; wall/open/solid streaming stayed CPU."
