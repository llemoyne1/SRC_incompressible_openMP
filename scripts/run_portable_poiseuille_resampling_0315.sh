...
portable_ensure_binary_0315() {
  ...
  local helper=""
  for h in scripts/build_src_mpcd_cuda_0314.sh scripts/build_src_mpcd_cuda_0308.sh scripts/build_src_mpcd_cuda_0313.sh scripts/build_src_mpcd_cuda_0307.sh scripts/build_src_mpcd_cuda_0306.sh scripts/build_src_mpcd_cuda_0305.sh scripts/build_src_mpcd_cuda_0303.sh; do
  ...
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash "$helper"
...
Lx="${Lx:-2.0}"; Ly="${Ly:-1.0}"; NX="${NX:-96}"; NY="${NY:-48}"
...
BODY_AX="${BODY_AX:-0.01}"; U0="${U0:-0.02}"; THERMOSTAT_ENABLE="${THERMOSTAT_ENABLE:-1}"
...
% Visualization modalities for the poiseuille validation.
% Select runMode='classic' or 'resampling_split_safe'.
% Select fieldMode among: N, Ux, speed, mass.
root = '${BASE_RUN_ROOT}';
...
echo "[0315-portable] visualization: edit runMode={classic,resampling_split_safe}, fieldMode={N, Ux, speed, mass}"
