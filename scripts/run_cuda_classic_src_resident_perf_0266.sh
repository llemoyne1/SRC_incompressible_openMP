...
# 0266 — performance instrumentation pass for the validated CUDA resident
...
BIN=${BIN:-build/src_mpcd_base_cuda_0266}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_resident_perf_0266}
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_resident_perf_validation_0266.csv}
THREADS=${THREADS:-8}
GAMMA=${GAMMA:-20}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0266-perf] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0266.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0266.sh
fi
...
# 0266 performance root and use the 0266 binary.
BIN="$BIN" \
...
python3 scripts/summarize_cuda_resident_perf_0266.py "$ART_DIR/validation_runs" \
  --summary-out "$ART_DIR/cuda_classic_src_resident_perf_summary_0266.csv" \
  --phases-out "$ART_DIR/cuda_classic_src_resident_perf_phases_0266.csv"

echo "[0266-perf] validation manifest: $OUT_CSV"
echo "[0266-perf] performance summary: $ART_DIR/cuda_classic_src_resident_perf_summary_0266.csv"
echo "[0266-perf] detailed phases: $ART_DIR/cuda_classic_src_resident_perf_phases_0266.csv"
echo "[0266-perf] Q6/resampling remain disabled only in these classic-only validators; reactivation paths are intentionally preserved."
echo "[0266-perf] CUDA thermostat wall/solid/piston/IO-aware remains a separate future chantier."
