...
export MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE="${MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE:-0}"
...
if suite_truthy_0434 "$MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE"; then
  grep -q '0493x14ai — production-candidate device-side Q6 resultant closure' "$SRC" || { echo '[0493x14ah] x14ai closure requested but source marker missing' >&2; exit 2; }
fi
...
MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE=$MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE
META

echo "===== 0493x14ah TRANSIENT GAS-POISEUILLE DROP DRAG ====="
echo "PATHS: runner=$ROOT/scripts/run_0493x14ah_drop_gas_transient_poiseuille_drag.sh"
...
echo "CHAIN: x14ad=1 x14aiDeviceClosure=$MPCD_X14V_DEVICE_APPLIED_Q6_RESULTANT_CLOSURE x14afBalanceDiag=$MPCD_X14V_GLOBAL_BALANCE_DIAGNOSTIC x14aeLossDiag=0"
...
if suite_truthy_0434 "${SKIP_ANALYSIS:-0}"; then
  echo "[0493x14ah] SKIP_ANALYSIS=1: binary timing run complete; time=$TF"
  exit 0
fi
