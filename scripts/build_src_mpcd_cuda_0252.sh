...
# 0252 — CUDA boundary stack + persistent SRC collision from the shared
...
OUT=${OUT:-build/src_mpcd_base_cuda_0252}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0252-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0252-cuda-build] Built $OUT"
