...
root=Path(sys.argv[1]) if len(sys.argv)>1 else Path('runs/0493x16p_q2_topology_qualification')
...
def parse(path):
    if not path.is_file(): raise SystemExit(f'[0493x16p] missing {path}')
    d={}; sec='root'
...
lines=[
 '0493x16p full-Q2-topology material-wall qualification',
...
out=root/'analysis'/'summary_0493x16p.txt'; out.parent.mkdir(parents=True,exist_ok=True)
