...
# 0214 — CUDA-enabled SRC/MPCD executable with persistent particle-state SRC collision path with CPU workspace moment download.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0214}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0214-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0214-cuda-build] Built $OUT"
