...
# 0297 — CUDA SRC classic build plus post-SRC local population guard.
# This build keeps the 0295 survey and 0296 mass reconditioning, then adds the first support-changing local CUDA population guard.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0297}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0297-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0297-cuda-build] Built $OUT"
