...
# 0255 — CUDA boundary stack + persistent SRC collision for piston/closed-capacity wall subset.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0255}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0255-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0255-cuda-build] Built $OUT"
