...
# 0400 — experimental CUDA-resident Q6 elliptic projection build.
# Keeps the validated topo/resampling source set and adds the resident Q6 prototype.
...
OUT=${OUT:-build/src_mpcd_base_cuda_q6_resident_0400}
...
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0400-q6-resident-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0400-q6-resident-build] Built $OUT"
