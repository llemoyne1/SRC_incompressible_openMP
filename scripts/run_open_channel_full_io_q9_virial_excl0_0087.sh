...
# 0087 open-channel full inlet/outlet with Q9+virial zero open-boundary exclusion.
#
# Purpose
# -------
...
#
# The geometry remains the simplified full-height inlet/outlet, slip/specular
# top/bottom channel.  No immersed solid and no VP/no-slip wall are present.
...
RUN_ROOT="${RUN_ROOT:-runs/open_channel_full_io_q9_virial_excl0_0087_g30}"
...
EOF_KV
  ...
  echo "[0087] running ${label}"
  echo "[0087] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0087] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
echo "[0087] full-height inlet/outlet, slip/specular y walls, no immersed solid"
echo "[0087] Uin=Uex=${UIN}; ramp ${INLET_RAMP_INITIAL_FACTOR}->${INLET_RAMP_FINAL_FACTOR}, t=${INLET_RAMP_START_TIME}->${INLET_RAMP_END_TIME}, profile=${INLET_RAMP_PROFILE}"
echo "[0087] Q9 limiter: mode=${Q9_CORRECTION_LIMITER_MODE}, overThermal=${Q9_CORRECTION_LIMITER_OVER_THERMAL}, thermalKBT=${Q9_CORRECTION_LIMITER_THERMAL_KBT}, legacyAbs=${Q9_CORRECTION_LIMITER}"
...
if [[ "$RUN_Q6" == "1" ]]; then
  write_case "openchan_q6_fullio_slip_${TAG}" "q6" "false" "false"
fi
if [[ "$RUN_Q9" == "1" ]]; then
  write_case "openchan_q9_fullio_slip_excl0_${TAG}" "q9" "true" "false"
fi
if [[ "$RUN_Q9_VIRIAL" == "1" ]]; then
  write_case "openchan_q9_virial_fullio_slip_excl0_${TAG}" "q9_virial" "true" "true"
fi

elapsed=$SECONDS
printf '[0087] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0087] Analyze with: cd matlab && R = analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}','caseGlob','openchan_*');"
...
if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    ...
    echo "[0087] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
