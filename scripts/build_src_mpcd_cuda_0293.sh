...
# 0293 — CUDA SRC classic build with pre-insertion outlet extraction for equilibrium/forced outlet regimes.
# This build keeps the 0291/0291b outlet and thermostat semantics. It fixes equilibrium_flux/forced_flux so outlet extraction can free inactive slots before hard inlet reservoir insertion. Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0293}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0293-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0293-cuda-build] Built $OUT"
