...
# 0326d — script-only strict non-regression harness for the post-0322 clean CUDA branch.
...
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_nonregression_q6_resampling_virial_0326d}
...
if [[ "$SRC_BUILD" == "1" ]]; then
  ...
  echo "[0326d-nonreg] building SRC binary: $BIN via $SRC_BUILD_SCRIPT"
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash "$SRC_BUILD_SCRIPT"
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0326d-nonreg] ERROR: missing executable BIN=$BIN" >&2
  echo "[0326d-nonreg]        Build once or rerun with SRC_BUILD=1." >&2
  exit 127
fi
if [[ ! -f scripts/run_validation_mono_config_0162.sh ]]; then
  echo "[0326d-nonreg] ERROR: missing scripts/run_validation_mono_config_0162.sh" >&2
  exit 127
fi
...
MANIFEST="$ART_DIR/gpu_nonregression_q6_resampling_virial_0326d_manifest.csv"
...
# 0326d correction: for hybrid checks, do NOT set srcClassicCudaModeEnable=true.
# In the solver, classic mode is an operator-level short-circuit for Q6 and
# closed-capacity.  To test the preserved hybrid architecture, activate the
# CUDA persistent collision subset by environment variables while keeping
# srcClassicCudaModeEnable=false in the parameter file:
#   CUDA persistent collision only -> CPU Q6/resampling/virial -> CPU thermostat.
# Piston/closed-capacity additionally needs the explicit 0255 resident piston
# collision subset.
export_cuda_collision_only_flags_0326d() {
  local piston_enable="${1:-0}"
  ...
  # Critical for Q6/resampling/virial non-regression:
  # keep CUDA collision, but leave thermostat to the normal CPU path.
  ...
  if [[ "$piston_enable" == "1" ]]; then
    export MPCD_CUDA_PERSISTENT_SRC_COLLISION_PISTON_0255=1
  else
    unset MPCD_CUDA_PERSISTENT_SRC_COLLISION_PISTON_0255 || true
  fi
...
run_target() {
  ...
  case "$target" in
    ...
    hybrid_cuda_q6_resampling_tg)
      ...
      projection_enable="true"
      resampling_enable="true"
      # Critical: keep srcClassicCudaModeEnable=false so Q6 is not short-circuited.
      # CUDA collision is selected only through MPCD_CUDA_PERSISTENT_SRC_COLLISION_USE.
      src_classic_cuda="false"
      note="env CUDA persistent collision + CPU Q6/resampling + CPU thermostat"
      ;;
    hybrid_cuda_piston_virial)
      case_list="piston_virial_full"
      projection_enable="true"
      resampling_enable="true"
      # Critical: keep srcClassicCudaModeEnable=false so Q6/capacity are not short-circuited.
      src_classic_cuda="false"
      steps="${PISTON_STEPS:-$STEPS}"
      cuda_piston_subset="1"
      note="env CUDA persistent piston collision + CPU Q6/resampling/virial + CPU thermostat"
      ;;
    *)
      echo "[0326d-nonreg] ERROR: unknown target=$target" >&2
      exit 2
      ;;
  esac
  ...
  echo "[0326d-nonreg] run target=$target cases=$case_list steps=$steps note=$note"
  if [[ "$target" == hybrid_cuda_* ]]; then
    export_cuda_collision_only_flags_0326d "$cuda_piston_subset"
  else
    clear_cuda_hybrid_flags_0326d
  fi
  ...
  if [[ "$rc" != "0" ]]; then
    echo "[0326d-nonreg] WARNING: target=$target failed rc=$rc" >&2
...
python3 scripts/summarize_gpu_nonregression_q6_resampling_virial_0326d.py "$ART_DIR" "$ART_DIR"

echo "[0326d-nonreg] manifest: $MANIFEST"
echo "[0326d-nonreg] summary : $ART_DIR/gpu_nonregression_q6_resampling_virial_0326d_summary.csv"
echo "[0326d-nonreg] flags   : $GREP_0325"
