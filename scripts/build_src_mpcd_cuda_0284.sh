...
# 0284 — CUDA immersed-circle SRC classic build.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0284}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0284-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0284-cuda-build] Built $OUT"
