...
# 0493x16i — prescribed deformable impermeable-wall comparison.
# Priority under test: one strict geometric solid formalism for fixed/mobile/
# deformable bodies while preserving the historical porous Darcy operator.
# The wall centerline is
#   x_c(y,t)=X_s(t)+A sin(2*pi*y/Ly) sin(omega*t)
# with constant thickness.  Historical chi remains binary for Darcy/bath/chiVP.
# A separate geometric occupancy is rasterized every step but is never fed to
# alpha(chi). Two CUDA-resident exclusion modes are compared:
#   binary_event   : remap only on binary fluid->solid cell transitions;
#   swept_geometry : remap continuously using the exact prescribed interface,
#                    including cut cells whose centers remain fluid.
# Both remaps change position only in this qualification patch.
# No porous-Darcy/topology law is changed.

ROOT="${ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
source "$ROOT/scripts/src_mpcd_run_common_0434.sh"
suite_root_cd_0434 || exit $?

CASE_LABEL="${CASE_LABEL:-0493x16i_deformable_impermeable_case}"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
EXCLUSION_MODE="${EXCLUSION_MODE:-swept_geometry}"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x16i.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x16i_deformable_impermeable_case}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x16i] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi
if [[ "$EXCLUSION_MODE" != "binary_event" && "$EXCLUSION_MODE" != "swept_geometry" ]]; then
  echo "[0493x16i] ERROR EXCLUSION_MODE must be binary_event or swept_geometry" >&2
  exit 2
fi
...
if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x16i] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
  ...
  if [[ -z "$RESTART_STEP" || ! -s "$OLD_SOLID" ]]; then
    echo "[0493x16i] ERROR restart requires matching chi_solid_dynamics_0493x16a.csv" >&2
    exit 2
  fi
...
# x16i experiment gates. Historical porous-Darcy runs remain unchanged when
...
export SRC_X16I_IMPERMEABLE_EXCLUSION_MODE="$EXCLUSION_MODE"
suite_export_livevis_0434
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x16i.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x16i.txt" <<META
case=$CASE_LABEL
architecture=cuda_resident_historical_darcy_plus_prescribed_deformable_geometric_exclusion
solidModel=rigid_slab_1d
solidMass=$SOLID_MASS
solidUx0=$SOLID_UX0
fluidMeanUx0=$U0
commonUx0=$COMMON_UX
exclusionMode=$EXCLUSION_MODE
...
printf '\n===== 0493x16i DEFORMABLE IMPERMEABLE EXCLUSION COMPARISON =====\n'
printf 'run=%s\n' "$RUN_ROOT"
...
printf 'fluid initialization: velocityMode=%s U0=%s\n' "$VELOCITY_MODE" "$U0"
printf 'deformation: A=%s cells period=%s steps exclusion=%s\n' "$DEFORM_AMPLITUDE_CELLS" "$DEFORM_PERIOD_STEPS" "$EXCLUSION_MODE"
...
[[ -s "$OUT/chi_solid_dynamics_0493x16a.csv" ]] || { echo "[0493x16i] ERROR missing dynamic solid diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x16i] ERROR missing Darcy/bath impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x16i] ERROR missing chiVP impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x16i] ERROR missing summary_runtime.csv" >&2; exit 2; }
head -n 1 "$OUT/chi_solid_impulse_0493x15a.csv" | grep -q 'deformableExclusion0493x16i' || {
  echo "[0493x16i] ERROR binary/output does not contain x16i exclusion diagnostics" >&2
  exit 2
}
python3 - "$OUT/chi_solid_impulse_0493x15a.csv" "$EXCLUSION_MODE" <<'PY_X16I'
import csv, sys
p=sys.argv[1]; mode=sys.argv[2]; expected=1 if mode=='binary_event' else 2
with open(p, newline='') as f: rows=list(csv.DictReader(f))
if not rows: raise SystemExit('[0493x16i] ERROR empty chi-solid impulse CSV')
for key in ['deformableExclusion0493x16i','deformableExclusionMode0493x16i','darcyTotalSeconds0493x16i']:
    if key not in rows[0]: raise SystemExit(f'[0493x16i] ERROR missing {key}')
if any(int(float(r['deformableExclusion0493x16i'])) != 1 for r in rows):
    raise SystemExit('[0493x16i] ERROR exclusion gate not active on every row')
if any(int(float(r['deformableExclusionMode0493x16i'])) != expected for r in rows):
    raise SystemExit('[0493x16i] ERROR wrong exclusion mode in diagnostics')
print('[0493x16i] deformable exclusion gate/mode PASS')
PY_X16I
head -n 1 "$OUT/chi_solid_dynamics_0493x16a.csv" | grep -q 'prescribedDeformable0493x16i' || {
  echo "[0493x16i] ERROR binary/output does not contain x16i geometry diagnostics" >&2
  exit 2
}
python3 - "$OUT/chi_solid_dynamics_0493x16a.csv" <<'PY_GEOM'
import csv, sys
p=sys.argv[1]
with open(p, newline='') as f: rows=list(csv.DictReader(f))
if not rows: raise SystemExit('[0493x16i] ERROR empty chi-solid dynamics CSV')
required=['cudaResidentSolid0493x16e','subcellRaster0493x16e','historicalBinaryMask0493x16f',
          'poststreamTemporalSync0493x16f','prescribedDeformable0493x16i',
          'positiveSolidFractionChange0493x16i','negativeSolidFractionChange0493x16i',
          'hostGeometryFieldUploadBytes0493x16e','hostLoadFieldDownloadBytes0493x16e']
for key in required:
    if key not in rows[0]: raise SystemExit(f'[0493x16i] ERROR missing {key}')
if any(int(float(r['cudaResidentSolid0493x16e'])) != 1 for r in rows): raise SystemExit('[0493x16i] non-resident row')
if any(int(float(r['subcellRaster0493x16e'])) != 0 for r in rows): raise SystemExit('[0493x16i] Darcy subcell raster unexpectedly active')
if any(int(float(r['historicalBinaryMask0493x16f'])) != 1 for r in rows): raise SystemExit('[0493x16i] historical binary Darcy mask inactive')
if any(int(float(r['poststreamTemporalSync0493x16f'])) != 1 for r in rows): raise SystemExit('[0493x16i] poststream sync inactive')
if any(int(float(r['prescribedDeformable0493x16i'])) != 1 for r in rows): raise SystemExit('[0493x16i] prescribed deformation inactive')
if any(int(float(r['hostGeometryFieldUploadBytes0493x16e'])) != 0 or int(float(r['hostLoadFieldDownloadBytes0493x16e'])) != 0 for r in rows):
    raise SystemExit('[0493x16i] full-grid host traffic detected')
print('[0493x16i] resident/deformable/historical-Darcy checks PASS')
PY_GEOM

echo "[0493x16i] COMPLETE run=$RUN_ROOT"
echo "[0493x16i] case complete; run the pair analyzer after both rest and boost cases"
