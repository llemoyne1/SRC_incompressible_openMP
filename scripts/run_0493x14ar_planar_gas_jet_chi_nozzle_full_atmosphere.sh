...
ATM_ANALYZER="$ROOT/scripts/analyze_gas_atmosphere_balance.py"
SRC_Q6="$ROOT/src/cuda_q6_resident_0400.cu"
SRC_BASE="$ROOT/src/src_mpcd_base.cpp"
SRC_PARAMS="$ROOT/src/params_io_base.cpp"
for f in "$GENERATOR" "$ANALYZER" "$ATM_ANALYZER" "$SRC_Q6" "$SRC_BASE" "$SRC_PARAMS"; do
  [[ -f "$f" ]] || { echo "[gas-jet-chi-nozzle-atmosphere] ERROR missing $f" >&2; exit 2; }
done
grep -q '0493x14ad — local-x6g-face gauge sampling' "$SRC_Q6" || {
  echo '[gas-jet-chi-nozzle-atmosphere] ERROR current local gas-traction projection source marker not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f prestream Darcy/chi was requested but not handled' "$SRC_BASE" || {
  echo '[gas-jet-chi-nozzle-atmosphere] ERROR Q6-g-f + resident Darcy/chi integration (0493x7g) not found' >&2; exit 2;
}
grep -q '0493x7g Q6-g-f Darcy requires darcyInitialDeactivateBelowChi<0' "$SRC_PARAMS" || {
  echo '[gas-jet-chi-nozzle-atmosphere] ERROR Q6-g-f Darcy/chi parameter contract (0493x7g) not found' >&2; exit 2;
}
grep -q -- '--incident-height-over-jet' "$ANALYZER" || {
  echo '[gas-jet-chi-nozzle-atmosphere] ERROR local jet analyzer with --incident-height-over-jet is required' >&2; exit 2;
}

# =============================================================================
# 0493x14ar — DARCY/CHI NOZZLE + FULL LATERAL ATMOSPHERIC RESERVOIR (2-D)
#
# Purpose: retain the x14ap similarity point and the same chi-Darcy nozzle, while
# changing ONLY the atmospheric boundary model.
#
# The whole accessible top face outside the two chi nozzle walls is now a
# hard-density gas reservoir at the top barometric occupancy:
#
#   ambient reservoir | chi wall | driven jet | chi wall | ambient reservoir
#
# There are NO separate passive outlet strips. This is deliberate, not a tuning
# sweep: hard_cell_density already behaves as an open bath. It deletes the
# particles occupying/re-entering its reservoir band and rebuilds that band at
# prescribed density, temperature target and segment mean velocity every step.
# Thus returned gas can be absorbed by the same zero-mean atmospheric reservoir
# that supplies gas when the interior is under-dense.
#
# Geometry:
...
#   - zero-mean atmospheric reservoirs fill the accessible top face outside the OUTER nozzle walls;
...
# the domain has external reservoir/inlet segments, outside x14ai's valid scope.
#
# No C++/CUDA change.  ./livevis_control.kv remains user-owned/read-only.
# =============================================================================
CASE_LABEL="${CASE_LABEL:-0493x14ar_planar_gas_jet_chi_nozzle_full_atmosphere}"
RUN_MODE="src-q6-g-f"
TOPOLOGY="segmented"
...
case "$DARCY_FORCING_MODE" in
  ...
  *) echo "[gas-jet-chi-nozzle-atmosphere] ERROR unsupported Q6-g-f Darcy forcing mode: $DARCY_FORCING_MODE" >&2; exit 2 ;;
esac
# No outlet segment is used in x14ar. Keep a valid dormant outlet policy in the
# parameter file because it is a canonical parameter, but it has no active
# segment to act on.
OUTLET_MODE="hybrid"
OUTLET_FEEDBACK_GAIN="0.0"
INLET_RESERVOIR_CELLS="${INLET_RESERVOIR_CELLS:-2}"
# Small solid margin at the two top corners. The atmospheric reservoir otherwise
# occupies ALL top cells available outside the nozzle walls; there is no width
# parameter to tune.
...
if ! awk -v x="$INLET_THERMAL_NOISE" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-atmosphere] ERROR resident segmented inlet requires INLET_THERMAL_NOISE=0" >&2
  exit 2
fi
...
if ! awk -v x="$WALL_ACCOMMODATION" 'BEGIN{exit !(x==0)}'; then
  echo "[gas-jet-chi-nozzle-atmosphere] ERROR first gas/liquid jet qualification fixes WALL_ACCOMMODATION=0" >&2
  exit 2
fi
...
CAMPAIGN_ROOT="${CAMPAIGN_ROOT:-runs/0493x14ar_planar_gas_jet_chi_nozzle_full_atmosphere_s${SURFACE_TENSION_SIGMA}_seed${SEED}}"
...
hx=lx/nx; hy=ly/ny
if abs(hx-hy)>1e-12*max(1,abs(hx),abs(hy)): raise SystemExit('[gas-jet-chi-nozzle-atmosphere] square cells required')
if abs(hx-1/256)>1e-12: raise SystemExit(f'[gas-jet-chi-nozzle-atmosphere] first qualification keeps h=1/256, got {hx:.17g}')
if abs(bh/hy-round(bh/hy))>1e-10: raise SystemExit('[gas-jet-chi-nozzle-atmosphere] BATH_HEIGHT must lie on a cell boundary')
W=wc*hx; smin=(xc-.5*W)/lx; smax=(xc+.5*W)/lx
if not (0.02<smin<smax<0.98): raise SystemExit('[gas-jet-chi-nozzle-atmosphere] jet segment too close to top corners')
A=hx*hy; pref=gam*kg/A; rhoG=gam*mg/A; rhoL=gam*ml/A
cth=math.sqrt(kg/mg)*dt/hx; cjet=abs(uj)*dt/hx; ccomb=cth+cjet
if ccomb>0.80: raise SystemExit(f'[gas-jet-chi-nozzle-atmosphere] unresolved gas flight: Cthermal+Cjet={ccomb:.6g} > 0.80 cell/step')
gabs=abs(gy)
if gy>1e-15: raise SystemExit('[gas-jet-chi-nozzle-atmosphere] default geometry expects GRAVITY_Y <= 0')
if gabs>0:
...
if abs(hx-hy) > 1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit("[gas-jet-chi-nozzle-atmosphere] square cells required")
if wcell < 2 or tcell < 1 or lcell < 1:
    raise SystemExit("[gas-jet-chi-nozzle-atmosphere] require innerWidthCells>=2, wallThicknessCells>=1, lengthCells>=1")
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1)):
    if not (0.0 < v < lx):
        raise SystemExit(f"[gas-jet-chi-nozzle-atmosphere] {name}={v} outside domain")
if not (outer0 < inner0 < inner1 < outer1):
    raise SystemExit("[gas-jet-chi-nozzle-atmosphere] invalid nozzle x ordering")
if exit_y <= bath + 4*hy:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-atmosphere] nozzle exit y={exit_y:.9g} too close to bath={bath:.9g}; "
...
for name,v in (("inner0",inner0),("inner1",inner1),("outer0",outer0),("outer1",outer1),("exit_y",exit_y)):
    ...
    if abs(q-round(q)) > 1e-9:
        raise SystemExit(f"[gas-jet-chi-nozzle-atmosphere] {name}={v:.17g} is not aligned with a cell face")
W=inner1-inner0
stand=exit_y-bath
...
# Top face:
# ambient hard-density reservoir | chi wall | driven jet |
# chi wall | ambient hard-density reservoir.
#
# There are no separate outlet segments. hard_cell_density itself is the
# atmospheric bath: all particles in the two-cell reservoir band are replaced
# every step at the target occupancy; outward/backflow particles at an inlet
# are deleted by the existing inlet-boundary logic.
read -r LEFT_AMBIENT_SMIN LEFT_AMBIENT_SMAX \
        RIGHT_AMBIENT_SMIN RIGHT_AMBIENT_SMAX \
        LEFT_AMBIENT_CELL_COUNT RIGHT_AMBIENT_CELL_COUNT \
        TOTAL_HARD_RESERVOIR_CELLS TOTAL_HARD_RESERVOIR_PARTICLES \
  <<<"$(python3 - \
    "$NX" "$INLET_RESERVOIR_CELLS" "$INLET_OCC" "$TOP_OPEN_MARGIN" \
    "$LEFT_WALL_SMIN" "$RIGHT_WALL_SMAX" "$JET_SMIN" "$JET_SMAX" <<'PY'
import sys
nx=int(sys.argv[1])
...
lw=float(sys.argv[5])
rw=float(sys.argv[6])
js0=float(sys.argv[7])
js1=float(sys.argv[8])

if not (0.0 <= margin < lw < js0 < js1 < rw < 1.0-margin <= 1.0):
    raise SystemExit("[gas-jet-chi-nozzle-atmosphere] invalid top atmosphere/nozzle ordering")

la0,la1=margin,lw
ra0,ra1=rw,1.0-margin

# Count actual top-face grid columns by cell-center membership, matching the
# compact-segment rasterization better than a nominal floating width.
centres=[(i+0.5)/nx for i in range(nx)]
nL=sum(la0 <= s < la1 for s in centres)
nR=sum(ra0 <= s < ra1 for s in centres)
nJ=sum(js0 <= s < js1 for s in centres)
if min(nL,nR,nJ) <= 0:
    raise SystemExit("[gas-jet-chi-nozzle-atmosphere] empty atmosphere or jet segment after rasterization")

nCells=depth*(nL+nR+nJ)
nParticles=nCells*occ
print(la0,la1,ra0,ra1,nL,nR,nCells,nParticles)
PY
)" || exit $?
...
echo "[gas-jet-chi-nozzle-atmosphere] topSegments: ambientL=[$LEFT_AMBIENT_SMIN,$LEFT_AMBIENT_SMAX] wallL=[$LEFT_WALL_SMIN,$LEFT_WALL_SMAX] jet=[$JET_SMIN,$JET_SMAX] wallR=[$RIGHT_WALL_SMIN,$RIGHT_WALL_SMAX] ambientR=[$RIGHT_AMBIENT_SMIN,$RIGHT_AMBIENT_SMAX]"
echo "[gas-jet-chi-nozzle-atmosphere] hardReservoir: lateralColumns=[$LEFT_AMBIENT_CELL_COUNT,$RIGHT_AMBIENT_CELL_COUNT] depth=$INLET_RESERVOIR_CELLS targetParticles=$TOTAL_HARD_RESERVOIR_PARTICLES"
...
import sys
la0,la1,lw0,lw1,ji0,ji1,rw0,rw1,ra0,ra1=map(float,sys.argv[1:])
if not (0 <= la0 < la1 <= lw0 < lw1 <= ji0 < ji1 <= rw0 < rw1 <= ra0 < ra1 <= 1):
    raise SystemExit(
        "[gas-jet-chi-nozzle-atmosphere] invalid top segmentation; expected "
        "ambient | left chi wall | jet | right chi wall | ambient"
    )
PY
...
if solid != expected or left_solid != tcell*lcell or right_solid != tcell*lcell:
    raise SystemExit(
        f"[gas-jet-chi-nozzle-atmosphere] chi cell-count mismatch solid={solid} expected={expected} "
...
print(
    f"[gas-jet-chi-nozzle-atmosphere] chi={out} solidCells={solid} "
...
# The complete accessible lateral top face is the zero-mean atmospheric bath.
openBoundarySegment1 = top inlet $LEFT_AMBIENT_SMIN $LEFT_AMBIENT_SMAX 0.0 0.0 $GAS_TYPE $GAS_MASS
openBoundarySegment2 = top inlet $RIGHT_AMBIENT_SMIN $RIGHT_AMBIENT_SMAX 0.0 0.0 $GAS_TYPE $GAS_MASS
...
speciesDiagnosticsFilename = species_runtime_0493x14ar.csv
...
ATMOSPHERE_BOUNDARY_MODE=full_hard_density_reservoir_no_passive_outlets
...
LEFT_AMBIENT_CELL_COUNT=$LEFT_AMBIENT_CELL_COUNT
RIGHT_AMBIENT_CELL_COUNT=$RIGHT_AMBIENT_CELL_COUNT
TOTAL_HARD_RESERVOIR_CELLS=$TOTAL_HARD_RESERVOIR_CELLS
TOTAL_HARD_RESERVOIR_PARTICLES=$TOTAL_HARD_RESERVOIR_PARTICLES
JET_WIDTH=$JET_WIDTH
JET_SPEED=$JET_SPEED
...
echo "PATHS: runner=$ROOT/scripts/run_0493x14ar_planar_gas_jet_chi_nozzle_full_atmosphere.sh"
...
echo "BOUNDARIES: solid base faces; top = ambient reservoir | chi wall | jet throat | chi wall | ambient reservoir"
echo "       full lateral atmosphere, Ntarget=$INLET_OCC, meanU=(0,0), NO passive outlet segments"
echo "       left ambient=[$LEFT_AMBIENT_SMIN,$LEFT_AMBIENT_SMAX] ($LEFT_AMBIENT_CELL_COUNT columns)"
echo "       right ambient=[$RIGHT_AMBIENT_SMIN,$RIGHT_AMBIENT_SMAX] ($RIGHT_AMBIENT_CELL_COUNT columns); hard target=$TOTAL_HARD_RESERVOIR_PARTICLES particles"
...
if suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  echo '[gas-jet-chi-nozzle-atmosphere] PREFLIGHT_ONLY complete'
  exit 0
fi
...
python3 "$ATM_ANALYZER" \
  --run-root "$CAMPAIGN_ROOT" \
  --bath-height "$BATH_HEIGHT" \
  --nozzle-exit-y "$NOZZLE_EXIT_Y" \
  --jet-width "$JET_WIDTH" \
  --cell-size "$H" \
  --inlet-target-occupancy "$INLET_OCC" \
  --post-start-time 2.0

OUT_TAR="$CAMPAIGN_ROOT/0493x14ar_planar_gas_jet_chi_nozzle_full_atmosphere_compact.tar.gz"
FILES=(
  analysis
  ...
  "output/species_runtime_0493x14ar.csv"
...
echo "[gas-jet-chi-nozzle-atmosphere] COMPLETE"
echo "[gas-jet-chi-nozzle-atmosphere] compact=$OUT_TAR"
