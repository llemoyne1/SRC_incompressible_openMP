...
# 0288 — CUDA SRC classic build with segmented inlet clipping fix.
# This build starts from the 0286 consolidated SRC classic CUDA stack and adds the 0288 segmented inlet reservoir clipping fix. Q6/resampling/virial remain separate modules.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0288}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0288-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0288-cuda-build] Built $OUT"
