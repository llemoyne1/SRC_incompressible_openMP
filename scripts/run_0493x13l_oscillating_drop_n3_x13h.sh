...
# 0493x13l — 2-D oscillating-drop qualification of the x13h fluid.
# Tooling/initial-condition only: no C++/CUDA physics change and no new source diagnostic.
# Mode n=3 requires a third-order observable.  No source diagnostic is added:
# q3 is reconstructed offline from COM-centred particle-state dumps.

CASE_LABEL="oscillating_drop_n3_0493x13l"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
TOPOLOGY="closed_box"

GENERATOR="$ROOT/scripts/generate_0493x13k_oscillating_drop_2d.py"
ANALYZER="$ROOT/scripts/analyze_0493x13l_oscillating_drop_n3_state.py"
[[ -f "$GENERATOR" ]] || { echo "[0493x13l] ERROR missing $GENERATOR" >&2; exit 2; }
[[ -f "$ANALYZER" ]] || { echo "[0493x13l] ERROR missing $ANALYZER" >&2; exit 2; }
...
# Qualified-amplitude mode-3 point.
...
# ~3.3 inviscid periods for n=3.  State dumps every 10 steps give ~9 samples/period;
# this is sufficient for N=10 phase-ensemble fitting while keeping I/O moderate.
STEPS="${STEPS:-300}"
...
RUN_ROOT="${RUN_ROOT:-runs/0493x13l_oscillating_drop_n3_x13h_s${SIGMA_DECLARED}_n${MODE}_r${RADIUS_CELLS}_eps${EPSILON}_seed${SEED}}"
...
# Tight preflight: this runner is intentionally locked to n=3.  The modal
# observable comes from offline third mass moments, not the x9f quadrupole.
...
hx,hy=lx/nx,ly/ny
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)): raise SystemExit('[0493x13l] ERROR square cells required')
if mode != 3: raise SystemExit('[0493x13l] ERROR this runner is intentionally MODE=3')
if not (0 < eps <= 0.05): raise SystemExit(f'[0493x13l] ERROR modal qualification requires 0<epsilon<=0.05, got {eps}')
if sigma <= 0 or rc <= 0 or nu <= 0 or dt <= 0: raise SystemExit('[0493x13l] ERROR sigma,R,nu,dt must be positive')
if rmin != 4: raise SystemExit(f'[0493x13l] ERROR current x12 dynamic drop uses minRadiusCells=4 here, got {rmin}')
if abs(angle-2.0943951023931953)>1e-12: raise SystemExit(f'[0493x13l] ERROR x13h requires rotationAngle=120deg, got {angle}')
...
LIVE_VIS_CONTROL_FILE="$RUN_ROOT/livevis_control_0493x13l.kv"
export LIVE_VIS_CONTROL_FILE
suite_prepare_livevis_control_0434 "$RUN_ROOT" "$RUN_MODE"
suite_export_livevis_0434
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x13l.env" "$RUN_MODE"
cat >> "$RUN_ROOT/logs/environment_0493x13l.env" <<META
X13L_MODE=$MODE
X13L_RADIUS_CELLS=$RADIUS_CELLS
X13L_EPSILON=$EPSILON
X13L_PHASE=$PHASE
X13L_SIGMA=$SIGMA_DECLARED
X13L_NU_REF=$NU_REF
X13L_OMEGA0_2D=$OMEGA0
X13L_PERIOD0_2D=$PERIOD0
X13L_BETA_LAMB=$BETA_LAMB
X13L_OMEGA_LAMB=$OMEGA_LAMB
X13L_PERIOD_LAMB=$PERIOD_LAMB
X13L_R_OVER_X12A_RC=$RCUT_RATIO
META

printf '%s\n' \
  '===== 0493x13l 2-D OSCILLATING DROP n=3 / x13h =====' \
  "[0493x13l] objective=dynamic capillary qualification n=3; no Young-Laplace sigma0 baseline" \
  "[0493x13l] initial r(theta)=R*[sqrt(1-eps^2/2)+eps*cos(3theta)]; exact continuous area=pi*R^2" \
  "[0493x13l] grid=${NX}x${NY} h=$H gamma=$GAMMA dt=$DT kBT=$KBT R/h=$RADIUS_CELLS eps=$EPSILON sigma=$SIGMA_DECLARED" \
  "[0493x13l] theory2D omega0=$OMEGA0 period0=$PERIOD0 LambBeta=$BETA_LAMB omegaDamped=$OMEGA_LAMB periodDamped=$PERIOD_LAMB" \
  "[0493x13l] x12a initial resolved-size ratio R/Rc=$RCUT_RATIO (target >1)" \
  "[0493x13l] steps=$STEPS tEnd=$(awk -v n="$STEPS" -v d="$DT" 'BEGIN{printf "%.9g",n*d}') summaryEvery=$SUMMARY_EVERY fitPeriods=$FIT_PERIODS" \
  "[0493x13l] diagnostics=offline COM-centred third mass moment from state dumps; x9e/x9r ancillary"
...
if ! suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  ...
  shopt -u nullglob
  (( ${#STATES[@]} >= 20 )) || { echo "[0493x13l] ERROR need >=20 state dumps, got ${#STATES[@]} in $OUT" >&2; exit 2; }
  python3 "$ANALYZER" \
  ...
  echo "[0493x13l] COMPLETE run=$RUN_ROOT analysis=$RUN_ROOT/analysis_0493x13l"
fi
