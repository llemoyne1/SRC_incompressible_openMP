...
# 0077 Poiseuille/open-channel hard-inlet / free-outlet validation.
# Scope: inlet/outlet only, no immersed solid.
# Purpose: verify whether a channel with solid y-walls, uniform hard inlet, and
# passive outlet relaxes toward a stable throughput using the validated Q6/Q9
# model parameters from feature/elliptic-q6-core.
...
RUN_ROOT="${RUN_ROOT:-runs/poiseuille_hard_inlet_free_outlet_validated_q9_0077}"
...
EOF_KV
  ...
  echo "[0077] running ${label}"
  echo "[0077] params: ${params_file}"
  ...
  if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
    ...
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "[0077] WARNING: ${label} failed with exit code ${rc}" | tee -a "$RUN_ROOT/FAILED_CASES.txt"
      return 0
    fi
  else
    build/src_mpcd_base "$params_file" 2>&1 | tee "$RUN_ROOT/logs/${label}.log"
...
if [[ "$RUN_CLASSIC" == "1" ]]; then
  write_case "poiseuille_classic_hard_inlet_u${UIN//./p}_48x24" "classic" "false" "false"
fi
if [[ "$RUN_Q6" == "1" ]]; then
  write_case "poiseuille_q6_hard_inlet_u${UIN//./p}_48x24" "q6" "false" "false"
fi
if [[ "$RUN_Q9" == "1" ]]; then
  write_case "poiseuille_q9_validated_hard_inlet_u${UIN//./p}_48x24" "q9" "true" "false"
fi
if [[ "$RUN_Q9_VIRIAL" == "1" ]]; then
  write_case "poiseuille_q9_virial_validated_hard_inlet_u${UIN//./p}_48x24" "q9_virial" "true" "true"
fi

elapsed=$SECONDS
printf '[0077] done in %02d:%02d:%02d\n' $((elapsed/3600)) $(((elapsed%3600)/60)) $((elapsed%60))

echo "[0077] Analyze with: cd matlab && R = analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"

if [[ "$AUTO_ANALYZE" == "1" ]]; then
  if command -v matlab >/dev/null 2>&1; then
    matlab -batch "cd('matlab'); analyze_poiseuille_hard_inlet_free_outlet_0077('root','..','runRoot','${RUN_ROOT}');"
  else
    echo "[0077] AUTO_ANALYZE=1 requested but matlab is not available." >&2
  fi
fi
