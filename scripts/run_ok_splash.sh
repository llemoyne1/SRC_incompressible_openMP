...
# run_ok_splash: stand-alone liquid/vacuum droplet-impact demonstration.
# Only scripts/src_mpcd_run_ok_common.sh is sourced; state generation is delegated to
# the geometry-only Python generator.
CASE_LABEL="splash"
...
SIGMA_ACTIVE="${SIGMA_ACTIVE:-10945.0}"
...
# Qualified liquid/vacuum closure controls, visible here.
...
RUN_ROOT="${RUN_ROOT:-runs/run_ok_splash_${NX}x${NY}_g${GAMMA}_s${SIGMA_ACTIVE}}"
...
RUN_OK_GENERATOR_PATH="$ROOT/scripts/generate_0493x9s_splash_state.py"
...
printf '%s\n' \
  "[run_ok_splash] target=$TARGET sigma=$SIGMA_ACTIVE minRadiusCells=$SURFACE_TENSION_MIN_RADIUS_CELLS contactAngle=$CONTACT_ANGLE_DEG" \
  "[0493x9t-suite] kineticReflectionFraction=$KINETIC_REFLECTION_FRACTION evaporationTargetType=$EVAPORATION_TARGET_TYPE" \
  "[run_ok_splash] drop R/h=$DROP_RADIUS_CELLS center=($DROP_CENTER_X,$DROP_CENTER_Y) v=($DROP_VX,$DROP_VY) gravityY=$GRAVITY_Y" \
  "[run_ok_splash] vacuum=1 puddleDepthCells=$([[ $TARGET == puddle ]] && echo "$PUDDLE_DEPTH_CELLS" || echo 0) dumpsEvery=$DUMP_STATE_EVERY steps=$STEPS dt=$DT" \
  "[run_ok_splash] objective=qualitative impact/sheet/rim/splash; wall and liquid-puddle targets use identical bulk/capillary settings"
...
echo "[run_ok_splash] completed target=$TARGET output=$OUT"
echo "[run_ok_splash] state dumps requested every $DUMP_STATE_EVERY steps"
