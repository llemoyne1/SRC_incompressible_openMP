...
# 0235 — real-state CUDA resampling transfer shadow coverage harness.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0235}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_0235}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620235}
...
TRANSFER_MAX_TRANSFERS=${TRANSFER_MAX_TRANSFERS:-8192}
TRANSFER_UNIQUE_RECEIVER=${TRANSFER_UNIQUE_RECEIVER:-0}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0235.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0235-resampling-transfer-shadow] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_transfer_shadow_0235.csv}
...
run_validation_logged() {
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      MPCD_CUDA_RESAMPLING_TRANSFER_SHADOW_UNIQUE_RECEIVER="$TRANSFER_UNIQUE_RECEIVER" \
...
print(f"[0235-resampling-transfer-shadow] {verdict} {label}/{mode}: rc={rc} wall={wall:.6g}s baseline={bwall:.6g}s guardRows={len(guard)} transferRows={len(transfer)} transferPass={transfer_pass} cellMismatch={cell_mismatch} roleMismatch={role_mismatch} failed={failed}/{compared}")
...
for spec in $GRID_CASES; do
  ...
  [[ -n "${nx:-}" && -n "${steps:-}" ]] || { echo "[0235-resampling-transfer-shadow] invalid GRID_CASES entry $spec" >&2; exit 2; }
  ny=${NY_OVERRIDE:-$nx}
  label="${nx}x${ny}_s${steps}"
  base_root="runs/cuda_resampling_transfer_shadow_0235_cpu_${label}"
  shadow_root="runs/cuda_resampling_transfer_shadow_0235_shadow_${label}"

  if [[ "$MODES" == *"cpu_baseline"* ]]; then
    stdout_log="$ART_DIR/cuda_resampling_transfer_shadow_0235_cpu_${label}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_transfer_shadow_0235_cpu_${label}.stderr.log"
    echo "[0235-resampling-transfer-shadow] running baseline $label"
    run_validation_logged "$nx" "$ny" "$steps" "$base_root" "cuda0235_cpu_${label}" 0 0 "none" "none" "$stdout_log" "$stderr_log"; rc=$?
  ...
  if [[ "$MODES" == *"cuda_resampling_transfer_shadow"* ]]; then
    guard_csv="$ART_DIR/cuda_resampling_guard_shadow_detail_0235_${label}.csv"
    transfer_csv="$ART_DIR/cuda_resampling_transfer_shadow_detail_0235_${label}.csv"
    compare_csv="$ART_DIR/cuda_resampling_transfer_shadow_compare_0235_${label}.csv"
    compare_summary="$ART_DIR/cuda_resampling_transfer_shadow_compare_summary_0235_${label}.csv"
    stdout_log="$ART_DIR/cuda_resampling_transfer_shadow_0235_shadow_${label}.stdout.log"
    stderr_log="$ART_DIR/cuda_resampling_transfer_shadow_0235_shadow_${label}.stderr.log"
    rm -f "$guard_csv" "$transfer_csv" "$compare_csv" "$compare_summary"
    echo "[0235-resampling-transfer-shadow] running transfer shadow $label"
    run_validation_logged "$nx" "$ny" "$steps" "$shadow_root" "cuda0235_shadow_${label}" 1 1 "$guard_csv" "$transfer_csv" "$stdout_log" "$stderr_log"; rc=$?
...
echo "[0235-resampling-transfer-shadow] wrote $OUT_CSV"
if [[ $fail_count -ne 0 ]]; then
  echo "[0235-resampling-transfer-shadow] completed with $fail_count failing row(s); see $OUT_CSV" >&2
  exit 1
fi
