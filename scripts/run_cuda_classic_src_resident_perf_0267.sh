...
# 0267 — performance instrumentation pass for the validated CUDA resident
# classic SRC stack.  This runner keeps the 0265c validation structure but
# enables the internal phase profile and the new resident CUDA phase profile.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0267}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_resident_perf_0267}
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_resident_perf_validation_0267.csv}
THREADS=${THREADS:-8}
GAMMA=${GAMMA:-20}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0267-perf] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0267.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0267.sh
fi
...
export MPCD_CUDA_RESIDENT_PROFILE_0267=1

# Keep child validation CSV parsing from 0265c, but place all artifacts under the
# 0267 performance root and use the 0267 binary.
BIN="$BIN" \
...
python3 scripts/summarize_cuda_resident_perf_0267.py "$ART_DIR/validation_runs" \
  --summary-out "$ART_DIR/cuda_classic_src_resident_perf_summary_0267.csv" \
  --phases-out "$ART_DIR/cuda_classic_src_resident_perf_phases_0267.csv"

echo "[0267-perf] validation manifest: $OUT_CSV"
echo "[0267-perf] performance summary: $ART_DIR/cuda_classic_src_resident_perf_summary_0267.csv"
echo "[0267-perf] detailed phases: $ART_DIR/cuda_classic_src_resident_perf_phases_0267.csv"
echo "[0267-perf] Q6/resampling remain disabled only in these classic-only validators; reactivation paths are intentionally preserved."
echo "[0267-perf] CUDA thermostat wall/solid/piston/IO-aware remains a separate future chantier."
