...
# 0493x14c — production-like dynamic two-species thermostat probe.
# Active SRC collision + random sign + random grid shift.  Because the final
# random collision-grid shift is not stored in .smpcd, this runner checks the
# invariant quantities exactly and reports (rather than hard-failing on) the
# global apparent per-type kBT.
...
RUN_OK_ENTRYPOINT="$ROOT/scripts/run_ok_0493x14c_species_thermostat_collision_shifted.sh"
BIN="${BIN:-build/src_mpcd_base_cuda_q6_resident_livevis_0486}"
RUN_ROOT="${RUN_ROOT:-runs/0493x14c_species_thermostat_collision_shifted}"
CLEAN_RUN_ROOT="${CLEAN_RUN_ROOT:-1}"
PREFLIGHT_ONLY="${PREFLIGHT_ONLY:-0}"
CASE_LABEL="${CASE_LABEL:-0493x14c_species_thermostat_collision_shifted}"
...
if (( GAMMA < 12 || GAMMA % 2 != 0 )); then
  echo "[0493x14c] ERROR GAMMA must be even and >=12" >&2
  exit 2
fi
...
STATE="$RUN_ROOT/init/two_type_uniform_dynamic_0493x14c.smpcd"
...
per=gamma//2; mean={1:(0.02,0.0),2:(-0.20,0.0)}
...
for j in range(ny):
    for i in range(nx):
        for t,m in ((1,m1),(2,m2)):
            ...
            for k in range(per):
                th=2.0*math.pi*(k+0.5)/per; ph=th+(0.19 if t==2 else 0.0)
...
print(f"[0493x14c-state] path={path} grid={nx}x{ny} gamma={gamma} perType={per} masses={m1}/{m2} initialKBT={T} N={N}")
PY
}

write_params() {
  local case_dir=$1 target1=$2 target2=$3
  local params="$case_dir/params/params_0493x14c.kv"
...
speciesDiagnosticsFilename = species_runtime_0493x14c.csv
...
run_one() {
  ...
  log="$dir/logs/run_0493x14c.log"; time_file="$dir/logs/time_0493x14c.txt"
  ...
  echo "GRID:  L=${Lx}x${Ly} N=${NX}x${NY} gamma=$GAMMA dt=$DT steps=$STEPS gridShift=$GRID_SHIFT_ENABLE seed=$SEED"
  ...
  echo "NOTE:  production-like random grid shift active; ./livevis_control.kv is untouched"
  ...
  [[ -s "$final_state" ]] || { echo "[0493x14c] FAIL missing final dump $final_state" >&2; exit 3; }
  python3 "$ROOT/tools/analyze_0493x14_species_thermostat_dynamic.py" \
    --initial "$STATE" --state "$final_state" \
    --species-csv "$dir/output/species_runtime_0493x14c.csv" \
    --label "$label" --nx "$NX" --ny "$NY" --min-particles "$THERMOSTAT_MIN_PARTICLES" \
    --target1 "$t1" --target2 "$t2" --grid-shift 1
}
...
if [[ "$PREFLIGHT_ONLY" != 1 ]]; then
  echo "[0493x14c] PASS production-like shifted-grid dynamic probe"
fi
