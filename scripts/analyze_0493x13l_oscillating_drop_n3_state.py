#!/usr/bin/env python3
"""0493x13l — offline mode-n modal-drop analysis from particle state dumps.
...
def main():
    ...
    args = ap.parse_args()
    if args.mode != 3:
        raise SystemExit("[0493x13l-analysis] this analyzer is locked to mode=3")
    ...
    if len(states) < 20:
        raise SystemExit(f"[0493x13l-analysis] need >=20 state dumps, got {len(states)}")
    ...
    lines = [
        "===== 0493x13l 2-D OSCILLATING DROP / n=3 =====",
    ...
    report = "\n".join(lines) + "\n"
    (analysis / "oscillating_drop_n3_state_moment_report.txt").write_text(report)
    print(report, end="")
