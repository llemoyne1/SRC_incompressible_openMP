...
# 0314 — CUDA SRC classic build with compact fluid-only summary/dump support.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0314}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0314-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0314-cuda-build] Built $OUT"
