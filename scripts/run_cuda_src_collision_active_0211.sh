...
# 0211 — run CUDA SRC collision in active mode inside the real SRC/MPCD step.
# The CUDA collision replaces the CPU SRC rotation under MPCD_CUDA_SRC_COLLISION_USE=1.
# The harness compares a CPU baseline run to the active CUDA collision run.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0211}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_collision_0211}
GRID_CASES=${GRID_CASES:-"64:200 128:100"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SUMMARY_EVERY_DEFAULT=${SUMMARY_EVERY:-50}
SEED_BASE=${SEED_BASE:-1620211}
...
if [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0211.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0211-src-active] ERROR: CUDA-enabled binary not found after build: $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_src_collision_active_0211.csv}
printf 'grid,NX,NY,steps,case,baselineElapsed_s,activeElapsed_s,baselineWallTime,activeWallTime,elapsedDelta_s,wallDelta_s,activeCalls,particlesVisitedPerCall,particlesRotatedPerCall,invalidCellParticles,numCells,totalActiveSeconds,uploadSeconds,kernelSeconds,downloadSeconds,avgActiveTotalSeconds,avgKernelSeconds,maxAbsVx,maxAbsVy,rmsV,velocityMismatches,failed_metrics,compared_metrics,verdict,activeCsv,compareCsv,compareSummary\n' > "$OUT_CSV"
...
if not rows:
    raise SystemExit(f"empty CUDA SRC collision active CSV: {active_csv}")
...
with open(out_csv, 'a', newline='') as fcsv:
    ...
    writer.writerow(row)
print(f"[0211-src-active] {verdict} {label}/{case_name}: activeWall={active_wall:.6g}s baselineWall={base_wall:.6g}s totalActive={row['totalActiveSeconds']:.6g}s kernel={row['kernelSeconds']:.6g}s maxV=({max_vx:.3g},{max_vy:.3g}) failed={failed}/{compared}")
if verdict != 'PASS':
    raise SystemExit(1)
PY
}
...
run_one() {
  ...
  local base_root="runs/cuda_src_collision_active_0211_baseline_${label}"
  local active_root="runs/cuda_src_collision_active_0211_${label}"

  if [[ "$RUN_BASELINE" != "1" ]]; then
    echo "[0211-src-active] ERROR: RUN_BASELINE=0 is not supported for this harness" >&2
    exit 4
  fi

  echo "[0211-src-active] baseline $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      ...
      RUN_ROOT="$base_root" RUN_TAG="cuda0211_baseline_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
  ...
  echo "[0211-src-active] active $label"
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$CASE_LIST" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$summary_every" \
      THREADS="$THREADS" SEED="$seed" DUMP_STATE_EVERY=0 \
      RUN_ROOT="$active_root" RUN_TAG="cuda0211_active_${label}" PROJECTION_BACKEND="$PROJECTION_BACKEND" \
      MPCD_CUDA_CELL_MOMENTS_USE=${MPCD_CUDA_CELL_MOMENTS_USE:-0} \
      MPCD_CUDA_THERMOSTAT_USE=${MPCD_CUDA_THERMOSTAT_USE:-0} \
  ...
  local compare_csv="$ART_DIR/cuda_src_collision_active_compare_0211_${label}.csv"
  local compare_summary="$ART_DIR/cuda_src_collision_active_compare_summary_0211_${label}.csv"
  ...
  for case_name in $CASE_LIST; do
    local active_csv="$active_root/$case_name/cuda_src_collision_active_0211.csv"
    if [[ ! -f "$active_csv" ]]; then
      echo "[0211-src-active] ERROR: missing $active_csv" >&2
      exit 3
    fi
...
for spec in $GRID_CASES; do
  ...
  if [[ -z "${nx:-}" || -z "${steps:-}" ]]; then
    echo "[0211-src-active] invalid GRID_CASES entry '$spec' (expected NX:steps)" >&2
    exit 2
  fi
  run_one "$nx" "$steps"
done

echo "[0211-src-active] wrote $OUT_CSV"
