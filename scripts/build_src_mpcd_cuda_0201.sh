...
# 0201 — CUDA-enabled SRC/MPCD executable with Q6 CUDA and optional
# active CUDA cell-moments deposit. CPU/OpenMP remains the default; set
...
OUT=${OUT:-build/src_mpcd_base_cuda_0201}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0201-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0201-cuda-build] Built $OUT"
