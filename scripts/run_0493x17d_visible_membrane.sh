...
# 0493x17d-fix3 publication membrane.
...
CASE_LABEL="${CASE_LABEL:-0493x17d_visible_membrane_fix3}"
MODE="${MODE:-src-q6}"
TOPOLOGY=periodic
GEN_CASE=uniform
...
# Six cells total thickness at refine=2: thinner than the old plate but still
# comfortably resolved.  Span is enlarged to make global curvature visible.
MEMBRANE_HALF_THICKNESS_CELLS="${MEMBRANE_HALF_THICKNESS_CELLS:-3.0}"
...
# Flexible publication law.  kStretch is intentionally much softer than the
# old x17c contour; angular bending regularizes local folding instead of using
# extensional stiffness to hide it.
SOLID_MASS="${SOLID_MASS:-4096.0}"
MEMBRANE_K_STRETCH="${MEMBRANE_K_STRETCH:-75000.0}"
MEMBRANE_K_AREA="${MEMBRANE_K_AREA:-3000000.0}"
MEMBRANE_DAMPING="${MEMBRANE_DAMPING:-4000.0}"
MEMBRANE_K_BENDING="${MEMBRANE_K_BENDING:-0.0005}"
...
RECORD_FIELDS="${RECORD_FIELDS:-rho,ux,uy}"; RECORD_EVERY="${RECORD_EVERY:-20}"; FILTER_MODE="${FILTER_MODE:-none}"; FILTER_SAMPLE_EVERY="${FILTER_SAMPLE_EVERY:-100}"; PARTICLE_TYPE_FILTER="${PARTICLE_TYPE_FILTER:--1}"

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x17d_visible_membrane_fix3}"
