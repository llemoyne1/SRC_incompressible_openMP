...
# 0493x16h — diagnostic ablation of the mask-capture impulse on RigidSlab1D.
# Keeps x16f historical binary chi + post-stream temporal synchronization, then
# locally remaps particles in cells newly swallowed by the mask across the outward
# binary cell face before the outward bath. The remap changes position only and
# bypasses the bath on that capture step; no collective reduction is used.
...
#   x16h: on newly-solid cells only, reflect x across the outward binary cell
#         face before the bath; preserve velocity and bypass bath on that step
...
CASE_LABEL="${CASE_LABEL:-0493x16h_capture_spatial_reinjection_galilean_case}"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x16h.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x16h_capture_spatial_reinjection_galilean_case}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x16h] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi
...
if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x16h] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
  ...
  if [[ -z "$RESTART_STEP" || ! -s "$OLD_SOLID" ]]; then
    echo "[0493x16h] ERROR restart requires matching chi_solid_dynamics_0493x16a.csv" >&2
    exit 2
  fi
...
# x16h is an explicitly gated diagnostic ablation. Other runs with the same
# binary remain on the x16f path unless this environment variable is set.
unset SRC_X16G_CAPTURE_BATH_MEAN_NEUTRALIZE
export SRC_X16H_CAPTURE_SPATIAL_REINJECT=1
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x16h.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x16h.txt" <<META
case=$CASE_LABEL
architecture=cuda_resident_historical_binary_poststream_sync_capture_spatial_reinjection
...
printf '\n===== 0493x16h CAPTURE-SPATIAL-REINJECTION GALILEAN / RIGID SLAB 1D =====\n'
printf 'run=%s\n' "$RUN_ROOT"
...
[[ -s "$OUT/chi_solid_dynamics_0493x16a.csv" ]] || { echo "[0493x16h] ERROR missing dynamic solid diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x16h] ERROR missing Darcy/bath impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x16h] ERROR missing chiVP impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x16h] ERROR missing summary_runtime.csv" >&2; exit 2; }
head -n 1 "$OUT/chi_solid_impulse_0493x15a.csv" | grep -q 'captureSpatialReinjection0493x16h' || {
  echo "[0493x16h] ERROR binary/output does not contain x16h capture spatial-reinjection diagnostics" >&2
  exit 2
}
python3 - "$OUT/chi_solid_impulse_0493x15a.csv" <<'PY_X16H'
import csv, sys
p=sys.argv[1]
with open(p, newline='') as f:
    rows=list(csv.DictReader(f))
if not rows:
    raise SystemExit('[0493x16h] ERROR empty chi-solid impulse CSV')
key='captureSpatialReinjection0493x16h'
if key not in rows[0]:
    raise SystemExit(f'[0493x16h] ERROR missing {key}')
if any(int(float(r[key])) != 1 for r in rows):
    raise SystemExit('[0493x16h] ERROR capture spatial-reinjection gate not active on every row')
print('[0493x16h] capture spatial reinjection gate PASS')
PY_X16H
head -n 1 "$OUT/chi_solid_dynamics_0493x16a.csv" | grep -q 'fictitiousFluidDiagnostic0493x16c' || {
  echo "[0493x16h] ERROR binary/output does not contain x16c fictitious-fluid diagnostics" >&2
  exit 2
}
...
with open(p, newline='') as f:
    rows=list(csv.DictReader(f))
if not rows:
    raise SystemExit('[0493x16h] ERROR empty chi-solid dynamics CSV')
...
for key in required:
    if key not in rows[0]:
        raise SystemExit(f'[0493x16h] ERROR missing resident column {key}')
if any(int(float(r['cudaResidentSolid0493x16e'])) != 1 for r in rows):
    raise SystemExit('[0493x16h] ERROR non-resident solid row detected')
if any(int(float(r['subcellRaster0493x16e'])) != 0 for r in rows):
    raise SystemExit('[0493x16h] ERROR subcell raster unexpectedly active')
if any(int(float(r['historicalBinaryMask0493x16f'])) != 1 for r in rows):
    raise SystemExit('[0493x16h] ERROR historical binary mask not active')
if any(int(float(r['poststreamTemporalSync0493x16f'])) != 1 for r in rows):
    raise SystemExit('[0493x16h] ERROR poststream temporal sync not active')
if any(int(float(r['hostGeometryFieldUploadBytes0493x16e'])) != 0 or
    ...
    raise SystemExit('[0493x16h] ERROR full-grid host traffic detected')
print('[0493x16h] resident/historical-binary/poststream-sync/full-grid-transfer checks PASS')
PY_RESIDENT

echo "[0493x16h] COMPLETE run=$RUN_ROOT"
echo "[0493x16h] case complete; run the pair analyzer after both rest and boost cases"
