#!/usr/bin/env python3
"""Summarize 0326b Q6/resampling/virial non-regression smoke results."""
...
def main() -> int:
    manifest_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("dev_history/artifacts/gpu_nonregression_q6_resampling_virial_0326b/gpu_nonregression_q6_resampling_virial_0326b_manifest.csv")
    ...
    out_path = art_dir / "gpu_nonregression_q6_resampling_virial_0326b_summary.csv"
    fieldnames = [
    ...
    print(f"[0326b-summary] wrote {out_path}")
    print(f"[0326b-summary] rows={len(summary_rows)} fail_or_unknown={fail_count}")
