...
# 0209a A/B profile for experimental incremental/reused resampling pool.
# The rebuild path keeps the validated full pool rebuilds. The incremental path
# reuses the current pool at step start and skips the post transfer rebuild when
# extraction/insertion is paired and leaves roles unchanged as a set.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

ROOT_RUN="${ROOT_RUN:-runs/profile_incremental_pool_0209a}"
CXX="${CXX:-g++}"
...
INACTIVE_FACTORS="${INACTIVE_FACTORS:-0 1 2 3}"
MODES_LIST="${MODES_LIST:-rebuild incremental}"
...
BIN="build/src_mpcd_base_incremental_pool_0209a"
echo "[0209a-build] $BIN"
...
GLOBAL_CSV="$ROOT_RUN/logs/timings_incremental_pool_0209a.csv"
...
run_one() {
  ...
  local tag="$2"
  local base_param="$3"
  local mode="$4"        # rebuild or incremental
  ...
  unset MPCD_DISABLE_FAST_RESAMPLING_INSERT_REMOVE || true
  unset MPCD_DISABLE_SPARSE_RESAMPLING_CELLID_RESET || true
  if [[ "$mode" == "incremental" ]]; then
    export MPCD_ENABLE_INCREMENTAL_RESAMPLING_POOL=1
  else
    unset MPCD_ENABLE_INCREMENTAL_RESAMPLING_POOL || true
  fi
  ...
  echo "[0209a-run] factor=$factor mode=$mode run=$run_kind threads=$threads phase=$phase_csv audit=$audit_csv"
...
print('\n[0209a-summary] wall by inactive factor')
for r in rows:
    if r['run_kind'] == 'perf':
...
print('\n[0209a-summary] phase comparison, perf rows')
keys=['total','stream','boundary','resamp_pool0','collision','q6','thermostat','resamp_deposit0','resamp_population_guard','resamp_pool_guard','resamp_deposit_guard','resamp_extract','resamp_insert','resamp_pool_edit','resamp_deposit_edit','resamp_thermal_late','resamp_velocity_refresh']
for r in rows:
...
print('\n[0209a-summary] speedup incremental vs rebuild')
by={}
for r in rows:
    if r['run_kind'] == 'perf':
        by[(r['inactive_factor'], r['threads'], r['mode'])]=float(r['elapsed'])
for factor in sorted(set(k[0] for k in by), key=lambda x: float(x)):
    for threads in sorted(set(k[1] for k in by if k[0]==factor), key=int):
        rebuild=by.get((factor,threads,'rebuild'))
        incremental=by.get((factor,threads,'incremental'))
        if rebuild and incremental:
            print(f"factor={factor} threads={threads} wall rebuild={rebuild:.6g}s incremental={incremental:.6g}s speedup={rebuild/incremental:.4g}")

print('\n[0209a-summary] audit scans by factor')
for r in rows:
