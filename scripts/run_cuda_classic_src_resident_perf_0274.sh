...
# 0274 — performance pass for fused stream+deposit in the validated CUDA resident classic SRC stack.
# This runner keeps the 0265c validation structure and enables the 0274
# stream+deposit fusion for periodic and wall-simple classic resident cases,
# while retaining the 0271/0272b/0273 fixed-overhead reductions.
...
BIN=${BIN:-build/src_mpcd_base_cuda_0274}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_classic_src_resident_perf_0274}
OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_classic_src_resident_perf_validation_0274.csv}
THREADS=${THREADS:-8}
GAMMA=${GAMMA:-20}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0274-perf] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0274.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0274.sh
fi
...
export MPCD_CUDA_RESIDENT_PROFILE_0274=1
# Keep 0271 fixed-overhead and 0272b/0273 collision-wrapper optimizations active;
# 0274 additionally fuses force/stream into the collision deposit for the
# strictly validated periodic and wall-simple resident subsets.
...
# Leave the 0274 explicit-gamma host role-count fast path enabled by default.
...
# Set MPCD_CUDA_PERSISTENT_SRC_COLLISION_DISABLE_FUSED_STREAM_DEPOSIT_0274=1
# to fall back to the separate 0271 streaming kernel plus 0273 collision path.

# Keep child validation CSV parsing from 0265c, but place all artifacts under the
# 0274 performance root and use the 0274 binary.
BIN="$BIN" \
...
python3 scripts/summarize_cuda_resident_perf_0274.py "$ART_DIR/validation_runs" \
  --summary-out "$ART_DIR/cuda_classic_src_resident_perf_summary_0274.csv" \
  --phases-out "$ART_DIR/cuda_classic_src_resident_perf_phases_0274.csv"

echo "[0274-perf] validation manifest: $OUT_CSV"
echo "[0274-perf] performance summary: $ART_DIR/cuda_classic_src_resident_perf_summary_0274.csv"
echo "[0274-perf] detailed phases: $ART_DIR/cuda_classic_src_resident_perf_phases_0274.csv"
echo "[0274-perf] Q6/resampling remain disabled only in these classic-only validators; reactivation paths are intentionally preserved."
echo "[0274-perf] CUDA thermostat wall/solid/piston/IO-aware remains a separate future chantier."
