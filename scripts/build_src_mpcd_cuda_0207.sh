...
# 0207 — CUDA-enabled SRC/MPCD executable for active cell-relative thermostat validation.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0207}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0207-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0207-cuda-build] Built $OUT"
