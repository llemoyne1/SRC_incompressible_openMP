#!/usr/bin/env python3
"""0326c strict parser for Q6/resampling/virial non-regression outputs.

This script intentionally does not run or modify the solver.  It reads an
existing 0326b artifact directory and checks whether each advertised physics
...
DEFAULT_ART_DIR = Path("dev_history/artifacts/gpu_nonregression_q6_resampling_virial_0326b")
DEFAULT_OUT_DIR = Path("dev_history/artifacts/gpu_nonregression_q6_resampling_virial_0326c_strict")
...
def main(argv: List[str]) -> int:
    ...
    manifest_path = art_dir / "gpu_nonregression_q6_resampling_virial_0326b_manifest.csv"
    manifest_rows = read_csv_rows(manifest_path)
    if not manifest_rows:
        print(f"[0326c-strict] ERROR: missing/empty manifest: {manifest_path}", file=sys.stderr)
        return 2

    out_csv = out_dir / "gpu_nonregression_q6_resampling_virial_0326c_strict_summary.csv"
    fieldnames = [
    ...
    print(f"[0326c-strict] wrote {out_csv}")
    print(f"[0326c-strict] rows={len(manifest_rows)} strict_fail={fail_count}")
