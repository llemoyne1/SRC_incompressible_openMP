...
# 0343 — CUDA-VIZ topo/Darcy build for SRC classic Brinkman validation.
...
OUT=${OUT:-build/src_mpcd_base_cuda_topo_0343}
...
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0343-topo-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0343-topo-cuda-build] Built $OUT"
