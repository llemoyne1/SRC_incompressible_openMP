...
for required in \
  ...
  "$ROOT/scripts/analyze_0493x14j_drop_radial.py"; do
  if [[ ! -f "$required" ]]; then
    echo "[0493x14o] ERROR missing prerequisite $required" >&2
    exit 2
  fi
done

# =============================================================================
# 0493x14o — x6g EOS-pressure ablation with the x14m physical chain unchanged
#
# Purpose:
#   Causal ablation of the x6g EOS pressure trace while preserving the entire
#   x14m system: qualified liquid chain x10o+CIC+Q2+x10p/q+x10u+x10v+x12a,
#   x14l gas-side local-moving-frame specular reflection, common SRC collisions,
#   differentiated thermostats, x9 capillarity, Q6-g-f and grid shift.
#
#   Only x6g's gas pressure provider changes: after the common run helper has
#   configured the production EOS mode, this runner overrides it to CONSTANT
#   with pConst == pRef.  Therefore the x6g gas-pressure potential is spatially
#   uniform and gauge-zero (pGas-pRef = 0), while x9 still supplies sigma*kappa.
#   This isolates the causal channel reflection -> gas cell occupancy -> x6g EOS.
#
#   No C++/CUDA source is changed.  ./livevis_control.kv is only read.
#
...
# -----------------------------------------------------------------------------
CASE_LABEL="${CASE_LABEL:-0493x14o_x6g_constant_pressure_ablation_drop}"
TOPOLOGY="closed_box"
RUN_MODE="src-q6-g-f"
...
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)):
    raise SystemExit('[0493x14o] square cells required')
r=rc*hx
area=hx*hy
pg=g*kbtg/area
lap=sigma/r
...
for n,v in [('mL',mL),('mG',mG),('R',R)]:
    if not (math.isfinite(v) and v>0): raise SystemExit(f'[0493x14o] {n} must be finite >0')
for n,v in [('kBT_L',tL),('kBT_G',tG)]:
    if not (math.isfinite(v) and v>=0): raise SystemExit(f'[0493x14o] {n} must be finite >=0')
if R >= min(cx,Lx-cx,cy,Ly-cy): raise SystemExit('[0493x14o] drop touches/overlaps a wall')
PY

BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x14o_x6g_constant_pressure_ablation_drop_${NX}x${NY}_g${GAMMA}_rc${R_CELLS}_TL${LIQUID_KBT}_TG${GAS_KBT}_s${SURFACE_TENSION_SIGMA}_pconst}"
...
speciesDiagnosticsFilename = species_runtime_0493x14o.csv
...
# 0493x14o causal ablation: suite_export_cuda_flags_0434 configures the normal
# x6g EOS provider for an explicit gas. Override it *after* that helper so this
# runner cannot silently fall back to EOS.  pConst == pRef makes the x6g gas
# pressure contribution exactly gauge-zero and spatially constant.  x9 later
# composes sigma*kappa on the same interface face values as in production.
export MPCD_Q6_PHASE_GAS_PRESSURE_0493X6G=1
export MPCD_Q6_PHASE_GAS_PRESSURE_MODE_0493X6G=constant
export MPCD_Q6_PHASE_GAS_PRESSURE_REFERENCE_0493X6G="$GAS_THERMAL_PRESSURE"
export MPCD_Q6_PHASE_GAS_PRESSURE_CONSTANT_0493X6G="$GAS_THERMAL_PRESSURE"
...
suite_write_env_file_0434 "$BASE_RUN_ROOT/logs/environment_0493x14o.env" "$RUN_MODE"
cat >> "$BASE_RUN_ROOT/logs/environment_0493x14o.env" <<META
...
X6G_PRESSURE_ABLATION=constant-pConst-equals-pRef
MPCD_Q6_PHASE_GAS_PRESSURE_0493X6G=$MPCD_Q6_PHASE_GAS_PRESSURE_0493X6G
MPCD_Q6_PHASE_GAS_PRESSURE_MODE_0493X6G=$MPCD_Q6_PHASE_GAS_PRESSURE_MODE_0493X6G
MPCD_Q6_PHASE_GAS_PRESSURE_REFERENCE_0493X6G=$MPCD_Q6_PHASE_GAS_PRESSURE_REFERENCE_0493X6G
MPCD_Q6_PHASE_GAS_PRESSURE_CONSTANT_0493X6G=$MPCD_Q6_PHASE_GAS_PRESSURE_CONSTANT_0493X6G
MPCD_Q6_PHASE_GAS_PRESSURE_SCALE_0493X6G=$MPCD_Q6_PHASE_GAS_PRESSURE_SCALE_0493X6G
...
run_ok_surface_print_0493x13zi "x9+x6g-CONSTANT-PRESSURE-ABLATION+liquid-qualified-full-x10uv-x12a+gas-specular-no-impulse-feedback"
echo "===== 0493x14o x6g CONSTANT-PRESSURE ABLATION + FULL LIQUID + GAS SPECULAR ====="
echo "PATHS: runner=$ROOT/scripts/run_ok_0493x14o_x6g_constant_pressure_ablation_drop.sh"
...
echo "X6G:   ABLATION mode=constant pRef=$MPCD_Q6_PHASE_GAS_PRESSURE_REFERENCE_0493X6G pConst=$MPCD_Q6_PHASE_GAS_PRESSURE_CONSTANT_0493X6G scale=$MPCD_Q6_PHASE_GAS_PRESSURE_SCALE_0493X6G"
echo "       expected gas contribution: pConst-pRef=0 everywhere; x9 sigma*kappa remains active"
...
# Keep the existing x14j analyzer untouched, but publish x14o-named copies so
# x6g-constant ablation results cannot be confused with production-EOS x14m.
cp -f "$ANALYSIS_DIR/radial_metrics_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_metrics_0493x14o.csv"
cp -f "$ANALYSIS_DIR/radial_profiles_0493x14j.csv" \
      "$ANALYSIS_DIR/radial_profiles_0493x14o.csv"

echo "[0493x14o] DONE"
echo "[0493x14o] return: $ANALYSIS_DIR/radial_metrics_0493x14o.csv"
echo "[0493x14o] return: $ANALYSIS_DIR/radial_profiles_0493x14o.csv"
echo "[0493x14o] return: $ANALYSIS_DIR/laplace_static_drop_0493x14o.json (if produced)"
