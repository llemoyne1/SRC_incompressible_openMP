...
# 0284 — immersed circle validation for the CUDA persistent SRC+thermostat path.
# Scope: periodic box + static circular immersed solid, classic SRC only.
# Q6/resampling/virial are disabled for this validation only; the flags are not
# architectural locks against later CPU Q6, CUDA/CPU resampling, virial, or Q6 CUDA.
#
# Important implementation detail:
#   MPCD_CUDA_CLASSIC_SRC_SOLID_RESIDENT_0262=1 is used here as the compatibility
#   switch expected by the 0245/0247 CUDA periodic+rectangle streaming kernels.
#   The old 0262 monolithic classic-only resident shortcut remains inactive because
#   THERMOSTAT_ENABLE=true; collision and thermostat are handled by the persistent
#   SRC+thermostat backend validated in 0276.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0284}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_circle_0284}
GRID_CASES=${GRID_CASES:-"64:64:120"}
CASES=${CASES:-"periodic_circle_obstacle_classic"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SEED_BASE=${SEED_BASE:-162840}
STOP_ON_FAIL=${STOP_ON_FAIL:-1}
FORCE_REBUILD=${FORCE_REBUILD:-1}
SUMMARY_EVERY_MODE=${SUMMARY_EVERY_MODE:-final}

BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_circle_src_thermostat}
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0284_cuda_circle_src_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0284-cuda-thermostat-circle] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0284.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0284.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0284-cuda-thermostat-circle] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_circle_0284.csv}
python3 - "$OUT_CSV" <<'PY'
import csv, sys
header = [
    'caseName','grid','NX','NY','steps','mode','classicSrcCudaMode','thermostatEnable',
    'wallThermalNoise','solidResidentCompatibilityFlag','runExitCode','compareExitCode',
    ...
    'thermostatKBTBeforeMean','thermostatKBTAfterMean','failed_metrics','compared_metrics',
    'verdict','stdoutLog','stderrLog','compareCsv','compareSummary'
]
...
run_validation_logged() {
  ...
  local periodic_stream=0 periodic_download=1 immersed_rect=0 immersed_download=1 solid_compat=0
  ...
  case "$mode" in
    ...
    "$CUDA_MODE_NAME")
      ...
      metadata_cache=1
      ;;
    *) echo "[0284-cuda-thermostat-circle] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$case_name" WALL_THERMAL_NOISE=0 \
      ...
      MPCD_CUDA_CLASSIC_SRC_SOLID_RESIDENT_0262="$solid_compat" \
      ...
      MPCD_CUDA_IMMERSED_CIRCLE_0284="$immersed_rect" \
      MPCD_CUDA_IMMERSED_CIRCLE_0284_DOWNLOAD_ALL="$immersed_download" \
      MPCD_CUDA_IMMERSED_CIRCLE_0284_THREADS="${MPCD_CUDA_IMMERSED_CIRCLE_0284_THREADS:-256}" \
      ...
      MPCD_CUDA_PERSISTENT_SRC_COLLISION_IMMERSED_CIRCLE_0284="$persistent_collision" \
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -z "${steps:-}" ]]; then
    echo "[0284-cuda-thermostat-circle] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "periodic_circle_obstacle_classic" ]]; then
      echo "[0284-cuda-thermostat-circle] ERROR: 0284 is limited to periodic_circle_obstacle_classic / static circular immersed solid, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0284-cuda-thermostat-circle] running baseline case=$case_name grid=$grid_tag"
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_thermostat_solid_0277_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
    base_rc=$?
    ...
    echo "[0284-cuda-thermostat-circle] running CUDA thermostat case=$case_name grid=$grid_tag"
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_thermostat_solid_0277_${case_name}_${CUDA_MODE_NAME}_${grid_tag}" "$CUDA_MODE_NAME" "$stdout_log" "$stderr_log"
    rc=$?
    cmp_rc=0
...
PY
    ...
    solid_compat=1
    append_csv_row \
      "$case_name" "$grid_tag" "$nx" "$ny" "$steps" "$CUDA_MODE_NAME" "1" "1" "0" "$solid_compat" \
      "$rc" "$cmp_rc" "$base_total" "$mode_total" "$delta" "$speedup" \
    ...
    echo "[0284-cuda-thermostat-circle] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0284-cuda-thermostat-circle] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0284-cuda-thermostat-circle] wrote $OUT_CSV"
