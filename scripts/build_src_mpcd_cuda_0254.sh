...
# 0254 — CUDA boundary stack + persistent SRC collision with immersed rectangle virtual-wall contribution.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0254}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0254-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0254-cuda-build] Built $OUT"
