...
# 0493x16d — Galilean common-translation case for the dynamic chi-solid RigidSlab1D.
...
CASE_LABEL="${CASE_LABEL:-0493x16d_galilean_case}"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x16d.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x16d_galilean_case}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x16d] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi
...
if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x16d] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
  ...
  if [[ -z "$RESTART_STEP" || ! -s "$OLD_SOLID" ]]; then
    echo "[0493x16d] ERROR restart requires matching chi_solid_dynamics_0493x16a.csv" >&2
    exit 2
  fi
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x16d.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x16d.txt" <<META
case=$CASE_LABEL
architecture=dynamic_chi_solid_galilean_common_translation
...
printf '\n===== 0493x16d GALILEAN COMMON TRANSLATION / RIGID SLAB 1D =====\n'
printf 'run=%s\n' "$RUN_ROOT"
...
[[ -s "$OUT/chi_solid_dynamics_0493x16a.csv" ]] || { echo "[0493x16d] ERROR missing dynamic solid diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x16d] ERROR missing Darcy/bath impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x16d] ERROR missing chiVP impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x16d] ERROR missing summary_runtime.csv" >&2; exit 2; }
head -n 1 "$OUT/chi_solid_dynamics_0493x16a.csv" | grep -q 'fictitiousFluidDiagnostic0493x16c' || {
  echo "[0493x16d] ERROR binary/output does not contain x16c fictitious-fluid diagnostics" >&2
  exit 2
}

echo "[0493x16d] COMPLETE run=$RUN_ROOT"
echo "[0493x16d] case complete; run the pair analyzer after both rest and boost cases"
