...
: "${BIN:=build/src_mpcd_base_cuda_q6_resident_periodic_equiv_0467b}"
: "${BASE_0467B_ROOT:=runs/0467b_external_resident_carrier_probe}"
...
mkdir -p "$BASE_0467B_ROOT"
RAW_ROOT="$BASE_0467B_ROOT/raw_scaling"
...
python3 - <<'PY' "$BASE_0467B_ROOT" "$RAW_ROOT" "$SCALE_CASES" "$SEEDS" "$STEPS" "$DEVICE_GATE_EVERY"
import csv, glob, sys
...
# Optional scaling summary for deltas and wall times.
scale_rows = []
...
pass_rows = sum(1 for r in records if r['handled'] > 0 and r['resident_rows'] == r['handled'] and r['external_rows'] == r['handled'] and r['external_min'] == 1)

flat = out_root / 'external_resident_carrier_summary_0467b.csv'
with flat.open('w', newline='') as f:
    fields = ['case','mode','seed','rows','handled','resident_rows','external_rows','full_gate_rows','cpu_wall_s','cuda_wall_s','speedup','max_summary_delta','max_upload_s','max_state_download_s','max_device_total_s']
    ...
    for r in records:
        w.writerow({
            ...
            'max_upload_s': r['max_upload'], 'max_state_download_s': r['max_state_dl'], 'max_device_total_s': r['max_total'],
        })

md = out_root / 'external_resident_carrier_report_0467b.md'
with md.open('w') as f:
    f.write('# 0467B external resident-carrier probe\n\n')
    f.write('Scope: call the 0467 resident CUDA carrier core from a caller-owned `CudaParticleState` in the 0448 transaction wrapper. This is still transaction-safe and still downloads the state for correctness; it verifies that ownership of the CUDA particle state can be lifted out of the legacy 0455 carrier wrapper.\n\n')
    f.write(f'Scale cases: `{scale_cases}`\n')
    f.write(f'Seeds: `{seeds}`\n')
    f.write(f'Steps: `{steps}`, DEVICE_GATE_EVERY: `{gate_every}`\n\n')
    f.write(f'External resident-carrier rows: **{pass_rows}/{len(records)}**\n\n')
    f.write('| case | mode | seed | rows | handled | resident rows | external rows | full gate rows | CPU wall s | CUDA wall s | speedup | max summary delta | max upload s | max state dl s | max device total s |\n')
    f.write('| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n')
    for r in records:
        def fmt(x):
        ...
        f.write(f"| {r['case']} | {r['mode']} | {r['seed']} | {r['rows']} | {r['handled']} | {r['resident_rows']} | {r['external_rows']} | {r['full_gate_rows']} | {fmt3(r['cpu_wall'])} | {fmt3(r['cuda_wall'])} | {fmt3(r['speedup'])} | {fmt(r['max_delta'])} | {r['max_upload']:.3e} | {r['max_state_dl']:.3e} | {r['max_total']:.3e} |\n")
    f.write(f'\nFlat CSV: `{flat}`\n')
    f.write('\nInterpretation: 0467B is not expected to remove upload/download costs yet. Success means every handled CUDA carrier row has both `residentCore0467=1` and `residentExternal0467B=1`, proving that the resident core can be driven by an externally owned CUDA particle state.\n')
