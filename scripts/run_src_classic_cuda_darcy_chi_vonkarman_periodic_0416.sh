...
# 0416 -- autonomous SRC classic CUDA + Darcy/Brinkman von Karman chi-file demo.
# Periodic left/right, solid top/bottom, no Q6, no resampling, livevis enabled.
# Domain: Lx=1.5, Ly=0.4, Nx=1200, Ny=640, gamma=6.
# Circular chi obstacle: xc=0.2, yc=0.205, rc=0.04.
# External .smpcd initial state is homogeneous with mean velocity U0 and contains no active particles inside the circle.
...
# Runtime uses CHI_FILE only; these variables are printed for traceability.
...
STATE_SOURCE=${STATE_SOURCE:-${INPUT_STATE:-./init/src_classic_darcy_vonkarman_U0p9_1200x640_g6_kBT5_no_circle.smpcd}}
...
if [[ -z "$CHI_FILE" ]]; then
  echo "[0416-darcy-vk] ERROR: provide CHI_FILE=/path/to/chi.f32 or DARCY_CHI_FILE=/path/to/chi.f32" >&2
  exit 2
fi
if [[ ! -f "$CHI_FILE" ]]; then
  echo "[0416-darcy-vk] ERROR: chi file not found: $CHI_FILE" >&2
  exit 2
fi
if [[ -z "$STATE_SOURCE" ]]; then
  echo "[0416-darcy-vk] ERROR: provide STATE_SOURCE=/path/to/state.smpcd or INPUT_STATE=/path/to/state.smpcd" >&2
  exit 2
fi
if [[ ! -f "$STATE_SOURCE" ]]; then
  echo "[0416-darcy-vk] ERROR: initial state file not found: $STATE_SOURCE" >&2
  exit 2
fi
...
if [[ ! -x "$BIN" ]]; then echo "[0416-darcy-vk] ERROR missing binary: $BIN" >&2; exit 127; fi
...
env | grep -E '^(MPCD_DARCY_FASTFLAGS_ENABLE=|MPCD_CUDA_|SRC_LIVE_VIS_|MPCD_LIVE_VIS_ENABLE=|OMP_|BIN=|CHI_FILE=|STATE_SOURCE=|INPUT_STATE=|DARCY_|TOPO_BENCHMARK_|ALPHA=|U0=|DT=|KBT=|NX=|NY=|CIRCLE_)' | sort > "$ENV_FILE"

echo "[0416-darcy-vk] binary=$BIN"
...
echo "[0416-darcy-vk] livevis=$LIVE_VIS_ENABLE control=$LIVE_VIS_CONTROL_FILE field=$LIVE_VIS_FIELD"

/usr/bin/time -o "$TIME_FILE" -f 'elapsed=%e user=%U sys=%S' "$BIN" "$PARAMS"

echo "[0416-darcy-vk] time=$(cat "$TIME_FILE")"
echo "[0416-darcy-vk] darcy csv=$OUT_DIR/darcy_cost_0343.csv"
if [[ "$TOPO_BENCHMARK_ENABLE" == "true" || "$TOPO_BENCHMARK_ENABLE" == "1" || "$TOPO_BENCHMARK_ENABLE" == "TRUE" ]]; then
  echo "[0416-darcy-vk] topo benchmark csv=$OUT_DIR/$TOPO_BENCHMARK_FILENAME"
fi
echo "[0416-darcy-vk] dumps/root=$RUN_ROOT"
