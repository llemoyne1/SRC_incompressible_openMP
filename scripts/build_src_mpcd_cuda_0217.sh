...
# 0217 — CUDA-enabled SRC/MPCD executable for the ordered persistent collision -> Q6 -> thermostat stack.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0217}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0217-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0217-cuda-build] Built $OUT"
