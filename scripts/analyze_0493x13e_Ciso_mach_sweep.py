...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--campaign-root',default='runs/0493x13e_compressible_mach_reach');ap.add_argument('--repo-root',default='.');ap.add_argument('--cs-ref',type=float,default=.3554482475790296);ap.add_argument('--nuT-ref',type=float,default=.0005328464868639473);ap.add_argument('--mode-max',type=int,default=4);a=ap.parse_args()
    root=Path(a.repo_root).resolve();camp=root/a.campaign_root
    manifest=camp/'Ciso_mach'/'manifest_0493x13e_Ciso.csv';rows=read_manifest(manifest)
    w1=load_module(root/'scripts/analyze_0493w1_src_fluid_calibrator.py','w1_x13e')
    run_rows=[];series_rows=[]
    for r in rows:
        run_dir=camp/'Ciso_mach'/r['runDir'];marker=run_dir/'RUN_COMPLETE_0493x13e_Ciso';failed=run_dir/'RUN_FAILED_0493x13e_Ciso'
        status='MISSING'
        ...
        elapsed=read_time(run_dir/'logs/time_0493x13e.txt')
    ...
    analysis=camp/'analysis';write_csv(analysis/'Ciso_runs_0493x13e.csv',run_rows);write_csv(analysis/'Ciso_mode_series_0493x13e.csv',series_rows)
    ...
    write_csv(analysis/'Ciso_group_summary_0493x13e.csv',groups)
    ...
    write_csv(analysis/'Ciso_mach_qualification_0493x13e.csv',quals)
    ...
    write_csv(analysis/'Ciso_Re_reach_0493x13e.csv',reach)
    print(f'[0493x13e-analysis] runs={len(run_rows)} groups={len(groups)} MachQual={len(quals)} H_h={H:.6g}')
    for q in quals:
        n10=math.ceil(10000/(f(q['mach'])*H));print(f"[0493x13e-analysis] Ma={q['mach']} grade={q['compressibleGrade']} cs256={q['cs256']:.6g} harm256={q['harmonicPeak256']:.4g} Re1e4_cells={n10}")
...
def self_test():
    ...
    print(f'[0493x13e-analysis-selftest] csErr={err:.3e} harmonic={r["harmonicFractionPeakCycle1"]:.3e}')
