...
CASE_LABEL="contact_angle_wallface_0493x9l"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"; TOPOLOGY=closed_box
RUN_ROOT="${RUN_ROOT:-runs/0493x9l_contact_angle_wallface}"
...
if ((${#ANGLE_LIST[@]} == 0)); then
  echo "[0493x9l] ERROR: ANGLES is empty" >&2
  exit 2
fi
...
if abs(lx/nx-ly/ny)>1e-12: raise SystemExit('[0493x9l] square cells required')
if not (r>0 and s>0): raise SystemExit('[0493x9l] radius and sigma must be positive')
for a in angles:
    if not (math.isfinite(a) and 0.0 < a < 180.0):
        raise SystemExit('[0493x9l] contact angles must satisfy 0 < theta < 180; endpoints are intentionally unsupported')
PY
...
run_one(){
  ...
  local state="$dir/init/sessile_cap_0493x9l.smpcd" params="$dir/params/contact_angle_wallface_0493x9l.kv" log="$dir/logs/contact_angle_wallface_0493x9l.log" tf="$dir/logs/contact_angle_wallface_0493x9l.time"
...
speciesDiagnosticsFilename = species_runtime_0493x9l.csv
...
PARAMS_EOF
  ...
  export MPCD_Q6_CONTACT_ANGLE_WALL_FACE_0493X9L=1
  BASE_RUN_ROOT="$dir"; LIVE_VIS_CONTROL_FILE="$dir/livevis_control_0493x9l.kv"; suite_prepare_livevis_control_0434 "$dir" "$RUN_MODE"; suite_export_livevis_0434
  echo "[0493x9l-suite] theta=$theta radius=$RADIUS exactKappa=$(awk -v r="$RADIUS" 'BEGIN{printf "%.8g",1/r}') sigma=$SIGMA closure=wall_face_normal"
...
if ! suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  python3 scripts/analyze_0493x9l_contact_angle_wallface.py \
    --root "$RUN_ROOT" --angles "${ANGLE_LIST[@]}" --radius "$RADIUS" \
    --baseline-x9k-root "$BASELINE_X9K_ROOT"
fi
