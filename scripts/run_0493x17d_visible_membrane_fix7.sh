...
# 0493x17d-fix7 bendable coherent strip: transverse thickness rungs with tunable diagonal shear braces.
...
CASE_LABEL="${CASE_LABEL:-0493x17d_visible_membrane_fix7}"
MODE="${MODE:-src-q6}"
TOPOLOGY=periodic
GEN_CASE=uniform
...
MEMBRANE_K_STRETCH="${MEMBRANE_K_STRETCH:-120000.0}"
MEMBRANE_K_AREA="${MEMBRANE_K_AREA:-10000000.0}"
MEMBRANE_DAMPING="${MEMBRANE_DAMPING:-5000.0}"
MEMBRANE_K_BENDING="${MEMBRANE_K_BENDING:-0.002}"
MEMBRANE_K_CROSSBRACE="${MEMBRANE_K_CROSSBRACE:-60000.0}"
MEMBRANE_CROSSBRACE_RANGE_EDGES="${MEMBRANE_CROSSBRACE_RANGE_EDGES:-16.0}"
MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION="${MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION:-0.0}"
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x17d_visible_membrane_fix7}"
...
python3 - "$REFINE" "$Lx" "$Ly" "$NX" "$NY" "$H" "$DT" "$KBT" "$PARTICLE_MASS" \
  ...
  "$MEMBRANE_K_STRETCH" "$MEMBRANE_K_BENDING" "$MEMBRANE_K_CROSSBRACE" "$MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION" "$SOLID_MASS" <<'PY'
import math,sys
(r,Lx,Ly,nx,ny,h,dt,kbt,m,xmin,xmax,ymin,ymax,ks,kb,kc,kdiag,ms) = map(float,sys.argv[1:])
nx=int(nx); ny=int(ny)
...
Sstretch=dt*math.sqrt((ks+kc*(1.0+2.0*kdiag))/node_mass)
...
print(f"[0493x17d-pub] estimatedNodes={n_est:.1f} estimatedNodeMass={node_mass:.6g} "
      f"localSpringStability={Sstretch:.6g} bendingStability={Sbend:.6g}")
if S >= 0.5:
...
chiSolidMembraneCrossBraceStiffness = $MEMBRANE_K_CROSSBRACE
chiSolidMembraneCrossBraceRangeEdges = $MEMBRANE_CROSSBRACE_RANGE_EDGES
chiSolidMembraneCrossBraceDiagonalFraction = $MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION
...
crossBraceStiffness=$MEMBRANE_K_CROSSBRACE
crossBraceRangeEdges=$MEMBRANE_CROSSBRACE_RANGE_EDGES
crossBraceDiagonalFraction=$MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION
...
printf 'membrane M=%s kS=%s kA=%s damping=%s bending=%s crossBrace=%s diagFrac=%s\n\n' "$SOLID_MASS" "$MEMBRANE_K_STRETCH" "$MEMBRANE_K_AREA" "$MEMBRANE_DAMPING" "$MEMBRANE_K_BENDING" "$MEMBRANE_K_CROSSBRACE" "$MEMBRANE_CROSSBRACE_DIAGONAL_FRACTION"
