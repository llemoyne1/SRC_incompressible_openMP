...
OUT=${OUT:-build/validate_cuda_resampling_planner_shadow_0441}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0441-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
"$NVCC" $NVCCFLAGS $CUDA_ARCH_FLAGS \
  ...
  src/main_validate_cuda_resampling_planner_shadow_0441.cu \
