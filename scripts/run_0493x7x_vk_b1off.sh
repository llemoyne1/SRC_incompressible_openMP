...
# 0493x7x controlled B1-off ablation; mechanically derived from scripts/run_ok_vk.sh.

ROOT="${ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
source "$ROOT/scripts/src_mpcd_run_common_0434.sh"

# -----------------------------------------------------------------------------
# 0493x7x controlled B1-off ablation.
#
# IMPORTANT:
#   * production scripts are not modified;
#   * every production preflight check is retained;
#   * only the production requirement "B1 must be 1" is virtualized while
#     preflight runs;
#   * the executable is launched with B1=0.
# -----------------------------------------------------------------------------
eval "$(declare -f suite_preflight_run_ok_0492 | sed '1s/suite_preflight_run_ok_0492/suite_preflight_run_ok_0492_0493x7x_production/')"

suite_preflight_run_ok_0492() {
  local mode="${SUITE_ACTIVE_MODE_0434:-unknown}"

  if [[ "$mode" == "src-q6-g-f" ]]; then
    local actual_b1="${MPCD_Q6_FACE_TO_PARTICLE_RT0_0493X6H_B1:-unset}"
    if [[ "$actual_b1" != "0" ]]; then
      echo "[0493x7x] ERROR controlled ablation requires actual B1=0 before preflight; got $actual_b1" >&2
      return 2
    fi

    export MPCD_Q6_FACE_TO_PARTICLE_RT0_0493X6H_B1=1
    local rc=0
    suite_preflight_run_ok_0492_0493x7x_production "$@" || rc=$?
    ...
    echo "[0493x7x] preflight=PASS production checks retained; actual launch ablation B1=0"
    return 0
  fi

  suite_preflight_run_ok_0492_0493x7x_production "$@"
}
# -----------------------------------------------------------------------------
suite_root_cd_0434
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x7x_vk_b1off_${NX}x${NY}_g${GAMMA}_u${U0}_kBT${KBT}}"
...
run_one_mode_0434() {
  ...
  # 0493x7x -- ONLY intentional numerical-path ablation.
  export MPCD_Q6_POSTAPPLY_REGION_DIAGNOSTICS_0493X6H_B0=0
  export MPCD_Q6_FACE_TO_PARTICLE_RT0_0493X6H_B1=0

  if [[ "${MPCD_Q6_PHASE_GEOMETRY_RESIDENT_0493X6C:-0}" != "1" ]]; then
    echo "[0493x7x] ERROR x6c unexpectedly disabled" >&2
    return 2
  fi
  if [[ "${MPCD_Q6_PHASE_INTERFACE_STENCIL_0493X6F:-0}" != "1" ]]; then
    echo "[0493x7x] ERROR x6f unexpectedly disabled" >&2
    return 2
  fi
  ...
  cat >> "$run_root/logs/environment_0434.env" <<META_0493X7X
RUN_OK_Q6_PROFILE_ABLATION=0493x7x_vk_b1off
...
META_0493X7X
  ...
  echo "[0493x7x] CONTROLLED ABLATION mode=$mode B0=$MPCD_Q6_POSTAPPLY_REGION_DIAGNOSTICS_0493X6H_B0 B1=$MPCD_Q6_FACE_TO_PARTICLE_RT0_0493X6H_B1"
