...
# 0305 — CUDA SRC classic build plus passive post-SRC adaptive support flag diagnostic.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0305}

mkdir -p build
if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0305-cuda-build] ERROR: nvcc not found" >&2
  exit 2
fi
...
echo "[0305-cuda-build] Built $OUT"
