...
# 0225 — CUDA-enabled SRC/MPCD executable with shared CudaParticleState
...
OUT=${OUT:-build/src_mpcd_base_cuda_0225}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0225-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0225-cuda-build] Built $OUT"
