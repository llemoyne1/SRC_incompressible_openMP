...
# 0204 — CUDA-enabled SRC/MPCD executable for combined TG benchmarks.
# Same build scope as 0203, with Q6 CUDA and CUDA cell-moment deposit enabled.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0204}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0204-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0204-cuda-build] Built $OUT"
