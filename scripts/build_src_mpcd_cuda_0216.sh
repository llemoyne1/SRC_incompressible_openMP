...
# 0216 — CUDA-enabled SRC/MPCD executable for the first persistent-collision -> Q6 CUDA -> CPU thermostat composition.
# This build keeps all CUDA kernels compiled but does not change the CPU default path.
...
OUT=${OUT:-build/src_mpcd_base_cuda_0216}

mkdir -p build

if ! command -v "$NVCC" >/dev/null 2>&1; then
  echo "[0216-cuda-build] ERROR: nvcc not found. Set NVCC=/path/to/nvcc or load CUDA." >&2
  exit 2
fi
...
echo "[0216-cuda-build] Built $OUT"
