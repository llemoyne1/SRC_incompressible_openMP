...
# 0276 — wall-simple validation for the CUDA persistent SRC+thermostat path.
# Scope: Poiseuille wall-simple, classic SRC only, Q6/resampling/virial disabled.
# Goal: compare CPU collision+CPU thermostat against CUDA streaming wall-simple
# + persistent CUDA SRC collision + persistent CUDA thermostat.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0276}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_persistent_src_thermostat_wall_0276}
GRID_CASES=${GRID_CASES:-"64:64:120"}
CASES=${CASES:-"poiseuille_wall_full"}
GAMMA=${GAMMA:-20}
THREADS=${THREADS:-8}
SEED_BASE=${SEED_BASE:-162760}
STOP_ON_FAIL=${STOP_ON_FAIL:-1}
FORCE_REBUILD=${FORCE_REBUILD:-1}
SUMMARY_EVERY_MODE=${SUMMARY_EVERY_MODE:-final}

BASELINE_MODE_NAME=${BASELINE_MODE_NAME:-cpu_wall_src_thermostat}
CUDA_MODE_NAME=${CUDA_MODE_NAME:-0276_cuda_wall_src_thermostat}

mkdir -p "$ART_DIR"

if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0276-cuda-thermostat-wall] rebuilding $BIN (FORCE_REBUILD=$FORCE_REBUILD)"
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0276.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS=${CUDA_ARCH_FLAGS:-} bash scripts/build_src_mpcd_cuda_0276.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0276-cuda-thermostat-wall] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_persistent_src_thermostat_wall_0276.csv}
printf 'caseName,grid,NX,NY,steps,mode,classicSrcCudaMode,thermostatEnable,wallThermalNoise,runExitCode,compareExitCode,baselineTotalWallTime,modeTotalWallTime,totalWallDelta_s,totalWallSpeedup,collisionActiveCalls,collisionTotalSeconds,collisionUploadSeconds,collisionKernelSeconds,collisionDownloadSeconds,collisionSharedParticleStateFraction,collisionSharedCellWorkspaceFraction,thermostatGpuAppliedFraction,thermostatCellsRescaled,thermostatParticlesRescaled,thermostatKBTBeforeMean,thermostatKBTAfterMean,failed_metrics,compared_metrics,verdict,stdoutLog,stderrLog,compareCsv,compareSummary\n' > "$OUT_CSV"
...
run_validation_logged() {
  ...
  local wall_stream=0 wall_download=1 persistent_collision=0 persistent_thermo=0 particle_state=0 cell_workspace=0 metadata_cache=0
  case "$mode" in
    "$BASELINE_MODE_NAME") ;;
    "$CUDA_MODE_NAME")
      wall_stream=1
      ...
      metadata_cache=1
      ;;
    *) echo "[0276-cuda-thermostat-wall] ERROR: unknown mode $mode" >&2; return 2 ;;
  esac
  ...
  env BIN="$BIN" BUILD_IF_MISSING=0 CASE_LIST="$case_name" WALL_THERMAL_NOISE=0 \
      ...
      MPCD_CUDA_STREAMING_WALL_SIMPLE_0246="$wall_stream" \
      MPCD_CUDA_STREAMING_WALL_SIMPLE_0246_DOWNLOAD_ALL="$wall_download" \
...
for grid_spec in $GRID_CASES; do
  ...
  if [[ -z "${steps:-}" ]]; then
    echo "[0276-cuda-thermostat-wall] ERROR: GRID_CASES entries must be NX:NY:STEPS, got $grid_spec" >&2
    exit 2
  fi
  grid_tag="${nx}x${ny}_s${steps}"
  for case_name in $CASES; do
    if [[ "$case_name" != "poiseuille_wall_full" ]]; then
      echo "[0276-cuda-thermostat-wall] ERROR: 0276 is limited to poiseuille_wall_full / wall-simple CL, got $case_name" >&2
      exit 2
    fi
    ...
    echo "[0276-cuda-thermostat-wall] running baseline case=$case_name grid=$grid_tag"
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$base_root" "cuda_thermostat_wall_0276_${case_name}_baseline_${grid_tag}" "$BASELINE_MODE_NAME" "$base_stdout" "$base_stderr"
    base_rc=$?
    ...
    echo "[0276-cuda-thermostat-wall] running CUDA thermostat case=$case_name grid=$grid_tag"
    run_validation_logged "$case_name" "$nx" "$ny" "$steps" "$run_root" "cuda_thermostat_wall_0276_${case_name}_${CUDA_MODE_NAME}_${grid_tag}" "$CUDA_MODE_NAME" "$stdout_log" "$stderr_log"
    rc=$?
    cmp_rc=0
...
PY
    ...
    printf '%s,%s,%s,%s,%s,%s,1,1,0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      "$case_name" "$grid_tag" "$nx" "$ny" "$steps" "$CUDA_MODE_NAME" "$rc" "$cmp_rc" \
    ...
    echo "[0276-cuda-thermostat-wall] $verdict case=$case_name grid=$grid_tag failed=$failed/$compared wall=$mode_total baseline=$base_total thermoGpu=$coll_thermo_gpu"
    if [[ "$STOP_ON_FAIL" == "1" && ( "$base_rc" != "0" || "$rc" != "0" || "$verdict" != "PASS" ) ]]; then
      echo "[0276-cuda-thermostat-wall] stopping after failure; see $stderr_log" >&2
      exit 1
    fi
  done
done

echo "[0276-cuda-thermostat-wall] wrote $OUT_CSV"
