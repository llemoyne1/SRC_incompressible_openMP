...
# 0493x16f — historical-binary chi + post-stream temporal synchronization / Galilean case for RigidSlab1D.
...
CASE_LABEL="${CASE_LABEL:-0493x16f_temporal_sync_galilean_case}"
MODE="src"
TOPOLOGY="periodic"
GEN_CASE="uniform"
...
TOPO_BENCHMARK_FILENAME=topo_benchmark_0493x16f.csv
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x16f_temporal_sync_galilean_case}"
...
if (( NX < 32 || NY < 16 || SLAB_CELLS < 1 || SLAB_CELLS >= NX/2 )); then
  echo "[0493x16f] ERROR require NX>=32 NY>=16 and 1<=SLAB_CELLS<NX/2" >&2
  exit 2
fi
...
if suite_truthy_0434 "$RESTART"; then
  if [[ -z "$RESTART_STATE" || ! -s "$RESTART_STATE" ]]; then
    echo "[0493x16f] ERROR RESTART=1 requires RESTART_STATE=/path/state_step_N.smpcd" >&2
    exit 2
  fi
  ...
  if [[ -z "$RESTART_STEP" || ! -s "$OLD_SOLID" ]]; then
    echo "[0493x16f] ERROR restart requires matching chi_solid_dynamics_0493x16a.csv" >&2
    exit 2
  fi
...
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x16f.env" "$MODE"

cat > "$RUN_ROOT/run_meta_0493x16f.txt" <<META
case=$CASE_LABEL
architecture=cuda_resident_historical_binary_poststream_temporal_sync
...
printf '\n===== 0493x16f TEMPORAL-SYNC GALILEAN / RIGID SLAB 1D =====\n'
printf 'run=%s\n' "$RUN_ROOT"
...
[[ -s "$OUT/chi_solid_dynamics_0493x16a.csv" ]] || { echo "[0493x16f] ERROR missing dynamic solid diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_solid_impulse_0493x15a.csv" ]] || { echo "[0493x16f] ERROR missing Darcy/bath impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/chi_vp_impulse_0493x15b.csv" ]] || { echo "[0493x16f] ERROR missing chiVP impulse diagnostic" >&2; exit 2; }
[[ -s "$OUT/summary_runtime.csv" ]] || { echo "[0493x16f] ERROR missing summary_runtime.csv" >&2; exit 2; }
head -n 1 "$OUT/chi_solid_dynamics_0493x16a.csv" | grep -q 'fictitiousFluidDiagnostic0493x16c' || {
  echo "[0493x16f] ERROR binary/output does not contain x16c fictitious-fluid diagnostics" >&2
  exit 2
}
...
if not rows:
    raise SystemExit('[0493x16f] ERROR empty chi-solid dynamics CSV')
...
for key in required:
    if key not in rows[0]:
        raise SystemExit(f'[0493x16f] ERROR missing resident column {key}')
if any(int(float(r['cudaResidentSolid0493x16e'])) != 1 for r in rows):
    raise SystemExit('[0493x16f] ERROR non-resident solid row detected')
if any(int(float(r['subcellRaster0493x16e'])) != 0 for r in rows):
    raise SystemExit('[0493x16f] ERROR subcell raster unexpectedly active')
if any(int(float(r['historicalBinaryMask0493x16f'])) != 1 for r in rows):
    raise SystemExit('[0493x16f] ERROR historical binary mask not active')
if any(int(float(r['poststreamTemporalSync0493x16f'])) != 1 for r in rows):
    raise SystemExit('[0493x16f] ERROR poststream temporal sync not active')
if any(int(float(r['hostGeometryFieldUploadBytes0493x16e'])) != 0 or
    ...
    raise SystemExit('[0493x16f] ERROR full-grid host traffic detected')
print('[0493x16f] resident/historical-binary/poststream-sync/full-grid-transfer checks PASS')
PY_RESIDENT

echo "[0493x16f] COMPLETE run=$RUN_ROOT"
echo "[0493x16f] case complete; run the pair analyzer after both rest and boost cases"
