...
# 0298 — post-SRC CUDA population guard plus cell-relative energy restoration validation.
#
# Default validation is intentionally non-triggering: the guard and 0298 restoration path are called and
# write diagnostics, but NMIN=0 and NMAX=0 disable split/merge.  This gives the
# same strict OFF/ON non-perturbation check used by 0295/0296/0297. Active mutation
# experiments are enabled explicitly by setting GUARD_NMIN/GUARD_NMAX.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

BIN=${BIN:-build/src_mpcd_base_cuda_0298}
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_cuda_resampling_moment_restore_0298}
NX=${NX:-64}
NY=${NY:-64}
GAMMA=${GAMMA:-20}
STEPS=${STEPS:-80}
...
if [[ "$FORCE_REBUILD" != "0" && "$FORCE_REBUILD" != "false" && "$FORCE_REBUILD" != "FALSE" ]]; then
  echo "[0298-restore] rebuilding $BIN"
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0298.sh
elif [[ ! -x "$BIN" ]]; then
  OUT="$BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash scripts/build_src_mpcd_cuda_0298.sh
fi
if [[ ! -x "$BIN" ]]; then
  echo "[0298-restore] ERROR: missing binary $BIN" >&2
  exit 127
fi

OUT_CSV=${OUT_CSV:-$ART_DIR/cuda_resampling_moment_restore_0298_manifest.csv}
printf 'caseName,guardOffRoot,guardOnRoot,offExitCode,onExitCode,comparedMetrics,failedMetrics,verdict,guardOffFiles,guardOffRows,guardOnFiles,guardOnRows,guardOnSplitApplied,guardOnMergeApplied,guardOnPoorCells,guardOnRichCells,compareCsv\n' > "$OUT_CSV"
...
rows=0; split=0; merge=0; poor=0; rich=0
for p in files:
    try:
        ...
        pass
print(f"{len(files)},{rows},{split},{merge},{poor},{rich}")
PY
}
...
run_demo_pair() {
  ...
  echo "[0298-restore] running case=$case_name guard=0 every=$guard_every script=$script"
  local off_rc=0
  set +e
  ...
  echo "[0298-restore] running case=$case_name guard=1 every=$guard_every script=$script"
  local on_rc=0
  set +e
  env BIN="$BIN" AUTO_BUILD=0 LIVE_PROGRESS="$LIVE_PROGRESS" CLEAN_RUN_ROOT="$CLEAN_RUN_ROOT" \
      NX="$nx" NY="$ny" GAMMA="$GAMMA" STEPS="$steps" SUMMARY_EVERY="$steps" DUMP_STATE_EVERY="$steps" \
  ...
  IFS=, read -r off_files off_rows off_split off_merge off_poor off_rich <<<"$(guard_count "$off_root")"
  IFS=, read -r on_files on_rows on_split on_merge on_poor on_rich <<<"$(guard_count "$on_root")"
  ...
  append_row "$case_name" "$off_root" "$on_root" "$off_rc" "$on_rc" "$compared" "$failed" "$verdict" "$off_files" "$off_rows" "$on_files" "$on_rows" "$on_split" "$on_merge" "$on_poor" "$on_rich" "$compare_csv"
  echo "[0298-restore] case=$case_name verdict=$verdict compared=$compared failed=$failed guardRowsOff=$off_rows guardRowsOn=$on_rows splitOn=$on_split mergeOn=$on_merge poorOn=$on_poor richOn=$on_rich"
  if [[ "$STOP_ON_FAIL" == "1" && "$verdict" != "PASS" ]]; then
    echo "[0298-restore] FAIL case=$case_name" >&2
    echo "[0298-restore] stdout/stderr: $case_art/guard_off.*.log $case_art/guard_on.*.log" >&2
    exit 1
  fi
}
...
print(f"[0298-restore] manifest={p}")
print(f"[0298-restore] cases={len(active)} failed={len(failed)} guardRowsOff={off_rows} guardRowsOn={on_rows} splitOn={on_split} mergeOn={on_merge} verdict={'PASS' if active and not failed and off_rows == 0 and on_rows > 0 else 'FAIL'}")
if failed:
    for r in failed:
        print(f"[0298-restore] failed-case={r.get('caseName')} offRc={r.get('offExitCode')} onRc={r.get('onExitCode')} compare={r.get('compareCsv')}")
if failed or not active or off_rows != 0 or on_rows <= 0:
    raise SystemExit(1)
PY

echo "[0298-restore] wrote $OUT_CSV"
