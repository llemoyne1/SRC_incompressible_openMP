#!/usr/bin/env python3
"""0300 active CUDA resampling sweep analyzer.

Consumes the run manifest produced by run_cuda_resampling_active_sweep_0300.sh
...
def main(argv: List[str]) -> int:
    ...
    write_csv(out_dir / "cuda_resampling_active_sweep_0300_per_run.csv", per_run)
    write_csv(out_dir / "cuda_resampling_active_sweep_0300_vs_classic.csv", deltas)
    ...
    print(f"[0300-analyze] manifest={manifest}")
    print(f"[0300-analyze] runs={len(rows)} failedRuns={len(failed_runs)} totalSplit={total_split:g} totalMerge={total_merge:g} maxMassErr={max_mass_err:g} maxMomentumErr={max_mom_err:g} maxKrelErr0298={max_krel_err:g}")
    print(f"[0300-analyze] wrote={out_dir / 'cuda_resampling_active_sweep_0300_per_run.csv'}")
    print(f"[0300-analyze] wrote={out_dir / 'cuda_resampling_active_sweep_0300_vs_classic.csv'}")
