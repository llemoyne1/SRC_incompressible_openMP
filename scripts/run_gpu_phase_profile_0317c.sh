...
# 0317c — phase profiling harness after 0317b.
...
ART_DIR=${ART_DIR:-dev_history/artifacts/gpu_phase_profile_0317c}
...
if [[ "$SRC_BUILD" == "1" && ( "$RUN_SRC_PERIODIC" == "1" || "$RUN_SRC_IO" == "1" ) ]]; then
  ...
  echo "[0317c-profile] building SRC binary: $SRC_BIN via $SRC_BUILD_SCRIPT"
  OUT="$SRC_BIN" CUDA_ARCH_FLAGS="${CUDA_ARCH_FLAGS:-}" bash "$SRC_BUILD_SCRIPT"
fi
if [[ ( "$RUN_SRC_PERIODIC" == "1" || "$RUN_SRC_IO" == "1" ) && ! -x "$SRC_BIN" ]]; then
  echo "[0317c-profile] ERROR: SRC_BIN missing/not executable: $SRC_BIN" >&2
  exit 127
fi

if [[ "$RUN_VKKH" == "1" ]]; then
  if [[ ! -f "$VKKH_CU" ]]; then
    echo "[0317c-profile] WARNING: VKKH_CU not found: $VKKH_CU" >&2
    echo "[0317c-profile]          Set VKKH_CU=/path/to/mpcd_vkkh_play.cu, or RUN_VKKH=0." >&2
    RUN_VKKH=0
  elif [[ "$VKKH_BUILD" == "1" ]]; then
    echo "[0317c-profile] building VKKH binary: $VKKH_BIN from $VKKH_CU"
...
if [[ "$RUN_VKKH" == "1" && ! -x "$VKKH_BIN" ]]; then
  echo "[0317c-profile] ERROR: VKKH_BIN missing/not executable: $VKKH_BIN" >&2
  exit 127
fi

MANIFEST="$ART_DIR/gpu_phase_profile_0317c_manifest.csv"
...
BIN="$SRC_BIN"; AUTO_BUILD=0; LIVE_PROGRESS=0; CLEAN_RUN_ROOT=1
THREADS="$THREADS"; OMP_NUM_THREADS="$THREADS"; export OMP_NUM_THREADS OMP_PROC_BIND OMP_PLACES OMP_DYNAMIC
CASE_NAME="vk_like_io_0317c"
prepare_demo_dirs_0283 "$run_root"
STATE_FILE="$run_root/init/vk_like_io_${NX}x${NY}_g${GAMMA}.smpcd"
PARAMS_FILE="$run_root/params/vk_like_io.kv"
OUT_DIR="$run_root/output"
LOG_FILE="$run_root/logs/vk_like_io.log"
TIME_FILE="$run_root/logs/vk_like_io.time"
...
$(write_src_classic_common_params_0283 "$steps" "$DT" "$KBT" "$SEED" 1000000000 0 "$THREADS" 1.5)
PARAMS
...
run_target() {
  ...
  case "$target" in
    ...
    *) echo "[0317c-profile] unknown target=$target" >&2; exit 2 ;;
  esac
  echo "[0317c-profile] run target=$target rep=$rep steps=$steps warmup=$warm"
  set +e
  ...
  if [[ "$rc" != "0" ]]; then
    echo "[0317c-profile] WARNING: target=$target rep=$rep failed rc=$rc" >&2
    tail -80 "$stderr_file" >&2 || true
  fi
}

if [[ "$WARMUP" == "1" ]]; then
  echo "[0317c-profile] warmups use WARMUP_STEPS=$WARMUP_STEPS and are excluded from manifest."
...
python3 scripts/summarize_gpu_phase_profile_0317c.py "$MANIFEST" "$ART_DIR"

echo "[0317c-profile] manifest: $MANIFEST"
echo "[0317c-profile] summary : $ART_DIR/gpu_phase_profile_0317c_summary.csv"
echo "[0317c-profile] SRC phase breakdown: $ART_DIR/gpu_phase_profile_0317c_src_phase_breakdown.csv"
echo "[0317c-profile] SRC CUDA resident breakdown: $ART_DIR/gpu_phase_profile_0317c_cuda_resident_breakdown.csv"
