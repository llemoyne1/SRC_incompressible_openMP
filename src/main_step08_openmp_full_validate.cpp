...
mpcd::State run_ref_step(const mpcd::State& state0,
                         const mpcd::Params& params,
                         mpcd::State& stateRef,
                         mpcd::ZonePassResult& baseResult,
                         mpcd::ZonePassResult& shiftedResult,
                         mpcd::LiquidClosureResult& closureResult,
    ...
    baseResult = mpcd::run_zone_pass(stateRef, params, "base", 88172645463325252ULL);
    shiftedResult = mpcd::run_zone_pass(baseResult.stateOut, params, "shifted", 88172645463325252ULL + 4099ULL);
    closureResult = mpcd::run_liquid_closure(stateRef, shiftedResult.stateOut, params);
    return closureResult.stateOut;
}

mpcd::State run_omp_step(const mpcd::State& state0,
                         const mpcd::Params& params,
                         int nthreads,
                         mpcd::State& stateRef,
                         mpcd::OpenMPPassResult& baseResult,
                         mpcd::OpenMPPassResult& shiftedResult,
                         mpcd::LiquidClosureResult& closureResult,
                         mpcd::WallInfo& wallInfo,
                         double& pBotInst,
                         double& pTopInst,
    ...
    baseResult = mpcd::run_zone_pass_openmp(stateRef, params, "base", 88172645463325252ULL, nthreads);
    shiftedResult = mpcd::run_zone_pass_openmp(baseResult.stateOut, params, "shifted", 88172645463325252ULL + 4099ULL, nthreads);
    closureResult = mpcd::run_liquid_closure(stateRef, shiftedResult.stateOut, params);
    return closureResult.stateOut;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        mpcd::State seqRef, ompRef;
        mpcd::ZonePassResult seqBase, seqShifted;
        mpcd::OpenMPPassResult ompBase, ompShifted;
        mpcd::LiquidClosureResult seqClosure, ompClosure;
        mpcd::WallInfo seqWall, ompWall;
        double seqPBot=0, seqPTop=0, seqPMean=0, ompPBot=0, ompPTop=0, ompPMean=0;

        const mpcd::State seqOut = run_ref_step(state0, params, seqRef, seqBase, seqShifted, seqClosure, seqWall, seqPBot, seqPTop, seqPMean);
        const mpcd::State ompOut = run_omp_step(state0, params, nthreads, ompRef, ompBase, ompShifted, ompClosure, ompWall, ompPBot, ompPTop, ompPMean);
        ...
        fout << "seq_pBotInst=" << seqPBot << '\n';
        fout << "omp_pBotInst=" << ompPBot << '\n';
        fout << "seq_pTopInst=" << seqPTop << '\n';
        fout << "omp_pTopInst=" << ompPTop << '\n';
        fout << "seq_pMeanInst=" << seqPMean << '\n';
        fout << "omp_pMeanInst=" << ompPMean << '\n';
