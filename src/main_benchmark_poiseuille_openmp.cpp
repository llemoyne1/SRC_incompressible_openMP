...
int env_threads_or_default(int dflt) {
    const char* s = std::getenv("OMP_NUM_THREADS");
    if (!s || !*s) return dflt;
    try { return std::max(1, std::stoi(std::string(s))); } catch (...) { return dflt; }
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        for (int step = 1; step <= nSteps; ++step) {
            ...
            const auto [baseOccStd, baseOutBand] = compute_occstd_outband(baseFields, params);
            const auto [shiftedOccStd, shiftedOutBand] = compute_occstd_outband(shiftedFields, params);
