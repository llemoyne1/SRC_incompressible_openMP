...
CudaResamplingOperationMaterialize0453Diagnostics try_apply_cuda_resampling_operation_materializer_0453(
    ...
    try {
        ...
        GpuMaterializedOps0453 gpu{};
        if (cuda_resampling_materializer_shared_state_0475_requested()) {
            CudaParticleStateDiagnostics stateUploadDiag{};
            CudaParticleState& sharedGpuState0475 = cuda_shared_particle_state_0251();
            const bool sharedFresh0475 = cuda_shared_particle_state_0251_is_fresh() &&
                                         sharedGpuState0475.size() == state.Np &&
                                         sharedGpuState0475.active_fluid_size() >= state.NactiveFluid;
            if (sharedFresh0475) {
                gpu.uploadSkipped0475 = 1u;
            } else {
                sharedGpuState0475.upload_all(state, &stateUploadDiag);
                cuda_shared_particle_state_0251_mark_fresh("operation_materializer_0475");
            }
            if (cuda_resampling_materializer_cell_list_0475b_requested()) {
            ...
            gpu.stateUploadSeconds0475 = stateUploadDiag.uploadSeconds;
            gpu.uploadSeconds += stateUploadDiag.uploadSeconds;
            gpu.totalSeconds += stateUploadDiag.uploadSeconds;
        } else {
            gpu = materialize_ops_gpu_0453(state, grid, params, operationWorkspace);
        }
        d.gpuOps = static_cast<std::uint64_t>(gpu.ops.size());
        d.invalidOps = gpu.invalidOps;
        d.materializerSharedState0475 = gpu.sharedState0475;
        d.materializerUploadSkipped0475 = gpu.uploadSkipped0475;
        d.materializerCompactDownload0475 = gpu.compactDownload0475;
        d.stateUploadSeconds0475 = gpu.stateUploadSeconds0475;
        d.planUploadSeconds0475 = gpu.planUploadSeconds0475;
...
void append_resident_upstream_coupled_probe_csv_0470(
    const SimulationParams& params,
    std::uint64_t step,
    std::uint64_t nActive,
    std::uint64_t passiveOps,
    ...
    try {
        ...
        if (cuda_resampling_device_carrier_0455_requested()) {
            ...
            if (cuda_resampling_resident_external_carrier_0467b_requested()) {
                ...
                if (cuda_resampling_resident_upstream_coupled_probe_0470_requested()) {
