...
cd "$ROOT"
export TARGET=wall
export RUN_ROOT="${RUN_ROOT:-runs/0493x9y_vacuum_drop_pointwise}"
...
printf '%s\n' \
  '[0493x9y-suite] pointwise alpha(start) side; no cell-center interior aliasing' \
  '[0493x9y-suite] actual endpoint first; 4 bisections only in apply pass for true crossings' \
  '[0493x9y-suite] last-known-inside crossing fraction; one-cell shell guard; no halo search' \
  '[0493x9y-suite] no new particle pass, no new O(N) production buffer'
bash scripts/run_0493x9s_splash.sh
CSV="$RUN_ROOT/output/cuda_phase_kinetic_crossing_0493x9y.csv"
[[ -f "$CSV" ]] || { echo "[0493x9y-suite] ERROR missing $CSV" >&2; exit 2; }
python3 scripts/analyze_0493x9y_pointwise.py "$CSV"
