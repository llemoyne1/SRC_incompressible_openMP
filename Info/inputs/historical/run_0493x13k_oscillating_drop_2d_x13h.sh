...
# 0493x13k — 2-D oscillating-drop qualification of the x13h fluid.
# Tooling/initial-condition only: no C++/CUDA physics change and no new source diagnostic.
# The first qualification is deliberately n=2 because existing x9f second moments
# provide a robust signed quadrupole observable without heavy field recording.

CASE_LABEL="oscillating_drop_2d_0493x13k"
RUN_MODE="${RUN_MODE:-src-q6-g-f}"
TOPOLOGY="closed_box"

GENERATOR="$ROOT/scripts/generate_0493x13k_oscillating_drop_2d.py"
ANALYZER="$ROOT/scripts/analyze_0493x13k_oscillating_drop_2d.py"
[[ -f "$GENERATOR" ]] || { echo "[0493x13k] ERROR missing $GENERATOR" >&2; exit 2; }
[[ -f "$ANALYZER" ]] || { echo "[0493x13k] ERROR missing $ANALYZER" >&2; exit 2; }
...
# First dynamic qualification point: small-amplitude n=2 drop.
...
# ~3.3 inviscid periods at the default point.  Summary every 2 gives ~90 samples/period.
STEPS="${STEPS:-600}"
SUMMARY_EVERY="${SUMMARY_EVERY:-2}"
DUMP_STATE_EVERY="${DUMP_STATE_EVERY:-0}"
FIT_PERIODS="${FIT_PERIODS:-2.5}"
NU_REF="${NU_REF:-0.00051}"

RUN_ROOT="${RUN_ROOT:-runs/0493x13k_oscillating_drop_2d_x13h_s${SIGMA_DECLARED}_n${MODE}_r${RADIUS_CELLS}_eps${EPSILON}_seed${SEED}}"
CLEAN_RUN_ROOT="${CLEAN_RUN_ROOT:-1}"
PREFLIGHT_ONLY="${PREFLIGHT_ONLY:-0}"

# No heavy visualization/recording path: x9f/x9e CSV diagnostics are sufficient.
...
# Tight preflight: keep this first runner on the agreed n=2 objective rather than
# silently pretending the x9f second moments also measure n=3/n=4.
...
hx,hy=lx/nx,ly/ny
if abs(hx-hy)>1e-12*max(1.0,abs(hx),abs(hy)): raise SystemExit('[0493x13k] ERROR square cells required')
if mode != 2: raise SystemExit('[0493x13k] ERROR this first dynamic qualification is intentionally MODE=2; x9f is a quadrupole diagnostic')
if not (0 < eps <= 0.05): raise SystemExit(f'[0493x13k] ERROR small-amplitude smoke requires 0<epsilon<=0.05, got {eps}')
if sigma <= 0 or rc <= 0 or nu <= 0 or dt <= 0: raise SystemExit('[0493x13k] ERROR sigma,R,nu,dt must be positive')
if rmin != 4: raise SystemExit(f'[0493x13k] ERROR current x12 dynamic drop uses minRadiusCells=4 here, got {rmin}')
if abs(angle-2.0943951023931953)>1e-12: raise SystemExit(f'[0493x13k] ERROR x13h requires rotationAngle=120deg, got {angle}')
...
LIVE_VIS_CONTROL_FILE="$RUN_ROOT/livevis_control_0493x13k.kv"
export LIVE_VIS_CONTROL_FILE
suite_prepare_livevis_control_0434 "$RUN_ROOT" "$RUN_MODE"
suite_export_livevis_0434
suite_write_env_file_0434 "$RUN_ROOT/logs/environment_0493x13k.env" "$RUN_MODE"
cat >> "$RUN_ROOT/logs/environment_0493x13k.env" <<META
X13K_MODE=$MODE
X13K_RADIUS_CELLS=$RADIUS_CELLS
X13K_EPSILON=$EPSILON
X13K_PHASE=$PHASE
X13K_SIGMA=$SIGMA_DECLARED
X13K_NU_REF=$NU_REF
X13K_OMEGA0_2D=$OMEGA0
X13K_PERIOD0_2D=$PERIOD0
X13K_BETA_LAMB=$BETA_LAMB
X13K_OMEGA_LAMB=$OMEGA_LAMB
X13K_PERIOD_LAMB=$PERIOD_LAMB
X13K_R_OVER_X12A_RC=$RCUT_RATIO
META

printf '%s\n' \
  '===== 0493x13k 2-D OSCILLATING DROP / x13h =====' \
  "[0493x13k] objective=dynamic capillary qualification n=2; no Young-Laplace sigma0 baseline" \
  "[0493x13k] initial r(theta)=R*[sqrt(1-eps^2/2)+eps*cos(2theta)]; exact continuous area=pi*R^2" \
  "[0493x13k] grid=${NX}x${NY} h=$H gamma=$GAMMA dt=$DT kBT=$KBT R/h=$RADIUS_CELLS eps=$EPSILON sigma=$SIGMA_DECLARED" \
  "[0493x13k] theory2D omega0=$OMEGA0 period0=$PERIOD0 LambBeta=$BETA_LAMB omegaDamped=$OMEGA_LAMB periodDamped=$PERIOD_LAMB" \
  "[0493x13k] x12a initial resolved-size ratio R/Rc=$RCUT_RATIO (target >1)" \
  "[0493x13k] steps=$STEPS tEnd=$(awk -v n="$STEPS" -v d="$DT" 'BEGIN{printf "%.9g",n*d}') summaryEvery=$SUMMARY_EVERY fitPeriods=$FIT_PERIODS" \
  "[0493x13k] diagnostics=x9f signed quadrupole from moment ellipticity+principal angle; x9e/x9r ancillary"
...
if ! suite_truthy_0434 "$PREFLIGHT_ONLY"; then
  ...
  [[ -s "$SHAPE" ]] || { echo "[0493x13k] ERROR missing x9f shape CSV: $SHAPE" >&2; exit 2; }
  python3 "$ANALYZER" \
  ...
  echo "[0493x13k] COMPLETE run=$RUN_ROOT analysis=$RUN_ROOT/analysis_0493x13k"
fi
