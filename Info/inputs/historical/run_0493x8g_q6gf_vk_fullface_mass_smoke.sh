...
# 0493x8g — Q6-g-f VK true-fullface mass-balance qualification, U=0.18.
#
# Design principles:
...
#
# Macroscopic design:
#   Uinf=.18, D=.3125=80a, Re=83.4165 using nuTG=6.743265812e-4
...
CASE_LABEL="0493x8g_q6gf_vk_fullface_u018"
GEN_CASE="vk"
TOPOLOGY="io_fullface"
RUN_MODES="src-q6-g-f"

# ---------------------------------------------------------------------------
# Qualified microscopic fluid: do not change in the reference x8g run.
# ---------------------------------------------------------------------------
Lx="${Lx:-3.125}"
Ly="${Ly:-1.5625}"
NX="${NX:-800}"
...
# True full-face inlet/outlet, matching the resident Q6 0404 family.
...
BASE_RUN_ROOT="${BASE_RUN_ROOT:-runs/0493x8g_q6gf_vk_fullface_u018}"
...
RECORD_SESSION_PREFIX="${RECORD_SESSION_PREFIX:-0493x8g_q6gf_vk_fullface_u018}"
...
if abs(dx-dy)>1e-14:
    raise SystemExit(f"[0493x8g-preflight] ERROR non-square cells dx={dx} dy={dy}")
if abs(dx-1/256)>1e-14:
    raise SystemExit(f"[0493x8g-preflight] ERROR calibrated cell size lost: a={dx}")
if not (0 < xc-r and xc+r < lx and 0 < yc-r and yc+r < ly):
    raise SystemExit("[0493x8g-preflight] ERROR cylinder intersects external boundary")
Re=u*D/nu
Ma=u/cs
alpha_dt=alpha*dt
up=xc/D
...
active=nx*ny*gamma
print("===== 0493x8g VK FULLFACE PREFLIGHT =====")
...
# Required by the resident Q6 full-face 0404 path.  At the Q6 level this sets
# equal prescribed volume fluxes on x-low/x-high.  At the particle level it
# remains passive (no extra equilibrium/forced extraction).
...
run_one_0493x8g() {
  ...
  write_params_0493x8g "$mode" "$state" "$out" "$chi" "$params"
  ...
  suite_write_env_file_0434 "$run_root/logs/environment_0493x8g.env" "$mode"
  cat >> "$run_root/logs/environment_0493x8g.env" <<META
X8G_REFERENCE_NU=${REFERENCE_NU}
X8G_REFERENCE_CS=${REFERENCE_CS}
X8G_CYLINDER_D=$(awk -v r="$CYLINDER_R" 'BEGIN{printf "%.17g",2*r}')
X8G_CYLINDER_CX=${CYLINDER_CX}
X8G_CYLINDER_CY=${CYLINDER_CY}
X8G_INLET=${INLET_FACE}:fullface
X8G_OUTLET=${OUTLET_FACE}:fullface:${OUTLET_MODE}
X8G_LIVE_VIS_ENABLE=${LIVE_VIS_ENABLE}
X8G_RECORD_EVERY=${RECORD_EVERY}
X8G_RECORD_STRIDE=${RECORD_STRIDE}
META

  echo "[0493x8g] mode=$mode topology=$TOPOLOGY root=$run_root"
  echo "[0493x8g] inlet=$INLET_FACE(fullface) U=$UIN -> outlet=$OUTLET_FACE(fullface) mode=$OUTLET_MODE"
  echo "[0493x8g] cylinder=($CYLINDER_CX,$CYLINDER_CY,r=$CYLINDER_R) alpha=$ALPHA forcing=$DARCY_BRINKMAN_FORCING_MODE chiVP=$DARCY_CHI_COLLISION_VP_ENABLE"
  echo "[0493x8g] livevis=$LIVE_VIS_ENABLE field=$LIVE_VIS_FIELD every=$LIVE_VIS_EVERY control=$LIVE_VIS_CONTROL_FILE"
  echo "[0493x8g] recording=$RECORD_ENABLE fields=$RECORD_FIELDS every=$RECORD_EVERY stride=$RECORD_STRIDE"
  echo "[0493x8g] Q6GF tau=$Q6_GF_DENSITY_RELAXATION_TIME tol=$PROJECTION_TOLERANCE maxIt=$PROJECTION_MAX_ITERATIONS x7j=1 resampling=off"

  suite_run_binary_0434 "$params" "$log" "$time" "$out"

  if ! suite_truthy_0434 "$PREFLIGHT_ONLY"; then
    echo
    echo "===== 0493x8g RUN COMPLETE ====="
    echo "root=$run_root"
    echo "log=$log"
